...
export class OverriddenCommand extends FunctionalTest {
	revisionCl = -1
	...
	async run() {
		...
		await this.reconsider('Main', this.revisionCl, 'Dev-Pootle', '#robomerge ' + this.fullBranchName('Dev-Pootle'))
	}
	...
	getBranches() {
		return [
			this.makeBranchDef('Main', ['Dev-Perkin', 'Dev-Pootle']),
