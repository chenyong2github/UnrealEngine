...
namespace P4VUtils.Commands
{
	class EdigrateCommand : Command
	{
		...
		public override async Task<int> Execute(string[] Args, IReadOnlyDictionary<string, string> ConfigValues, ILogger Logger)
		{
			...
			await ReopenFiles("delete", AddActions, OpenedClientToLocalMap, x => Perforce.DeleteAsync(Change, DeleteOptions.None, x, CancellationToken.None), Debug, Logger);
			await ReopenFiles("add", AddActions, OpenedClientToLocalMap, x => Perforce.AddAsync(Change, x, CancellationToken.None), Debug, Logger);
			await ReopenFiles("edit", AddActions, OpenedClientToLocalMap, x => Perforce.EditAsync(Change, x, CancellationToken.None), Debug, Logger);
			...
			Logger.LogInformation("Getting the list of files reopened in changelist {Change}...", Change);
		...
		async Task ReopenFiles(string Operation, List<FStatRecord> Records, Dictionary<string, string> OpenedClientToLocalMap, Func<string[], Task> ReopenAsync, bool Debug, ILogger Logger)
		{
			if (Records.Count > 0)
			{
