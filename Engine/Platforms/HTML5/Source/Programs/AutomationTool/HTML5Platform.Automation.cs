...
public class HTML5Platform : Platform
{
	...
	static bool bUseFixedTimeStep = false;
	...
	public override void Package(ProjectParams Params, DeploymentContext SC, int WorkingCL)
	{
		...
		ConfigCache.GetBool("/Script/HTML5PlatformEditor.HTML5TargetSettings", "UseFixedTimeStep", out bUseFixedTimeStep);
		...
		string TemplateFilesPath = CombinePaths(CmdEnv.LocalRoot, "Engine/Platforms/HTML5/Build/TemplateFiles");
		...
		string MacBashTemplateFile = CombinePaths(TemplateFilesPath, "RunMacHTML5LaunchHelper_template.command");
		...
		File.Copy(CombinePaths(TemplateFilesPath, "htaccess_template.txt"), htaccesspath, true);
		...
		File.AppendAllText(DestinationFile, File.ReadAllText(CombinePaths(TemplateFilesPath, "json2.js")));
		File.AppendAllText(DestinationFile, File.ReadAllText(CombinePaths(TemplateFilesPath, "jstorage.js")));
		File.AppendAllText(DestinationFile, File.ReadAllText(CombinePaths(TemplateFilesPath, "moz_binarystring.js")));
		...
		using (StreamReader reader = new StreamReader(InTemplateFile))
		{
			...
			while (reader.Peek() != -1)
			{
				...
				if (LineStr.Contains("%UE4CMDLINE%"))
				{
					...
					{
						ArgumentString += "'-UseFixedTimeStep',";
					}
