...
bool FHTML5TargetPlatform::IsSdkInstalled(bool bProjectHasCode, FString& OutDocumentationPath) const
{
	...
	// NEW PLATFORMS PATH
	FString SDKPath = FPaths::EnginePlatformExtensionsDir() / TEXT("HTML5/Build/emsdk") /
#if PLATFORM_WINDOWS
		TEXT("Win64");
#elif PLATFORM_MAC
		TEXT("Mac");
	...
	// OLD ENGINE PATH
	SDKDirectory = SDKDirectory.Replace(TEXT("Platforms/HTML5/Build"), TEXT("Extras/ThirdPartyNotUE"));
	if (IFileManager::Get().DirectoryExists(*SDKDirectory))
	{
		return true;
	}
