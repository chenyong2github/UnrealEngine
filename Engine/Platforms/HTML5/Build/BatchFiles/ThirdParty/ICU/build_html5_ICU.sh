...
# ----------------------------------------
ICU_HTML5=$(pwd)
ICU_VERSION='icu4c-53_1'
ICU_HTML5_SRC="$UE4_TPS_SRC/ICU/$ICU_VERSION"
ICU_HTML5_DST="$HTML5_TPS_LIBS/ICU/$ICU_VERSION"


# local destination
if [ ! -d "$ICU_HTML5_DST" ]; then
	mkdir -p "$ICU_HTML5_DST"
fi
# TODO change this to p4 checkout
if [ ! -z "$(ls -A "$ICU_HTML5_DST")" ]; then
	chmod +w "$ICU_HTML5_DST"/*
fi


# github version needs some loving
cd "$ICU_HTML5_SRC"/source
	chmod +x configure
...
build_all()
{
	...
	rm -rf "$ICU_HTML5"/html5_build$OPTIMIZATION
...
#	emmake make clean VERBOSE=1
	...
	cd ..
	mv html5_build$OPTIMIZATION "$ICU_HTML5"/.
}
...
cp -vp "$ICU_HTML5"/html5_build*/lib/* "$ICU_HTML5"/html5_build*/stubdata/lib* "$ICU_HTML5_DST"/.
#ls -l "$ICU_HTML5_DST"
