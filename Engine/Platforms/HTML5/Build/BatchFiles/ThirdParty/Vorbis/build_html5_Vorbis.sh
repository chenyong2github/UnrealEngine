...
# local destination
if [ ! -d "$VORBIS_HTML5_DST/lib" ]; then
	mkdir -p "$VORBIS_HTML5_DST/lib"
fi
# TODO remove this p4 hack after HTML5 becomes community driven only
if [ ! -z "$(ls -A "$VORBIS_HTML5_DST/lib")" ]; then
	chmod +w "$VORBIS_HTML5_DST"/lib/*
fi
...
#	TYPE=${type^^} # OSX-bash doesn't like this
	...
	emcmake cmake -G "Unix Makefiles" \
		...
		-DOGG_LIBRARIES=$OGG_HTML5_DST/lib \
	...
	cp lib/libvorbis.$UE_LIB_EXT "$VORBIS_HTML5_DST"/lib/libvorbis${SUFFIX}.$UE_LIB_EXT
#	cp lib/libvorbisenc.$UE_LIB_EXT "$VORBIS_HTML5_DST"/libvorbisenc${SUFFIX}.$UE_LIB_EXT
	cp lib/libvorbisfile.$UE_LIB_EXT "$VORBIS_HTML5_DST"/lib/libvorbisfile${SUFFIX}.$UE_LIB_EXT
	cd ..
}
