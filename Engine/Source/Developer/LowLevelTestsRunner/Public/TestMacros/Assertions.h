...
#define REQUIRE_ENSURE(...) INTERNAL_UE_ENSURE( "REQUIRE_ENSURE", Catch::ResultDisposition::Normal, __VA_ARGS__ )

//requires that an UE `ensure` fails with a message that matches the supplied message
#define REQUIRE_ENSURE_MSG(msg, ...) INTERNAL_UE_ENSURE_MSG(msg, "REQUIRE_ENSURE", Catch::ResultDisposition::Normal, __VA_ARGS__ )

//checks that an UE `ensure` fails in this call
#define CHECK_ENSURE(...) INTERNAL_UE_ENSURE( "CHECK_ENSURE", Catch::ResultDisposition::ContinueOnFailure, __VA_ARGS__ )

//checks that an UE `ensure` fails with a message that matches the supplied message
#define CHECK_ENSURE_MSG(msg, ...) INTERNAL_UE_ENSURE_MSG(msg, "CHECK_ENSURE", Catch::ResultDisposition::ContinueOnFailure, __VA_ARGS__ )

//requires that a UE `check` fails in this call
#define REQUIRE_CHECK(...) INTERNAL_UE_CHECK( "REQUIRE_ENSURE", Catch::ResultDisposition::Normal, __VA_ARGS__ )

//requires that a UE `check` fails in this call containg the supplies message
#define REQUIRE_CHECK_MSG(msg, ...) INTERNAL_UE_CHECK_MSG(msg, "REQUIRE_ENSURE", Catch::ResultDisposition::Normal, __VA_ARGS__ )

#define INTERNAL_UE_ENSURE( macroName, resultDisposition, ... ) \
	do { \
...
#define INTERNAL_UE_ENSURE_MSG(msg, macroName, resultDisposition, ... ) \
	do { \
		Catch::AssertionHandler catchAssertionHandler( macroName##_catch_sr, CATCH_INTERNAL_LINEINFO, CATCH_INTERNAL_STRINGIFY(__VA_ARGS__), resultDisposition ); \
		INTERNAL_CATCH_TRY { \
...
#define INTERNAL_UE_CHECK(macroName, resultDisposition, ... ) \
	do { \
		Catch::AssertionHandler catchAssertionHandler( macroName##_catch_sr, CATCH_INTERNAL_LINEINFO, CATCH_INTERNAL_STRINGIFY(__VA_ARGS__), resultDisposition ); \
		INTERNAL_CATCH_TRY { \
...
#define INTERNAL_UE_CHECK_MSG(msg, macroName, resultDisposition, ... ) \
	do { \
		Catch::AssertionHandler catchAssertionHandler( macroName##_catch_sr, CATCH_INTERNAL_LINEINFO, CATCH_INTERNAL_STRINGIFY(__VA_ARGS__), resultDisposition ); \
		INTERNAL_CATCH_TRY { \
