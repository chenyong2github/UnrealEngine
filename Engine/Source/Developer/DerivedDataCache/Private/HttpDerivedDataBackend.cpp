...
TRACE_DECLARE_INT_COUNTER(HttpDDC_BytesRecieved, TEXT("HttpDDC Bytes Recieved"));
...
public:
	...
	const size_t GetBytesRecieved() const
	{
...
private:
	...
	void LogResult(CURLcode Result, const TCHAR* Uri, RequestVerb Verb) const
	{
		if (Result == CURLE_OK)
		{
			...
			case Get:
				...
				AdditionalInfo = FString::Printf(TEXT("Recieved: %d bytes."), BytesRecieved);
				break;
			case Put:
	...
	static size_t StaticWriteBodyFn(void* Ptr, size_t SizeInBlocks, size_t BlockSizeInBytes, void* UserData)
	{
		...
		if (WriteDataBufferPtr && WriteSize > 0)
		{
			...
			if (Request->BytesRecieved == 0 && Request->WriteHeaderBufferPtr)
			{
			...
			Request->BytesRecieved += WriteSize;
			return WriteSize;
		}
 ...
 */
bool VerifyPayload(const FRequest* Request, const TArray<uint8>& Payload)
{
	...
	if (Request->GetHeader("X-Jupiter-Sha1", RecievedHashStr))
	{
		...
		if (RecievedHash != PayloadHash)
		{
			UE_LOG(LogDerivedDataCache, Warning, TEXT("Checksum from server did not match recieved data (%s vs %s). Discarding cached result."), *RecievedHashStr, *PayloadHash.ToString());
			return false;
		}
	}
	else
...
bool FHttpDerivedDataBackend::CachedDataProbablyExists(const TCHAR* CacheKey)
{
	...
	FString Uri = FString::Printf(TEXT("api/v1/c/%s/%s/%s"), *Namespace, *DefaultBucket, CacheKey);
...
bool FHttpDerivedDataBackend::GetCachedData(const TCHAR* CacheKey, TArray<uint8>& OutData)
{
	...
	FString Uri = FString::Printf(TEXT("api/v1/c/%s/%s/%s.raw"), *Namespace, *DefaultBucket, CacheKey);
	...
	while (ResponseCode == 0 && ++Attempts < UE_HTTPDDC_MAX_ATTEMPTS)
	{
		...
		if (Request.IsValid())
		{
			...
			if (ResponseCode == 200 && VerifyPayload(Request.Get(), OutData))
			{
				TRACE_COUNTER_ADD(HttpDDC_GetHit, int64(1));
				TRACE_COUNTER_ADD(HttpDDC_BytesRecieved, int64(Request->GetBytesRecieved()));
				COOK_STAT(Timer.AddHit(Request->GetBytesRecieved()));
				return true;
			}
