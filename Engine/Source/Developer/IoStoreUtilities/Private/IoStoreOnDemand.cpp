...
class FOnDemandIoStoreWriter final
	: public IIoStoreWriter
{
	...
	static constexpr int32 MaxConcurrentWrites{64};

public:
	FOnDemandIoStoreWriter(const FOnDemandWriterSettings& WriterSettings)
		: Settings(WriterSettings)
	{
		Settings.TocFilePath = FPaths::SetExtension(Settings.TocFilePath, TEXT("utocmanifest"));
	}

	virtual ~FOnDemandIoStoreWriter()
	{ }
		...
		{
			{
				...
				if (WriteStats.PendingCount.load() < MaxConcurrentWrites)
				{
					...
					// Add compression/encryption tasks if needed
					...
					UE::Tasks::FTask WriteChunkTask = PendingWrite->TaskPipe.Launch(TEXT("WriteChunk"), [this, PendingWrite]() mutable
					{
						...
						{
							FString FileName = LexToString(PendingWrite->ChunkId) + TEXT(".iochunk");
							FString FilePath = Settings.OutputDirectory / FileName; 
	...
	virtual TIoStatusOr<FIoStoreWriterResult> GetResult() override
	{
		...
		UE_LOG(LogIoStore, Display, TEXT("Saving ondemand JSON manifest '%s'"), *Settings.TocFilePath);
		if (!FFileHelper::SaveStringToFile(Json, *Settings.TocFilePath))
		{
			return FIoStatus(EIoErrorCode::CorruptToc, TEXTVIEW("Failed writing ondemand JSON manifest")); 
		}
...
private:
	FOnDemandWriterSettings Settings;
...
{
	return MakeUnique<FOnDemandIoStoreWriter>(WriterSettings);
}
