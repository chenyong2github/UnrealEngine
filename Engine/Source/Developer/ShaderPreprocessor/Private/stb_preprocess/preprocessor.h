...
extern char * preprocess_file(char * output_storage, char *filename, char **include_paths, int num_include_paths, char **sys_include_paths, int num_sys_include_paths, macro_definition **predefined_macros, int num_predefined_macros, pp_diagnostic **pd, int *num_pd);
extern void preprocessor_file_free(char *text, pp_diagnostic *pd);
typedef struct stb_arena stb_arena;
extern macro_definition *pp_define(stb_arena *a, char *p);
