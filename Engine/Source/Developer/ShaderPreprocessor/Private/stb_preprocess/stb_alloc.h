...
typedef struct stb_arena stb_arena;

// allocate from an arena
STB_ALLOC_DEF void *stb_arena_alloc        (stb_arena *a, size_t size);
STB_ALLOC_DEF void *stb_arena_alloc_aligned(stb_arena *a, size_t size, size_t align);

// allocate a string from an arena
STB_ALLOC_DEF char *stb_arena_alloc_string       (stb_arena *a, char *str);
STB_ALLOC_DEF char *stb_arena_alloc_string_length(stb_arena *a, char *str, size_t length);

// free the entire arena, leaves arena reset so it can be used again
STB_ALLOC_DEF void stb_arena_free(stb_arena *a);
...
void *stb_arena_alloc_aligned(stb_arena *a, size_t size, size_t align)
{
  void *p;
  ...
  if (a->last_block_alloc_offset + size > a->last_block_size) {
    ...
    if (a->num_blocks == 0) {
      ...
      if ((a->num_blocks & (a->num_blocks-1)) == 0) {
        // grow table
        a->blocks = realloc(a->blocks, sizeof(a->blocks[0]) * a->num_blocks * 2);
      }
    }
...
{
  if ((size & 7) != 0) return 4;
  if (size >= 16) {
    if (size >= 32 && (size & 31) == 0) return 32;
    if (              (size & 15) == 0) return 16;
  }
  return 8;
}

void *stb_arena_alloc(stb_arena *a, size_t size)
{
  return stb_arena_alloc_aligned(a, size, stb_arena_align_size(size));
}

char *stb_arena_alloc_string_length(stb_arena *a, char *str, size_t length)
{
...
char *stb_arena_alloc_string(stb_arena *a, char *str)
{
  return stb_arena_alloc_string_length(a, str, strlen(str));
}

void stb_arena_free(stb_arena *a)
{
  int i;
