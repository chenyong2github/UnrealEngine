...
static void ce_next(ppcexp *c)
{
  ...
  switch (*p) {
    ...
    numeric_suffixes:
      ...
      while (*p == 'u' || *p == 'l') {
        if (*p == 'u') c->token_value.is_signed = 1;
        ++p;
      }
      c->token = PPLEX_int_literal;
      break;
...
{
  int err;
  assert(evaluate_integer_constant_expression("4+3*2",&err)==10);              assert(err==0);
  assert(evaluate_integer_constant_expression("4+3*2==10",&err) == 1);         assert(err==0);
  assert(evaluate_integer_constant_expression("0xffffffff+1==1<<32",&err)==1); assert(err==0);
  evaluate_integer_constant_expression("1+=1",&err); assert(err>0);
  evaluate_integer_constant_expression("1+(",&err) ; assert(err>0);
  evaluate_integer_constant_expression("(1+",&err) ; assert(err>0);
  assert(evaluate_integer_constant_expression("2/1",&err)==2); assert(err==0);
  evaluate_integer_constant_expression("1/0",&err); assert(err<0);
  evaluate_integer_constant_expression("1 || 1/0",&err); assert(err==0);
  evaluate_integer_constant_expression("0 || 1/0",&err); assert(err<0);
  evaluate_integer_constant_expression("1 && 1/0",&err); assert(err<0);
  evaluate_integer_constant_expression("0 && 1/0",&err); assert(err==0);
  evaluate_integer_constant_expression("0 ? 1/0 : 2/1",&err); assert(err==0);
  evaluate_integer_constant_expression("1 ? 1/0 : 2/1",&err); assert(err<0);
  evaluate_integer_constant_expression("0 ? 2/1 : 1/0",&err); assert(err<0);
  evaluate_integer_constant_expression("1 ? 2/1 : 1/0",&err); assert(err==0);
}
