...
struct macro_definition
{
  ...
  uint8    disabled;
  macro_definition *next; // push/pop stack
};
...
static macro_definition predefined_FILE    = { "__FILE__"   ,  8, { 0 }, 0, MACRO_NUM_PARAMETERS_file   , 1 };
static macro_definition predefined_LINE    = { "__LINE__"   ,  8, { 0 }, 0, MACRO_NUM_PARAMETERS_line   , 1 };
static macro_definition predefined_COUNTER = { "__COUNTER__", 11, { 0 }, 0, MACRO_NUM_PARAMETERS_counter, 1 };
static macro_definition predefined_defined = { "defined"    ,  7, { 0 }, 0, MACRO_NUM_PARAMETERS_defined, 1 };
...
typedef struct pp_context 
{
  ...
  char *last_ifndef;
  macro_definition *last_macro_definition;
...
static char *arena_alloc_padded_string(stb_arena *a, char *text, size_t stringlen)
{
...
void *stringhash_get(pphash *ht, char *key, size_t keylen)
{
  ...
  if (ht->verify) { char temp[1024]; memcpy(temp, key, keylen); temp[keylen] = 0; vresult = shget(ht->verify, temp); }

  if (hash <= HASH_TOMBSTONE) hash+=2;
  if (table[slot].hash == hash) {
    uint32 i = table[slot].index;
    if (ht->pair[i].key_length == keylen && 0==memcmp(key, ht->pair[i].key, keylen)) {
      if (ht->verify) assert(vresult == ht->pair[i].value);
...
int stringhash_delete(pphash *ht, char *key, size_t keylen)
{
  ...
  if (ht->verify) { char temp[1024]; memcpy(temp, key, keylen); temp[keylen] = 0; shdel(ht->verify, temp); }
...
void stringhash_put(pphash *ht, char *key, size_t keylen, void *value)
{
  ...
  if (ht->verify) { assert(stringhash_get(ht, key, keylen) == NULL); }
  if (ht->verify) { char temp[1024]; memcpy(temp, key, keylen); temp[keylen] = 0; shput(ht->verify, temp, value); }
...
char *get_string(int n)
{
  ...
  if      (bottom_bits <  2) len = (len >> 2) + 3;
  else if (bottom_bits == 2) len = (len >> 1) + 3;
  else                       len =  len       + 3;
  if ((n>>18) > 0) ++len;
  len += 5;
...
#define char_is_argument_parse_action(x)   (pp_char_flags1[(uint8) (x)] & (CHAR1_IS_BALANCE_SCAN_ACTION | CHAR1_IS_QUOTE | CHAR1_IS_BACKSLASH | CHAR1_IS_NUL | CHAR1_IS_NEWLINE))
  ...
  for(;;) {
    ...
    if (state == PP_STATE_saw_backslash) {
      ...
      if (!char_is_newline(*p)) {
        ...
        cs->state_limit = PP_STATE_saw_cr;     // line_number mismatches, so need to output match newlines at first opportunity, namely next time we see a newline not in a comment
...
static char *preprocessor_skip_whitespace_in_directive(char *s, int *newline_count)
{
  for(;;) {
    ...
    if (*s == '/') {
      // skip comments
      if (s[1] == '/') goto scan_to_end_of_line;
      if (s[1] != '*')
        break;
...
static void maybe_expand_macro(parse_state *cs, macro_definition *pending, int in_macro_expansion, char *in_macro, macro_definition **md);

static void preprocess_string(parse_state *cs, int in_macro_expansion, char *in_macro, macro_definition **final_function_macro)
{
  ...
  if (in_macro_expansion == IN_MACRO_include_guard_scan) {
    ...
    if (0 != memcmp(p, "define", 6)) {
     handle_action:
      ...
      switch (action) {
        case PP_WHITESCAN_STATE_stop_seen_onechar : rewind_to_start_of_line(cs); break;
        case PP_WHITESCAN_STATE_stop_seen_twochars: rewind_to_start_of_line(cs); break;
        case PP_WHITESCAN_STATE_stop_eof          : break;
        case PP_WHITESCAN_STATE_error_unterminated_multiline_comment: goto unterminated_multline_comment; break;
      }
     done:
      ;
    } else {
  ...
  for (;;) {
    ...
    action = in_macro_expansion > IN_MACRO_no
                     ? copy_to_action_point_macro_expansion(cs)
                     : copy_to_action_point(cs);
...
static macro_definition *create_macro_definition(parse_state *ps, char *def)
{
  ...
  macro_definition *m = stb_arena_alloc(&c->macro_arena, sizeof(*m));
  ...
  if (*p != '(') {
    ...
    m->expansion = NULL;
    token_paste_before_text = 0; // @TODO: this used to be 1 to make sure we didn't generate a leading space, but this was wrong with reparsing. but may be problematic with nesting now?
    for(;;) {
...
// if it's followed by '('
static void maybe_expand_macro(parse_state *cs, macro_definition *pending, int in_macro_expansion, char *in_macro, macro_definition **final_function_macro)
{
  pp_context *c = cs->context;
  macro_definition *md;
  ...
  if (md->num_parameters < 0) {
    ...
    switch (md->num_parameters) {
      ...
      case MACRO_NUM_PARAMETERS_line: {
        if (cs->fast_dest)
          cs->fast_dest += sprintf(cs->fast_dest, "%u", cs->src_line_number);
        else {
          int len;
          q = identifier;
          arrsetcap(cs->dest, arrlen(cs->dest)+12);
          len = sprintf(cs->dest+arrlen(cs->dest), "%u", cs->src_line_number);
      ...
      case MACRO_NUM_PARAMETERS_counter: {
        ...
        if (cs->fast_dest)
          cs->fast_dest += sprintf(cs->fast_dest, "%u", c->counter);
        else {
          int len;
          q = identifier;
          arrsetcap(cs->dest, arrlen(cs->dest)+12);
          len = sprintf(cs->dest+arrlen(cs->dest), "%u", c->counter);
          arrsetlen(cs->dest, arrlen(cs->dest)+len);
        }
        ++c->counter;
        break;
    ...
    for (i=0; i < md->num_parameters; ++i) {
      ...
      if (*s == ',') {
        if (i < md->num_parameters-1)
          ...
          int err = do_error_code(cs, PP_RESULT_too_many_arguments_to_macro, "Too many arguments supplied to macro '%s'; only %d expected", md->symbol_name, md->num_parameters);
          if (err)
            return;
          parse_rest = 1;
        }
...

      {
        ...
        if (e-1 > p) {
          ...
          assert(e > p);
          if (e < p) e = p; // this should be impossible
        }
        *e = 0;
...
static char *resolve_file(char path[4096], parse_state *ps, ppbool check_source_dirs, ppbool check_normal_includes, ppbool check_system_includes, char *filename, ptrdiff_t filename_len);
...
static void process_directive(parse_state *cs, conditional_state *cons)
{
  ...
  switch (n & 31) { // mask to try to avoid range-checked switch
    ...
    case 14: case 16: case 17: case 19: case 20: case 21:
          ...
          if (!char_is_pp_identifier_first(*p)) {
            ...
            if (pp_result_mode[PP_RESULT_unrecognized_directive] != PP_RESULT_MODE_no_warning) {
              ...
              if (i < sizeof(identifier) && pp_result_mode[PP_RESULT_unrecognized_directive] <= did_you_mean_threshold) {
                match = find_closest_matching_string(identifier, directives, sizeof(directives)/sizeof(directives[0]),
                                                       i > 7 ? 2 + i-7 : 2); // if the string is long, allow more errors, e.g. they put junk on the end
              }
              if (match)
                do_error_code(cs, PP_RESULT_unrecognized_directive, "Unrecognized preprocessor directive '#%s', did you mean '%s'?", identifier, match);
              else
                do_error_code(cs, PP_RESULT_unrecognized_directive, "Unrecognized preprocessor directive '#%s'", identifier);
            }
          }
          if (alloc) arrfree(alloc);
          return;
      }
      break;
    }
      ...
      if (md != NULL) {
        ...
        if (pp_result_mode[PP_RESULT_undef_of_undefined_macro] != PP_RESULT_MODE_no_warning) {
          if (shget(c->undef_map, identifier)) {
            ...
            if (do_error_code(cs,  PP_RESULT_undef_of_undefined_macro, "Tried to #undef '%s', which is not a defined macro name. However, it was defined previously.", identifier))
              return;
            ...
            if (n > 1) {
              ...
              if (n >= 4) {
                ...
                for (i=0; i < arrlen(c->macro_map.pair); ++i) {
                  ...
                  if (s) {
                    if (s[0] && s[1] && s[2] && s[3] && s[0] != identifier[0] && s[1] != identifier[1] && s[2] != identifier[2] && s[3] != identifier[3])
                      ;
                    else
            ...
            if (dist <= min_dist ? do_error_code(cs,  PP_RESULT_undef_of_undefined_macro, "Tried to #undef '%s', which is not a defined macro name. Did you mean '%s'?", identifier, best)
...
//
    ...
    case HASH_ifdef: {
      ...
      if (*p != 0) {
        char cbuf[12];
        do_error_code(cs, PP_RESULT_unexpected_character_after_end_of_directive, "Expected end of line in #ifdef, but found character %s", error_char(*p, cbuf));
      }
      break;
    }
    ...
    handle_else_after_disabled_block:
    {
      ...
      if (*p != 0) {
        char cbuf[12];
        if (do_error_code(cs, PP_RESULT_unexpected_character_after_end_of_directive, "Expected end of line in #else, but found character %s.", error_char(*p, cbuf)))
          return;
      }
    ...
    case HASH_endif: {
      ...
      if (*p != 0) {
        char cbuf[12];
        if (do_error_code(cs, PP_RESULT_unexpected_character_after_end_of_directive, "Expected end of line in #endif, but found character %s.", error_char(*p, cbuf)))
          return;
      }
...
void init_preprocessor_scanner(void)
{
  ...
  for (i=0; i < 127; ++i) {
    if (isalpha(i)) pp_char_class[i] = PP_CHAR_CLASS_idtype;
    if (isdigit(i)) pp_char_class[i] = PP_CHAR_CLASS_digit;
    if (isspace(i)) pp_char_class[i] = PP_CHAR_CLASS_whitespace;
  }
...
macro_definition *pp_define(struct stb_arena *a, char *p)
{
  macro_definition *m;
  pp_context c = { 0 };
  parse_state ps = { 0 };
...
static void define_macro(pphash *map, macro_definition *m)
{
  stringhash_put(map, m->symbol_name, m->symbol_name_length, m);
}

char * preprocess_file(char *output_autobuffer, char *filename, char **include_paths, int num_include_paths, char **sys_include_paths, int num_sys_include_paths, macro_definition **predefined_macros, int num_predefined_macros, pp_diagnostic **pd, int *num_pd)
{
  char *output = 0;
  pp_context c = { 0 };
  int main_mode, i;
...
static stb_arena        resolve_arena;

static size_t max_cached_headers = sizeof(size_t) > 4
                                             ? (128U << 20) // 128MB in 64-bit
