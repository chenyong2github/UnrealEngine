...
//	FString SDKStatus = GetSectionString(Section, TEXT("SDKStatus"));
	FString TutorialPath = GetSectionString(Section, TEXT("TutorialPath"));
#if PLATFORM_WINDOWS
	bool bIsEnabled = GetSectionBool(Section, TEXT("bSupportsHostWindows"));
#elif PLATFORM_MAC
	bool bIsEnabled = GetSectionBool(Section, TEXT("bSupportsHostMac"));
#elif PLATFORM_LINUX
	bool bIsEnabled = GetSectionBool(Section, TEXT("bSupportsHostLinux"));
#else
	bool bIsEnabled = false;
// @todo platplug: for some reason this is being compiled on target devices! TargetPlatform pulls in DesktopPlatform, but can't figure out why TP is being pulled in
//#error Unknown Host Platform!
	...
	BuildPlatformInfo(Name, TargetPlatformName, FText::FromString(DisplayName), EPlatformTypeFromString(*PlatformType), ConvertPlatformFlags(PlatformFlags), FPlatformIconPaths(NormalIconPath, LargeIconPath, XLargeIconPath), UATCommandLine,
		AutoSDKPath, PlatformInfo::EPlatformSDKStatus::Unknown, TutorialPath, bIsEnabled, BinariesDirectoryName, IniPlatformName, bUsesHostCompiler, bUATClosesAfterLaunch, bIsConfidential, UBTTargetID, PlatformGroupName);
}
...
const TArray<FPlatformInfo>& GetPlatformInfoArray(bool bAccessiblePlatformsOnly)
{
	if (bAccessiblePlatformsOnly)
	{
		...
		if (!bHasSearchedForPlatforms)
		{
			...
			for (const FPlatformInfo& PlatformInfo : AllPlatformInfoArray)
			{
				...
				const FString IniFolderPath = FPaths::Combine(*FPaths::EngineConfigDir(), *PlatformInfo.IniPlatformName);
				const FString ExtIniFolderPath = FPaths::Combine(*FPaths::PlatformExtensionsDir(), *PlatformInfo.IniPlatformName, TEXT("Engine"), TEXT("Config"));
				IFileManager::Get().FindFiles(FoundFiles, *IniFolderPath);
				IFileManager::Get().FindFiles(FoundFiles, *ExtIniFolderPath);
...
TArray<FVanillaPlatformEntry> BuildPlatformHierarchy(const EPlatformFilter InFilter, bool bAccessiblePlatformsOnly)
{
	...
	for (const PlatformInfo::FPlatformInfo& PlatformInfo : GetPlatformInfoArray(bAccessiblePlatformsOnly))
	{
