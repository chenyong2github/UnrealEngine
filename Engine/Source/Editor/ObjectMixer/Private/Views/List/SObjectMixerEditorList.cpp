...
void SObjectMixerEditorList::RebuildList(const FString& InItemToScrollTo)
{
	...
	if (!InItemToScrollTo.IsEmpty())
	{
		...
		for (const FObjectMixerEditorListRowPtr& Item : TreeViewRootObjects)
		{
			if (false)
			{
				ScrollToItem = Item;
...
{
	for (TSharedPtr<FObjectMixerEditorListRow> Item : TreeViewPtr->GetSelectedItems())
	{
		Item->SetObjectVisibility(bNewIsVisible);
	}
}
...
TSharedPtr<SHeaderRow> SObjectMixerEditorList::GenerateHeaderRow()
{
	...
	for (SHeaderRow::FColumn Column : HeaderRow->GetColumns())
	{
