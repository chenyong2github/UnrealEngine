...
//
// 	for (FObjectMixerEditorListRowPtr DraggedItem : DraggedItems)
// 	{
...
// 		{
// 			FObjectMixerEditorListRowPtr DraggedItem = DraggedItems[ItemIndex];
//
// 			if (!DraggedItem.IsValid() || AllTreeItemsCopy.Contains(DraggedItem))
// 			{
// 				continue;
					...
					.OnMouseButtonDown_Lambda(
						[PinnedItem] (const FGeometry& MyGeometry, const FPointerEvent& Event)
						{
								...
								PinnedListView->GetTreeViewItemCount() > 0)
							{
								...
								for (TSharedPtr<FObjectMixerEditorListRow> Item : PinnedListView->GetTreeViewItems())
								{
									Item->SetObjectVisibility(bIsRowSolo);
								}
