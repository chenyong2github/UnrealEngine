...
{
	const FTreeItemType FActorFolderTreeItem::Type(&FFolderTreeItem::Type);

	struct SActorFolderTreeLabel : FCommonLabelData, public SCompoundWidget
	{
		SLATE_BEGIN_ARGS(SActorFolderTreeLabel) {}
		SLATE_END_ARGS()

		void Construct(const FArguments& InArgs, FActorFolderTreeItem& FolderItem, ISceneOutliner& SceneOutliner, const STableRow<FTreeItemPtr>& InRow)
		{
	...
	void FActorFolderTreeItem::Delete(FName InNewParentPath)
	{
		...
		for (const TWeakPtr<ITreeItem>& ChildPtr : GetChildren())
		{
			FTreeItemPtr Child = ChildPtr.Pin();
	...
	TSharedRef<SWidget> FActorFolderTreeItem::GenerateLabelWidget(ISceneOutliner& Outliner, const STableRow<FTreeItemPtr>& InRow)
	{
