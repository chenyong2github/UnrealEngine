...
namespace SceneOutliner
{
	...
	FActorBrowsingMode::FActorBrowsingMode(SSceneOutliner* InSceneOutliner, TWeakObjectPtr<UWorld> InSpecifiedWorldToDisplay)
		: FActorModeInteractive(InSceneOutliner, true, InSpecifiedWorldToDisplay)
		, FilteredActorCount(0)
	{
		...
		FOutlinerFilterInfo HideTemporaryActorsInfo(LOCTEXT("ToggleHideTemporaryActors", "Hide Temporary Actors"), LOCTEXT("ToggleHideTemporaryActorsToolTip", "When enabled, hides temporary/run-time Actors."), SharedSettings->bHideTemporaryActors, FCreateSceneOutlinerFilter::CreateStatic(&FActorBrowsingMode::CreateHideTemporaryActorsFilter));
	...
	TSharedRef<FOutlinerFilter> FActorBrowsingMode::CreateShowOnlySelectedActorsFilter()
	{
		auto IsActorSelected = [](const AActor* InActor)
		{
			return InActor && InActor->IsSelected();
		};
		return MakeShareable(new FActorFilter(FActorTreeItem::FFilterPredicate::CreateStatic(IsActorSelected), EDefaultFilterBehaviour::Fail, FActorTreeItem::FFilterPredicate::CreateStatic(IsActorSelected)));
	}

	TSharedRef<FOutlinerFilter> FActorBrowsingMode::CreateHideTemporaryActorsFilter()
	{
	...
	TSharedRef<FOutlinerFilter> FActorBrowsingMode::CreateIsInCurrentLevelFilter()
	{
		return MakeShareable(new FActorFilter(FActorTreeItem::FFilterPredicate::CreateStatic([](const AActor* InActor)
			{
				if (InActor->GetWorld())
	...
	TSharedRef<FOutlinerFilter> FActorBrowsingMode::CreateHideComponentsFilter()
	{
		return MakeShared<TOutlinerPredicateFilter<FComponentTreeItem>>(TOutlinerPredicateFilter<FComponentTreeItem>(FComponentTreeItem::FFilterPredicate::CreateStatic([](const UActorComponent* Component)
			{
				// Filter out all components
				return false;
			}), EDefaultFilterBehaviour::Pass));
	}
	...
	TSharedPtr<SWidget> FActorBrowsingMode::BuildContextMenu()
	{
		...
		FItemSelection ItemSelection(SceneOutliner->GetSelection());
	...
	void FActorBrowsingMode::OnItemAdded(FTreeItemPtr Item)
	{
	...
	void FActorBrowsingMode::OnItemRemoved(FTreeItemPtr Item)
	{
		if (const FActorTreeItem* ActorItem = Item->CastTo<FActorTreeItem>())
		{
	...
	void FActorBrowsingMode::OnItemSelectionChanged(FTreeItemPtr TreeItem, ESelectInfo::Type SelectionType, const FItemSelection& Selection)
	{
		TArray<AActor*> SelectedActors = Selection.GetData<AActor*>(FActorSelector());
	...
	void FActorBrowsingMode::OnItemDoubleClick(FTreeItemPtr Item)
	{
		if (Item->CanInteract())
		{
			if (Item->IsA<FActorTreeItem>())
			{
				FItemSelection Selection(SceneOutliner->GetSelection());
	...
	void FActorBrowsingMode::OnFilterTextCommited(FItemSelection& Selection, ETextCommit::Type CommitType)
	{
		...
		for (AActor* Actor : Selection.GetData<AActor*>(FActorSelector()))
		{
			const bool bShouldSelect = true;
	...
	void FActorBrowsingMode::OnItemPassesFilters(const ITreeItem& Item)
	{
		if (const FActorTreeItem* ActorItem = Item.CastTo<FActorTreeItem>())
		{
	...
	FReply FActorBrowsingMode::OnKeyDown(const FKeyEvent& InKeyEvent)
	{
		...
		if (InKeyEvent.GetKey() == EKeys::F2)
		{
			if (Selection.Num() == 1)
			{
				FTreeItemPtr ItemToRename = Selection.SelectedItems[0].Pin();
	...
	{
		const FItemSelection ItemSelection = SceneOutliner->GetSelection();
		const uint32 NumberOfFolders = ItemSelection.Num<FFolderTreeItem>();
		return (NumberOfFolders == 1 && NumberOfFolders == ItemSelection.Num());
	}

	bool FActorBrowsingMode::CanRenameItem(const ITreeItem& Item) const
	{
	...
	TSharedPtr<FDragDropOperation> FActorBrowsingMode::CreateDragDropOperation(const TArray<FTreeItemPtr>& InTreeItems) const
	{
		FDragDropPayload DraggedObjects(InTreeItems);
		...
		if (DraggedObjects.Has<FFolderTreeItem>())
		{
			...
			FolderOperation->Init(DraggedObjects.GetData<FName>(FFolderPathSelector()));
	...
	bool FActorBrowsingMode::ParseDragDrop(FDragDropPayload& OutPayload, const FDragDropOperation& Operation) const
	{
	...
	FDragValidationInfo FActorBrowsingMode::ValidateDrop(const ITreeItem& DropTarget, const FDragDropPayload& Payload) const
	{
		if (const FActorTreeItem* ActorItem = DropTarget.CastTo<FActorTreeItem>())
		{
			if (Payload.Has<FFolderTreeItem>())
			{
				return FDragValidationInfo(DropCompatibility::Drop_IncompatibleGeneric, LOCTEXT("FoldersOnActorError", "Cannot attach folders to actors"));
			}
			...
			const auto& DragActors = Payload.GetData<TWeakObjectPtr<AActor>>(FWeakActorSelector());
			for (const auto& DragActorPtr : DragActors)
			{
				...
				if (DragActor)
				{
					if (bCanAttach)
					{
						...
						if (!GEditor->CanParentActors(ActorTarget, DragActor, &AttachErrorMsg))
						{
							bCanAttach = false;
						}
					}
			...
			if (bDraggedOntoAttachmentParent)
			{
				if (DragActors.Num() == 1)
				{
					return FDragValidationInfo(DropCompatibility::Drop_CompatibleDetach, ActorLabel);
				}
				else
				{
					return FDragValidationInfo(DropCompatibility::Drop_CompatibleMultipleDetach, ActorLabel);
				}
			}
			else if (bCanAttach)
			{
				if (DragActors.Num() == 1)
				{
					return FDragValidationInfo(DropCompatibility::Drop_CompatibleAttach, ActorLabel);
				}
				else
				{
					return FDragValidationInfo(DropCompatibility::Drop_CompatibleMultipleAttach, ActorLabel);
				}
			}
			else
			{
				if (DragActors.Num() == 1)
				{
					return FDragValidationInfo(DropCompatibility::Drop_IncompatibleGeneric, AttachErrorMsg);
				}
				else
				{
					const FText ReasonText = FText::Format(LOCTEXT("DropOntoText", "{0}. {1}"), ActorLabel, AttachErrorMsg);
					return FDragValidationInfo(DropCompatibility::Drop_IncompatibleMultipleAttach, ReasonText);
				}
			}
		}
		else if (DropTarget.IsA<FFolderTreeItem>() || DropTarget.IsA<FWorldTreeItem>())
		{
			...
			if (Payload.Has<FFolderTreeItem>())
			{
				...
				for (FName DraggedFolder : Payload.GetData<FName>(FFolderPathSelector()))
				{
					const FName Leaf = GetFolderLeafName(DraggedFolder);
					const FName Parent = GetParentPath(DraggedFolder);
					...
					if (Parent == DestinationPath)
					{
						...
						return FDragValidationInfo(DropCompatibility::Drop_IncompatibleGeneric, Text);
					}
					...
					if (FActorFolders::Get().GetFolderProperties(*RepresentingWorld, FName(*NewPath)))
					{
						...
						return FDragValidationInfo(DropCompatibility::Drop_IncompatibleGeneric,
					...
					else if (DragFolderPath == DstFolderPath || DstFolderPath.StartsWith(DragFolderPath + "/"))
					{
						...
						return FDragValidationInfo(
							DropCompatibility::Drop_IncompatibleGeneric,
			...
			if (Payload.Has<FActorTreeItem>())
			{
				...
				for (const TWeakObjectPtr<AActor>& WeakActor : Payload.GetData<TWeakObjectPtr<AActor>>(FWeakActorSelector()))
				{
					...
					{
						return FDragValidationInfo(DropCompatibility::Drop_IncompatibleGeneric, FText::Format(LOCTEXT("Error_AttachChildActor", "Cannot move {0} as it is a child actor."), FText::FromString(Actor->GetActorLabel())));
					}
			...
			if (DestinationPath.IsNone())
			{
				return FDragValidationInfo(DropCompatibility::Drop_CompatibleGeneric, LOCTEXT("MoveToRoot", "Move to root"));
			}
			else
			{
				FFormatNamedArguments Args;
				Args.Add(TEXT("DestPath"), FText::FromName(DestinationPath));
				return FDragValidationInfo(DropCompatibility::Drop_CompatibleGeneric, FText::Format(LOCTEXT("MoveInto", "Move into \"{DestPath}\""), Args));
			}
		}
	...
	void FActorBrowsingMode::OnDrop(ITreeItem& DropTarget, const FDragDropPayload& Payload, const FDragValidationInfo& ValidationInfo) const
	{
		if (const FActorTreeItem* ActorItem = DropTarget.CastTo<FActorTreeItem>())
		{
			...
			if (ValidationInfo.CompatibilityType == DropCompatibility::Drop_CompatibleMultipleDetach || ValidationInfo.CompatibilityType == DropCompatibility::Drop_CompatibleDetach)
			{
			...
			else if (ValidationInfo.CompatibilityType == DropCompatibility::Drop_CompatibleMultipleAttach || ValidationInfo.CompatibilityType == DropCompatibility::Drop_CompatibleAttach)
			{
		...
		else if (DropTarget.IsA<FFolderTreeItem>() || DropTarget.IsA<FWorldTreeItem>())
		{
			...
			if (Payload.Has<FActorTreeItem>())
			{
				...
				for (const auto& WeakActor : Payload.GetData<TWeakObjectPtr<AActor>>(FWeakActorSelector()))
				{
	...
	bool FActorBrowsingMode::ReparentItemToFolder(const FName& FolderPath, const FTreeItemPtr& Item)
	{
	...
	void FActorBrowsingMode::SelectFoldersDescendants(const TArray<FFolderTreeItem*>& FolderItems, bool bSelectImmediateChildrenOnly)
	{
		...
		TFunctionRef<void(const FTreeItemPtr&)> RecursiveExpand = [&](const FTreeItemPtr&  Item)
		{
		...
		TFunctionRef<void(const FTreeItemPtr&)> RecursiveActorSelect = [&](const FTreeItemPtr& Item)
		{
			...
			for (const TWeakPtr<ITreeItem> Child : Item->GetChildren())
			{
				...
				if (!bSelectImmediateChildrenOnly)
				{
					for (const TWeakPtr<ITreeItem>& Grandchild : Child.Pin()->GetChildren())
					{
	...
	FCreateOutlinerMode FActorBrowsingMode::CreateFolderPickerMode() const
	{
		auto MoveSelectionTo = [this](const FTreeItemRef& NewParent)
		{
		...
		return FCreateOutlinerMode::CreateLambda([this, MoveSelectionTo](SceneOutliner::SSceneOutliner* Outliner)
			{
	...
	{
		// Only a callback in actor browsing mode
		const TArray<FName> SelectedFolderPaths = SceneOutliner->GetSelection().GetData<FName>(FFolderPathSelector());
