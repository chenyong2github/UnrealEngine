...
{
	using FActorFilter = TOutlinerPredicateFilter<FActorTreeItem>;

	bool FWeakActorSelector::operator()(const TWeakPtr<ITreeItem>& Item, TWeakObjectPtr<AActor>& DataOut) const
	{
	...
	bool FActorSelector::operator()(const TWeakPtr<ITreeItem>& Item, AActor*& ActorPtrOut) const
	{
		if (FActorTreeItem* ActorItem = Item.Pin()->CastTo<FActorTreeItem>())
		{
			if (ActorItem->IsValid())
	...
	void FActorMode::SynchronizeActorSelection()
	{
		...
		const FItemSelection Selection(SceneOutliner->GetSelection());
		auto DeselectActors = [this](FActorTreeItem& Item)
		{
			if (!Item.Actor.IsValid() || !Item.Actor.Get()->IsSelected())
			{
				SceneOutliner->RemoveFromSelection(Item.AsShared());
			}
		};
		...
		if (!Selection.Has<FComponentTreeItem>())
		{
			...
			for (FSelectionIterator SelectionIt(*SelectedActors); SelectionIt; ++SelectionIt)
			{
				...
				if (FTreeItemPtr ActorItem = SceneOutliner->GetTreeItem(Actor))
				{
	...
	void FActorMode::OnFilterTextChanged(const FText& InFilterText)
	{
		...
		if (const AActor* LastSelectedActor = GEditor->GetSelectedActors()->GetBottom<AActor>())
		{
			...
			const FTreeItemPtr TreeItem = SceneOutliner->GetTreeItem(LastSelectedActor);
	...
	int32 FActorMode::GetTypeSortPriority(const ITreeItem& Item) const
	{
