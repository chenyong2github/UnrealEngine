...
namespace SceneOutliner
{
	...
	public:
		...
		virtual FCreateOutlinerMode CreateFolderPickerMode() const override;
		virtual void CreateViewContent(FMenuBuilder& MenuBuilder) override;
		virtual TSharedPtr<FDragDropOperation> CreateDragDropOperation(const TArray<FTreeItemPtr>& InTreeItems) const override;
		virtual bool ParseDragDrop(FDragDropPayload& OutPayload, const FDragDropOperation& Operation) const override;
		virtual FDragValidationInfo ValidateDrop(const ITreeItem& DropTarget, const FDragDropPayload& Payload) const override;
		virtual TSharedPtr<SWidget> CreateContextMenu() override;
		virtual void OnItemAdded(FTreeItemPtr Item) override;
		virtual void OnItemRemoved(FTreeItemPtr Item) override;
		...
		virtual void OnItemDoubleClick(FTreeItemPtr Item) override;
		virtual void OnFilterTextCommited(FItemSelection& Selection, ETextCommit::Type CommitType) override;
		virtual void OnItemPassesFilters(const ITreeItem& Item) override;
		virtual FReply OnKeyDown(const FKeyEvent& InKeyEvent) override;
		virtual void OnDuplicateSelected() override;
		virtual void OnDrop(ITreeItem& DropTarget, const FDragDropPayload& Payload, const FDragValidationInfo& ValidationInfo) const override;
		virtual bool CanRenameItem(const ITreeItem& Item) const override;
		...
		virtual bool ReparentItemToFolder(const FName& FolderPath, const FTreeItemPtr& Item) override;
	...
	private:
		...
		/** Filter factories */
		static TSharedRef<FOutlinerFilter> CreateShowOnlySelectedActorsFilter();
		static TSharedRef<FOutlinerFilter> CreateHideTemporaryActorsFilter();
		static TSharedRef<FOutlinerFilter> CreateIsInCurrentLevelFilter();
		static TSharedRef<FOutlinerFilter> CreateHideComponentsFilter();
	private:
