...
namespace SceneOutliner
{
	...
	public:
		...
		virtual FCreateOutlinerMode CreateFolderPickerMode() const { return FCreateOutlinerMode(); }
	public:
		/** Construct a new Drag and drop operation */
		virtual TSharedPtr<FDragDropOperation> CreateDragDropOperation(const TArray<FTreeItemPtr>& InTreeItems) const { return nullptr; }
		 ...
		 */
		virtual bool ParseDragDrop(FDragDropPayload& OutPayload, const FDragDropOperation& Operation) const { return false; /* Default does not support drag/drop */ }

		/** Test whether the specified payload can be dropped onto this tree item */
		virtual FDragValidationInfo ValidateDrop(const ITreeItem& DropTarget, const FDragDropPayload& Payload) const { return FDragValidationInfo::Invalid(); }

		/** Called when a payload is dropped onto a target */
		virtual void OnDrop(ITreeItem& DropTarget, const FDragDropPayload& Payload, const FDragValidationInfo& ValidationInfo) const {}

		/** Called when a payload is dragged over an item */
		virtual FReply OnDragOverItem(const FDragDropEvent& Event, const ITreeItem& Item) const { return FReply::Handled(); }
	public:
		/* Events */

		/** Called by the outliner when a new item is added to the outliner tree */
		virtual void OnItemAdded(FTreeItemPtr Item) {}
		/** Called by the outliner when an item is removed from the outliner tree */
		virtual void OnItemRemoved(FTreeItemPtr Item) {}
		/** Called by the outliner when an item is selected in the tree. Used to partially synchronize selection */
		virtual void OnItemSelectionChanged(FTreeItemPtr Item, ESelectInfo::Type SelectionType, const FItemSelection& Selection) {}
		/** Called by the outliner when a tree item is double clicked */
		virtual void OnItemDoubleClick(FTreeItemPtr Item) {}
		...
		virtual void OnFilterTextCommited(FItemSelection& Selection, ETextCommit::Type CommitType) {}
		/** Called by the outliner when an item passes filters but isn't yet added to the tree */
		virtual void OnItemPassesFilters(const ITreeItem& Item) {}
		...
		virtual bool CanRenameItem(const ITreeItem& Item) const { return false; }
	...
	public:
		...
		virtual int32 GetTypeSortPriority(const ITreeItem& Item) const { return 0; }
	...
	public:
		...
		virtual bool ReparentItemToFolder(const FName& FolderPath, const FTreeItemPtr& Item) { return false; }
	...
	public:
		...
		TMap<FName, FOutlinerFilterInfo>& GetFilterInfos() { return FilterInfoMap; }
	protected:
		virtual TUniquePtr<ISceneOutlinerHierarchy> CreateHierarchy() = 0;
	protected:
		TMap<FName, FOutlinerFilterInfo> FilterInfoMap;
