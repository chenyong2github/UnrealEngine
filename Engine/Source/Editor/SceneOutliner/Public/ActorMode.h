...
namespace SceneOutliner
{
	...
	{
		bool operator()(const TWeakPtr<ITreeItem>& Item, TWeakObjectPtr<AActor>& DataOut) const;
	};

	/** Functor which can be used to get actors from a selection including component parents */
	struct FActorSelector
	{
		bool operator()(const TWeakPtr<ITreeItem>& Item, AActor*& ActorPtrOut) const;
	};
	...
	public:
		...
		virtual int32 GetTypeSortPriority(const ITreeItem& Item) const override;
	private:
