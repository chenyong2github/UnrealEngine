...
UK2Node::ERedirectType UK2Node::DoPinsMatchForReconstruction(const UEdGraphPin* NewPin, int32 NewPinIndex, const UEdGraphPin* OldPin, int32 OldPinIndex) const
{
	...
	{
		const bool bPinsAreTypeCompatible = K2Schema->ArePinTypesCompatible(OldPin->PinType, NewPin->PinType);
		...
		else
		{
			...
			else if (K2Schema->SearchForAutocastFunction(OldPin->PinType, NewPin->PinType).IsSet())
			{
				RedirectType = ERedirectType_Type;
			}
		}
	}
	else
	...
	for (int32 OldPinIndex = InOldPins.Num()-1; OldPinIndex >= 0; --OldPinIndex)
	{
		...
		for (int32 NewPinCount = NumNewPins - 1; NewPinCount >= 0; --NewPinCount)
		{
			...
			if (!NewPinMatched[NewPinIndex])
			{
				...
				const bool bShouldReconstruct = (RedirectType != ERedirectType_None);
				const bool bNeedsConversion = (RedirectType & ERedirectType_Type);
				...
				if (bShouldReconstruct)
				{
					...
					if (bNeedsConversion)
					{
						...
						TArray<UEdGraphPin*> LinkedToCopy = NewPin->LinkedTo;
						for (UEdGraphPin* LinkedPin : LinkedToCopy)
						{
							// A linked pin could've had its type updated independently of ours.
							// In some cases, it might've been changed to the same type, which alleviates the need for a cast.
							if (NewPin->PinType != LinkedPin->PinType)
							{
								FFormatNamedArguments Args;

								Args.Add(TEXT("PinName"), FText::FromName(NewPin->GetFName()));

								if (Schema->CreateAutomaticConversionNodeAndConnections(NewPin, LinkedPin))
								{
									FText ConversionInfo = FText::Format(
										NSLOCTEXT("K2Node", "PinConversion_InfoFmt", "Pin '{PinName}' on node @@ changed type. A conversion node had to be inserted into the graph."),
										Args
									);
								...
								{
									Args.Add(TEXT("LinkName"), FText::FromName(LinkedPin->GetFName()));

									FText ErrorInfo = FText::Format(
										NSLOCTEXT("K2Node", "PinConversion_ErrorFmt", "Pin '{PinName}' on node @@ changed type, but no suitable conversion node could be found for link '{LinkName}'!"),
										Args
									);

									Message_Error(ErrorInfo.ToString(), this);
								}
							}
						}
					}
