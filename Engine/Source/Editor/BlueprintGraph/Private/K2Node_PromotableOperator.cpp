...
void UK2Node_PromotableOperator::EvaluatePinsFromChange(UEdGraphPin* ChangedPin)
{
	...
	auto HasDeterminingComparisonTypes = [](const TArray<UEdGraphPin*>& PinsToCheck) -> bool
	{
		// For comparison ops, we only want to check the input pins as the output
		// will always be a bool
		for(const UEdGraphPin* Pin : PinsToCheck)
		{
	...
	if ((bWasAFullDisconnect && !HasAnyConnectionsOrDefaults()) || (bIsComparisonOp && !HasDeterminingComparisonTypes(Pins)))
	{
		ResetNodeToWildcard();
		return;
	}
