		...
		if (Schema->ConvertPropertyToPinType(Param, /* out */ ParamType) && ArgCount >= 0 && ArgCount < Pins.Num())
		{
			...
			if (bNeedsTypeUpdate)
			{
				...
				if (bIsWildcard && PinToChange->LinkedTo.Num() > 0)
				{
					...
					// If this is a wildcard pin, then check the compatibility with a linked type instead of the wildcard itself
					if (FTypePromotion::IsValidPromotion(bIsWildcard ? LinkedType : PinToChange->PinType, ParamType))
					{
