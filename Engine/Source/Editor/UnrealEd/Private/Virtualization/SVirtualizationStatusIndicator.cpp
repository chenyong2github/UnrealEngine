...
private:
	...
	int64 PayloadsUploaded = 0; // Covers both pushing to persistent backends and caching to local ones	

	FCurveSequence FadePullArrow;
	FCurveSequence FadeUploadArrow;
};
...
FText SVirtualizationToolTip::GetToolTipText() const
{
	...
	Builder << TEXT("Cached: ") << Info.Cache.TotalBytes / (int64)(1024 * 1024) << TEXT(" MB in ");
	Builder.Appendf(TEXT("%.3f Seconds\n"), Info.Cache.CyclesSpent * FPlatformTime::GetSecondsPerCycle());

	Builder << TEXT("Pushed: ") << Info.Push.TotalBytes / (int64)(1024 * 1024) << TEXT(" MB in ");
	Builder.Appendf(TEXT("%.3f Seconds\n"), Info.Push.CyclesSpent * FPlatformTime::GetSecondsPerCycle());

	Builder << TEXT("Pulled: ") << Info.Pull.TotalBytes / (int64)(1024 * 1024) << TEXT(" MB in ");
	Builder.Appendf(TEXT("%.3f Seconds"), Info.Pull.CyclesSpent * FPlatformTime::GetSecondsPerCycle());
...
void SVirtualizationStatusIndicator::Construct(const FArguments& InArgs)
{
	...
	FPayloadActivityInfo Info = FVirtualizationManager::Get().GetPayloadActivityInfo();
	PayloadsPulled = Info.Pull.PayloadCount;
	PayloadsUploaded = Info.Push.PayloadCount + Info.Cache.PayloadCount;

	FadeUploadArrow = FCurveSequence(0.0f, ArrowFadeTime, ECurveEaseFunction::Linear);
	...
	[
		...
		[
			...
			[
				...
				.ColorAndOpacity_Lambda([this] { return FLinearColor::Green.CopyWithNewOpacity(FadeUploadArrow.GetLerp()); })
			]
...
EActiveTimerReturnType SVirtualizationStatusIndicator::UpdateLastDataAccess(double InCurrentTime, float InDeltaTime)
{
	...
	{
		FadePullArrow.PlayRelative(this->AsShared(), PayloadsPulled != Info.Pull.PayloadCount);
		PayloadsPulled = Info.Pull.PayloadCount;
	}

	if (!FadeUploadArrow.IsPlaying())
	{
		int64 NewPayloadsUploaded = Info.Push.PayloadCount + Info.Cache.PayloadCount;

		FadeUploadArrow.PlayRelative(this->AsShared(), PayloadsUploaded != NewPayloadsUploaded);
		PayloadsUploaded = NewPayloadsUploaded;
	}
