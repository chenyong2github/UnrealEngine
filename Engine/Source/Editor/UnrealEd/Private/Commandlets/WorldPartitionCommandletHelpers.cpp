...
namespace WorldPartitionCommandletHelpers
{
	...
	{
		UE_LOG(LogWorldParitionCommandletUtils, Verbose, TEXT("Adding Package %s."), *SourceControlHelpers::PackageFilename(PackagesToCheckout));
	...
	bool Save(const TArray<UPackage*>& PackagesToSave, FPackageSourceControlHelper& SCHelper)
	{
		...
		{
			if (!Save(PackagesToSave, SCHelper))
			{
				return false;
			}
		}	
	...
	bool Save(UPackage* PackageToSave, FPackageSourceControlHelper& SCHelper)
	{
	...
	bool Delete(const TArray<UPackage*>& PackagesToDelete, FPackageSourceControlHelper& SCHelper)
	{
		...
		{
			if (!Delete(PackageToDelete, SCHelper))
			{
				return false;
			}
		}
