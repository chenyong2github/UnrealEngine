...
void UDataLayerConversionInfo::SetDataLayerToConvert(const TObjectPtr<UDataLayer>& InDataLayerToConvert)
{
	DataLayerToConvert = InDataLayerToConvert;

	DataLayerAsset->DataLayerType = DataLayerToConvert->IsRuntime() ? EDataLayerType::Runtime : EDataLayerType::Editor;
	DataLayerAsset->DebugColor = DataLayerToConvert->GetDebugColor();
}

void UDataLayerConversionInfo::SetDataLayerInstance(const TObjectPtr<UDataLayerInstance>& InDataLayerInstance)
{
	...
	if (DataLayerToConvert != nullptr)
	{
		...
		DataLayerInstance->bIsVisible = DataLayerToConvert->IsVisible();
		DataLayerInstance->bIsInitiallyVisible = DataLayerToConvert->IsInitiallyVisible();
		DataLayerInstance->bIsInitiallyLoadedInEditor = DataLayerToConvert->IsInitiallyLoadedInEditor();
		DataLayerInstance->bIsLocked = DataLayerToConvert->IsLocked();
		DataLayerInstance->InitialRuntimeState = DataLayerToConvert->GetInitialRuntimeState();
	}
	else
	{
...
UDataLayerConversionInfo* UDataLayerToAssetCommandletContext::GetDataLayerConversionInfo(const TObjectPtr<UDataLayer>& DataLayer) const
{
...
UDataLayerConversionInfo* UDataLayerToAssetCommandletContext::GetDataLayerConversionInfo(const TObjectPtr<UDataLayerInstance>& DataLayerInstance) const
{
...
UDataLayerConversionInfo* UDataLayerToAssetCommandletContext::GetDataLayerConversionInfo(const FActorDataLayer& ActorDataLayer) const
{
	...
	{ 
		return Other->DataLayerToConvert != nullptr && ActorDataLayer.Name == Other->DataLayerToConvert->GetFName();
	});
...
UDataLayerConversionInfo* UDataLayerToAssetCommandletContext::StoreDataLayerAssetConversion(const TObjectPtr<UDataLayer>& DataLayerToConvert, TObjectPtr<UDataLayerAsset>& NewDataLayerAsset)
{
	if (UDataLayerConversionInfo* ConversionInfo = GetDataLayerConversionInfo(DataLayerToConvert))
	{
		UE_LOG(LogDataLayerToAssetCommandlet, Error, TEXT("Failed to assign asset %s to data Layer %s. The data layer is already associated to asset %s."), 
			*NewDataLayerAsset->GetFullName() , *DataLayerToConvert->GetDataLayerLabel().ToString() , *ConversionInfo->DataLayerAsset->GetFullName());
		return nullptr;
	}
	...
	if (ConversionInfo != nullptr)
	{
		if (ConversionInfo->DataLayerToConvert != nullptr)
		{
			UE_LOG(LogDataLayerToAssetCommandlet, Error, TEXT("Failed to assign asset %s to data Layer %s. The asset is already associated to data layer %s."),
				*NewDataLayerAsset->GetFullName(), *DataLayerToConvert->GetDataLayerLabel().ToString(), *ConversionInfo->DataLayerToConvert->GetDataLayerLabel().ToString());
			return nullptr;
		}
	}
	else
	...
	UE_LOG(LogDataLayerToAssetCommandlet, Log, TEXT("Data Layer Asset %s is associated to Data Layer %s for conversion"),
		*ConversionInfo->DataLayerAsset->GetFullName(), *ConversionInfo->DataLayerToConvert->GetDataLayerLabel().ToString());
	return ConvertingDataLayerInfo.Add_GetRef(ConversionInfo).Get();
}

UDataLayerConversionInfo* UDataLayerToAssetCommandletContext::StoreDataLayerInstanceConversion(const UDataLayerAsset* DataLayerAsset, UDataLayerInstance* NewDataLayerInstance)
{
	if (UDataLayerConversionInfo* ConversionInfo = GetDataLayerConversionInfo(NewDataLayerInstance))
	{
		UE_LOG(LogDataLayerToAssetCommandlet, Error, TEXT("Failed to assign asset %s to data layer instance %s. The instance is already associated to asset %s."),
			*DataLayerAsset->GetFullName(), *NewDataLayerInstance->GetName(), *ConversionInfo->DataLayerAsset->GetFullName());
		return nullptr;
	}
	...
	if (UDataLayerConversionInfo* ConversionInfo = GetDataLayerConversionInfo(DataLayerAsset)) 
	{
		...
		UE_LOG(LogDataLayerToAssetCommandlet, Log, TEXT("Data Layer Instance %s is associated to Data Layer Asset %s for conversion"),
			*ConversionInfo->DataLayerInstance->GetName(), *ConversionInfo->DataLayerAsset->GetFullName());
	...
	UE_LOG(LogDataLayerToAssetCommandlet, Error, TEXT("Failed to assign asset %s to data layer instance %s. The asset was not retrieved in conversion data."),
		*DataLayerAsset->GetFullName(), *NewDataLayerInstance->GetName());
	return nullptr;
}
...
bool UDataLayerToAssetCommandletContext::FindDataLayerConversionInfos(FName DataLayerAssetName, TArray<TWeakObjectPtr<UDataLayerConversionInfo>>& OutConversionInfos) const
{
	...
	{
		if (ConversionInfo->DataLayerAsset->GetFName() == DataLayerAssetName)
		{
	...
	if (LogDataLayerToAssetCommandlet.GetVerbosity() >= ELogVerbosity::Verbose)
	{
		for (const TObjectPtr<UDataLayerConversionInfo>& info : DataLayerConversionInfo)
		{
			...
			UE_LOG(LogDataLayerToAssetCommandlet, Verbose, TEXT("[Conversion Info] Data Layer %s\t\tData Layer Asset: %s\t\t\t\t\t\tData Layer Instance: %s\t\tConverting By: %s\t\t\t\t\t\tConflicting Previous Conversion: %s"),
				info->DataLayerToConvert != nullptr ? *info->DataLayerToConvert->GetDataLayerLabel().ToString() : TEXT("None"),
				info->DataLayerAsset != nullptr ? *info->DataLayerAsset->GetFullName() : TEXT("None"),
				info->DataLayerInstance != nullptr ? *info->DataLayerInstance->GetName() : TEXT("None"),
...
bool UDataLayerToAssetCommandlet::BuildConversionInfos(TStrongObjectPtr<UDataLayerToAssetCommandletContext>& CommandletContext)
{
	...
	TArray<TObjectPtr<UDataLayerInstance>> DataLayerInstances;
	Algo::Copy(WorldDataLayers->DataLayerInstances, DataLayerInstances);
	for (int32 i = DataLayerInstances.Num() -1; i >= 0; --i)
	{
		TObjectPtr<UDataLayerInstance>& DataLayerInstance = DataLayerInstances[i];
		if (UDataLayerConversionInfo* ConversionInfo = CommandletContext->GetDataLayerConversionInfo(DataLayerInstance->GetAsset()))
		{
	...
	uint32 ErrorCount = 0;
	for (const TObjectPtr<UDataLayer>& DataLayer : WorldDataLayers->WorldDataLayers)
	{
		if (!CreateConversionFromDataLayer(CommandletContext, DataLayer))
		{
			ErrorCount++;
		}
	}
...
bool UDataLayerToAssetCommandlet::CreateConversionFromDataLayer(TStrongObjectPtr<UDataLayerToAssetCommandletContext>& CommandletContext, const TObjectPtr<UDataLayer> & DataLayer)
{
	if(TObjectPtr<UDataLayerAsset> DataLayerAsset = GetOrCreateDataLayerAssetForConversion(CommandletContext, DataLayer->GetDataLayerLabel()))
	{
...
TObjectPtr<UDataLayerAsset> UDataLayerToAssetCommandlet::GetOrCreateDataLayerAssetForConversion(TStrongObjectPtr<UDataLayerToAssetCommandletContext>& CommandletContext, FName DataLayerAssetName)
{
	...
	if (UObject* Asset = AssetTools.CreateAsset(DataLayerAssetName.ToString(), ConversionFolder, UDataLayerAsset::StaticClass(), DataLayerFactory))
	{
...
bool UDataLayerToAssetCommandlet::RemapActorDataLayersToAssets(TStrongObjectPtr<UDataLayerToAssetCommandletContext>& CommandletContext)
{
	...
	FWorldPartitionHelpers::ForEachActorWithLoading(MainWorld->GetWorldPartition(), AActor::StaticClass(),
		[WorldDataLayersActor, &ErrorCount, &CommandletContext, this](const FWorldPartitionActorDesc* ActorDesc)
		{
			...
			{
				const TArray<FName>& ActDescDataLayers = ActorDesc->GetDataLayers();
...
uint32 UDataLayerToAssetCommandlet::RemapActorDataLayers(TStrongObjectPtr<UDataLayerToAssetCommandletContext>& CommandletContext, AActor* Actor)
{
	...
	for (int32 i = ActorDataLayers.Num() - 1; i >= 0; --i)
	{
		...
		if (UDataLayerConversionInfo* DataLayerConversionInfo = CommandletContext->GetDataLayerConversionInfo(ActorDataLayer))
		{
			...
			if (Actor->AddDataLayer(DataLayerConversionInfo->DataLayerAsset))
			{
				if (!Actor->RemoveDataLayer(DataLayerConversionInfo->DataLayerToConvert))
				{
					...
					UE_LOG(LogDataLayerToAssetCommandlet, Error, TEXT("Failed to remove data layer %s in Actor %s"),
						*DataLayerConversionInfo->DataLayerToConvert->GetDataLayerLabel().ToString(), *Actor->GetName());
				}
			}
			else
			{
...
uint32 UDataLayerToAssetCommandlet::RemapDataLayersAssetsFromPreviousConversions(TStrongObjectPtr<UDataLayerToAssetCommandletContext>& CommandletContext, AActor* Actor)
{
	...
	for (int32 i = ActorDataLayerAssets.Num() - 1; i >= 0; --i)
	{
		...
		if (UDataLayerConversionInfo* DataLayerConversionInfo = CommandletContext->GetDataLayerConversionInfo(ActorDataLayerAsset.Get()))
		{
			...
			{
				if (Actor->AddDataLayer(DataLayerConversionInfo->GetCurrentConversion()->DataLayerAsset))
				{
...
{
	UE_SCOPED_TIMER(TEXT("Updating WorldDataLayers Actor"), LogDataLayerToAssetCommandlet, Display);
	...
	for (const TWeakObjectPtr<UDataLayerConversionInfo>& ConvertingInfo : CommandletContext->GetConvertingDataLayerConversionInfo())
	{
		...
		{
			UDataLayerInstance* DataLayerInstance = WorldDataLayers->CreateDataLayer(ConvertingInfo->DataLayerAsset.Get());
			UE_LOG(LogDataLayerToAssetCommandlet, Log, TEXT("Created new Data Layer Instance %s"),
				*DataLayerInstance->GetName());

			CommandletContext->StoreDataLayerInstanceConversion(ConvertingInfo->DataLayerAsset, DataLayerInstance);
		}

		WorldDataLayers->DeprecatedDataLayerNameToDataLayerInstance.Add(ConvertingInfo->DataLayerToConvert->GetFName(), ConvertingInfo->DataLayerInstance);
	}
		...
		if (ConversionInfo->DataLayerToConvert != nullptr)
		{
			if (WorldDataLayers->RemoveDataLayer(ConversionInfo->DataLayerToConvert))
			{
				UE_LOG(LogDataLayerToAssetCommandlet, Log, TEXT("Deleted old data layer %s, it is now converted to Data Layer Asset %s and Data Layer Instance %s"),
					*ConversionInfo->DataLayerToConvert->GetDataLayerLabel().ToString(), *ConversionInfo->DataLayerAsset->GetFullName(), *ConversionInfo->DataLayerInstance->GetName());
			}
			else
			{
				UE_LOG(LogDataLayerToAssetCommandlet, Error, TEXT("Failed to delete converting data layer %s."),
					*ConversionInfo->DataLayerToConvert->GetDataLayerLabel().ToString());
				ErrorCount++;
			}
		}
	}
