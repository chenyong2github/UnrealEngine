	...
	const TSet<FName>& CategoriesFromBlueprints, TSet<FName>& CategoriesFromProperties, TArray<FName>& SortedCategories)
{
	...
	// 
	if (CurrentProperty->HasMetaData(Name_InlineEditConditionToggle))
	{
		bHidden = true;
	}

	bool bMetaDataAllowVisible = true;
	const FString& MetaDataVisibilityCheckString = CurrentProperty->GetMetaData(Name_bShowOnlyWhenTrue);
	...
	{
		const bool bShowIfNonHiddenEditableProperty = CurrentProperty->HasAnyPropertyFlags(CPF_Edit) && !bHidden;
		const bool bShowIfDisableEditOnInstance = !CurrentProperty->HasAnyPropertyFlags(CPF_DisableEditOnInstance) || bShouldShowDisableEditOnInstance;
