...
static void GetPropertyRowRowNumbersRecursive(const TArray<TSharedRef<FDetailTreeNode>>& TreeNodes, int32& CurrentRowNumber, TArray<TPair<int32, FPropertyPath>>& OutResults)
{
	for (const TSharedRef<FDetailTreeNode>& TreeNode : TreeNodes)
	{
		...
		if (!TreeNode->IsLeaf() && TreeNode->ShouldBeExpanded())
		{
			TArray<TSharedRef<FDetailTreeNode>> Children;
			TreeNode->GetChildren(Children);
			GetPropertyRowRowNumbersRecursive(Children, CurrentRowNumber, OutResults);
		}
	}
}
...
TArray<TPair<int32, FPropertyPath>> SDetailsViewBase::GetPropertyRowNumbers() const
{
	...
	int32 OverallIdx = 0;
	GetPropertyRowRowNumbersRecursive(RootTreeNodes, OverallIdx, Results);
	return Results;
}

void SDetailsViewBase::ScrollLockDetailsViews(TSharedRef<IDetailsView> OtherDetailsView,
	const TAttribute<FInvertiblePiecewiseLinearFunction>& ScrollRate)
{
	const TSharedPtr<SDetailTree> NewDetailTree = DetailTree;
	const TSharedPtr<SDetailTree> OldDetailTree = StaticCastSharedRef<SDetailsViewBase>(OtherDetailsView)->DetailTree;
	if (NewDetailTree && OldDetailTree)
	{
		const TSharedPtr<FScrollLockedTableView> ScrollLock = MakeShared<FScrollLockedTableView>(NewDetailTree, OldDetailTree, ScrollRate);
		NewDetailTree->SetScrollLockedTable(ScrollLock);
		OldDetailTree->SetScrollLockedTable(ScrollLock);
	}
}

static void SetCategoryExpansionSavingRecursive(const TArray<TSharedRef<FDetailTreeNode>>& TreeNodes, bool bShouldCategoriesSaveExpansion)
{
	for (const TSharedRef<FDetailTreeNode>& TreeNode : TreeNodes)
	{
		if (!TreeNode->IsLeaf())
		{
			TArray<TSharedRef<FDetailTreeNode>> Children;
			TreeNode->GetChildren(Children);
			SetCategoryExpansionSavingRecursive(Children, bShouldCategoriesSaveExpansion);
		}

		if(const TSharedPtr<FDetailCategoryImpl> ParentCategory = TreeNode->GetParentCategory())
		{
			ParentCategory->RestoreExpansionState(bShouldCategoriesSaveExpansion);
		}
	}
}

void SDetailsViewBase::SetCategoryExpansionSaving(bool bShouldCategoriesSaveExpansion)
{
	SetCategoryExpansionSavingRecursive(RootTreeNodes, bShouldCategoriesSaveExpansion);
}
