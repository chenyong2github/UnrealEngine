...
#if PLATFORM_SUPPORTS_MESH_SHADERS
	case SV_Mesh:
		return GRHISupportsMeshShaders ? D3D12_ROOT_SIGNATURE_FLAG_DENY_MESH_SHADER_ROOT_ACCESS : D3D12_ROOT_SIGNATURE_FLAG_NONE;
	case SV_Amplification:
		return GRHISupportsMeshShaders ? D3D12_ROOT_SIGNATURE_FLAG_DENY_AMPLIFICATION_SHADER_ROOT_ACCESS : D3D12_ROOT_SIGNATURE_FLAG_NONE;
#endif
	case SV_All:
...
{
	const EShaderVisibility ShaderVisibilityPriorityOrder[] = { SV_Pixel, SV_Vertex, SV_Geometry, SV_Hull, SV_Domain, SV_Mesh, SV_Amplification, SV_All };
