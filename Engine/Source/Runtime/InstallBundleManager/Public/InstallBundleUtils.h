...
namespace InstallBundleUtil
{
	...
	{
		//Stats used to store timing information in FInstallBundleIndividualPersistentTimingData format
		enum class ETimingStatNames : uint8
		{
			  RealTotalTime
			, ActiveTotalTime
			, EstimatedTotalBGTime
			, RealChunkDBDownloadTime
			, ActiveChunkDBDownloadTime
			, EstimatedBackgroundChunkDBDownloadTime
			, ActiveInstallTime
			, EstimatedBGInstallTime
			, ActivePSOTime
			, EstimatedBGPSOTime 
		...
		public:
			...
			void GetRequiredBundles(TArray<FString>& OutRequiredBundles);
		...
		public:
			FPersistentStatContainerBase()
				...
				, TimerStat_ResetTimerValue(5.0f)
				, TimerStat_UpdateTimer(5.0f)
				, bShouldAutoUpdateInTick(true)
				, bShouldAutoUpdateBackgroundStats(true)
				, bShouldSaveDirtyStatsOnTick(true)
				, bShouldSaveStatsEveryUpdate(false)
			{
				InitializeBase();
			}
		...
		protected:
			...
			float TimerStat_ResetTimerValue;
			float TimerStat_UpdateTimer;
			bool bShouldAutoUpdateInTick;
			bool bShouldAutoUpdateBackgroundStats;
			bool bShouldSaveDirtyStatsOnTick;
			bool bShouldSaveStatsEveryUpdate;
		};
	}
