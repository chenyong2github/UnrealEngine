...
bool FCompositeBufferTest::RunTest(const FString& Parameters)
{
	// Test Null
	{
		...
		TestTrue(TEXT("FCompositeBuffer().Flatten().IsNull()"), Buffer.Flatten().IsNull());
	...
	// Test Empty
	{
		...
		TestEqual(TEXT("FCompositeBuffer(Empty).Flatten()"), Buffer.Flatten(), EmptyView);
		TestEqual(TEXT("FCompositeBuffer(Empty).Mid(0, 0)"), Buffer.Mid(0, 0).Flatten(), EmptyView);
	...
	}
	{
		...
		TestEqual(TEXT("FCompositeBuffer(Empty, Empty).Mid(0, 0)"), Buffer.Mid(0, 0).Flatten(), EmptyView1);
	...
	// Test Flat
	{
		...
		TestEqual(TEXT("FCompositeBuffer(Flat).Flatten()"), Buffer.Flatten(), FlatView);
		TestEqual(TEXT("FCompositeBuffer(Flat).MakeOwned().Flatten()"), Buffer.MakeOwned().Flatten(), FlatView);
		TestEqual(TEXT("FCompositeBuffer(Flat).Mid(0)"), Buffer.Mid(0).Flatten(), FlatView);
		TestEqual(TEXT("FCompositeBuffer(Flat).Mid(4)"), Buffer.Mid(4).Flatten().GetView(), FlatView.GetView().Mid(4));
		TestEqual(TEXT("FCompositeBuffer(Flat).Mid(8)"), Buffer.Mid(8).Flatten().GetView(), FlatView.GetView().Mid(8));
		TestEqual(TEXT("FCompositeBuffer(Flat).Mid(4, 2)"), Buffer.Mid(4, 2).Flatten().GetView(), FlatView.GetView().Mid(4, 2));
		TestEqual(TEXT("FCompositeBuffer(Flat).Mid(8).GetData()"), Buffer.Mid(8).Flatten().GetView().GetData(), FlatView.GetView().Mid(8).GetData());
		TestEqual(TEXT("FCompositeBuffer(Flat).Mid(4, 2).GetData()"), Buffer.Mid(4, 2).Flatten().GetView().GetData(), FlatView.GetView().Mid(4, 2).GetData());
		TestEqual(TEXT("FCompositeBuffer(Flat).Mid(8, 0).GetData()"), Buffer.Mid(8, 0).Flatten().GetView().GetData(), FlatView.GetView().Mid(8, 0).GetData());
	...
	// Test Composite
	{
		...
		TestTrue(TEXT("FCompositeBuffer(Composite).Flatten()"), Buffer.Flatten().GetView().EqualBytes(MakeMemoryView(FlatArray)));
		TestTrue(TEXT("FCompositeBuffer(Composite).Mid(2, 4)"), Buffer.Mid(2, 4).Flatten().GetView().EqualBytes(MakeMemoryView(FlatArray).Mid(2, 4)));
		TestEqual(TEXT("FCompositeBuffer(Composite).Mid(0, 4)"), Buffer.Mid(0, 4).Flatten(), FlatView1);
		TestEqual(TEXT("FCompositeBuffer(Composite).Mid(4, 4)"), Buffer.Mid(4, 4).Flatten(), FlatView2);
		TestEqual(TEXT("FCompositeBuffer(Composite).Mid(4, 0).GetData()"), Buffer.Mid(4, 0).Flatten().GetView().GetData(), static_cast<const void*>(FlatArray + 4));
		TestEqual(TEXT("FCompositeBuffer(Composite).Mid(8, 0).GetData()"), Buffer.Mid(8, 0).Flatten().GetView().GetData(), static_cast<const void*>(FlatArray + 8));
