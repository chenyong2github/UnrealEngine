...
bool FCbAttachmentTest::RunTest(const FString& Parameters)
{
	const auto TestSaveLoadValidate = [this](const TCHAR* Test, const FCbAttachment& Attachment)
	{
		...
		FromArchive.Load(ReadAr, [](uint64 Size) { return FSharedBuffer::Alloc(Size); });
	...
	// Empty Attachment
	{
		...
		TestFalse(TEXT("FCbAttachment(Null).AsBinary()"), Attachment.AsBinary().IsValid());
		...
		TestEqual(TEXT("FCbAttachment(Null).GetHash()"), Attachment.GetHash(), FBlake3Hash());
		TestSaveLoadValidate(TEXT("Null"), Attachment);
	}

	// Binary Attachment
	{
		FSharedBufferConstPtr Buffer = FSharedBuffer::MakeReadOnly(FSharedBuffer::Clone(MakeMemoryView<uint8>({0, 1, 2, 3})));
		...
		TestEqual(TEXT("FCbAttachment(Binary).GetHash()"), Attachment.GetHash(), FBlake3::HashBuffer(Buffer->GetView()));
	...
	// Binary View
	{
		...
		FSharedBufferConstPtr Buffer = FSharedBuffer::MakeView(MakeMemoryView(Value));
		FCbAttachment Attachment(Buffer);
		...
		TestEqual(TEXT("FCbAttachment(BinaryView).GetHash()"),
			Attachment.GetHash(), FBlake3::HashBuffer(Buffer->GetView()));
	}
	...
	{
		const uint8 Value[]{0, 1, 2, 3};
		const FSharedBufferConstPtr Buffer = FSharedBuffer::MakeView(MakeMemoryView(Value));
		FCbAttachment Attachment(Buffer);
		...
		TestEqual(TEXT("FCbAttachment(LoadBinaryView).GetHash()"),
			Attachment.GetHash(), FBlake3::HashBuffer(MakeMemoryView(Value)));
	}
	...
	{
		const FSharedBufferConstPtr Buffer = FSharedBuffer::Clone(MakeMemoryView<uint8>({0, 1, 2, 3}));
		...
		const FSharedBufferConstPtr Binary = Attachment.AsBinary();
	...
	// Binary Null
	{
		const FCbAttachment Attachment(FSharedBufferConstPtr{});
		...
		TestEqual(TEXT("FCbAttachment(BinaryNull).GetHash()"), Attachment.GetHash(), FBlake3Hash());
	}

	// Binary Empty
	{
		const FCbAttachment Attachment(FSharedBuffer::Alloc(0));
		...
		TestEqual(TEXT("FCbAttachment(BinaryEmpty).GetHash()"), Attachment.GetHash(), FBlake3Hash());
	}
	...
	// Compact Binary Empty
	{
		...
		TestEqual(TEXT("FCbAttachment(CompactBinaryEmpty).GetHash()"), Attachment.GetHash(), FBlake3Hash());
	}
...
bool FCbPackageTest::RunTest(const FString& Parameters)
{
	...
	// Attachment Only
	{
		...
		TestEqual(TEXT("FCbPackage(Attachments).GetObjectHash()"), Package.GetObjectHash(), FBlake3Hash());
	...
	FSharedBufferConstPtr Level4 = FSharedBuffer::MakeView(MakeMemoryView(Level4Values));
	const FBlake3Hash Level4Hash = FBlake3::HashBuffer(*Level4);
	...
	const FBlake3Hash Level3Hash = Level3.GetHash();
	...
	FCbArrayRef Level2;
	{
		...
		Writer.Name("Level3");
		Writer.BeginArray();
		Writer.Reference(Level3Hash);
		Writer.EndArray();
		Level2 = Writer.Save().AsArrayRef();
	}
	const FBlake3Hash Level2Hash = Level2.AsField().GetHash();

	FCbObjectRef Level1;
	{
		TCbWriter<256> Writer;
		Writer.BeginObject();
		Writer.Name("Level2").Reference(Level2Hash);
		Writer.EndObject();
		Level1 = Writer.Save().AsObjectRef();
	}
	const FBlake3Hash Level1Hash = Level1.AsField().GetHash();

	const auto Resolver = [&Level2, &Level2Hash, &Level3, &Level3Hash, &Level4, &Level4Hash]
		(const FBlake3Hash& Hash) -> FSharedBufferConstPtr
		{
			return
				Hash == Level2Hash ? Level2.GetBuffer() :
				Hash == Level3Hash ? Level3.GetBuffer() :
				Hash == Level4Hash ? Level4 :
				FSharedBufferConstPtr();
		};
	...
	// Object + Attachments
	{
		...
		TestEqual(TEXT("FCbPackage(Object+Attachments).GetObjectHash(Null)"), Package.GetObjectHash(), FBlake3Hash());
	}
	...
	// Out of Order
	{
		...
		++Saved;
		TestEqual(TEXT("FCbPackage(OutOfOrder).Save()->Level1Hash"), Saved.AsReference(), Level1Hash);
		++Saved;
		TestTrue(TEXT("FCbPackage(OutOfOrder).Save()->Level3"), Saved.AsBinary().EqualBytes(Level3.GetView()));
		++Saved;
		TestEqual(TEXT("FCbPackage(OutOfOrder).Save()->Level3Hash"), Saved.AsReference(), Level3Hash);
		++Saved;
		TestTrue(TEXT("FCbPackage(OutOfOrder).Save()->Level2"), Saved.AsBinary().EqualBytes(Level2.GetView()));
		++Saved;
		TestEqual(TEXT("FCbPackage(OutOfOrder).Save()->Level2Hash"), Saved.AsReference(), Level2Hash);
		++Saved;
	...
	// Resolve After Merge
	{
		...
		Package.AddAttachment(FCbAttachment(FCbFieldRefIterator::MakeSingle(Level3)),
			[&bResolved](const FBlake3Hash& Hash) -> FSharedBufferConstPtr
			{
				bResolved = true;
