...
static FName UntrackedMemoryName(TEXT("Untracked Memory"));
...
FResourceSizeEx::FResourceSizeEx(const EResourceSizeMode::Type InResourceSizeMode, const SIZE_T InDedicatedSystemMemoryBytes, const SIZE_T InDedicatedVideoMemoryBytes)
	: ResourceSizeMode(InResourceSizeMode)
{
	DedicatedSystemMemoryBytesMap.Add(UntrackedMemoryName, InDedicatedSystemMemoryBytes);
	DedicatedVideoMemoryBytesMap.Add(UntrackedMemoryName, InDedicatedVideoMemoryBytes);
}

FResourceSizeEx::FResourceSizeEx(const EResourceSizeMode::Type InResourceSizeMode, const SIZE_T InUnknownMemoryBytes)
	: ResourceSizeMode(InResourceSizeMode)
{
	UnknownMemoryBytesMap.Add(UntrackedMemoryName, InUnknownMemoryBytes);
}
...
{
	SIZE_T& CurrentSize = DedicatedSystemMemoryBytesMap.FindOrAdd(UntrackedMemoryName);
	CurrentSize += InMemoryBytes;
	return *this;
}
...
{
	SIZE_T& CurrentSize = DedicatedVideoMemoryBytesMap.FindOrAdd(UntrackedMemoryName);
	CurrentSize += InMemoryBytes;
	return *this;
}
...
{
	SIZE_T& CurrentSize = UnknownMemoryBytesMap.FindOrAdd(UntrackedMemoryName);
	CurrentSize += InMemoryBytes;
	return *this;
}
