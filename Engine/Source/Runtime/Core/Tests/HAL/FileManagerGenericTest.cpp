...
class FArchiveFileReaderGenericTest : public FAutomationTestFixture
{
public:
...
void FArchiveFileReaderGenericTest::TestInternalPrecache()
{
	...
	TEST_EQUAL(TEXT("Initial Pos should be 0"), Reader->Pos, 0LL);
	TEST_EQUAL(TEXT("Size should be what was passed in"), Reader->Size, FileSize);
	TEST_EQUAL(TEXT("BufferSize should be what was passed in"), Reader->BufferSize, BufferSize);

	// Vanilla InternalPrecache at start of file
	bool Result = Reader->InternalPrecache(0, BufferSize);
	TEST_EQUAL(TEXT("Vanilla0 - InternalPrecache should succeed"), Result, true);
	TEST_EQUAL(TEXT("Vanilla0 - InternalPrecache should not alter Pos"), Reader->Pos, 0LL);
	TEST_EQUAL(TEXT("Vanilla0 - BufferBase should have be set to PrecacheOffset aka Pos"), Reader->BufferBase, 0LL);
	TEST_EQUAL(TEXT("Vanilla0 - Bytes precached should be set to BufferSize unless it runs out of room"), Reader->BufferArray.Num(), BufferSize);
	...
	TEST_EQUAL(TEXT("PrecacheNotAtPos - Should fail"), Result, false);
	TEST_EQUAL(TEXT("PrecacheNotAtPos - BufferBase should not be altered"), Reader->BufferBase, 0LL);
	TEST_EQUAL(TEXT("PrecacheNotAtPos - Buffe should not be altered"), Reader->BufferArray.Num(), BufferSize);
	...
	TEST_EQUAL(TEXT("PrecacheNotAtPos - Should succeed since the first byte at PrecacheOffset is buffered"), Result, true);
	TEST_EQUAL(TEXT("PrecacheNotAtPos - BufferBase should not be altered"), Reader->BufferBase, 0LL);
	TEST_EQUAL(TEXT("PrecacheNotAtPos - Buffe should not be altered"), Reader->BufferArray.Num(), BufferSize);
	...
	TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBuffer - Should succeed"), Result, true);
	if (FArchiveFileReaderGeneric::bPrecacheAsSoonAsPossible)
	{
		TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBuffer - BufferBase should be updated"), Reader->BufferBase, PosStart);
	}
	else
	{
		TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBuffer - BufferBase should not be updated"), Reader->BufferBase, 0LL);
	}
	TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBuffer - BufferCount should be set to BufferSize"), Reader->BufferArray.Num(), BufferSize);
	...
	TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBufferReallocation - Should succeed"), Result, true);
	if (FArchiveFileReaderGeneric::bPrecacheAsSoonAsPossible)
	{
		TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBufferReallocation - BufferBase should be updated"), Reader->BufferBase, 3*BufferSize/4);
		TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBufferReallocation - BufferCount should be set to BufferSize"), Reader->BufferArray.Num(), BufferSize);
	}
	else
	{
		TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBufferReallocation - BufferBase should not be updated"), Reader->BufferBase, BufferSize/2);
		TEST_EQUAL(TEXT("PrecachePartwayPosMoreThanBufferReallocation - BufferCount should not be updated"), Reader->BufferArray.Num(), BufferSize/2);
	}
	...
	TEST_EQUAL(TEXT("PrecachePartwayPosLessThanBuffer - Should succeed"), Result, true);
	TEST_EQUAL(TEXT("PrecachePartwayPosLessThanBuffer - BufferBase should not be updated"), Reader->BufferBase, 0LL);
	TEST_EQUAL(TEXT("PrecachePartwayPosLessThanBuffer - BufferCount should be set to BufferSize"), Reader->BufferArray.Num(), BufferSize);
	...
	SetPosAndBuffer(Reader, PosStart, 0, BufferSize);
	Result = Reader->InternalPrecache(PosStart, BufferSize);
	TEST_EQUAL(TEXT("PrecachePosEndOfBuffer - Should succeed"), Result, true);
	TEST_EQUAL(TEXT("PrecachePosEndOfBuffer - BufferBase should be updated"), Reader->BufferBase, PosStart);
	TEST_EQUAL(TEXT("PrecachePosEndOfBuffer - BufferCount should be set to BufferSize"), Reader->BufferArray.Num(), BufferSize);
	...
	Result = Reader->InternalPrecache(PosStart, BufferSize);
	TEST_EQUAL(TEXT("PrecacheNearEndOfFile - Should succeed"), Result, true);
	TEST_EQUAL(TEXT("PrecacheNearEndOfFile - BufferBase should be updated"), Reader->BufferBase, PosStart);
	TEST_EQUAL(TEXT("PrecacheNearEndOfFile - BufferCount should be set Min(BufferSize, Size - Pos)"), Reader->BufferArray.Num(), 1LL);
	...
	TEST_EQUAL(TEXT("PrecacheEndOfFile - Should fail"), Result, false);
	TEST_EQUAL(TEXT("PrecacheEndOfFile - BufferBase should not be updated"), Reader->BufferBase, 0LL);
	TEST_EQUAL(TEXT("PrecacheEndOfFile - BufferCount should not be updated"), Reader->BufferArray.Num(), 16LL);
	...
	TEST_EQUAL(TEXT("PrecacheReadFailsBytesRemain - Should succeed if bytes are left in the buffer"), Result, true);
	if (FArchiveFileReaderGeneric::bPrecacheAsSoonAsPossible)
	{
		TEST_EQUAL(TEXT("PrecacheReadFailsBytesRemain - BufferBase should be updated"), Reader->BufferBase, 8LL);
		TEST_EQUAL(TEXT("PrecacheReadFailsBytesRemain - BufferCount should be updated"), Reader->BufferArray.Num(), 8LL);
	}
	else
	{
		TEST_EQUAL(TEXT("PrecacheReadFailsBytesRemain - BufferBase should not be updated"), Reader->BufferBase, 0LL);
		TEST_EQUAL(TEXT("PrecacheReadFailsBytesRemain - BufferCount should not be updated"), Reader->BufferArray.Num(), 16LL);
	}
	...
	TEST_EQUAL(TEXT("PrecacheReadFails - Should fail if bytes are not left in the buffer"), Result, false);
	TEST_EQUAL(TEXT("PrecacheReadFails - BufferCount should be updated to empty"), Reader->BufferArray.Num(), 0LL);
...
bool FArchiveFileReaderGenericTest::CreateTestFile()
{
	...
	if (!WriteHandle)
	{
		FAIL_CHECK(FString::Printf(TEXT("Could not create test file %s."), *TestFileName));
		return false;
	}
	...
	if (!ReadHandle)
	{
		FAIL_CHECK(FString::Printf(TEXT("Could not open test file %s."), *TestFileName));
		return false;
	}
	if (ReadHandle->Size() != FileSize)
	{
		FAIL_CHECK(FString::Printf(TEXT("Received incorrect file size from test file %s. Expected = %lld. Actual = %lld."), *TestFileName, FileSize, ReadHandle->Size()));
		return false;
	}
...
void FArchiveFileReaderGenericTest::TestBytesValid(const TCHAR* What, uint8* InData, int64 InNumBytes, int64 InFileOffset)
{
	...
	if (StartByte != 0)
	{
		...
		for (int n = 0; n < NumBytes && n < sizeof(int32) - StartByte; ++n)
		{
			...
			{
				TEST_EQUAL(What, Data[n], reinterpret_cast<uint8*>(&ExpectedValue)[StartByte + n]);
				return;
			}
		}
	...
	for (int n = 0; n < NumInts; ++n)
	{
		...
		{
			TEST_EQUAL(What, IntData[n], GetExpectedValue(IntOffset + n));
			return;
		}
	}
	...
	if (NumEndBytes != 0)
	{
		...
		for (int n = 0; n < NumEndBytes; ++n)
		{
			...
			{
				TEST_EQUAL(What, DataStart[n], reinterpret_cast<uint8*>(&ExpectedValue)[n]);
				return;
			}
		}
	}
...
void FArchiveFileReaderGenericTest::SetPosAndBuffer(TUniquePtr<FArchiveFileReaderGeneric>& Reader, int64 Pos, int64 BufferBase, int64 BufferSize)
{
	...
	if (BufferSize)
	{
		...
		TEST_EQUAL(TEXT("SetPosAndBuffer - ReadLowLevel read the requested bytes"), BytesRead, BufferSize);
	}
	...
	TEST_EQUAL(TEXT("SetPosAndBuffer - Pos set correctly"), Reader->Pos, Pos);
	TEST_EQUAL(TEXT("SetPosAndBuffer - BufferBase set correctly"), Reader->BufferBase, BufferBase);
	TEST_EQUAL(TEXT("SetPosAndBuffer - BufferSize set correctly"), Reader->BufferArray.Num(), BufferSize);
...
TEST_CASE_METHOD(FAutomationTestFixture, "Core::HAL::FArchiveFileReaderGeneric::Smoke Test", "[Core][HAL][Smoke]")
{
	FArchiveFileReaderGenericTest ArchiveFileReaderGenericTest;
	ArchiveFileReaderGenericTest.TestInternalPrecache();
}
