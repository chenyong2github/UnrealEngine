...
bool FArchiveFileReaderGenericTest::CreateTestFile()
{
	...
	if (!WriteHandle)
	{
		TestAddError(FString::Printf(TEXT("Could not create test file %s."), *TestFileName));
		return false;
	}
	...
	if (!ReadHandle)
	{
		TestAddError(FString::Printf(TEXT("Could not open test file %s."), *TestFileName));
		return false;
	}
	if (ReadHandle->Size() != FileSize)
	{
		TestAddError(FString::Printf(TEXT("Received incorrect file size from test file %s. Expected = %lld. Actual = %lld."), *TestFileName, FileSize, ReadHandle->Size()));
		return false;
	}
...
TEST_CASE("Core::HAL::FArchiveFileReaderGeneric::Smoke Test", "[Core][HAL][Smoke]")
{
