...
namespace UE { namespace TasksTests
{
	...
	TEST_CASE_METHOD(FCoreTestFixture, "Core::Async::Task::Basic", "[Core][Async][Smoke]")
	{
		...
		{	// FTaskEvent
			...
			Event.Trigger();
			CHECK(Event.IsCompleted());
			CHECK(Event.Wait(FTimespan::Zero()));
			CHECK(Event.BusyWait(FTimespan::Zero()));
		}
		...
		{	// one nested task
			...
			CHECK(ExecutedSignal.IsCompleted());
		...
		{	// nested in square
			...
			FTask Task{ Launch(UE_SOURCE_LOCATION,
				[&Signal]
				{
					...
					CHECK(!NestedTask.Wait(FTimespan::FromMilliseconds(100)));
					NestedSignal.Trigger();
					CHECK(NestedTask.Wait());
				}
			)};

			CHECK(!Task.Wait(FTimespan::FromMilliseconds(200)));
			Signal.Trigger();
			CHECK(Task.Wait());
		}
