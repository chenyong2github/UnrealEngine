	...
	TEST_CASE_METHOD(FAutomationTestFixture, "Core::Containers::TMpscQueue::ConcurrentQueuesTest", "[Core][Containers][Smoke]")
	{
		{	// test for support of not default constructible types
...

			{
				...
				CHECK((Res.IsSet() && (Res.GetValue().Value == 1)));
			}
		...
		{	// test queue destruction and not default movable types
			...
			{	// enqueue and dequeue multiple items
				...
				CHECK((Res.IsSet() && (*Res.GetValue().Value) == 1));
				Res = Q.Dequeue();
				CHECK((Res.IsSet() && (*Res.GetValue().Value == 2)));
			}
...
namespace ClosableMpscQueueTests
{
	...
	public:
		bool Enqueue(T Value)
		{
			...
			CHECK(SubscribingThreadsNum.fetch_add(1, std::memory_order_acq_rel) >= 0);
			// `acquire` to "synchronise-with" the consumer
			if (!bIsComplete.load(std::memory_order_acquire))
			{
				REQUIRE(Queue.Enqueue(MoveTemp(Value)));
				bRes = true;
			}
			// `release` to "synchonise-with" other threads
			CHECK(SubscribingThreadsNum.fetch_sub(1, std::memory_order_release) >= 1);
...
namespace DepletableMpscQueueTests
{
	...
	public:
		...
		void Enqueue(T Value)
		{
			...
			CHECK(SubscribingThreadsNum.fetch_add(1, std::memory_order_acquire) >= 0);
	...
	public:
		...
		void Enqueue(T* Value)
		{
			// `acquire`/`release` to keep `Queue` usage inside the scope
			CHECK(SubscribingThreadsNum.fetch_add(1, std::memory_order_acquire) >= 0);
			REQUIRE(Queue.load(std::memory_order_relaxed)->PushIfNotClosed(MoveTemp(Value)));
			// `release` to "synchonise-with" other threads
			CHECK(SubscribingThreadsNum.fetch_sub(1, std::memory_order_release) >= 1);
		}
	...
	public:
		...
		void Enqueue(T Value)
		{
			// `acquire`/`release` to keep `Queue` usage inside the scope
			CHECK(SubscribingThreadsNum.fetch_add(1, std::memory_order_acquire) >= 0);
			REQUIRE(Queue.load(std::memory_order_relaxed)->Enqueue(MoveTemp(Value)));
			// `release` to "synchonise-with" other threads
			CHECK(SubscribingThreadsNum.fetch_sub(1, std::memory_order_release) >= 1);
		}
	...
	TEST_CASE_METHOD(FAutomationTestFixture, "Core::Containers::TDepletableMpscQueue::DepletableMpscQueueTest", "[Core][Containers][Smoke]")
	{
		...
		{	// EnqueueAndReturnWasEmpty
			TDepletableMpscQueue<int> Q;
			CHECK(Q.EnqueueAndReturnWasEmpty(1));
			CHECK(!Q.EnqueueAndReturnWasEmpty(2));
			Q.Deplete([](int) {});
			CHECK(Q.EnqueueAndReturnWasEmpty(1));
			CHECK(!Q.EnqueueAndReturnWasEmpty(2));
			Q.Deplete([](int) {});
