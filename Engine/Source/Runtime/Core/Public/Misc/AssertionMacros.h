 ...
 */
	...
	#define UE_ENSURE_IMPL(Capture, Always, InExpression, InFormat) \
		...
		{ \
			...
			const bool CheckVerifyResult = CheckVerifyImpl(bExecuted, Always, __FILE__, __LINE__, PLATFORM_RETURN_ADDRESS(), #InExpression, InFormat); \
			PLATFORM_BREAK_IF_DESIRED_INLINE(); \
			return CheckVerifyResult; \
		})))
	...
	#define UE_ENSURE_IMPL2(Capture, Always, InExpression, ...) \
		...
		{ \
			...
			const bool CheckVerifyResult = CheckVerifyImpl(bExecuted, Always, __FILE__, __LINE__, PLATFORM_RETURN_ADDRESS(), #InExpression, ##__VA_ARGS__); \
			PLATFORM_BREAK_IF_DESIRED_INLINE(); \
			return CheckVerifyResult; \
		})))
