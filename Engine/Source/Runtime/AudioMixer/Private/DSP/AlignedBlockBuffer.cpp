	...
	,	ReadPos(0)
{
	...
	{
		UE_LOG(LogAudioMixer, Warning, TEXT("MaxNumInspectSamples [%d] reduced to maximum storage [%d]"), MaxNumInspectSamples, MaxStorage);
		MaxNumInspectSamples = MaxStorage;
	}

	if (SampleCapacity > MaxStorage)
	{
		UE_LOG(LogAudioMixer, Warning, TEXT("SampleCapacity [%d] reduced to maximum storage [%d]"), SampleCapacity, MaxStorage);
		SampleCapacity = MaxStorage;
	}

	if (MaxNumInspectSamples > SampleCapacity)
	{
		UE_LOG(LogAudioMixer, Warning, TEXT("SampleCapacity [%d] increased to Maxmimum Inspection [%d]"), SampleCapacity, MaxNumInspectSamples);
