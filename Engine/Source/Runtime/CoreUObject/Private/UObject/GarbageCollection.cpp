...
static bool IncrementalDestroyGarbage(bool bUseTimeLimit, float TimeLimit, double StartTime);
...
void IncrementalPurgeGarbage(bool bUseTimeLimit, float TimeLimit)
{
	...
	{
		bCompleted = IncrementalDestroyGarbage(bUseTimeLimit, TimeLimit, StartTime);
	}
}

bool IncrementalDestroyGarbage(bool bUseTimeLimit, float TimeLimit, double StartTime)
{
	...
	if (GObjFinishDestroyHasBeenRoutedToAllObjects && !bTimeLimitReached)
	{
		...
		GAsyncPurge->TickPurge(bUseTimeLimit, TimeLimit, StartTime);
