...
bool FVirtualizationWrapperTestBasic::RunTest(const FString& Parameters)
{
	...
	auto ValidateBulkData = [this, &SourceBuffer, BufferSize](const FVirtualizedUntypedBulkData& BulkDataToValidate, const TCHAR* Label)
		{
			...
			TestEqual(FString::Printf(TEXT("%s buffer length"),Label), (int64)RetrievedBuffer->Size(), BufferSize);
	...
	BulkData.UpdatePayload(MakeSharedBuffer(FSharedBuffer::Wrap, SourceBuffer.Get(), BufferSize));

	// Test access via callback
	BulkData.GetData([this,BufferSize,&SourceBuffer](FSharedBufferConstPtr Payload)
		{
			TestEqual(TEXT("Callback buffer length"), (int64)Payload->Size(), BufferSize);
	...
	BulkDataAssignment.UpdatePayload(MakeSharedBuffer(128));
...
bool FVirtualizationWrapperTestUpdatePayload::RunTest(const FString& Parameters)
{
	...
	FByteVirtualizedBulkData BulkData;
	BulkData.UpdatePayload(MakeSharedBuffer(FSharedBuffer::Wrap, OriginalData.Get(), BufferSize));
	...
	// Access the payload, edit it and push it back into the bulkdata object
	{
		...
		FSharedBufferConstPtr Payload = BulkData.GetData().Get();
		TestEqual(TEXT("Payload length"), (int64)Payload->Size(), BufferSize);
		...
		TestTrue(TEXT("Orginal buffer == Payload data"), FMemory::Memcmp(OriginalData.Get(), Payload->GetData(), Payload->Size()) == 0);

		// Make a copy of the payload that we can edit
		FSharedBufferRef EditablePayload = MakeSharedBuffer(FSharedBuffer::Clone, Payload->GetView());

		const uint8 NewValue = 255;
		FMemory::Memset(EditablePayload->GetData(), NewValue, EditablePayload->Size());

		// Update the bulkdata object with the new edited payload
		BulkData.UpdatePayload(EditablePayload);

		Payload = BulkData.GetData().Get();
		TestEqual(TEXT("Updated payload length"), (int64)Payload->Size(), BufferSize);
		...
		const bool bAllElementsCorrect = Algo::AllOf(TArrayView<uint8>((uint8*)Payload->GetData(), Payload->Size()), [NewValue](uint8 Val)
			{ 
				return Val == NewValue; 
			});
...

	{
		...
		// the bulkdata object.
		BulkData.UpdatePayload(MakeSharedBuffer(FSharedBuffer::AssumeOwnership, OriginalData.Release(), BufferSize));
		...
		const bool bAllElementsCorrect = Algo::AllOf(TArrayView<uint8>((uint8*)Payload->GetData(), Payload->Size()), [](uint8 Val)
			{
				return Val == 0;
			});
