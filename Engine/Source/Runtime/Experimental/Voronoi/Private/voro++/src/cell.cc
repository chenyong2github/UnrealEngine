 ...
 * routine from .
 * \param[in] vc a reference to the specialized version of the calling class.
 * \param[in,out] upRef reference to the up member used to initialize search, 
				  just to make it extra clear that this function changes this value */
template<class vc_class>
inline bool voronoicell_base::search_for_outside_edge(vc_class &vc,int &upRef) {
...
void voronoicell_neighbor::neighborsTArray(TArray<int> &nbrs, bool excludeBounds) {
	nbrs.Empty();
	int i, j, k, l, m;
	for (i = 1; i < p; i++) for (j = 0; j < nu[i]; j++) {
		k = ed[i][j];
