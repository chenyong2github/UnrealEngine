...
bool FStreamingFileSystem::BlockingReadFromVFS(FIoChunkId ChunkId, TArray<uint8>& Result, FString& ReadError)
{
	...
	{
		ReadError = FString::Printf(TEXT("Hash not found in chunk map for hash %s"), *Hash.ToString());
		return false;
	}
	...
	if (VFS->DoesChunkExist(Hash))
	{
		...
		{
			// TODO: Report error
			ReadError = FString::Printf(TEXT("Empty data found for hash %s"), *Hash.ToString());
			return false;
		}
		else
		{
