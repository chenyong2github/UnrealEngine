...
#if PLATFORM_APPLE | PLATFORM_UNIX | PLATFORM_ANDROID
#	define CORE_HTTP_SUPPORTED
#	include <arpa/inet.h>
#	include <fcntl.h>
#	include <netdb.h>
...
#	include <unistd.h>
	...
	struct FAddrInfoCharlatan
	{
		int32				ai_family;
		int32				ai_socktype;
		int32				ai_protocol;
		sockaddr_in			ai_addr[1];
		FAddrInfoCharlatan*	ai_next;
	};

	using addrinfo = FAddrInfoCharlatan;
	...
	static int getaddrinfo(const char* HostName, void*, const addrinfo*, addrinfo** Out)
	{
		static_assert(sizeof(sockaddr_in::sin_addr) == sizeof(uint32));
		static_assert(sizeof(SceNetInAddr_t) == sizeof(uint32));
		static_assert(SCE_NET_AF_INET == AF_INET);

		int MemPool = sceNetPoolCreate("CoreHttpResolve", 4096, 0);
		if (MemPool < 0)
			return -1;
		ON_SCOPE_EXIT { sceNetPoolDestroy(MemPool); };

		SceNetId Resolver = sceNetResolverCreate("CoreHttpResolve", MemPool, 0);
		if (Resolver < 0)
			return -1;
		ON_SCOPE_EXIT { sceNetResolverDestroy(Resolver); };

		uint32 ResolveFlags = SCE_NET_RESOLVER_ASYNC;

		enum { EnableAsync = 0 };
		int32 PollId = EnableAsync ? sceNetEpollCreate("CoreHttpResolve", 0) : -1;
		if (PollId < 0)
			ResolveFlags = 0;
		ON_SCOPE_EXIT { if (PollId >= 0) sceNetEpollDestroy(PollId); };

		SceNetResolverInfo DnsInfo;
		int Result = sceNetResolverStartNtoaMultipleRecords(
			Resolver,
			HostName,
			&DnsInfo,
			0, /* timeout (us) */
		...
		if (EnableAsync && PollId >= 0)
		{
			SceNetEpollEvent Event = { SCE_NET_EPOLLDESCID };
			sceNetEpollControl(PollId, SCE_NET_EPOLL_CTL_ADD, Result, &Event);

			SceNetEpollEvent WaitOut;
			int32 WaitResult = sceNetEpollWait(PollId, &WaitOut, 1, -1);
			if (WaitResult < 0) return -1;
			check(WaitResult == 1);
		}

		addrinfo* AddrInfos = new FAddrInfoCharlatan[DnsInfo.records];
		for (int i = 0, n = DnsInfo.records; i < n; i++)
		{
			AddrInfos->ai_family = AF_INET;
			AddrInfos->ai_socktype = SOCK_STREAM;
			AddrInfos->ai_protocol = IPPROTO_TCP;
			AddrInfos->ai_next = (i < n - 1) ? AddrInfos + i + 1 : nullptr;

			sockaddr_in* SockAddr = AddrInfos->ai_addr;
			SockAddr->sin_family = AF_INET;
			SockAddr->sin_port = 0;
			::memcpy(&(SockAddr->sin_addr), &(DnsInfo.addrs[i].un.addr.s_addr), sizeof(uint32));
		}
...
#		include <nn/socket/socket_Api.h>
		using SocketType	= int;
		using MsgFlagType	= nn::socket::MsgFlag;
#		define WRAP(CanonicalName, NnName) \
			template <typename... ArgTypes> auto CanonicalName(ArgTypes... Args) { \
				return nn::socket::NnName(Forward<ArgTypes>(Args)...); \
			}
			WRAP(closesocket,	Close)
			WRAP(connect,		Connect)
			WRAP(fcntl,			Fcntl)
			WRAP(freeaddrinfo,	FreeAddrInfo)
			WRAP(getaddrinfo,	GetAddrInfo)
			WRAP(getsockopt,	GetSockOpt)
			WRAP(htons,			InetHtons)
			WRAP(recv,			Recv)
			WRAP(send,			Send)
			WRAP(socket,		Socket)
#		undef WRAP
		using addrinfo					= nn::socket::AddrInfo;
		using sockaddr					= nn::socket::SockAddr;
		using sockaddr_in				= nn::socket::SockAddrIn;
		using socklen_t					= nn::socket::SockLenT;
		const static auto AF_INET		= nn::socket::Family::Af_Inet;
		const static auto SOCK_STREAM	= nn::socket::Type::Sock_Stream;
		const static auto IPPROTO_TCP	= nn::socket::Protocol::IpProto_Tcp;
		const static auto F_GETFL		= nn::socket::FcntlCommand::F_GetFl;
		const static auto F_SETFL		= nn::socket::FcntlCommand::F_SetFl;
		const static auto O_NONBLOCK	= nn::socket::FcntlFlag::O_NonBlock;
		const static auto SOL_SOCKET	= nn::socket::Level::Sol_Socket;
		const static auto SO_ERROR		= nn::socket::Option::So_Error;
		inline auto& GetAddrRef(nn::socket::InAddr& Parent) { return Parent.S_addr; }
#	endif
#endif

// }}}

#if defined(CORE_HTTP_SUPPORTED)

#undef CORE_HTTP_SUPPORTED
...
#else // CORE_HTTP_SUPPORTED

namespace UE::HTTP {
FEventLoop::~FEventLoop() {}
} // namespace UE::HTTP

#endif // CORE_HTTP_SUPPORTED
