...
FSkeletalMeshPhysicsObject::FSkeletalMeshPhysicsObject(UObject* InOwner, const FInitFunc& InInitFunc)
	: TSolverObject<FSkeletalMeshPhysicsObject>(InOwner)
...
FSkeletalMeshPhysicsObject::~FSkeletalMeshPhysicsObject()
{
}

void FSkeletalMeshPhysicsObject::Initialize()
{
	check(IsInGameThread());
...
void FSkeletalMeshPhysicsObject::Reset()
{
	bInitializedState = false;
}

bool FSkeletalMeshPhysicsObject::IsSimulating() const
{
	return Parameters.bSimulating;
}

void FSkeletalMeshPhysicsObject::UpdateKinematicBodiesCallback(const FParticlesType& Particles, const float Dt, const float Time, FKinematicProxy& Proxy)
{
	...
	if (bInitializedState && Parameters.bSimulating)
	{
		...
		const FSkeletalMeshPhysicsObjectInputs* InputBuffer = InputBuffers.ExchangeConsumerBuffer();
...
void FSkeletalMeshPhysicsObject::StartFrameCallback(const float InDt, const float InTime)
{
}

void FSkeletalMeshPhysicsObject::EndFrameCallback(const float InDt)
{
}

void FSkeletalMeshPhysicsObject::CreateRigidBodyCallback(FParticlesType& Particles)
{
	if (!bInitializedState && Parameters.bSimulating)
	{
		...
		for (const int32 BoneIndex : Parameters.BoneHierarchy.GetBoneIndices())
		{
			...
			UE_LOG(USkeletalMeshSimulationComponentLogging, Log,
				TEXT("FSkeletalMeshPhysicsObject::CreateRigidBodyCallback() - this: %p - "
...
void FSkeletalMeshPhysicsObject::ParameterUpdateCallback(FParticlesType& InParticles, const float InTime)
{}

void FSkeletalMeshPhysicsObject::DisableCollisionsCallback(TSet<TTuple<int32, int32>>& InPairs)
{}

void FSkeletalMeshPhysicsObject::AddForceCallback(FParticlesType& InParticles, const float InDt, const int32 InIndex)
{}

void FSkeletalMeshPhysicsObject::BindParticleCallbackMapping(Chaos::TArrayCollectionArray<SolverObjectWrapper>& SolverObjectReverseMap, Chaos::TArrayCollectionArray<int32>& ParticleIDReverseMap)
{
	if (bInitializedState)
	{
		for (int32 Index = 0; Index < RigidBodyIds.Num(); Index++)
		{
			...
			{
				SolverObjectReverseMap[RigidBodyIds[Index]] = { this, ESolverObjectType::SkeletalMeshType };
...
void FSkeletalMeshPhysicsObject::SyncBeforeDestroy() 
{}

void FSkeletalMeshPhysicsObject::OnRemoveFromScene()
{
	// Disable the particle we added
	Chaos::FPBDRigidsSolver* CurrSolver = GetSolver();
	if(CurrSolver && RigidBodyIds.Num())
	{
		Chaos::FPBDRigidsSolver::FParticlesType &Particles = CurrSolver->GetRigidParticles();
...
void FSkeletalMeshPhysicsObject::CacheResults()
{
	...
	if (GetSolver() != nullptr)
	{
		...
		FSkeletalMeshPhysicsObjectOutputs& Outputs = OutputBuffers.GetPhysicsDataForWrite();
		...
		FPBDRigidsSolver::FParticlesType& Particles = GetSolver()->GetRigidParticles();
		for (const int32 BoneIndex : Parameters.BoneHierarchy.GetBoneIndices())
		{
...
void FSkeletalMeshPhysicsObject::FlipCache()
{
	// PHYSICS THREAD
...
void FSkeletalMeshPhysicsObject::SyncToCache()
{
	// GAME THREAD
...
void FSkeletalMeshPhysicsObject::CaptureInputs(const float Dt, const FInputFunc& InputFunc)
{
	// GAME THREAD
	...
	if (InputFunc(Dt, Parameters))
	{
		...
		FSkeletalMeshPhysicsObjectInputs& PhysicsInputs = *NextInputProducerBuffer;
