...
namespace Chaos
{
	...
	bool FSolverCollisionEventFilter::Pass(const TCollisionData<float, 3>& InData)
	{
	...
	bool FSolverTrailingEventFilter::Pass(const TTrailingData<float, 3>& InData)
	{
	...
	bool FSolverBreakingEventFilter::Pass(const TBreakingData<float, 3>& InData)
	{
		const float MinSpeedThresholdSquared = Settings.MinSpeed * Settings.MinSpeed;
