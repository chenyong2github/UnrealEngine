...
struct FSolverObjectKinematicUpdate
{
	FTransform NewTransform;
	FVector NewVelocity;
};

class CHAOSSOLVERS_API FStaticMeshPhysicsObject : public TSolverObject<FStaticMeshPhysicsObject>
{
public:
	...
	using FCallbackInitFunc = TFunction<void(FStaticMeshPhysicsObject::Params&)>;
	using FSyncDynamicFunc = TFunction<void(const FTransform&)>;

	FStaticMeshPhysicsObject() = delete;
	FStaticMeshPhysicsObject(UObject* InOwner, FCallbackInitFunc InInitFunc, FSyncDynamicFunc InSyncFunc);

	void Initialize();
	void Reset();

	/** Stores latest update, to be applied at next opportunity (via UpdateKinematicBodiesCallback). */
	void BufferKinematicUpdate(const FSolverObjectKinematicUpdate& InParamUpdate);
...
private:
	...
	FSolverObjectKinematicUpdate BufferedKinematicUpdate;
};
