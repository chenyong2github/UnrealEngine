...
#include "Chaos/PBDConstraintColor2.h"
...
namespace Chaos
{
	...
	class CHAOS_API TPBDConstraintGraphRuleImpl2 : public TPBDConstraintGraphRule2<T, d>
	{
		typedef TPBDConstraintGraphRule2<T, d> Base;
	public:
		typedef T_CONSTRAINTS FConstraints;
		typedef TPBDConstraintGraph2<T, d> FConstraintGraph;

		TPBDConstraintGraphRuleImpl2(FConstraints& InConstraints)
	 ...
	 */
	template<typename T_CONSTRAINTS, typename T, int d>
	class CHAOS_API TPBDConstraintIslandRule2 : public TPBDConstraintGraphRuleImpl2<T_CONSTRAINTS, T, d>
	{
		typedef TPBDConstraintGraphRuleImpl2<T_CONSTRAINTS, T, d> Base;

	public:
		typedef typename Base::FConstraints FConstraints;
		typedef typename Base::FConstraintGraph FConstraintGraph;

		TPBDConstraintIslandRule2(FConstraints& InConstraints)
			: TPBDConstraintGraphRuleImpl2<T_CONSTRAINTS, T, d>(InConstraints)
		{
		}
	 ...
	 */
	template<typename T_CONSTRAINTS, typename T, int d>
	class CHAOS_API TPBDConstraintColorRule2 : public TPBDConstraintGraphRuleImpl2<T_CONSTRAINTS, T, d>
	{
		typedef TPBDConstraintGraphRuleImpl2<T_CONSTRAINTS, T, d> Base;
	...
	public:
		typedef typename Base::FConstraints FConstraints;
		typedef typename Base::FConstraintGraph FConstraintGraph;
		typedef TPBDConstraintColor2<T, d> FConstraintColor;

		TPBDConstraintColorRule2(FConstraints& InConstraints, const int32 InPushOutIterations)
			: TPBDConstraintGraphRuleImpl2<T_CONSTRAINTS, T, d>(InConstraints)
					...
					for (int32 Color = 0; Color <= MaxColor; ++Color)
					{
						if (LevelToColorToConstraintListMap[Level].Contains(Color))
						{
							for (int32 Edge = 0; Edge < LevelToColorToConstraintListMap[Level][Color].Num(); ++Edge)
							{
								...
								const TVector<TGeometryParticleHandle<T,d>*, 2> Particles = Constraints.ConstraintParticleIndices(ConstraintIndex);
								if (Iteration == PushOutIterations - 1)
								{
									if (Particles[0]->ToDynamic() == nullptr || IsTemporarilyStatic.Contains(Particles[0]))
									{
										IsTemporarilyStatic.Add(Particles[1]);
									}
									else if (Particles[1]->ToDynamic() == nullptr || IsTemporarilyStatic.Contains(Particles[1]))
									{
	 ...
	 */
	template<typename T, int d>
	struct CHAOS_API TConstraintRuleFactory2
	{
	public:
		typedef TPBDConstraintGraph2<T, d> FConstraintGraph;

		template<typename T_CONSTRAINTS>
		static TPBDConstraintIslandRule2<T_CONSTRAINTS, T, d> CreateIslandRule(T_CONSTRAINTS& Constraints)
		{
			return TPBDConstraintIslandRule2<T_CONSTRAINTS, T, d>(Constraints);
		}

		template<typename T_CONSTRAINTS>
		static TPBDConstraintColorRule2<T_CONSTRAINTS, T, d> CreateColorRule(T_CONSTRAINTS& Constraints, const int32 InPushOutIterations)
		{
			return TPBDConstraintColorRule2<T_CONSTRAINTS, T, d>(Constraints, InPushOutIterations);
		}
	};
