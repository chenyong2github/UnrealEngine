...
namespace Chaos
{
	...
	public:
		...
		inline PMatrix<FReal, 3, 3> ToMatrix() const
		{
			PMatrix<FReal, 3, 3> R;
			BaseQuat::ToMatrix(R);
			return R;
		...
		*/
		 ...
		 */
		CHAOS_API static TRotation<FReal, 3> Conjugate(const ::Chaos::TRotation<FReal, 3>& InR);

		/**
		 * Negate all values of the quaternion (note: not the inverse rotation. See Conjugate)
		 */
		CHAOS_API static TRotation<FReal, 3> Negate(const ::Chaos::TRotation<FReal, 3>& InR);
		 ...
		 */
		CHAOS_API static TRotation<FReal, 3> FromVector(const ::Chaos::TVector<FReal, 3>& V);

		/**
		 * Generate a Rotation that would rotate vector InitialVector to FinalVector
		 */
		CHAOS_API static TRotation<FReal, 3> FromRotatedVector(
		 ...
		 */
		CHAOS_API static TVector<FReal, 3> CalculateAngularVelocity1(const TRotation<FReal, 3>& R0, const TRotation<FReal, 3>& InR1, const FReal InDt);
		 ...
		 */
		CHAOS_API static TVector<FReal, 3> CalculateAngularVelocity2(const TRotation<FReal, 3>& R0, const TRotation<FReal, 3>& InR1, const FReal InDt);
		 ...
		 * Uses the relation: DQ/DT = (W * Q)/2
		 */
		CHAOS_API static TRotation<FReal, 3> IntegrateRotationWithAngularVelocity(const TRotation<FReal, 3>& InR0, const TVector<FReal, 3>& InW, const FReal InDt);
