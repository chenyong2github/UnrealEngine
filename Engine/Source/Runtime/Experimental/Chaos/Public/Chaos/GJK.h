...
namespace Chaos
{
	...
	bool GJKIntersection(const TGeometryA& A, const TGeometryB& B, const TRigidTransform<T,3>& BToATM, const T Thickness = 0, const TVector<T,3>& InitialDir = TVector<T,3>(1,0,0))
	{
		...
		do 
		{
			...
			const TVector<T, 3> SupportA = A.Support(NegV, Thickness);	//todo: add thickness to quadratic geometry to avoid quadratic vs quadratic when possible
