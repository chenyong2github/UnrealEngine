...
namespace Chaos
{
	...
	/** Determines the distance between two convex bodies
	// Improvement over previous function: calculates distances using core shapes allowing for better performance and negative distances
	// The distances can be negative
	 ...
	 @InitialDir The first direction we use to search the CSO
	 @ThicknessB The amount of geometry inflation for Geometry B(for example if the surface distance of two geometries with thickness 0 would be 2, a thickness of 0.5 would give a distance of 1.5)
	 @OutDistance if returns true, the minimum distance between A and B, otherwise not modified.
	 @OutNearestA if returns true, the near point on A in local-space, otherwise not modified.
	 @OutNearestB if returns true, the near point on B in local-space, otherwise not modified.
	 @return false if the geometries overlap sop much that a distance cannot be calculated by GJK*/

	template <typename T, typename TGeometryA, typename TGeometryB>
	bool GJKDistanceImproved(const TGeometryA& A, const TGeometryB& B, const TRigidTransform<T, 3>& BToATM, const T InThicknessA = 0, const TVector<T, 3>& InitialDir = TVector<T, 3>(1, 0, 0), const T InThicknessB = 0,
		T* OutDistance = nullptr, TVector<T, 3>* OutNearestA = nullptr, TVector<T, 3>* OutNearestB = nullptr)
	{
		...
		FSimplex SimplexIDs;
		TVector<T, 3> Simplex[4];
		TVector<T, 3> SimplexA[4];
		TVector<T, 3> SimplexB[4];
		...
		const T ThicknessA = A.GetMargin() + InThicknessA;
		const T ThicknessB = B.GetMargin() + InThicknessB;
		const T Inflation = ThicknessA + ThicknessB + Epsilon;
		const T Inflation2 = Inflation * Inflation;
		do
		{
			if (!ensure(NumIterations++ < 32))	//todo: take this out
			...
			T NewDist2 = V.SizeSquared();
			bNearZero = NewDist2 < Epsilon * Epsilon;//Inflation2;
			...
			if (bTerminate && !bNearZero)
			{
				// Geometries do not overlap, so we can populate the output parameters
				*OutDistance = V.Size() - Inflation + Epsilon;
				...
				*OutNearestA -= V * ThicknessA;
				*OutNearestB += AToBRotation * V * ThicknessB;
