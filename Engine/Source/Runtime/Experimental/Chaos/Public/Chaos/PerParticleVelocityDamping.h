...
namespace Chaos
{
	...
	class TPerParticleVelocityDamping : public TPerParticleRule<T, d>
	{
	public:
		TPerParticleVelocityDamping() {}
		virtual ~TPerParticleVelocityDamping() {}
		...
		{
			ApplyHelper(InParticles.V(Index), InParticles.W(Index), InParticles.LinearDamping(Index), InParticles.AngularDamping(Index), Dt);
		}

		inline void Apply(TTransientPBDRigidParticleHandle<T, d>& Particle, const T Dt) const override //-V762
		{
			ApplyHelper(Particle.V(), Particle.W(), Particle.LinearDamping(), Particle.AngularDamping(), Dt);
		}
	};
}
