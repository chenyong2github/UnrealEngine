...
namespace Chaos::Softs
{
	...
	/**
	 * Property collection adapter class to use a ManagedArrayCollection property collection and add simulation properties.
	 * Note: Int properties are limited to 24 bits (-16777215 to 16777215).
	 */
	class FPropertyCollectionAdapter final : public FPropertyCollectionConstAdapter
	{
	public:
		explicit FPropertyCollectionAdapter(const TSharedPtr<FManagedArrayCollection>& ManagedArrayCollection);
		...
		template<typename T, TEMPLATE_REQUIRES(TIsWeightedType<T>::Value)>
		int32 AddValue(const FString& Key, const T& Value, bool bEnabled = true, bool bAnimatable = false) { return AddWeightedValue(Value, Value, bEnabled, bAnimatable); }
	...
	template<typename T, typename TEnableIf<TIsWeightedType<T>::Value, int>::type>
	inline int32 FPropertyCollectionAdapter::AddWeightedValue(const FString& Key, const T& LowValue, const T& HighValue, bool bEnabled, bool bAnimatable)
	{
	...
	inline int32 FPropertyCollectionAdapter::AddWeightedFloatValue(const FString& Key, const FVector2f& Value, bool bEnabled, bool bAnimatable)
	{
		const int32 KeyIndex = AddProperty(Key, bEnabled, bAnimatable);
		SetWeightedFloatValue(KeyIndex, Value);
		return KeyIndex;
	...
	inline int32 FPropertyCollectionAdapter::AddStringValue(const FString& Key, const FString& Value, bool bEnabled, bool bAnimatable)
	{
		const int32 KeyIndex = AddProperty(Key, bEnabled, bAnimatable);
