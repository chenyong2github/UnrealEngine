...
#define CHAOS_ENABLE_STAT_NARROWPHASE 0
	...
	public:
		 ...
		 */
		void GenerateCollisions(FCollisionConstraintsArray& NewConstraints, FReal Dt, TGeometryParticleHandle<FReal, 3>* Particle0, TGeometryParticleHandle<FReal, 3>* Particle1, const FReal CullDistance, const FReal ShapePadding, CollisionStats::FStatData& StatData)
		{
			...
			if (ensure(Particle0 && Particle1))
			{
				...
				Collisions::ConstructConstraints(Particle0, Particle1, Particle0->Geometry().Get(), RigidParticle0 ? RigidParticle0->CollisionParticles().Get() : nullptr, Particle1->Geometry().Get(), RigidParticle1 ? RigidParticle1->CollisionParticles().Get() : nullptr, FRigidTransform3(), FRigidTransform3(), CullDistance, ShapePadding, Context, NewConstraints);
