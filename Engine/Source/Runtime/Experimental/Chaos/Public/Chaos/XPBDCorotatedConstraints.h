...
namespace Chaos::Softs
{
	...
	public:
		...
		void Init() const 
		{
			for (FReal& Lambdas : LambdaArray) { Lambdas = (FReal)0.; }
		}
	...
	private:
		...
		void InitColor(const FSolverParticles& Particles)
		{

			{
				...
				TArray<FReal> ReorderedMeasure;
				TArray<FReal> ReorderedDmInverse;
	...
	protected:
		...
		TVec4<FSolverVec3> GetDeterminantDelta(const FSolverParticles& Particles, const FReal Dt, const int32 ElementIndex, const FReal Tol = 1e-3) const
		{
			const TVec4<int32>& Constraint = MeshConstraints[ElementIndex];

			const PMatrix<FReal, 3, 3> Fe = F(ElementIndex, Particles);
			PMatrix<FReal, 3, 3> DmInvT = ElementDmInv(ElementIndex).GetTransposed();
			TVec4<FSolverVec3> dC2(FSolverVec3((FReal)0.));

			FReal J = Fe.Determinant();
			if (J - 1 < Tol)
			{
				return TVec4<FSolverVec3>(FSolverVec3((FReal)0.));
			}

			PMatrix<FReal, 3, 3> JFinvT;
			...
			PMatrix<FReal, 3, 3> JinvTDmInvT = DmInvT * JFinvT;
			...
			FReal AlphaTilde = (FReal)2. / (Dt * Dt * Lambda * Measure[ElementIndex]);
			...
			FReal DLambda = (1 - J) - AlphaTilde * LambdaArray[2 * ElementIndex + 1];
		...
		TVec4<FSolverVec3> GetPolarDelta(const FSolverParticles& Particles, const FReal Dt, const int32 ElementIndex, const FReal Tol = 1e-3) const
		{
			...
			TVector<FReal, 81> dRdF((FReal)0.);
			Chaos::dRdFCorotated(Fe, dRdF);

			PMatrix<FReal, 3, 3> DmInvT = ElementDmInv(ElementIndex).GetTransposed();

			//TODO: deifnitely test the initialization
			TVec4<FSolverVec3> dC1(FSolverVec3((FReal)0.));
			//dC1 = dC1dF * dFdX
			for (int alpha = 0; alpha < 3; alpha++) {
				for (int l = 0; l < 3; l++) {
			...
			FReal AlphaTilde = (FReal)1. / (Dt * Dt * Mu * Measure[ElementIndex]);
			...
			FReal DLambda = -C1 - AlphaTilde* LambdaArray[2 * ElementIndex + 0];
	...
	protected:
		mutable TArray<FReal> LambdaArray;
		mutable TArray<FReal> DmInverse;
