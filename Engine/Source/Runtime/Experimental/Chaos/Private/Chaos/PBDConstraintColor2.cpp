...
void TPBDConstraintColor2<T, d>::ComputeIslandColoring(const int32 Island, const FConstraintGraph& ConstraintGraph, uint32 ContainerId)
{
	...
	for (const TGeometryParticleHandle<T,d>* Particle :IslandParticles)
	{
		...
		if (ProcessedNodes.Contains(ParticleNodeIndex) || Particle->ToDynamic() == nullptr)
		{
			continue;
		}
		...
		while (NodesToProcess.Num())
		{
			...
			for (const int32 EdgeIndex : GraphNode.Edges)
			{
				...
				if ((ColorToUse == ColorNode.NextColor) && Particle->ToDynamic() != nullptr)
				{
					ColorNode.NextColor++;
				}
				...
				if (OtherNodeIndex != INDEX_NONE)
				{
					...
					if (Particle->ToDynamic() != nullptr)
					{
						OtherColorNode.UsedColors.Add(ColorEdge.Color);
					}

					// Queue other node for processing
					if (!ProcessedNodes.Contains(OtherNodeIndex) && OtherGraphNode.Particle->ToDynamic() != nullptr)
					{
...
#ifdef USE_CONTACT_LEVELS
template<typename T, int d>
void TPBDConstraintColor2<T, d>::ComputeContactGraph(const int32 Island, const FConstraintGraph& ConstraintGraph, uint32 ContainerId)
{
	...
	for (const TGeometryParticleHandle<T,d>* Particle : ConstraintGraph.GetIslandParticles(Island))
	{
		...
		if (Particle->ToDynamic() == nullptr && NodeIndexPtr)
		{
...
template<typename T, int d>
void TPBDConstraintColor2<T, d>::InitializeColor(const FConstraintGraph& ConstraintGraph)
{
	Nodes.Reset();
	Edges.Reset();
	IslandData.Reset();
...
template<typename T, int d>
void TPBDConstraintColor2<T, d>::ComputeColor(const int32 Island, const FConstraintGraph& ConstraintGraph, uint32 ContainerId)
{
#ifdef USE_CONTACT_LEVELS
...
template<typename T, int d>
const typename TPBDConstraintColor2<T, d>::FLevelToColorToConstraintListMap& TPBDConstraintColor2<T, d>::GetIslandLevelToColorToConstraintListMap(int32 Island) const
{
...
template<typename T, int d>
int TPBDConstraintColor2<T, d>::GetIslandMaxColor(int32 Island) const
{
	if (Island < IslandData.Num())
	{
...
template<typename T, int d>
int TPBDConstraintColor2<T, d>::GetIslandMaxLevel(int32 Island) const
{
	if (Island < IslandData.Num())
	{
#ifdef USE_CONTACT_LEVELS
...
template class Chaos::TPBDConstraintColor2<float, 3>;
