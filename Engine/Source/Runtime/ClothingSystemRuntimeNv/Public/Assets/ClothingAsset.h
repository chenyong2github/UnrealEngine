...
namespace ClothingAssetUtils
{
	 ...
	 */
	void CLOTHINGSYSTEMRUNTIMENV_API GetMeshClothingAssetBindings(USkeletalMesh* InSkelMesh, TArray<FClothingAssetMeshBinding>& OutBindings);
	
	// Similar to above, but only inspects the specified LOD
	void CLOTHINGSYSTEMRUNTIMENV_API GetMeshClothingAssetBindings(USkeletalMesh* InSkelMesh, TArray<FClothingAssetMeshBinding>& OutBindings, int32 InLodIndex);

#if WITH_EDITOR
	// Clears the clothing tracking struct of a section.
	CLOTHINGSYSTEMRUNTIMENV_API void ClearSectionClothingData(FSkelMeshSection& InSection);
#endif
}
...
class CLOTHINGSYSTEMRUNTIMENV_API UClothingAsset : public UClothingAssetBase
{
	GENERATED_BODY()
