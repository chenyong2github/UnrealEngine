...
namespace Audio
{
	 ...
	 */
	class SIGNALPROCESSING_API FSimpleUpmixer : public FBaseChannelFormatConverter 
	{
		public:
			 ...
			 *
			 * @param InInputFormat - The input audio format to the upmixer.
			 * @param InOutputFormat - The output audio format to the upmixer. 
			 * @param OutEntries - An array of channel mix entries. 
			 *
			 * @return True on success, false on failure. 
			 */
			static bool GetSimpleUpmixerStaticMixEntries(const FInputFormat& InInputFormat, const FOutputFormat& InOutputFormat, TArray<FChannelMixEntry>& OutEntries);
			 ...
			 * @param InNumFramesPerCall - The number of frames used for each call to ProcessAudio.
			 */
			static TUniquePtr<FSimpleUpmixer> CreateSimpleUpmixer(const FInputFormat& InInputFormat, const FOutputFormat& InOutputFormat, int32 InNumFramesPerCall);

		protected:

			FSimpleUpmixer(const FInputFormat& InInputFormat, const FOutputFormat& InOutputFormat, TArrayView<const FChannelMixEntry> InMixEntries, int32 InNumFramesPerCall);

		private:

			void UpdateOutputGain(bool bFadeToGain);

			void GetFrontChannelIndices(TArray<int32>& OutFrontChannelIndices) const;

			// Returns the channels associated with a rear channel indices. 
			void GetPairedRearChannelIndices(int32 InInputChannelIndex, TArray<int32>& OutRearChannelIndices) const;
