...
protected:
	struct FTriangle
	{
		...
		void Set(const int32& Index0, const int32& Index1, const int32& Index2, const TArray<FPoint2D>& InVertices)
		{
			...
			SquareRadius += KINDA_SMALL_NUMBER;
		}
	};
...
#ifdef DEBUG_BOWYERWATSON
			F3DDebugSession _(FString::Printf(TEXT("Step %d"), VertexIndex));
			if(bDisplay)
			{
				F3DDebugSession _(TEXT("New Point"));
				DisplayPoint(NewVertex, EVisuProperty::YellowPoint);
			}
#endif
...
private:
	...
	void DisplayEdges()
	{
		...
		for (int32 Index = 0; Index < EdgeVertexIndices.Num(); Index += 2)
		{
			...
			{
				DisplaySegment(Vertices[EdgeVertexIndices[Index]], Vertices[EdgeVertexIndices[Index + 1]], 0, EVisuProperty::PurpleCurve);
			}
		}
	};
	...
	void DisplayTriangles()
	{
		...
		{
			DisplayTriangle(Index, EVisuProperty::YellowCurve);
		}
		//Wait();
	};
	...
	void DisplayTriangle(int32 Index, EVisuProperty Property)
	{
		...
		DisplaySegment(Vertices[TriangleSet[Index].VertexIndices[0]], Vertices[TriangleSet[Index].VertexIndices[1]], 0, EVisuProperty::BlueCurve);
		DisplaySegment(Vertices[TriangleSet[Index].VertexIndices[1]], Vertices[TriangleSet[Index].VertexIndices[2]], 0, EVisuProperty::BlueCurve);
		DisplaySegment(Vertices[TriangleSet[Index].VertexIndices[2]], Vertices[TriangleSet[Index].VertexIndices[0]], 0, EVisuProperty::BlueCurve);
		//Wait();
	};
