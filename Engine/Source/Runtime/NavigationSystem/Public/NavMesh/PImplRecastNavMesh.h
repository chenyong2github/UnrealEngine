...
public:
	...
	UE_DEPRECATED(4.25, "Use the version with the added MaxCost parameter (FLT_MAX can be used as default).")
	...
	/** Generates path from the given query. Synchronous. */
	ENavigationQueryResult::Type FindPath(const FVector& StartLoc, const FVector& EndLoc, const float MaxCost, FNavMeshPath& Path, const FNavigationQueryFilter& Filter, const UObject* Owner) const;
	...
	int32 DebugPathfinding(const FVector& StartLoc, const FVector& EndLoc, const float MaxCost, const FNavigationQueryFilter& Filter, const UObject* Owner, TArray<FRecastDebugPathfindingData>& Steps);
