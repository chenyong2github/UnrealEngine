...
enum class EAGXRHIClearUAVType : uint8
{
	VertexBuffer,
	StructuredBuffer
};
...
class FAGXRHICommandContext : public IRHICommandContext
{
public:
	FAGXRHICommandContext(class FAGXProfiler* InProfiler, FAGXContext* WrapContext);
	virtual ~FAGXRHICommandContext();

	/** Get the internal context */
	FORCEINLINE FAGXContext& GetInternalContext() const { return *Context; }
...
protected:
	...
	TSharedPtr<FAGXCommandBufferFence, ESPMode::ThreadSafe> CommandBufferFence;
	
	/** Profiling implementation details. */
	class FAGXProfiler* Profiler;
	
	/** Some local variables to track the pending primitive information used in RHIEnd*UP functions */
	FAGXBuffer PendingVertexBuffer;
	uint32 PendingVertexDataStride;
	
	FAGXBuffer PendingIndexBuffer;
...
class FAGXRHIComputeContext : public FAGXRHICommandContext
{
public:
	FAGXRHIComputeContext(class FAGXProfiler* InProfiler, FAGXContext* WrapContext);
...
class FAGXRHIImmediateCommandContext : public FAGXRHICommandContext
{
public:
	FAGXRHIImmediateCommandContext(class FAGXProfiler* InProfiler, FAGXContext* WrapContext);
