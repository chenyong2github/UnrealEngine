...
#pragma mark - Public C++ Boilerplate -
	 ...
	 */
	FAGXCommandList(FAGXCommandQueue& InCommandQueue, bool const bInImmediate);
	
	/** Destructor */
	~FAGXCommandList(void);
...
#pragma mark - Public Command List Accessors -
	 ...
	 */
	bool IsParallel(void) const { return !bImmediate && Num > 0 && FAGXCommandQueue::SupportsFeature(EAGXFeaturesParallelRenderEncoders); }
	...
	FAGXCommandQueue& GetCommandQueue(void) const { return CommandQueue; }
	
private:
#pragma mark - Private Member Variables -
	FAGXCommandQueue& CommandQueue;
	TSharedPtr<TArray<FAGXCommandBufferTiming>, ESPMode::ThreadSafe> FrameCommitedBufferTimings;
	TSharedPtr<FAGXCommandBufferTiming, ESPMode::ThreadSafe> LastCompletedBufferTiming;
