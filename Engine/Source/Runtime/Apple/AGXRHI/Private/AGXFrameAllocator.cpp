...
void FAGXFrameAllocator::SetTargetAllocationLimitInBytes(uint32 LimitInBytes)
{
    TargetAllocationLimit = LimitInBytes;
}

void FAGXFrameAllocator::SetDefaultAllocationSizeInBytes(uint32 DefaultInBytes)
{
    DefaultAllocationSize = DefaultInBytes;
}

void FAGXFrameAllocator::SetStatIds(const TStatId& InTotalAllocationStat, const TStatId& InMemoryInFlightStat, const TStatId& InBytesPerFrameStat)
{
...
FAGXFrameAllocator::AllocationEntry FAGXFrameAllocator::AcquireSpace(uint32 SizeInBytes)
{
...
void FAGXFrameAllocator::MarkEndOfFrame(uint32 FrameNumberThatEnded, id <MTLCommandBuffer> LastCommandBufferInFrame)
{
    __block FAGXFrameAllocator* Allocator = this;
...
id <MTLBuffer> FAGXFrameAllocator::FindOrAllocateBufferUnsafe(uint32 SizeInBytes)
{
...
void FAGXFrameAllocator::RecycleBuffers(NSArray <id <MTLBuffer>>* BuffersToRecycle)
{
