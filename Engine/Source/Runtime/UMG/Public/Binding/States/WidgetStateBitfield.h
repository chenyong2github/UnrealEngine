 ...
 */
USTRUCT()
struct UMG_API FWidgetStateBitfield
{
	GENERATED_BODY()
...
public:
	...
	FWidgetStateBitfield(const FName InStateName);

	/** Interprets name value pair as enum state */
	FWidgetStateBitfield(const FName InStateName, const uint8 InValue);
...
public:
	 ...
	 */
	FWidgetStateBitfield operator~() const;
	 ...
	 */
	FWidgetStateBitfield Intersect(const FWidgetStateBitfield& Rhs) const;
	 ...
	 */
	FWidgetStateBitfield Union(const FWidgetStateBitfield& Rhs) const;
...
public:
	...
	void SetState(const FWidgetStateBitfield& InBitfield);
	 ...
	 * @param BinaryStateValue Value to assign for given Binary state
	 */
	void SetBinaryState(const FWidgetStateBitfield& BinaryStateBitfield, bool BinaryStateValue);

	/** Set Binary state to given value, slow version w/ FName map lookup */
	void SetBinaryStateSlow(FName BinaryStateName, bool BinaryStateValue);
	 ...
	 */
	void SetEnumState(const FWidgetStateBitfield& EnumStateBitfield);

	/** Set enum state to given value, clearing all existing values for that state, slow version w/ FName map lookup */
	void SetEnumStateSlow(FName EnumStateName, uint8 EnumStateValue);
	 ...
	 */
	void ClearEnumState(const FWidgetStateBitfield& EnumStateBitfield);
	...
	void ClearEnumState(FName EnumStateName);
