...
// are crushed.
typedef U32 RADLINK   DecodeMemoryFnType(U32 rate, U32 chans);

// Initialize the decoder, returns 0 on invalid parameters. Unreal should always be encoding bink_audio_2
typedef U32 RADLINK   DecodeOpenFnType(void* BinkAudioDecoderMemory, U32 rate, U32 chans, bool interleave_output, bool is_bink_audio_2);
...
typedef U32 RADLINK   DecodeFnType(void* BinkAudioDecoderMemory, char* OutputBuffer, U32 OutputBufferLen, char const** InputBuffer, char const* InputBufferEnd);

// Call this when seeking/looping, this clears the overlap buffers.
typedef void RADLINK  DecodeResetStartFrameFnType(void* BinkAudioDecoderMemory);
...
RADEXPFUNC UEBinkAudioDecodeInterface* RADEXPLINK UnrealBinkAudioDecodeInterface();
...
#define BinkAudioDecodeOutputMaxSize() ( 2048 * sizeof(S16) * 2 )  // 2 is channels
