...
void ULandscapeComponent::UpdateMaterialInstances_Internal(FMaterialUpdateContext& Context)
{
	...
	for (auto It = MaterialPerLOD.CreateConstIterator(); It; ++It)
	{
		...
		if (CombinationMaterialInstance != nullptr)
		{
			...
			{
				ULandscapeMaterialInstanceConstant* TessellationMaterialInstance = NewObject<ULandscapeMaterialInstanceConstant>(GetOuter());
...
void ULandscapeComponent::ReallocateWeightmaps(FLandscapeEditDataInterface* DataInterface, bool InCanUseCurrentEditingWeightmap, bool InSaveToTransactionBuffer, bool InInitPlatformDataAsync, TArray<UTexture2D*>* OutNewCreatedTextures)
{
	...
	TArray<ULandscapeWeightmapUsage*>& ComponentWeightmapTexturesUsage = GetWeightmapTexturesUsage(InCanUseCurrentEditingWeightmap);
...
void ULandscapeComponent::GeneratePlatformPixelData()
{
	...
	else
	{
		...
		for (auto It = MaterialPerLOD.CreateConstIterator(); It; ++It)
		{
			...
			UMaterialInstanceConstant* NewMobileMaterialInstance = NewObject<ULandscapeMaterialInstanceConstant>(GetOuter());
...
UTexture2D* ALandscapeProxy::CreateLandscapeTexture(int32 InSizeX, int32 InSizeY, TextureGroup InLODGroup, ETextureSourceFormat InFormat, UObject* OptionalOverrideOuter, bool bCompress)
{
	UObject* TexOuter = OptionalOverrideOuter ? OptionalOverrideOuter : this;
