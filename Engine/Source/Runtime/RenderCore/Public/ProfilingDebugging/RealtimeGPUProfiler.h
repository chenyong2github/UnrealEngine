...
#ifndef WANT_BREADCRUMB_EVENTS
#define WANT_BREADCRUMB_EVENTS 0 // Breadcrumbs are always enabled.
#endif
...
struct RENDERCORE_API FBreadcrumbEvent
{
	...
	//overload, to isolate TCHAR literals
	template<typename T, size_t N = 0>
	FBreadcrumbEvent(FRHIComputeCommandList& InRHICmdList, T t); // not implemented


	//Static literals
	template<size_t N>
	FORCEINLINE FBreadcrumbEvent(FRHIComputeCommandList& InRHICmdList, TCHAR const (&Name)[N])
		: RHICmdList(&InRHICmdList)
	{
		if (RHICmdList)
		{
			RHICmdList->PushBreadcrumb_Static(Name);
		}
	}
