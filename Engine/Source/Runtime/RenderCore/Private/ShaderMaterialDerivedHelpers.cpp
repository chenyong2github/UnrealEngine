	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	Dst.WRITES_VELOCITY_TO_GBUFFER_USE_POS_INTERPOLATOR = (Dst.WRITES_VELOCITY_TO_GBUFFER);// && USING_TESSELLATION);
	...
	// No, just no
	Dst.TESSELLATION_SUPPORTED = false; // (PC_D3D || GL4_PROFILE || SWITCH_PROFILE || SWITCH_PROFILE_FORWARD || METAL_TESSELLATION_PROFILE || VULKAN_PROFILE_SM5);
...
////Materials also have to opt in to these features.
	...
	Dst.MATERIAL_WORKS_WITH_DUAL_SOURCE_COLOR_BLENDING = (Dst.MATERIAL_SHADINGMODEL_THIN_TRANSLUCENT || Mat.STRATA_TRANSLUCENT_ENABLED);
