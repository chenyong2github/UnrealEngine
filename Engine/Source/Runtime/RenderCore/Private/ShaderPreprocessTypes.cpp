...
{
	if (!Diagnostic.ErrorLineString.IsEmpty() && !Diagnostic.ErrorVirtualFilePath.IsEmpty())
	{
		if (Diagnostic.ErrorVirtualFilePath != FilenameSentinel)
		{
		...
		// Some backends give us already-parsed FShaderCompilerError objects which simplifies the remapping process somewhat
	...
	// need to parse/remap/reconstruct the full error message
	else
	{
		const FString& Message = Diagnostic.StrippedErrorMessage;
		int32 FilenameIndex = Message.Find(FilenameSentinel);
		if (FilenameIndex < 0)
		{
			// only attempt remapping if we find our sentinel filename in the diagnostic message; otherwise it's likely
			// a non-line-specific error that doesn't need remapping
			return;
		}
		...
		while (!FChar::IsDigit(Message[LineNumberStart]))
		{
			++LineNumberStart;
		}
		int32 LineNumberEnd = LineNumberStart;
		int32 StrippedLineNum = 0;
		while (FChar::IsDigit(Message[LineNumberEnd]) && LineNumberEnd < Message.Len())
		{
			StrippedLineNum = StrippedLineNum * 10 + (Message[LineNumberEnd++] - TEXT('0'));
		}
		...
		int32 RemappedLen = Message.Len() - FilenameSentinelLen + RemapData.Filename.Len();
		FString RemappedMessage;
		RemappedMessage.Reserve(RemappedLen);
		RemappedMessage.AppendChars(Message.GetCharArray().GetData(), FilenameIndex);
		RemappedMessage.Append(RemapData.Filename);
		int32 SeparatorStart = FilenameIndex + FilenameSentinelLen;
		RemappedMessage.Append(Message.GetCharArray().GetData() + SeparatorStart, LineNumberStart - SeparatorStart);
		RemappedMessage.AppendInt(RemapData.LineNumber);
		RemappedMessage.Append(Message.GetCharArray().GetData() + LineNumberEnd, Message.Len() - LineNumberEnd);
...
void FShaderDiagnosticRemapper::AddSourceBlock(int32 OriginalLineNum, int32 StrippedLineNum, FString && OriginalPath)
{
	...
	else
	{
		...
		// sanity check to ensure if we're not given a path we must have a previous block from which to retrieve the path
		check(!OriginalPath.IsEmpty() || Blocks.Num() > 0);
