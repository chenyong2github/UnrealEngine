	...
	static void DeltaEncode(const TArray<uint32>& InValues, TArray<uint16>& OutDeltas)
	{
		if (InValues.Num() > 0)
		{
			...
			for (int32 i = 1; i < InValues.Num(); ++i)
			{
				...
				check(Delta >= 0);
				check(Delta < MAX_uint16);
				OutDeltas[i] = static_cast<uint16>(Delta);
			}
		}
	}
...
bool FStreamedAudioChunkSeekTable::Parse(const uint8* InMemory, uint32 InSize, uint32& InOutOffset, FStreamedAudioChunkSeekTable& OutTable)
{
...
struct FConstantRateSeekTable : public FStreamedAudioChunkSeekTable::ISeekTableImpl
{
	...
	void Add(uint32 InTimeInAudioFrames, uint32 InOffset) override
	{
		...
		else if(Table.Num() == 1)
		{
			...
			check(DeltaFrames >= 0);
			check(DeltaFrames < MAX_uint16);
			AudioFramesPerEntry = static_cast<uint16>(DeltaFrames);
		}		
		Table.Add(InOffset);
	}
