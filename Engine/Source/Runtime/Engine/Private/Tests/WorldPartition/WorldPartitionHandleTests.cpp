...
namespace WorldPartitionTests
{
	...
	bool FWorldPartitionSoftRefTest::RunTest(const FString& Parameters)
	{
...

		{
			...
			Handle = FWorldPartitionHandle(ActorDescContainer, FGuid(TEXT("5D9F93BA407A811AFDDDAAB4F1CECC6A")));
			...
			// Make sure the container gets destroyed so we can test destrucing dangling handles
			CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS);
		}

		TestTrue(TEXT("Invalid container"), true);
#endif
		return true;
	}
}
