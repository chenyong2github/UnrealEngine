...
void FStreamedAudioChunk::Serialize(FArchive& Ar, UObject* Owner, int32 ChunkIndex)
{
	...
	{
		const FPlatformAudioCookOverrides* Overrides = FPlatformCompressionUtilities::GetCookOverrides(*CookingTarget->PlatformName());
		check(Overrides);
...
void USoundWave::Serialize( FArchive& Ar )
{
	...
	if (bCooked)
	{
		...
		const bool bSoundCueDataHasStreamingDisabled = []() { return FParse::Param(FCommandLine::Get(), TEXT("SoundWaveDataHasStreamingDisabled")); }();
		bShouldStreamSound = bShouldStreamSound && !bSoundCueDataHasStreamingDisabled;
#endif
