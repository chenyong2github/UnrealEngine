...
namespace UE_NETWORK_PHYSICS
{
	...
	FAutoConsoleVariableRef CVarEnableNetworkPhysicsPrediction(TEXT("np2.EnableNetworkPhysicsPreidction"), EnableNetworkPhysicsPrediction, TEXT("Enables network physics prediction"));
}
	...
	: Super(ObjectInitializer)
{
	...
	bAsyncPhysicsTickEnabled = !!UE_NETWORK_PHYSICS::EnableNetworkPhysicsPrediction;
}
...
void APlayerController::TickActor( float DeltaSeconds, ELevelTick TickType, FActorTickFunction& ThisTickFunction )
{
	...
	if(!!UE_NETWORK_PHYSICS::EnableNetworkPhysicsPrediction && GetLocalRole() == ROLE_AutonomousProxy && bIsClient)
	{
...
FAsyncPhysicsTimestamp APlayerController::GetAsyncPhysicsTimestamp(float DeltaSeconds)
{
	...
	if(UWorld* World = GetWorld())
	{
		if (FPhysScene* PhysScene = World->GetPhysicsScene())
		{
			...
			{
				const int32 PendingSteps = DeltaSeconds / Solver->GetAsyncDeltaTime();
