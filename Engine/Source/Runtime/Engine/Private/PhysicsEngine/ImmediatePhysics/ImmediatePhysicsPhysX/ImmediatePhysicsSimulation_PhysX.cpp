...
#include "Physics/ImmediatePhysics/ImmediatePhysicsPhysX/ImmediatePhysicsActorHandle_PhysX.h"
#include "Physics/ImmediatePhysics/ImmediatePhysicsPhysX/ImmediatePhysicsJointHandle_PhysX.h"
...
void FSimulation::SetNumActiveBodies(int32 InNumActiveBodies)
{
...
FJointHandle* FSimulation::CreateJoint(FConstraintInstance* ConstraintInstance, FActorHandle* Actor1, FActorHandle* Actor2)
{
	...
	PxD6Joint* Joint = ConstraintInstance->GetPhysicsConstraintRef().ConstraintData;
...
bool FSimulation::GetIsKinematic(int32 ActorDataIndex)
{
...
void FSimulation::SetIsKinematic(int32 ActorDataIndex, bool bKinematic)
{
	const int32 NumMobileBodies = NumSimulatedBodies + NumKinematicBodies;
	const bool bIsSimulated = ActorDataIndex < NumSimulatedBodies;
...
int32 FSimulation::CreateActor(PxRigidActor* RigidActor, const FTransform& WorldTM)
{
	...
	PxRigidBody* const RigidBody = ActorType == EActorType::StaticActor ? nullptr : static_cast<PxRigidBody*>(RigidActor);	//Note the const pointer - this is for templated code
	PxRigidDynamic* const RigidDynamic = ActorType == EActorType::DynamicActor ? static_cast<PxRigidDynamic*>(RigidActor) : nullptr;	//Note the const pointer - this is for templated code
	...
	if(RigidDynamic)
	{
		...
		PxU32 PositionIterations, VelocityIterations;
		RigidDynamic->getSolverIterationCounts(PositionIterations, VelocityIterations);

		NumPositionIterations = FMath::Max((int32)PositionIterations, NumPositionIterations);
		NumVelocityIterations = FMath::Max((int32)VelocityIterations, NumVelocityIterations);
	...
	if (ActorType == EActorType::StaticActor)
	{
	...
	if (ActorType == EActorType::DynamicActor)	//dynamic first
	{
	...
	else if(ActorType == EActorType::KinematicActor)  //then kinematics
	{
...
FActorHandle* FSimulation::InsertActorData(const FActor& InActor, const FActorData& InData)
{
	// @todo(ccaulfield): does not seem to be called anywhere...
...
void FSimulation::RemoveActor(FActorHandle* InHandle)
{
	// @todo(ccaulfield): this is never called. Also I think it should delete the handle?
	...
	bool bDynamic = DataIndex < NumSimulatedBodies;
	bool bKinematic = !bDynamic && DataIndex < NumKinematicBodies;
...
FActorHandle* FSimulation::CreateDynamicActor(FBodyInstance* BodyInstance, const FTransform& TM)
{
	...
	{
		const int32 ActorDataIndex = CreateActor<EActorType::DynamicActor>(PDynamic, TM);
		NumActiveSimulatedBodies = NumSimulatedBodies;
		bDirtyJointData = true;
...
FActorHandle* FSimulation::CreateKinematicActor(FBodyInstance* BodyInstance, const FTransform& TM)
{
	PxRigidActor* PRigidActor = FPhysicsInterface::GetPxRigidActor_AssumesLocked(BodyInstance->GetPhysicsActorHandle());
	if (PxRigidBody* PRigidBody = PRigidActor->is<PxRigidBody>())
	{
		const int32 ActorDataIndex = CreateActor<EActorType::KinematicActor>(PRigidBody, TM);
		return ActorHandles[ActorDataIndex];
	}
	return nullptr;
}
...
{
	if (PxRigidActor* RigidActor = FPhysicsInterface_PhysX::GetPxRigidActor_AssumesLocked(BodyInstance->ActorHandle))
	{
		const int32 ActorDataIndex = CreateActor<EActorType::StaticActor>(RigidActor, P2UTransform(RigidActor->getGlobalPose()));
		return ActorHandles[ActorDataIndex];
	}
	return nullptr;
}

FActorHandle* FSimulation::CreateActor(EActorType ActorType, FBodyInstance* BodyInstance, const FTransform& TM)
{
	switch (ActorType)
	{
	case EActorType::StaticActor:
		return CreateStaticActor(BodyInstance);
	case EActorType::KinematicActor:
		return CreateKinematicActor(BodyInstance, TM);
	case EActorType::DynamicActor:
		return CreateDynamicActor(BodyInstance, TM);
	}
	return nullptr;
}
...
void FSimulation::Simulate(float DeltaTime, const FVector& Gravity)
{
	...
	FScopedSharedResourceLock<LockMode::Read> ScopeLock;
...
void FSimulation::ConstructSolverBodies(float DeltaTime, const FVector& Gravity)
{
	...
	for(int32 BodyIdx = 0; BodyIdx < NumActiveSimulatedBodies; ++BodyIdx)
	{
	...
	for (int32 KinematicBodyIdx = NumSimulatedBodies; KinematicBodyIdx < NumActors; ++KinematicBodyIdx)
	{
	...
	if (bDirtyJointData)
	{
		...
		if(NumJoints > 0)
		{
			...
			for (int32 JointIdx = 0; JointIdx < NumJoints; ++JointIdx)
			{
				...
				const int32 DynamicActorIdx = Joint.DynamicActor->ActorDataIndex;
				const int32 OtherActorIdx = Joint.OtherActor != nullptr ? Joint.OtherActor->ActorDataIndex : INDEX_NONE;
			...
			else
			{
				...
				for(int32 Offset = 0; Offset < NumActiveJoints; ++Offset)
				{
			...
			for(int32 DescriptorIdx = 0; DescriptorIdx < NumActiveJoints; ++DescriptorIdx)
			{
...
void FSimulation::PrepareConstraints(float DeltaTime)
{
	...
	for (PxU32 HeaderIdx = 0; HeaderIdx < NumJointHeaders; ++HeaderIdx)
	{
		...
		for (PxU32 BatchInnerIdx = 0; BatchInnerIdx < Header.mStride; ++BatchInnerIdx)
		{
			...
			JointDescriptor.bodyState1 = JointDescriptor.body1 == nullptr ? PxSolverConstraintPrepDescBase::eSTATIC_BODY : (OrderedDescriptor.bodyBDataIndex < (uint32)NumSimulatedBodies ? PxSolverConstraintPrepDescBase::eDYNAMIC_BODY : PxSolverConstraintPrepDescBase::eSTATIC_BODY);
			...
			int32 NumRows = D6JointSolverPrep(&Rows[CurRows], Body0WorldOffset, 0, InvMassScale, (const void*)&Joint, JointDescriptor.bodyFrame0, JointDescriptor.bodyFrame1);
	...
	for (PxU32 HeaderIdx = NumJointHeaders; HeaderIdx < NumTotalHeaders; ++HeaderIdx)
	{
		...
		for (PxU32 BatchInnerIdx = 0; BatchInnerIdx < Header.mStride; ++BatchInnerIdx)
		{
			...
			ContactDescriptor.bodyState1 = ContactPair.OtherActorDataIndex < (uint32)NumSimulatedBodies ? PxSolverConstraintPrepDescBase::eDYNAMIC_BODY : PxSolverConstraintPrepDescBase::eSTATIC_BODY;
...
void FSimulation::SolveAndIntegrate(float DeltaTime)
{
	...
	for (int32 DynamicsBodyIdx = 0; DynamicsBodyIdx < NumActiveSimulatedBodies; ++DynamicsBodyIdx)
	{
