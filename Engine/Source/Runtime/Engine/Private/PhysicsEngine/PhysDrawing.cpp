...
void FKAggregateGeom::GetAggGeom(const FTransform& Transform, const FColor Color, const FMaterialRenderProxy* MatInst, bool bPerHullColor, bool bDrawSolid, int32 ViewIndex, FMeshElementCollector& Collector) const
{
	...
	if(ConvexElems.Num() > 0)
	{
		if(bDrawSolid)
		{
			...
			if(RenderInfo->HasValidGeometry())
			{
				...
				DynamicPrimitiveUniformBuffer.Set(LocalToWorld.ToMatrixWithScale(), LocalToWorld.ToMatrixWithScale(), WorldBounds, LocalBounds, true, false);
...
void UPhysicsAsset::GetCollisionMesh(int32 ViewIndex, FMeshElementCollector& Collector, const USkeletalMesh* SkelMesh, const TArray<FTransform>& SpaceBases, const FTransform& LocalToWorld, const FVector& Scale3D)
{
	for( int32 i=0; i<SkeletalBodySetups.Num(); i++)
	{
		...
		{
			SkeletalBodySetups[i]->AggGeom.GetAggGeom(BoneTransform, *BoneColor, NULL, false, false, ViewIndex, Collector);
		}
	}
}
