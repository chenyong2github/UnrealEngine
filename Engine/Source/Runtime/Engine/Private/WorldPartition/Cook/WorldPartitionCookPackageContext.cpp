...
const FWorldPartitionCookPackage* FWorldPartitionCookPackageContext::AddPackageToGenerateInternal(IWorldPartitionCookPackageGenerator* CookPackageGenerator, const FString& Root, const FString& RelativePath, FWorldPartitionCookPackage::EType Type)
{
	...
	{
		FWorldPartitionCookPackage::IDType PackageId = FWorldPartitionCookPackage::MakeCookPackageID(Root, RelativePath);
		TUniquePtr<FWorldPartitionCookPackage>* ExistingPackage = PackagesToCookById.Find(PackageId);
		if (ExistingPackage == nullptr)
		{
			TUniquePtr<FWorldPartitionCookPackage>& CookPackage = PackagesToCookById.Emplace(PackageId, MakeUnique<FWorldPartitionCookPackage>(Root, RelativePath, Type));
			...
			UE_LOG(LogWorldPartition, Verbose, TEXT("[Cook] Added Package %s with ID %llu in context"), *CookPackage->GetFullPath(), PackageId);
		...
		else
		{
			UE_LOG(LogWorldPartition, Error, TEXT("[Cook] Trying to add package %s in context but there is already a package to generate with the same ID (%llu). Other package: %s Id %llu"),
				*FWorldPartitionCookPackage::MakeFullPath(Root, RelativePath), PackageId, *(*ExistingPackage)->GetFullPath(), (*ExistingPackage)->PackageId);
		}
	}
	else
	{
		UE_LOG(LogWorldPartition, Error, TEXT("[Cook] Trying to add package %s in context, but its generator is not registered."), *FWorldPartitionCookPackage::MakeFullPath(Root, RelativePath));
	}
