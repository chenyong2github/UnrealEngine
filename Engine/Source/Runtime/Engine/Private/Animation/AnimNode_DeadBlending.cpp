	...
	const FInertializationPose& SrcPoseCurr)
{
	...
	const int32 CurveNum = InCurves.IsValid() ? InCurves.UIDToArrayIndexLUT->Num() : 0;

	CurveValid.Init(false, CurveNum);
	CurveValues.Init(0.0f, CurveNum);
	CurveVelocities.Init(0.0f, CurveNum);
	CurveDecayHalfLives.Init(ExtrapolationHalfLifeMin, CurveNum);

	// Record curve state

	for (int32 CurveUID = 0; CurveUID < CurveNum; CurveUID++)
	{
		// Check if the curve is in our filter set. Leave Curves zeroed if it is.
		if (CachedFilteredCurvesUIDs.Contains((SmartName::UID_Type)CurveUID))
		{
			continue;
		}

		const int32 SrcCurveIdxCurr = SrcPoseCurr.Curves.BlendedCurve.GetArrayIndexByUID(CurveUID);
		
		if (SrcCurveIdxCurr == INDEX_NONE)
		{
			continue;
		}
		...
		CurveValid[SrcCurveIdxCurr] = true;

		// Get Curve Value

		const float SrcCurveCurr = SrcPoseCurr.Curves.BlendedCurve.CurveWeights[SrcCurveIdxCurr];

		CurveValues[SrcCurveIdxCurr] = SrcCurveCurr;

		const int32 SrcCurveIdxPrev = SrcPosePrev.Curves.BlendedCurve.GetArrayIndexByUID(CurveUID);
		const int32 DstCurveIdx = InCurves.GetArrayIndexByUID(CurveUID);

		if (SrcPoseCurr.DeltaTime > UE_SMALL_NUMBER && SrcCurveIdxPrev != INDEX_NONE && DstCurveIdx != INDEX_NONE)
		{
			// Get Curve Velocity

			const float SrcCurvePrev = SrcPosePrev.Curves.BlendedCurve.CurveWeights[SrcCurveIdxPrev];

			CurveVelocities[SrcCurveIdxCurr] = (SrcCurveCurr - SrcCurvePrev) / SrcPoseCurr.DeltaTime;

			// Compute Decay Halflife

			CurveDecayHalfLives[SrcCurveIdxCurr] = UE::Anim::DeadBlending::Private::ComputeDecayHalfLifeFromDiffAndVelocity(
				InCurves.CurveWeights[DstCurveIdx] - SrcCurveCurr,
				CurveVelocities[SrcCurveIdxCurr],
				ExtrapolationHalfLife,
				ExtrapolationHalfLifeMin,
				ExtrapolationHalfLifeMax);
		}
...
void FAnimNode_DeadBlending::ApplyTo(FCompactPose& InOutPose, FBlendedCurve& InOutCurves)
{
	...
	if (InOutCurves.IsValid())
	{
		...
		const int32 CurveNum = FMath::Min(InOutCurves.UIDToArrayIndexLUT->Num(), CurveValues.Num());

		for (int32 CurveUID = 0; CurveUID != CurveNum; ++CurveUID)
		{
			const int32 CurveIdx = InOutCurves.GetArrayIndexByUID(CurveUID);

			if (CurveIdx == INDEX_NONE || !CurveValid[CurveIdx])
			{
				continue;
			}
			...
			const float ExtrapolatedCurve = UE::Anim::DeadBlending::Private::ExtrapolateCurve(
				CurveValues[CurveIdx],
				CurveVelocities[CurveIdx],
				InertializationTime,
				CurveDecayHalfLives[CurveIdx]);
			...
			if (CurveAlpha != 0.0f)
			{
				...
				if (bShowExtrapolations)
				{
					InOutCurves.CurveWeights[CurveIdx] = ExtrapolatedCurve;
					InOutCurves.ValidCurveWeights[CurveIdx] = true;
					continue;
				}
#endif
				InOutCurves.CurveWeights[CurveIdx] = FMath::Lerp(InOutCurves.CurveWeights[CurveIdx], ExtrapolatedCurve, CurveAlpha);
				InOutCurves.ValidCurveWeights[CurveIdx] = true;
			}
		}
	}
...
void FAnimNode_DeadBlending::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	...
	CachedFilteredCurvesUIDs.Empty(FilteredCurves.Num());
	...
	CurveValid.Empty();
	CurveValues.Empty();
	CurveVelocities.Empty();
	CurveDecayHalfLives.Empty();
...
void FAnimNode_DeadBlending::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	else
	{
		...
		for (int32 SnapshotIndex = 0; SnapshotIndex < UE::Anim::DeadBlending::Private::MaxPoseSnapShotNum - 1; ++SnapshotIndex)
		{
			...
			checkSlow(PoseSnapshots[SnapshotIndex + 0].Curves.BlendedCurve.UIDToArrayIndexLUT == &PoseSnapshots[SnapshotIndex + 0].Curves.CurveUIDToArrayIndexLUT);
			checkSlow(PoseSnapshots[SnapshotIndex + 1].Curves.BlendedCurve.UIDToArrayIndexLUT == &PoseSnapshots[SnapshotIndex + 1].Curves.CurveUIDToArrayIndexLUT);
		}
