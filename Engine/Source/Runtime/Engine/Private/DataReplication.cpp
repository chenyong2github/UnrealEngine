		...
		if (!Connection->IsInternalAck())
		{
			if (FSendingRepState * SendingRepState = RepState.IsValid() ? RepState->GetSendingRepState() : nullptr)
			{
				...
				if (OwningChannel->SpawnAcked)
				{
					...
					UE_LOG(LogRep, Verbose, TEXT("FObjectReplicator::InitRecentProperties -  OwningChannel->SpawnAcked is true, also setting SendingRepState->bOpenAckedCalled to true"), OwningChannel->ChIndex);
				}
			}
		}
