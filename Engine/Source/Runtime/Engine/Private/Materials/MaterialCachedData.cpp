...
bool FMaterialCachedExpressionData::AddParameter(const FMaterialParameterInfo& ParameterInfo, const FMaterialParameterMetadata& ParameterMeta, UObject*& OutReferencedTexture)
{
	...
	else
	{
		...
		case EMaterialParameterType::Scalar:
			bSameValue =
				ScalarValues[Index] == ParameterMeta.Value.AsScalar()
				&& ScalarPrimitiveDataIndexValues[Index] == ParameterMeta.PrimitiveDataIndex;
			if (ParameterMeta.bUsedAsAtlasPosition)
			{
				bSameValue &=
					EditorOnlyData->ScalarCurveValues[Index] == ParameterMeta.ScalarCurve.Get()
					&& EditorOnlyData->ScalarCurveAtlasValues[Index] == ParameterMeta.ScalarAtlas.Get();
			}
			break;

		case EMaterialParameterType::Vector:
			bSameValue = VectorValues[Index] == ParameterMeta.Value.AsLinearColor()
				&& EditorOnlyData->VectorChannelNameValues[Index] == ParameterMeta.ChannelNames
				&& EditorOnlyData->VectorUsedAsChannelMaskValues[Index] == ParameterMeta.bUsedAsChannelMask
				&& VectorPrimitiveDataIndexValues[Index] == ParameterMeta.PrimitiveDataIndex;
			break;
		...
		case EMaterialParameterType::Texture:
			bSameValue = TextureValues[Index] == ParameterMeta.Value.Texture
				&& EditorOnlyData->TextureChannelNameValues[Index] == ParameterMeta.ChannelNames;
			break;

		case EMaterialParameterType::Font:
			bSameValue = FontValues[Index] == ParameterMeta.Value.Font.Value
				&& FontPageValues[Index] == ParameterMeta.Value.Font.Page;
			break;
		...
		case EMaterialParameterType::StaticSwitch:
			bSameValue = EditorOnlyData->StaticSwitchValues[Index] == ParameterMeta.Value.AsStaticSwitch();
			break;
