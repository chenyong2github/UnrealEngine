...
FMaterialHLSLGenerator::FMaterialHLSLGenerator(EShaderPlatform InShaderPlatform,
...
bool FMaterialHLSLGenerator::GenerateHLSL(FMaterial& InMaterial,
	FMaterialCompilationOutput& OutCompilationOutput,
	TRefCountPtr<FSharedShaderCompilerEnvironment>& OutMaterialEnvironment)
{
	UMaterialInterface* MaterialInterface = InMaterial.GetMaterialInterface();
	...
	UE::HLSLTree::FStatement* RootStatement = AcquireStatement(HLSLTree->GetRootScope(), BaseExpression);
	...
	HLSLTree->EmitHLSL(EmitContext, *CodeWriter);

	InMaterial.CompileErrors = MoveTemp(CompileErrors);
	InMaterial.ErrorExpressions = MoveTemp(ErrorExpressions);

	const FString MaterialTemplateSource = GenerateMaterialTemplateHLSL(ShaderPlatform, InMaterial, EmitContext, CodeWriter->StringBuilder->ToString());

	OutMaterialEnvironment = new FSharedShaderCompilerEnvironment();
	OutMaterialEnvironment->TargetPlatform = TargetPlatform;
	GetMaterialEnvironment(ShaderPlatform, InMaterial, OutCompilationOutput, *OutMaterialEnvironment);
...
UE::HLSLTree::FTextureParameterDeclaration* FMaterialHLSLGenerator::AcquireTextureDeclaration(UE::HLSLTree::FScope& Scope, const UE::HLSLTree::FTextureDescription& Value)
{
	...
	if (!UMaterialExpressionTextureBase::VerifySamplerType(FeatureLevel, TargetPlatform, Value.Texture, Value.SamplerType, SamplerTypeError))
	{
...
{
	FString SamplerTypeError;
	if (!UMaterialExpressionTextureBase::VerifySamplerType(FeatureLevel, TargetPlatform, DefaultValue.Texture, DefaultValue.SamplerType, SamplerTypeError))
	{
		Errorf(TEXT("%s"), *SamplerTypeError);
		return nullptr;
	}
