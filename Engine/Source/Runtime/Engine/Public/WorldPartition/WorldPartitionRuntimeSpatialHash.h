...
private:
...
#if WITH_EDITOR
	FName GetCellName(FName InGridName, int32 InLevel, int32 InCellX, int32 InCellY) const;
	...
	virtual FVector2D GetShowDebugDesiredFootprint(const FVector2D& CanvasSize) const override;
...
#if WITH_EDITOR
	bool CreateStreamingGrid(const FSpatialHashRuntimeGrid& RuntimeGrid, const FSquare2DGridHelper& PartionnedActors, EWorldPartitionStreamingMode Mode, UWorldPartitionStreamingPolicy* StreamingPolicy);
#endif
};