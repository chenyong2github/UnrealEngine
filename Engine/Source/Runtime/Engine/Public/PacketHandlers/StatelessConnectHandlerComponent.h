...
private:
	 ...
	 */
	void SendConnectChallenge(TSharedPtr<const FInternetAddr> ClientAddress);
	 ...
	 * @param InCookie			The cookie value to send
	 */
	void SendChallengeAck(TSharedPtr<const FInternetAddr> ClientAddress, uint8 InCookie[COOKIE_BYTE_SIZE]);
	 ...
	 */
	void SendRestartHandshakeRequest(const TSharedPtr<const FInternetAddr> ClientAddress);
