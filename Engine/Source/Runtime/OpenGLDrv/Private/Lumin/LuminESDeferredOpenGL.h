...
struct FLuminESDeferredOpenGL : public FOpenGLESDeferred
{
	static FORCEINLINE EShaderPlatform GetShaderPlatform()
	{
		return SP_OPENGL_ES31_EXT;
	}
	...
	{
		if (glTexStorage2D != NULL && InternalFormat != GL_LUMINANCE && InternalFormat != GL_ALPHA)
		{
	...
	{
		if (glTexStorage3D && InternalFormat != GL_LUMINANCE && InternalFormat != GL_ALPHA)
		{
	...
	static FORCEINLINE void DeleteSync(UGLsync Sync)
	{
		...
		{ 
			...
			{
				//handle error here
				EGLBoolean Result = eglDestroySyncKHR(LuminEGL::GetInstance()->GetDisplay(), Sync.EGL);
				if (Result == EGL_FALSE)
				{
					//handle error here
				}
	...
	static FORCEINLINE UGLsync FenceSync(GLenum Condition, GLbitfield Flags)
	{
		if (IsES2())
		{
			...
			return UGLsync(GUseThreadedRendering ? eglCreateSyncKHR(LuminEGL::GetInstance()->GetDisplay(), EGL_SYNC_FENCE_KHR, NULL) : 0);
		}
		else
		{
	...
	static FORCEINLINE EFenceResult ClientWaitSync(UGLsync Sync, GLbitfield Flags, GLuint64 Timeout)
	{
		if (IsES2())
		{
			if (GUseThreadedRendering)
			{
				...
				GLenum Result = eglClientWaitSyncKHR(LuminEGL::GetInstance()->GetDisplay(), Sync.EGL, EGL_SYNC_FLUSH_COMMANDS_BIT_KHR, Timeout);
				switch (Result)
				{
	...
	static FORCEINLINE bool IsES2()				{ return false; }
...
typedef FLuminESDeferredOpenGL FOpenGL;
