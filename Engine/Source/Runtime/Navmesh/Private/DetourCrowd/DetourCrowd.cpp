...
void dtCrowd::updateMoveRequest(const float /*dt*/)
{
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		if (ag->targetState == DT_CROWDAGENT_TARGET_REQUESTING)
		{
			...
			const float maxCost = FLT_MAX; //@UE4
			m_navquery->initSlicedFindPath(path[0], ag->targetRef, ag->npos, ag->targetPos, maxCost, &m_filters[ag->params.filter]); //@UE4
	...
	for (int i = 0; i < nqueue; ++i)
	{
		...
		ag->targetPathqRef = m_pathq.request(ag->corridor.getLastPoly(), ag->targetRef,
			ag->corridor.getTarget(), ag->targetPos, maxCost, &m_filters[ag->params.filter], ag->params.linkFilter); //@UE4
