...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FPathTracingLightData, RENDERER_API)
	...
	SHADER_PARAMETER_ARRAY(float, FalloffExponent, [RAY_TRACING_LIGHT_COUNT_MAXIMUM])
	...
	// IES index (valid if >= 0)
	SHADER_PARAMETER_ARRAY(float, IESTextureSlice , [RAY_TRACING_LIGHT_COUNT_MAXIMUM])
	// Flags
