...
void FSceneRenderer::RenderSkyAtmosphereEditorNotifications(FRDGBuilder& GraphBuilder, FRDGTextureRef SceneColorTexture) const
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		if (View.bSceneHasSkyMaterial && View.Family->EngineShowFlags.Atmosphere && View.Family->EngineShowFlags.Lighting)
		{
			...
			PassParameters->ViewUniformBuffer = View.ViewUniformBuffer;
			PassParameters->MiniFontTexture = GEngine->MiniFontTexture ? GEngine->MiniFontTexture->GetResource()->TextureRHI : GSystemTextures.WhiteDummy->GetRenderTargetItem().TargetableTexture;
