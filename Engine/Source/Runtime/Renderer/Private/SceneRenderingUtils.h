...
template <typename PassParametersType>
inline void AddBatchedElementsPass(const FBatchedElements& BatchedElements, FRDGBuilder& GraphBuilder, PassParametersType* PassParameters, const FViewInfo& View, const FMeshPassProcessorRenderState& DrawRenderState, bool bNeedToSwitchVerticalAxis)
{
	GraphBuilder.AddPass(
		...
		[&View, &BatchedElements, DrawRenderState, bNeedToSwitchVerticalAxis](FRHICommandList& RHICmdList)
		{
			...
			BatchedElements.Draw(RHICmdList, DrawRenderState, View.FeatureLevel, bNeedToSwitchVerticalAxis, View, false);
		}
	);
}
