	...
	FRDGTextureRef SceneSeparateTranslucency)
{
	...
	RDG_EVENT_SCOPE(GraphBuilder, "DiaphragmDOF");
	...
	// Setup at lower resolution from full resolution scene color and scene depth.
	{
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("DiaphragmDOF Setup(%s CoC=[%d;%d]) %dx%d",
	...
	FDOFTileClassificationTextures TileClassificationTextures;
	{
		...
		FDOFTileClassificationTextures FlattenedTileClassificationTextures;
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("DiaphragmDOF FlattenCoc(Gather4=%s) %dx%d",
			...
			int32 SampleRadiusCount, int32 SampleOffsetMultipler)
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("DiaphragmDOF DilateCoc(1/16 %s radius=%d step=%d) %dx%d",
	...
	FRDGBufferRef BackgroundScatterDrawListBuffer = nullptr;
	{
		...
		if (bForegroundHybridScattering || bBackgroundHybridScattering)
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("DiaphragmDOF Downsample %dx%d", PassViewSize.X, PassViewSize.Y),
				*ComputeShader,
				PassParameters,
		...
		// Adds the reduce pass.
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("DiaphragmDOF Reduce(Mips=%d FgdScatter=%s BgdScatter=%s%s) %dx%d",
					MipLevelCount,
		...
		if (bForegroundHybridScattering || bBackgroundHybridScattering)
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("DiaphragmDOF ScatterGroupPack"),
				*ComputeShader,
				PassParameters,
				FIntVector(2, 1, 1));
		}
	...
	auto AddBuildBokehLUTPass = [&](EDiaphragmDOFBokehLUTFormat LUTFormat)
	{
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("DiaphragmDOF BuildBokehLUT(Blades=%d Shape=%s, LUT=%s) %dx%d",
	...
	FDOFConvolutionTextures SlightOutOfFocusConvolutionTextures;
	{
			...
			FRDGTextureRef* ScatterOcclusionTexture)
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("DiaphragmDOF Gather(%s %s Bokeh=%s Rings=%d%s%s) %dx%d",
			...
			FDOFConvolutionTextures* ConvolutionTextures)
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("DiaphragmDOF Postfilter(%s %s%s) %dx%d",
			...
			FRDGBufferRef ScatterDrawList)
		{
			...
			GraphBuilder.AddPass(
				RDG_EVENT_NAME("DiaphragmDOF IndirectScatter(%s Bokeh=%s Occlusion=%s) %dx%d",
	...
	FRDGTextureRef NewSceneColor;
	{
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("DiaphragmDOF Recombine(%s Quality=%d Bokeh=%s alpha=no) %dx%d",
