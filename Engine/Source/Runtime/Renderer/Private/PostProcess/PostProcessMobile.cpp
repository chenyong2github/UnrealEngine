...
#undef A_RESTRICT
#endif
#include "../Shaders/Private/Mobile/FSR/ffx_a.ush"
#include "../Shaders/Private/Mobile/FSR/ffx_fsr1.ush"
...
public:
	...
	using FPermutationDomain = TShaderPermutationDomain<FFSR_UseFP16Dim>;
...
IMPLEMENT_GLOBAL_SHADER(FFSRCS, "/Engine/Private/Mobile/FSR/PostProcessFFX_FSR.usf", "MainCS", SF_Compute);
...
IMPLEMENT_GLOBAL_SHADER(FFSRPS, "/Engine/Private/Mobile/FSR/PostProcessFFX_FSR.usf", "MainPS", SF_Pixel);
...
FScreenPassTexture AddEASUPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const ISpatialUpscaler::FInputs& PassInputs)
{
	...
	if (!bOutputSupportsUAV || GMobileFSRDisableCompute)
	{
		...
		PSPermutationVector.Set<FFSR_UseFP16Dim>(bUseFP16);
	...
	else
	{
		...
		CSPermutationVector.Set<FFSR_UseFP16Dim>(bUseFP16);
...
public:
	...
	using FPermutationDomain = TShaderPermutationDomain<FFSR_UseFP16Dim, FRCAS_DenoiseDim>;
...
IMPLEMENT_GLOBAL_SHADER(FRCASCS, "/Engine/Private/Mobile/FSR/PostProcessFFX_RCAS.usf", "MainCS", SF_Compute);
...
IMPLEMENT_GLOBAL_SHADER(FRCASPS, "/Engine/Private/Mobile/FSR/PostProcessFFX_RCAS.usf", "MainPS", SF_Pixel);
...
FScreenPassTexture AddCASPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, const ISpatialUpscaler::FInputs& PassInputs)
{
	...
	if (!bOutputSupportsUAV || GMobileFSRDisableCompute)
	{
		...
		PSPermutationVector.Set<FFSR_UseFP16Dim>(bUseFP16);
		PSPermutationVector.Set<FRCAS_DenoiseDim>(GMobileFSRCASDenoise > 0);
	...
	else
	{
		...
		CSPermutationVector.Set<FFSR_UseFP16Dim>(bUseFP16);
		CSPermutationVector.Set<FRCAS_DenoiseDim>(GMobileFSRCASDenoise > 0);
