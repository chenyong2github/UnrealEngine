...
void FMobileSceneRenderer::InitAmbientOcclusionOutputs(FRHICommandListImmediate& RHICmdList, const TRefCountPtr<IPooledRenderTarget>& SceneDepthZ)
{
	...
	const int32 DownsampleFactor = MobileAmbientOcclusionQuality < 3 ? 2 : 1; // Epic quality use full size, otherwise use half size
...
void FMobileSceneRenderer::RenderAmbientOcclusion(FRHICommandListImmediate& RHICmdList, const TRefCountPtr<IPooledRenderTarget>& SceneDepthZ, const TRefCountPtr<IPooledRenderTarget>& WorldNormalRoughness)
{
	...
	FRDGTextureRef WorldNormalRoughnessTexture = TryRegisterExternalTexture(GraphBuilder, WorldNormalRoughness);
