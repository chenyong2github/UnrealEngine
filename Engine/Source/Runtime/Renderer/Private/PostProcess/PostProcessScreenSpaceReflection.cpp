...
void FMobileSceneRenderer::RenderScreenSpaceReflection(FRDGBuilder& GraphBuilder, const FViewInfo& View, FSceneRenderTargets& SceneContext)
{
	...
	GRenderTargetPool.FindFreeElement(GraphBuilder.RHICmdList, Desc, SceneContext.ScreenSpaceReflection, TEXT("ScreenSpaceReflectionTexture"));
	...
	FRDGTextureRef ScreenSpaceReflectionTexture = GraphBuilder.RegisterExternalTexture(SceneContext.ScreenSpaceReflection, TEXT("ScreenSpaceReflectionTexture"));
...
{
	{
		...
		FIntPoint BufferSize = ScreenSpaceReflectionTexture->Desc.Extent;
		...
		{			
			...
			PassParameters->SSRParams = FLinearColor(
				...
				RoughnessMaskScale,
				View.ViewState ? View.ViewState->GetCurrentTemporalAASampleIndex() * 1551 : 0,
				View.ViewState ? View.ViewState->GetFrameIndex(8) * 1551 : 0);
		}
		...
		GraphBuilder.AddPass(
			RDG_EVENT_NAME("SSR RayMarch(Quality=%d RayPerPixel=1) %dx%d", SSRQuality, BufferSize.X, BufferSize.Y),
			PassParameters,
			ERDGPassFlags::Raster,
			[PassParameters, &View, BufferSize, PixelShader](FRHICommandList& RHICmdList)
			{
...

	{
		...
		GraphBuilder.AddPass(
			RDG_EVENT_NAME("SSR Composite %dx%d", BufferSize.X, BufferSize.Y),
			PassParameters,
			ERDGPassFlags::Raster,
			[PassParameters, &View, BufferSize, PixelShader](FRHICommandList& RHICmdList)
			{
