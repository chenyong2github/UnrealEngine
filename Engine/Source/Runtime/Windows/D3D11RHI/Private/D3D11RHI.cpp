...
void UpdateBufferStats(TRefCountPtr<ID3D11Buffer> Buffer, bool bAllocating)
{
	...
	if (bAllocating)
	{
		...
		LLM_SCOPED_PAUSE_TRACKING_WITH_ENUM_AND_AMOUNT(ELLMTag::VertexData, Desc.ByteWidth, ELLMTracker::Default, ELLMAllocType::None);
	...
	{ //-V523
		...
		// to hook the actual d3d allocations we can't track the memory in the normal way.
		// Instead we simply tell LLM the size of these resources.
		LLM_SCOPED_PAUSE_TRACKING_WITH_ENUM_AND_AMOUNT(ELLMTag::VertexData, -(int64)Desc.ByteWidth, ELLMTracker::Default, ELLMAllocType::None);
