...
inline bool EnsureColorSpace(IDXGISwapChain* SwapChain, EDisplayColorGamut DisplayGamut, EDisplayOutputFormat OutputDevice, EPixelFormat PixelFormat)
{
	if (!SwapChain)
	{
		return false;
	...
	if (FAILED(SwapChain->GetContainingOutput(Output.GetInitReference())))
	{
		return false;
	}
	
	TRefCountPtr<IDXGIOutput6> Output6;
	if (FAILED(Output->QueryInterface(IID_PPV_ARGS(Output6.GetInitReference()))))
	{
		return false;
	}
	...
	bool bHdrSupported = (desc.ColorSpace == DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020);
...
void FD3D11Viewport::CheckHDRMonitorStatus()
{
	...
	//if (CVarHDREnable->GetInt() != 0 || IsHDREnabled())
	{
		...
		bHDRSupported = EnsureColorSpace(SwapChain, DisplayGamut, OutputDevice, PixelFormat);
	}
