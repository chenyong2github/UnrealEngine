...
bool AggressiveDCEPass::AggressiveDCE(Function* func) {
  ...
  // UE Change Begin: Track function-variable stores to handle intrinsics that
  // use such variables as operands but DCE only considers the declaration of
  // the variable.
  function_stores_.clear();
  // UE Change End: Track function-variable stores to handle intrinsics that use
  // such variables as operands but DCE only considers the declaration of the
  // variable.
    ...
    for (auto ii = (*bi)->begin(); ii != (*bi)->end(); ++ii) {
      ...
      switch (op) {
        case SpvOpStore: {
          ...
          // UE Change Begin: Track function-variable stores to handle
          // intrinsics that use such variables as operands but DCE only
          // considers the declaration of the variable.
          else if (IsVarOfStorage(varId, SpvStorageClassFunction))
            function_stores_[varId] = &*ii;
          else
            // UE Change End: Track function-variable stores to handle
            // intrinsics that use such variables as operands but DCE only
            // considers the declaration of the variable.
            AddToWorklist(&*ii);
        } break;
  ...
  while (!worklist_.empty()) {
    ...
    liveInst->ForEachInId([&liveInst, this](const uint32_t* iid) {
      ...
      // UE Change Begin: Track function-variable stores to handle intrinsics
      // that use such variables as operands but DCE only considers the
      ...
      // UE Change End: Track function-variable stores to handle intrinsics that
      // use such variables as operands but DCE only considers the declaration
      // of the variable.
