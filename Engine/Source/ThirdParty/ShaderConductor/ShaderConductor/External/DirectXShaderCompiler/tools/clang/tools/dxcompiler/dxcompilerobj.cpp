...
struct CompilerVersionPartWriter {
  ...
  void Init(IDxcVersionInfo *pVersionInfo) {
    ...
    // UE Change Begin: Handle empty commit SHA string
    else {
      m_CommitSha = "";
      m_Header.CommitCount = 0;
      m_Header.VersionStringListSizeInBytes = /*null term*/ 1 + /*another null term for the empty custom string*/ 1;
    }
    // UE Change End: Handle empty commit SHA string
  }
