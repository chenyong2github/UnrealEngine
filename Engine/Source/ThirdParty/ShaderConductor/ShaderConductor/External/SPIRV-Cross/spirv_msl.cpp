...
void CompilerMSL::build_implicit_builtins()
{
	...
	if (need_subpass_input || need_sample_pos || need_vertex_params || need_tesc_params)
	{
		...
		ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
			...
			if (need_subpass_input && !msl_options.ios_use_framebuffer_fetch_subpasses && ir.meta[var.self].decoration.builtin_type == BuiltInFragCoord)
			{
                                     ...
                                     uint32_t sample, bool *p_forward)
{
	...
	case DimBuffer:
		...
		if (is_fetch && !msl_options.supports_msl_version(2, 1))
		{
			if (msl_options.texel_buffer_texture_width > 0)
			{
...
CompilerMSL::SPVFuncImpl CompilerMSL::OpCodePreprocessor::get_spv_func_impl(Op opcode, const uint32_t *args)
{
	...
	case OpImageWrite:
	{
		...
		/* UE Change Begin: Add support for Metal 2.1's new texture_buffer type. */
		if (tid && !compiler.msl_options.supports_msl_version(2, 1) && compiler.get<SPIRType>(tid).image.dim == DimBuffer)
