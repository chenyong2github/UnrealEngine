...
xcodebuild clean build -target "CrashReporter macOS" -configuration "Debug" -xcconfig BuildForUE/Mac/XcodeConfig
xcodebuild clean build -target "CrashReporter macOS" -configuration "Release" -xcconfig BuildForUE/Mac/XcodeConfig

## 
## Move the output file into the expected place
OUTPUTFILES=( "${TMPDIR}/CrashReporter/build/macosx/Release/libCrashReporter.a" "${TMPDIR}/CrashReporter/build/macosx/Debug/libCrashReporter.a")
cp -v ${OUTPUTFILES[0]} ${LIBFILES[0]}
cp -v ${OUTPUTFILES[1]} ${LIBFILES[1]}
