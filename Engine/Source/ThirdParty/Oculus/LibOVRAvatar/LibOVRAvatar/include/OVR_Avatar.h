...
#include <jni.h>
	...
	typedef enum ovrAvatarLookAndFeelVersion_
	{
		...
		ovrAvatarLookAndFeelVersion_One = 0,
		ovrAvatarLookAndFeelVersion_Two = 1,
	...
	///
	OVRN_EXPORT(void) ovrAvatarSpecificationRequest_SetExpressiveFlag(
		ovrAvatarSpecificationRequest* request,
		bool enable);
	...
	OVRN_EXPORT(void) ovrAvatarSpecificationRequest_SetLookAndFeelVersion(
		ovrAvatarSpecificationRequest* request,
	...
	///
	OVRN_EXPORT(void) ovrAvatarPose_UpdateHands(
		ovrAvatar* avatar,
	...
	///
	OVRN_EXPORT(void) ovrAvatarPose_UpdateHandsWithType(
		ovrAvatar* avatar,
		ovrAvatarHandInputState inputStateLeft,
		ovrAvatarHandInputState inputStateRight,
...
///
	...
	typedef struct ovrAvatarBlendShapeParams_ {
		...
		float blendShapeParams[OVR_AVATAR_MAXIMUM_BLEND_SHAPE_PARAM_COUNT]; ///< Array of floats for blend shape parameters
	...
	///
	OVRN_EXPORT(const ovrAvatarBlendShapeParams*) ovrAvatarSkinnedMeshRender_GetBlendShapeParams(
		const ovrAvatarRenderPart* renderPart);
	...
	typedef enum ovrAvatarBodyPartType_ {
		ovrAvatarBodyPartType_Body,
		ovrAvatarBodyPartType_Clothing,
		ovrAvatarBodyPartType_Eyewear,
		ovrAvatarBodyPartType_Hair,
		ovrAvatarBodyPartType_Beard,
		ovrAvatarBodyPartType_Count
	} ovrAvatarBodyPartType;

	OVRN_EXPORT(ovrAvatarBodyPartType) ovrAvatarRenderPart_GetBodyPartType(
		ovrAvatar* avatar,
		const ovrAvatarRenderPart* renderPart);
	...
	///
	OVRN_EXPORT(uint32_t) ovrAvatarAsset_GetMeshBlendShapeCount(
		const ovrAvatarAsset* asset);
	...
	///
	OVRN_EXPORT(const char*) ovrAvatarAsset_GetMeshBlendShapeName(
		const ovrAvatarAsset* asset,
		uint32_t blendIndex);
	...
	///
	OVRN_EXPORT(const ovrAvatarBlendVertex*) ovrAvatarAsset_GetMeshBlendShapeVertices(
		const ovrAvatarAsset* asset);
	...
	///
	OVRN_EXPORT(uint32_t) ovrAvatarAsset_GetSubmeshCount(
		const ovrAvatarAsset* asset);
	...
	///
	OVRN_EXPORT(uint32_t) ovrAvatarAsset_GetSubmeshLastIndex(
		const ovrAvatarAsset* asset,
		uint32_t index);
	...
	OVRN_EXPORT(ovrAvatarExpressiveParameters) ovrAvatar_GetExpressiveParameters(const ovrAvatar* avatar);
...
// Current viseme activation values from 0 to 1.
	typedef struct ovrAvatarVisemes_ {
		uint32_t visemeParamCount;
		float visemeParams[OVR_AVATAR_MAXIMUM_VISEME_PARAM_COUNT];
	} ovrAvatarVisemes;

	OVRN_EXPORT(void) ovrAvatar_SetVisemes(ovrAvatar* avatar, const ovrAvatarVisemes* state);


	OVRN_EXPORT(void) ovrAvatar_UpdateWorldTransform(ovrAvatar* avatar, ovrAvatarTransform transform);

	typedef enum ovrAvatarGazeTargetType_ {
		ovrAvatarGazeTargetType_AvatarHead,
		ovrAvatarGazeTargetType_AvatarHand,
		ovrAvatarGazeTargetType_Object,
		ovrAvatarGazeTargetType_ObjectStatic,
		ovrAvatarGazeTargetType_Count
	} ovrAvatarGazeTargetType;

	typedef struct ovrAvatarGazeTarget_ {
		uint32_t id;
		ovrAvatarVector3f worldPosition;
		ovrAvatarGazeTargetType type;
	} ovrAvatarGazeTarget;

	// The maximum number of gaze targets.
#define OVR_AVATAR_MAXIMUM_GAZE_TARGET_COUNT 128

	typedef struct ovrAvatarGazeTargetSet_ {
		uint32_t targetCount;
		ovrAvatarGazeTarget targets[OVR_AVATAR_MAXIMUM_GAZE_TARGET_COUNT];
	} ovrAvatarGazeTargetSet;

	// TODO - Add doxygen info for new API calls.

	/// Add/update gaze targets. Updating an existing target only affects position (changes to type are ignored).
	OVRN_EXPORT(void) ovrAvatar_UpdateGazeTargets(const ovrAvatarGazeTargetSet* targets);
	OVRN_EXPORT(void) ovrAvatar_RemoveGazeTargets(uint32_t targetCount, const uint32_t* ids);
	OVRN_EXPORT(void) ovrAvatar_RemoveAllGazeTargets();


	typedef enum ovrAvatarLightType_ {
		ovrAvatarLightType_Point,
		ovrAvatarLightType_Direction,
		ovrAvatarLightType_Spot,
		ovrAvatarLightType_Count
	} ovrAvatarLightType;

	typedef struct ovrAvatarLight_ {
		uint32_t id;
		ovrAvatarLightType type;
		float intensity;
		ovrAvatarVector3f worldDirection;
		ovrAvatarVector3f worldPosition;
		float range;
		float spotAngleDeg;
	} ovrAvatarLight;
...
#define OVR_AVATAR_MAXIMUM_LIGHT_COUNT 16

	typedef struct ovrAvatarLightSet_ {
		float ambientIntensity;
		uint32_t lightCount;
		ovrAvatarLight lights[OVR_AVATAR_MAXIMUM_LIGHT_COUNT];
	} ovrAvatarLightSet;

	OVRN_EXPORT(void) ovrAvatar_UpdateLights(const ovrAvatarLightSet* lights);
	OVRN_EXPORT(void) ovrAvatar_RemoveLights(uint32_t lightCount, const uint32_t* ids);
	OVRN_EXPORT(void) ovrAvatar_RemoveAllLights();
