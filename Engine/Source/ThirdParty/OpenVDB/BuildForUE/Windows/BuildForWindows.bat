...
set OPENVDB_INSTALL_BIN_PATH=%OPENVDB_INSTALL_PATH%\bin\Win64
set OPENVDB_INSTALL_LIB_PATH=%OPENVDB_INSTALL_PATH%\lib\Win64
...
set PATH_TO_BLOSC=%PATH_TO_THIRDPARTY%\Blosc\c-blosc-1.5.0\Win64\VS2019
...
if exist %OPENVDB_INSTALL_INCLUDE_PATH% (rmdir %OPENVDB_INSTALL_INCLUDE_PATH% /s/q)
if exist %OPENVDB_INSTALL_BIN_PATH% (rmdir %OPENVDB_INSTALL_BIN_PATH% /s/q)
if exist %OPENVDB_INSTALL_LIB_PATH% (rmdir %OPENVDB_INSTALL_LIB_PATH% /s/q)

REM Build OpenVDB for VS2019 (64-bit)
...
cmake -G "Visual Studio 16 2019" -A x64^
	-DCMAKE_PREFIX_PATH="%PATH_TO_ZLIB_SRC%;%PATH_TO_ZLIB_LIB%;%PATH_TO_TBB_SRC%;%PATH_TO_TBB_LIB%;%PATH_TO_BLOSC%"^
	...
	-DOPENVDB_CORE_SHARED=ON -DOPENVDB_CORE_STATIC=ON^
	-DUSE_STATIC_DEPENDENCIES=ON -DCMAKE_MSVC_RUNTIME_LIBRARY=MultiThreadedDLL^
