...
echo Generating tokengen solution for VS2015 (32-bit)...
if exist "%VS2015_X86_PATH%" (rmdir "%VS2015_X86_PATH%" /s/q)
mkdir "%VS2015_X86_PATH%"
cd "%VS2015_X86_PATH%"
cmake -G "Visual Studio 14 2015" -DCMAKE_SUPPRESS_REGENERATION=1 -DOPENSSL_PATH=../../../OpenSSL/1.0.2g/include/Win32/VS2015 %PATH_TO_CMAKE_FILE%
echo Building tokengen solution for VS2015 (32-bit, Debug)...
"%_msbuild%msbuild.exe" tokengen.sln /t:build /p:Configuration=Debug
echo Building tokengen solution for VS2015 (32-bit, Release)...
"%_msbuild%msbuild.exe" tokengen.sln /t:build /p:Configuration=Release
cd "%PATH_TO_CMAKE_FILE%"
rmdir "%VS2015_X86_PATH%" /s/q

REM Build for VS2015 (64-bit)
echo Generating tokengen solution for VS2015 (64-bit)...
if exist "%VS2015_X64_PATH%" (rmdir "%VS2015_X64_PATH%" /s/q)
mkdir "%VS2015_X64_PATH%"
cd "%VS2015_X64_PATH%"
cmake -G "Visual Studio 14 2015 Win64" -DCMAKE_SUPPRESS_REGENERATION=1 -DOPENSSL_PATH=../../../OpenSSL/1.0.2g/include/Win64/VS2015 %PATH_TO_CMAKE_FILE%
echo Building tokengen solution for VS2015 (64-bit, Debug)...
"%_msbuild%msbuild.exe" tokengen.sln /t:build /p:Configuration=Debug
echo Building tokengen solution for VS2015 (64-bit, Release)...
"%_msbuild%msbuild.exe" tokengen.sln /t:build /p:Configuration=Release
cd "%PATH_TO_CMAKE_FILE%"
