...
namespace WindowsMixedReality
{
	...
	{
		Log(L"SpatialAnchorHelper constructor started.");
		m_spatialAnchorStoreAsyncOperation = SpatialAnchorManager::RequestStoreAsync();
		{ std::wstringstream string; string << L"SpatialAnchorHelper constructor status = " << static_cast<INT32>(m_spatialAnchorStoreAsyncOperation.Status()); Log(string); }
#if 0
		...
		m_spatialAnchorStoreAsyncOperation.Completed([this, Log = m_logCallback](winrt::Windows::Foundation::IAsyncOperation<SpatialAnchorStore> asyncOperation, winrt::Windows::Foundation::AsyncStatus status)
		{
			if (asyncOperation.Status() == winrt::Windows::Foundation::AsyncStatus::Completed)
			{
				...
				if (Log) Log(L"SpatialAnchorHelper RequestStoreAsync succeeded!");
			}
			else if (asyncOperation.Status() != winrt::Windows::Foundation::AsyncStatus::Canceled)
			{
				if (Log){ std::wstringstream string; string << L"SpatialAnchorHelper RequestStoreAsync failed with status = " << static_cast<INT32>(asyncOperation.Status()); Log(string.str().c_str()); }
			}
		});
#endif
		Log(L"SpatialAnchorHelper constructor finished.");
	}

	SpatialAnchorHelper::~SpatialAnchorHelper()
	{
		if (m_spatialAnchorStoreAsyncOperation.Status() != winrt::Windows::Foundation::AsyncStatus::Completed)
		{
