...
# Make sure all the optional libraries are not enabled, they will be optional looked for so you'll have to manually edit the
# CMakeLists.txt to remove these checks
#
#  HarfBuzz
#  PNG
#  ZLIB
...
cd $TEMP_DIR_RELEASE
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_PREFIX_PATH="$LIB_PNG_PATH;$ZLIB_PATH" -DCMAKE_C_COMPILER=clang "$BASE_DIR"
make -j4
cp -v libfreetype.a "$BASE_DIR/lib/Mac/libfreetype.a"

cd $TEMP_DIR_DEBUG
cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_PREFIX_PATH="$LIB_PNG_PATH;$ZLIB_PATH" -DCMAKE_C_COMPILER=clang "$BASE_DIR"
make -j4
