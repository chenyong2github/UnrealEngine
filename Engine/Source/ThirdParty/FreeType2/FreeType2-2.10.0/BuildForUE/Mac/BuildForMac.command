...
cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_PREFIX_PATH="$LIB_PNG_PATH;$ZLIB_PATH" -DCMAKE_C_COMPILER=clang -DCMAKE_OSX_DEPLOYMENT_TARGET="$OSX_VERSION" -DCMAKE_DISABLE_FIND_PACKAGE_HarfBuzz=TRUE -DCMAKE_DISABLE_FIND_PACKAGE_BZip2=TRUE -DCMAKE_OSX_ARCHITECTURES="$ARCHFLAGS" "${LIB_ROOT_DIR}"
make clean && make -j$(get_core_count)
