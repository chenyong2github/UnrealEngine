...
namespace UnrealVS
{
    ...
    public partial class FileBrowserWindowControl : UserControl
    {
		...
		public FileBrowserWindowControl(FileBrowserWindow window)
        {
            ...
            FilesListTab.KeyDown += FilesListTab_KeyDown;
            FilesListTab.SelectedIndex = 0;

			ActiveFilesListBox = AllFilesListBox;
        }

        private void FilesListTab_KeyDown(object sender, KeyEventArgs e)
        {
            /*
            if (e.Key == Key.Right && FilesListTab.SelectedIndex < 2)
                FilesListTab.SelectedIndex = (int)FilesListTab.SelectedIndex + 1;
            else if (e.Key == Key.Left && FilesListTab.SelectedIndex > 0)
