...
namespace HordeServerTests
{
    ...
    /// </summary>
	[TestClass]
    public class JobsControllerDbTest : DatabaseIntegrationTest
    {
        ...
        {
	        TestSetup TestSetup = await GetTestSetup();
	        JobsController JobsController = TestSetup.JobsController;

	        IJob Job = TestSetup.Fixture!.Job1;
	        SubResourceId BatchId = Job.Batches[0].Id;
	        SubResourceId StepId = Job.Batches[0].Steps[0].Id;

	        object Obj = (await JobsController.GetStepAsync(Job.Id.ToString(), BatchId.ToString(), StepId.ToString())).Value;
	        ...
	        Obj = (await JobsController.UpdateStepAsync(Job.Id.ToString(), BatchId.ToString(), StepId.ToString(), UpdateReq)).Value;
	        UpdateStepResponse UpdateRes = (Obj as UpdateStepResponse)!;
	        
	        Obj = (await JobsController.GetStepAsync(Job.Id.ToString(), BatchId.ToString(), StepId.ToString())).Value;
