...
namespace HordeServer.Storage
{
	...
	public interface IStorageService
	{
		...
		/// from a volatile cache. Call <see cref="ShouldPutBlobAsync(BlobHash)"/> to determine if a blob needs to
		/// be put to the store.
		/// 
		...
		Task<ReadOnlyMemory<byte>?> TryGetBlobAsync(BlobHash Hash, DateTime Deadline = default(DateTime));
		...
		Task PutBlobAsync(BlobHash Hash, ReadOnlyMemory<byte> Value);
		...
		Task<bool> ShouldPutBlobAsync(BlobHash Hash);
		...
		Task<BlobHash?> GetRefAsync(BlobHash Key, TimeSpan MaxDrift = default(TimeSpan));
		...
		/// <returns>Async task</returns>
		Task SetRefAsync(BlobHash Key, BlobHash? Value);
		...
		/// <returns>Async task</returns>
		Task TouchRefAsync(BlobHash Key);
	}
	...
	static class StorageServiceExtensions
	{
		...
		public static async Task<ReadOnlyMemory<byte>> GetBlobAsync(this IStorageService StorageService, BlobHash Hash, DateTime Deadline = default(DateTime))
		{
		...
		/// <summary>
		/// Hashes a blob, then adds it to the storage service. If the hash is already known, prefer <see cref="IStorageService.PutBlobAsync(BlobHash, ReadOnlyMemory{byte})"/>
		/// </summary>
		/// <param name="StorageService">The storage service</param>
		/// <param name="Data">The data to add</param>
		/// <returns>Hash of the data that was added</returns>
		public static async Task<BlobHash> PutBlobAsync(this IStorageService StorageService, ReadOnlyMemory<byte> Data)
		{
			BlobHash Hash = BlobHash.Compute(Data);
