...
namespace GeometryCollectionTest
{
	...
	void AllOnSurface(const TImplicitShape *Shape, const TArray<Chaos::FVec3> &Points, const float Tolerance = KINDA_SMALL_NUMBER)
	{
		for (auto &Pt : Points)
		{
	...
	void TestSphere(const Chaos::FVec3& Center, const float Radius, const FTransform &BoneRelXf)
	{
	...
	{
		TestSphere(Chaos::FVec3(0, 0, 0), 1.f, FTransform::Identity);
		TestSphere(Chaos::FVec3(0, 0, 0), 1.f, FTransform(FVector(1,0,0)));
		TestSphere(Chaos::FVec3(0, 0, 0), 1.f, FTransform(FVector(1,1,0)));
		TestSphere(Chaos::FVec3(0, 0, 0), 1.f, FTransform(FVector(1,1,1)));

		//This test fails in Dev-Physics for spheres of larger radii
		//TestSphere(R, Chaos::FVec3(0, 0, 0), 10.f, FTransform::Identity);
		//TestSphere(R, Chaos::FVec3(0, 0, 0), 10.f, FTransform(FVector(1,0,0)));
		//TestSphere(R, Chaos::FVec3(0, 0, 0), 10.f, FTransform(FVector(1,1,0)));
		//TestSphere(R, Chaos::FVec3(0, 0, 0), 10.f, FTransform(FVector(1,1,1)));
	...
	void RunBoneHierarchyTest()
	{
		...
		Hierarchy.InitPreAdd(2);
		{
			...
			Group1->SetParentBoneIndex(INDEX_NONE);
			Group1->Add(FTransform::Identity, new Chaos::TSphere<float, 3>(Chaos::FVec3(0), 1.));
			...
			Group2->Add(FTransform::Identity, new Chaos::TSphere<float, 3>(Chaos::FVec3(0), 1.));
