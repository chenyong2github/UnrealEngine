...
TArray<const FUnrealScriptStructDefinitionInfo*> GetSparseDataStructsToExport(FUnrealClassDefinitionInfo& ClassDef)
{
	TArray<const FUnrealScriptStructDefinitionInfo*> out;

	TArray<FString> SparseClassDataTypes;
	ClassDef.GetSparseClassDataTypes(SparseClassDataTypes);

	TSet<FString> SuperSparseClassDataTypes;
	if (FUnrealClassDefinitionInfo* SuperClass = ClassDef.GetSuperClass())
	{
		TArray<FString> TempTypes;
		SuperClass->GetSparseClassDataTypes(TempTypes);
		for (const FString& Type : TempTypes)
		{
			SuperSparseClassDataTypes.Add(Type);
		}
	}

	for (const FString& SparseClass : SparseClassDataTypes)
	{
		if (FUnrealScriptStructDefinitionInfo* SparseScriptStructDef = GTypeDefinitionInfoMap.FindByName<FUnrealScriptStructDefinitionInfo>(*SparseClass))
		{
			while (SparseScriptStructDef != nullptr && !SuperSparseClassDataTypes.Contains(SparseScriptStructDef->GetName()))
			{
				out.Add(SparseScriptStructDef);
				SparseScriptStructDef = SparseScriptStructDef->GetSuperScriptStruct();
			}
		}
	}
...
void FNativeClassHeaderGenerator::ExportNativeFunctions(FOutputDevice& OutGeneratedHeaderText, FOutputDevice& OutGeneratedCPPText, FOutputDevice& OutMacroCalls, FOutputDevice& OutNoPureDeclsMacroCalls, FReferenceGatherers& OutReferenceGatherers, const FUnrealSourceFile& SourceFile, FUnrealClassDefinitionInfo& ClassDef) const
{
	...
	TArray<const FUnrealScriptStructDefinitionInfo*> SparseClassDataTypes = GetSparseDataStructsToExport(ClassDef);
	FString FullClassName = ClassDef.GetNameWithPrefix();
	for (const FUnrealScriptStructDefinitionInfo* SparseClassData : SparseClassDataTypes)
	{
		FString Empty;
		const FString& API = ClassDef.HasAnyClassFlags(CLASS_MinimalAPI) ? GetAPIString() : Empty;

		const FString SparseClassDataString = SparseClassData->GetName();
		...
		while (SparseClassDataStructDef != nullptr)
		{
			for (TSharedRef<FUnrealPropertyDefinitionInfo> PropertyDef : SparseClassDataStructDef->GetProperties())
			{
				...
				if (!PropertyDef->HasMetaData(NAME_NoGetter))
				{
					...
					RuntimeStringBuilders.AutogeneratedStaticDataFuncs.Logf(TEXT("\treturn Get%s(EGetSparseClassDataMethod::ArchetypeIfNull)->%s;\r\n"), *SparseClassDataString, *VarName);
					RuntimeStringBuilders.AutogeneratedStaticDataFuncs.Logf(TEXT("}\r\n"));
				}
			}
