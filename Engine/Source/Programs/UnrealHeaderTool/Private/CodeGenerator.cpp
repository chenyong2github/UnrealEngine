	...
	for (TSharedRef<FUnrealFunctionDefinitionInfo> FunctionDef : Functions)
	{
		...
		if (!bWillBeProgrammerTyped)
		{
			...
			if (FunctionDef->HasAnyFunctionFlags(FUNC_NetValidate))
			{
				...
				bool AddApi = !FunctionDef->GetOwnerClass()->HasAnyClassFlags(CLASS_RequiredAPI) && (FunctionData.FunctionExportFlags & FUNCEXPORT_RequiredAPI);

				ValidDecl.Logf(TEXT("\t%s %s bool %s(%s);\r\n"), AddApi ? *api : TEXT(""), Virtual, *FunctionData.CppValidationImplName, *ParameterList);
