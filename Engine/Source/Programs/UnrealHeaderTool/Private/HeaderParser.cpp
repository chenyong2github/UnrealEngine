	...
	void ValidateMetaDataFormat(FUnrealTypeDefinitionInfo& TypeDef, const FName InKey, const FString& InValue)
	{
		switch (GetCheckedMetadataSpecifier(InKey))
		{
			...
			case ECheckedMetadataSpecifier::BlueprintProtected:
			{
				if (FUnrealFunctionDefinitionInfo* FuncDef = TypeDef.AsFunction())
				{
					...
					if (FuncDef->HasAnyFunctionFlags(FUNC_Static))
					{
						...
						FUnrealClassDefinitionInfo* ClassDef = FuncDef->GetOuterClass();
				...
				if (FUnrealFunctionDefinitionInfo* FuncDef = TypeDef.AsFunction())
				{
					...
					for (const FString& RawGroup : RawGroupings)
					{
						...
						for (const FString& Entry : IndividualEntries)
						{
							...
							FField* FoundField = FHeaderParser::FindProperty(*FuncDef, *Entry, false);
							if (!FoundField)
							{
			...
			case ECheckedMetadataSpecifier::Units:
			{
				...
				{
					if (TypeDef.AsField() != nullptr)
					{
						FError::Throwf(TEXT("Unrecognized units (%s) specified for property '%s'"), *InValue, *TypeDef.AsFieldChecked().GetField()->GetFullName());
					}
					else
					{
						FError::Throwf(TEXT("Unrecognized units (%s) specified for property '%s'"), *InValue, *TypeDef.AsPropertyChecked().GetProperty()->GetFullName());
					}
				}
			}
			break;
...
FUnrealClassDefinitionInfo* FHeaderParser::GetQualifiedClass(const TCHAR* Thing)
{
	...
	return FUnrealClassDefinitionInfo::FindScriptClassOrThrow(ClassName);
}
		...
		for (FUnrealStructDefinitionInfo* Scope = &InScope; Scope; Scope = UHTCast<FUnrealStructDefinitionInfo>(Scope->GetOuter()))
		{
			for (FUnrealPropertyDefinitionInfo* PropertyDef : TUHTFieldRange<FUnrealPropertyDefinitionInfo>(*Scope))
			{
				...
				if (Property->GetFName() == InName)
				{
					if (Thing)
					{
						FError::Throwf(TEXT("%s: expecting @todo: FProp, got %s"), Thing, /**FieldClass->GetName(),*/ *Property->GetClass()->GetName());
					}
					return nullptr;
				}
			...
			for (FUnrealFunctionDefinitionInfo* FunctionDef : TUHTFieldRange<FUnrealFunctionDefinitionInfo>(*Scope))
			{
				...
				if (Function->GetFName() == InName)
				{
					if (!Function->IsA(FieldClass))
					{
						if (Thing)
						{
							FError::Throwf(TEXT("%s: expecting %s, got %s"), Thing, *FieldClass->GetName(), *Function->GetClass()->GetName());
						}
						return nullptr;
					}
					return Function;
...
FField* FHeaderParser::FindProperty(FUnrealStructDefinitionInfo& InScope, const TCHAR* InIdentifier, bool bIncludeParents, FFieldClass* FieldClass, const TCHAR* Thing)
{
	check(InIdentifier);
	FName InName(InIdentifier, FNAME_Find);
	if (InName != NAME_None)
		...
		for (FUnrealStructDefinitionInfo* Scope = &InScope; Scope; Scope = UHTCast<FUnrealStructDefinitionInfo>(Scope->GetOuter()))
		{
			for (FUnrealFunctionDefinitionInfo* FunctionDef : TUHTFieldRange<FUnrealFunctionDefinitionInfo>(*Scope))
			{
				...
				if (Function->GetFName() == InName)
				{
					if (Thing)
					{
						FError::Throwf(TEXT("%s: expecting a property, got %s"), Thing, *Function->GetClass()->GetName());
					}
					return nullptr;
				}
			}
			for (FUnrealPropertyDefinitionInfo* PropertyDef : TUHTFieldRange<FUnrealPropertyDefinitionInfo>(*Scope))
			{
				...
				if (Property->GetFName() == InName)
				{
					if (!Property->IsA(FieldClass))
					{
						if (Thing)
						{
							FError::Throwf(TEXT("%s: expecting %s: FProp, got %s"), Thing, *FieldClass->GetName(), *Property->GetClass()->GetName());
						}
						return nullptr;
					}
					return Property;
...
FUnrealEnumDefinitionInfo& FHeaderParser::CompileEnum()
{
	...
	{
		UE_LOG_WARNING_UHT(TEXT("'%s' does not have a 0 entry! (This is a problem when the enum is initalized by default)"), *EnumDef.GetName());
	}
	...
	for (FUnrealPropertyDefinitionInfo* PropertyDef : StructDef.GetProperties())
	{
		...
		if (DelegateProperty != nullptr || MulticastDelegateProperty != nullptr)
		{
			...
			if( DelegateProperty != NULL )
			{
				...
				{
					DelegateProperty->SignatureFunction = SourceDelegateFunctionDef->GetFunction();
					DelegatePropertyToken.FunctionDef = SourceDelegateFunctionDef;
				}
				else
				{
					FError::Throwf(TEXT("Unable to declare a single-cast delegate property for a multi-cast delegate type '%s'.  Either add a 'multicast' qualifier to the property or change the delegate type to be single-cast as well."), *SourceDelegateFunctionDef->GetName());
				}
			}
			else if( MulticastDelegateProperty != NULL )
			{
				if(SourceDelegateFunctionDef->HasAnyFunctionFlags( FUNC_MulticastDelegate ) )
				{
					MulticastDelegateProperty->SignatureFunction = SourceDelegateFunctionDef->GetFunction();
					DelegatePropertyToken.FunctionDef = SourceDelegateFunctionDef;
					...
					if(MulticastDelegateProperty->HasAnyPropertyFlags(CPF_BlueprintAssignable | CPF_BlueprintCallable))
					{
						for (FUnrealPropertyDefinitionInfo* FuncParamDef : SourceDelegateFunctionDef->GetProperties())
						{
							...
							if (!FPropertyTraits::IsSupportedByBlueprint(*FuncParamDef, false))
							{
								...
								UE_LOG_ERROR_UHT(TEXT("Type '%s%s' is not supported by blueprint. %s.%s"), *CPPType, *ExtendedCPPType, *SourceDelegateFunctionDef->GetName(), *FuncParam->GetName());
							}
							...
							if(FuncParam->HasAllPropertyFlags(CPF_OutParm) && !FuncParam->HasAllPropertyFlags(CPF_ConstParm)  )
							{
								...
								{
									UE_LOG_ERROR_UHT(TEXT("BlueprintAssignable delegates do not support non-const references at the moment. Function: %s Parameter: '%s'"), *SourceDelegateFunctionDef->GetName(), *FuncParam->GetName());
								}
							}
						}
					}
				...
				{
					FError::Throwf(TEXT("Unable to declare a multi-cast delegate property for a single-cast delegate type '%s'.  Either remove the 'multicast' qualifier from the property or change the delegate type to be 'multicast' as well."), *SourceDelegateFunctionDef->GetName());
				}
			}
		}
	}
...
void FHeaderParser::VerifyBlueprintPropertyGetter(FUnrealPropertyDefinitionInfo& PropertyDef, FUnrealFunctionDefinitionInfo* TargetFuncDef)
{
	...
	{
		UE_LOG_ERROR_UHT(TEXT("Blueprint Property getter function %s must not have parameters."), *TargetFuncDef->GetName());
	}

	if (ReturnPropDef == nullptr || !Prop->SameType(ReturnPropDef->GetProperty()))
	{
		FString ExtendedCPPType;
		FString CPPType = Prop->GetCPPType(&ExtendedCPPType);
		UE_LOG_ERROR_UHT(TEXT("Blueprint Property getter function %s must have return value of type %s%s."), *TargetFuncDef->GetName(), *CPPType, *ExtendedCPPType);
	}
...
void FHeaderParser::VerifyBlueprintPropertySetter(FUnrealPropertyDefinitionInfo& PropertyDef, FUnrealFunctionDefinitionInfo* TargetFuncDef)
{
	...
	if (ReturnPropDef)
	{
		UE_LOG_ERROR_UHT(TEXT("Blueprint Property setter function %s must not have a return value."), *TargetFuncDef->GetName());
	}
	else
	{
		const TArray<FUnrealPropertyDefinitionInfo*>& Properties = TargetFuncDef->GetProperties();
		if (TargetFuncDef->GetProperties().Num() != 1 || !Prop->SameType(Properties[0]->GetProperty()))
		{
			FString ExtendedCPPType;
			FString CPPType = Prop->GetCPPType(&ExtendedCPPType);
			UE_LOG_ERROR_UHT(TEXT("Blueprint Property setter function %s must have exactly one parameter of type %s%s."), *TargetFuncDef->GetName(), *CPPType, *ExtendedCPPType);
		}
	}
...
void FHeaderParser::VerifyRepNotifyCallback(FUnrealPropertyDefinitionInfo& PropertyDef, FUnrealFunctionDefinitionInfo* TargetFuncDef)
{
	...
	if (TargetFuncDef)
	{
		...
		{
			UE_LOG_ERROR_UHT(TEXT("Replication notification function %s must not have return value."), *TargetFuncDef->GetName());
		}
		...
		if (Properties.Num() >= 1)
		{
			const FProperty* Parm = Properties[0]->GetProperty();
			// First parameter is always the old value:
			if ( !Prop->SameType(Parm) )
			{
				FString ExtendedCPPType;
				FString CPPType = Prop->GetCPPType(&ExtendedCPPType);
				UE_LOG_ERROR_UHT(TEXT("Replication notification function %s has invalid parameter for property %s. First (optional) parameter must be of type %s%s."), *TargetFuncDef->GetName(), *Prop->GetName(), *CPPType, *ExtendedCPPType);
			}
		}

		if (TargetFuncDef->GetProperties().Num() >= 2)
		{
			FProperty* Parm = Properties[1]->GetProperty();
			...
			{
				UE_LOG_ERROR_UHT(TEXT("Replication notification function %s (optional) second parameter must be of type 'const TArray<uint8>&'"), *TargetFuncDef->GetName());
			}
		}
	}
	else
...
)
{
	...
	else if (FUnrealEnumDefinitionInfo* EnumDef = GTypeDefinitionInfoMap.FindByName<FUnrealEnumDefinitionInfo>(VarType.Identifier))
	{
		...
		if (VariableCategory == EVariableCategory::Member)
		{
			...
			{
				FError::Throwf(TEXT("You cannot use the raw enum name as a type for member variables, instead use TEnumAsByte or a C++11 enum class with an explicit underlying type."), *EnumDef->GetCppType());
			}
		}
				...
				if (TempClassDef != NULL)
				{
					...
					if (TempClassDef->GetClass()->IsChildOf(UClass::StaticClass()))
					{
						if (MatchSymbol(TEXT('<')))
						{
							...
							VarProperty.MetaClassDef = FUnrealClassDefinitionInfo::FindScriptClassOrThrow(Limitor.Identifier);
...
)
{
	...
	UField* ExistingField = FindField(ParentStruct, Identifier, true, UField::StaticClass(), NULL);
	FField* ExistingProperty = FindProperty(ParentStruct, Identifier, true, FField::StaticClass(), NULL);
	...
	if ((LayoutMacroType == ELayoutMacroType::None && MatchSymbol(TEXT('['))) || LayoutMacroType == ELayoutMacroType::Array || LayoutMacroType == ELayoutMacroType::ArrayEditorOnly)
	{
		...
		if (LayoutMacroType == ELayoutMacroType::None)
		{
			...
			{
				FError::Throwf(TEXT("%s %s: Missing ']'"), HintText, Identifier);
			}
		}
		else
		{
...
void FHeaderParser::ParseRigVMMethodParameters(FUnrealStructDefinitionInfo& StructDef)
{
	...
	{
		FProperty const* const Prop = PropertyDef->GetProperty();
...
void FHeaderParser::CompileFunctionDeclaration()
{
	...
	if (SuperStruct)
	{
		if (UFunction* OverriddenFunction = ::FindUField<UFunction>(SuperStruct, FuncDefFuncInfo.Function.Identifier))
		{
			...
			UE_LOG_ERROR_UHT(TEXT("%s: Override of UFUNCTION in parent class (%s) cannot have a UFUNCTION() declaration above it; it will use the same parameters as the original declaration."), FuncDefFuncInfo.Function.Identifier, *OverriddenFunction->GetOuter()->GetName());
		}
	}
...
void FHeaderParser::ComputeFunctionParametersSize(FUnrealClassDefinitionInfo& ClassDef)
{
	...
	for (FUnrealFunctionDefinitionInfo* FunctionDef : ClassDef.GetFunctions())
	{
		...
		if (FunctionDef->HasAnyFunctionFlags(FUNC_Delegate))
		{
			...
			{
				FProperty* Param = PropertyDef->GetProperty();
...
FHeaderParser::FHeaderParser(FUnrealPackageDefinitionInfo& InPackageDef, FFeedbackContext* InWarn, FUnrealSourceFile& InSourceFile)
	: FBaseParser()
	, Warn(InWarn)
...
ECompilationResult::Type FHeaderParser::Parse(
		...
		else
		{
			...
			else if (const TCHAR* UEnumMacroDecl = FCString::Strfind(Str, TEXT("UENUM")))
			{
				if (UEnumMacroDecl == FCString::Strspn(Str, TEXT("\t ")) + Str)
				{
					...
					TSharedRef<FUnrealTypeDefinitionInfo> EnumDecl = Parser.ParseEnumDeclaration(SourceFile, StartOfLine + (UEnumMacroDecl - Str), CurrentLine);
			...
			else if (const TCHAR* UStructMacroDecl = FCString::Strfind(Str, TEXT("USTRUCT")))
			{
				if (UStructMacroDecl == FCString::Strspn(Str, TEXT("\t ")) + Str)
				{
					...
					TSharedRef<FUnrealTypeDefinitionInfo> StructDecl = Parser.ParseStructDeclaration(SourceFile, StartOfLine + (UStructMacroDecl - Str), CurrentLine);
...
TSharedRef<FUnrealTypeDefinitionInfo> FHeaderPreParser::ParseClassDeclaration(FUnrealSourceFile& SourceFile, const TCHAR* InputText, int32 InLineNumber, bool bClassIsAnInterface, const TCHAR* StartingMatchID)
{
...
// Compile an enumeration definition.
//
TSharedRef<FUnrealTypeDefinitionInfo> FHeaderPreParser::ParseEnumDeclaration(FUnrealSourceFile& SourceFile, const TCHAR* InputText, int32 InLineNumber)
{
...
//
TSharedRef<FUnrealTypeDefinitionInfo> FHeaderPreParser::ParseStructDeclaration(FUnrealSourceFile& SourceFile, const TCHAR* InputText, int32 InLineNumber)
{
		...
		for (; InterfaceDef; InterfaceDef = InterfaceDef->GetSuperClass())
		{
			...
			for (FUnrealFunctionDefinitionInfo* InterfaceFunctionDef : TUHTFieldRange<FUnrealFunctionDefinitionInfo>(*InterfaceDef, EFieldIteratorFlags::ExcludeSuper))
			{
				...
				for (FUnrealFunctionDefinitionInfo* ClassFunctionDef : TUHTFieldRange<FUnrealFunctionDefinitionInfo>(CurrentClassDef))
				{
					...
					{
						FError::Throwf(TEXT("Implementation of function '%s::%s' must be declared as 'event' to match declaration in interface '%s'"), *ClassFunctionDef->GetOuter()->GetName(), *ClassFunctionDef->GetName(), *InterfaceDef->GetName());
					}

					if (InterfaceFunctionDef->HasAnyFunctionFlags(FUNC_Delegate) && !ClassFunctionDef->HasAnyFunctionFlags(FUNC_Delegate))
					{
						FError::Throwf(TEXT("Implementation of function '%s::%s' must be declared as 'delegate' to match declaration in interface '%s'"), *ClassFunctionDef->GetOuter()->GetName(), *ClassFunctionDef->GetName(), *InterfaceDef->GetName());
					}
					...
					{
						FError::Throwf(TEXT("Implementation of function '%s' conflicts with interface '%s' - different number of parameters (%i/%i)"), *InterfaceFunctionDef->GetName(), *InterfaceDef->GetName(), ClassFunctionDef->GetProperties().Num(), InterfaceFunctionDef->GetProperties().Num());
					}
					...
					for (int32 Index = 0, End = ClassFunctionDef->GetProperties().Num(); Index != End; ++Index)
					{
						...
						{
							FProperty* Property = InterfaceParamDef->GetProperty();
							if (Property->HasAnyPropertyFlags(CPF_ReturnParm))
							{
							...
							{
								FError::Throwf(TEXT("Implementation of function '%s' conflicts with interface '%s' - parameter %i '%s'"), *InterfaceFunctionDef->GetName(), *InterfaceDef->GetName(), End, *Property->GetName());
							}
						}
					}
				}
				...
				{
					FError::Throwf(TEXT("Missing UFunction implementation of function '%s' from interface '%s'.  This function needs a UFUNCTION() declaration."), *InterfaceFunctionDef->GetName(), *InterfaceDef->GetName());
				}
			}
		}
	}
...
void FHeaderParser::CheckDocumentationPolicyForEnum(FUnrealEnumDefinitionInfo& EnumDef, const TMap<FName, FString>& MetaData, const TArray<TMap<FName, FString>>& Entries)
{
	...
	if (DocumentationPolicy.bClassOrStructCommentRequired)
	{
		...
		{
			UE_LOG_ERROR_UHT(TEXT("Enum '%s' does not provide a tooltip / comment (DocumentationPolicy)."), *EnumDef.GetName());
		}
	}
	...
	for (const TMap<FName, FString>& Entry : Entries)
	{
		...
		if (ToolTip == nullptr)
		{
			UE_LOG_ERROR_UHT(TEXT("Enum entry '%s::%s' does not provide a tooltip / comment (DocumentationPolicy)."), *EnumDef.GetName(), *(*EntryName));
			continue;
		}
		...
		{
			UE_LOG_ERROR_UHT(TEXT("Enum entries '%s::%s' and '%s::%s' have identical tooltips / comments (DocumentationPolicy)."), *EnumDef.GetName(), *(*ExistingEntry), *EnumDef.GetName(), *(*EntryName));
		}
