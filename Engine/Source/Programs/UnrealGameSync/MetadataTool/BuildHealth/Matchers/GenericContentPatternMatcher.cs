...
namespace MetadataTool
{
	...
	/// </summary>
	abstract class GenericContentPatternMatcher : PatternMatcher
	{
		/// <summary>
		/// Set of extensions to treat as code
		/// </summary>
		static readonly string[] ContentExtensions =
		{
			".uasset",
			".umap",
			".ini",
		...
		public override List<ChangeInfo> FindCausers(PerforceConnection Perforce, BuildHealthIssue Issue, IReadOnlyList<ChangeInfo> Changes)
		{
			...
			Causers.RemoveAll(x => !ContainsAnyFileWithExtension(Perforce, x, ContentExtensions));
			return Causers;
		}
	}
}
