...
namespace EpicGames.UHT.Exporters.CodeGen
{
		...
		: UhtHeaderCodeGenerator
	{
		...
		private StringBuilder AppendClass(StringBuilder builder, UhtClass classObj)
		{
			...
			AppendRpcFunctions(builder, classObj, api, reversedFunctions, false);
			if (hasEditorRpc)
			{
				AppendRpcFunctions(builder, classObj, api, reversedFunctions, true);
			}
		...
		private StringBuilder AppendRpcFunctions(StringBuilder builder, UhtClass classObj, string api,  List<UhtFunction> reversedFunctions, bool editorOnly)
		{
			...
			{
				AppendAutogeneratedBlueprintFunctionDeclarations(builder, classObj, api, reversedFunctions, editorOnly);
			...
			using (UhtMacroCreator macro = new(builder, this, classObj, editorOnly ? EditorOnlyRpcWrappersNoPureDeclsMacroSuffix : RpcWrappersNoPureDeclsMacroSuffix))
			{
				...
				{
					AppendAutogeneratedBlueprintFunctionDeclarationsOnlyNotDeclared(builder, classObj, api, reversedFunctions, editorOnly);
				}
				AppendRpcWrappers(builder, reversedFunctions, editorOnly);
			}
		...
		private StringBuilder AppendAutogeneratedBlueprintFunctionDeclarations(StringBuilder builder, UhtClass classObj, string api, List<UhtFunction> reversedFunctions, bool editorOnly)
		{
			foreach (UhtFunction function in reversedFunctions)
			{
				...
				AppendNetValidateDeclaration(builder, classObj, api, function);
		...
		private StringBuilder AppendAutogeneratedBlueprintFunctionDeclarationsOnlyNotDeclared(StringBuilder builder, UhtClass classObj, string api, List<UhtFunction> reversedFunctions, bool editorOnly)
		{
			foreach (UhtFunction function in reversedFunctions)
			{
				if (!IsRpcFunction(function, editorOnly) || function.CppImplName == function.SourceName)
		...
		private static StringBuilder AppendNetValidateDeclaration(StringBuilder builder, UhtClass classObj, string api, UhtFunction function)
		{
