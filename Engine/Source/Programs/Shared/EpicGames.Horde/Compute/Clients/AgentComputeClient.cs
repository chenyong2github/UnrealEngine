...
namespace EpicGames.Horde.Compute.Clients
{
	...
	public sealed class AgentComputeClient : IComputeClient
	{
		...
		{
			readonly IAsyncEnumerator<IComputeSocket> _source;
			...
			/// <inheritdoc/>
			public IComputeSocket Socket => _source.Current;

			public LeaseImpl(IAsyncEnumerator<IComputeSocket> source) => _source = source;
		...
		public async Task<IComputeLease?> TryAssignWorkerAsync(ClusterId clusterId, Requirements? requirements, CancellationToken cancellationToken)
		{
			...
			IAsyncEnumerator<IComputeSocket> source = ConnectAsync(cancellationToken).GetAsyncEnumerator(cancellationToken);
		...
		async IAsyncEnumerable<IComputeSocket> ConnectAsync([EnumeratorCancellation] CancellationToken cancellationToken)
		{
