...
namespace EpicGames.Horde.Storage
{
	...
	public interface IBlob
	{
		...
		/// </summary>
		/// <returns></returns>
		IReadOnlyList<BlobLocator> References { get; }
	}
	...
	public interface IBlobStore
	{
		...
		/// <returns></returns>
		Task<IBlob> ReadBlobAsync(BlobLocator id, CancellationToken cancellationToken = default);
		...
		Task<BlobLocator> WriteBlobAsync(ReadOnlySequence<byte> data, IReadOnlyList<BlobLocator> references, Utf8String prefix = default, CancellationToken cancellationToken = default);
		...
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>Unique identifier for the blob</returns>
		Task<BlobLocator> WriteRefAsync(RefName name, ReadOnlySequence<byte> data, IReadOnlyList<BlobLocator> references, CancellationToken cancellationToken = default);
		...
		/// <param name="cancellationToken">Cancellation token for the operation</param>
		/// <returns>Unique identifier for the blob</returns>
		Task WriteRefTargetAsync(RefName name, BlobLocator blobId, CancellationToken cancellationToken = default);
	...
	public static class BlobStoreExtensions
	{
		class TypedBlobStore<T> : IBlobStore<T>
		{
			...
			/// <inheritdoc/>
			public Task<IBlob> ReadBlobAsync(BlobLocator id, CancellationToken cancellationToken = default) => _inner.ReadBlobAsync(id, cancellationToken);

			/// <inheritdoc/>
			public Task<BlobLocator> WriteBlobAsync(ReadOnlySequence<byte> data, IReadOnlyList<BlobLocator> references, Utf8String prefix = default, CancellationToken cancellationToken = default) => _inner.WriteBlobAsync(data, references, prefix, cancellationToken);
			...
			/// <inheritdoc/>
			public Task<BlobLocator> WriteRefAsync(RefName name, ReadOnlySequence<byte> data, IReadOnlyList<BlobLocator> references, CancellationToken cancellationToken = default) => _inner.WriteRefAsync(name, data, references, cancellationToken);
	...
	public static class Blob
	{
		...
		{
			/// <inheritdoc/>
			public BlobLocator Id { get; }

			/// <inheritdoc/>
			public ReadOnlyMemory<byte> Data { get; }

			/// <inheritdoc/>
			public IReadOnlyList<BlobLocator> References { get; }

			public InMemoryBlob(BlobLocator id, ReadOnlyMemory<byte> data, IReadOnlyList<BlobLocator> references)
			{
				Id = id;
				Data = data;
		...
		public static IBlob FromMemory(BlobLocator id, ReadOnlyMemory<byte> data, IReadOnlyList<BlobLocator> references) => new InMemoryBlob(id, data, references);
		...
		/// <returns>Serialized data</returns>
		public static ReadOnlySequence<byte> Serialize(ReadOnlyMemory<byte> data, IReadOnlyList<BlobLocator> references)
		{
		...
		/// <param name="references">List of references to other blobs</param>
		/// <returns>Serialized data</returns>
		public static ReadOnlySequence<byte> Serialize(ReadOnlySequence<byte> data, IReadOnlyList<BlobLocator> references)
		{
			ByteArrayBuilder writer = new ByteArrayBuilder();
			writer.WriteVariableLengthArray(references, x => writer.WriteBlobLocator(x));
		...
		public static IBlob Deserialize(BlobLocator id, ReadOnlyMemory<byte> memory) => Deserialize(id, new ReadOnlySequence<byte>(memory));
		...
		/// <returns>Deserialized blob data. May reference the supplied memory.</returns>
		public static IBlob Deserialize(BlobLocator id, ReadOnlySequence<byte> sequence)
		{
			MemoryReader reader = new MemoryReader(sequence.First);
			IReadOnlyList<BlobLocator> references = reader.ReadVariableLengthArray(() => reader.ReadBlobLocator());
