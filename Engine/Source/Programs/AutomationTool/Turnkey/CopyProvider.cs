...
namespace Turnkey
{
	...
	abstract class CopyProvider
	{
		...
		private static void ParseOperation(string Operation, out CopyProvider Provider, out string ProviderParam)
		{
		...
		public static string ExecuteCopy(string CopyOperation, CopyExecuteSpecialMode SpecialMode=CopyExecuteSpecialMode.None, string SpecialModeHint=null)
		{
			...
			ParseOperation(CopyOperation, out Provider, out ProviderParam);
			...
			if (OutputPath != null)
			{
				...
				if (Ext == ".zip" || Ext == ".7z")
				{
					...
					// we can't trust it, so we re-decompress
					string Tag = OutputPath + "_Decompressed";

					string CachedLocation = LocalCache.GetCachedPathByTag(Tag);
					...
					{
						// make a random temp directory, or use the download directory
						string TempDir = SpecialMode == CopyExecuteSpecialMode.UsePermanentStorage ? Path.GetDirectoryName(OutputPath) : LocalCache.CreateTempDirectory();
						bool bFailed = false;
						if (Ext == ".zip")
						{
							try
							{
								...
								{
									TurnkeyUtils.Log("Unzipping {0} to {1}...", OutputPath, TempDir);
						...
						{
							TurnkeyUtils.Log("7zip decompressing {0} to {1}...", OutputPath, TempDir);
							...
							int ExitCode = UnrealBuildTool.Utils.RunLocalProcessAndLogOutput(TurnkeyUtils.ExpandVariables("$(EngineDir)/Restricted/NotForLicensees/Extras/ThirdPartyNotUE/7-Zip/7z.exe"),
								string.Format("x -o{0} {1}", TempDir, OutputPath));
							if (ExitCode != 0)
							{
						...
						{
							OutputPath = TempDir;
							LocalCache.CacheLocationByTag(Tag, OutputPath);
						}
