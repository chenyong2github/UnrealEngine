...
public class MemreportHelper : BuildCommand
{
	...
	public override ExitCode Execute()
	{
		...
		string MemreportFile = ParseParamValue("report", "");
		string OutputDir = ParseParamValue("outcsvdir", "");
		...
		if (string.IsNullOrWhiteSpace(MemreportFile))
		{
			LogError("No memreport file specified");
			return ExitCode.Error_Arguments;
		}

		if (string.IsNullOrEmpty(OutputDir))
		{
			OutputDir = Path.GetDirectoryName(MemreportFile);
		}
		...
		if (File.Exists(MemreportFile)== false)
		{
			LogError("Memreport file not found: " + MemreportFile);
			return ExitCode.Error_Arguments;
		}

		// Pull the file into a buffer of lines (this will be stomped by the process)
		InputFileAllLines = File.ReadAllLines(MemreportFile);
		...
		if (CsvFound)
		{
			...
			string OutputRootFileName = Path.Combine(OutputDir, Path.GetFileName(MemreportFile));
		...
		LogInformation("************************ MemreportToGSheet WORK COMPLETED ************************");
