...
public partial class Project : CommandUtils
{
	...
	static void StageConfigFiles(DeploymentContext SC, DirectoryReference ConfigDir, string PlatformExtensionName)
	{
		...
		// Stage JSON manifests that were generated by the SteamVR Input action system
		List<FileReference> ManifestFiles = SC.FindFilesToStage(ConfigDir, "*.json", StageFilesSearch.AllDirectories);
		foreach (FileReference ManifestFile in ManifestFiles)
		{
			CommandUtils.LogInformation("Adding manifest file {0}", ManifestFile);
			SC.StageFile(StagedFileType.NonUFS, ManifestFile);
		}
	}
	...
	public static void WriteStageCommandline(FileReference IntermediateCmdLineFile, ProjectParams Params, DeploymentContext SC)
	{
		...
		if (!string.IsNullOrEmpty(Params.StageCommandline) || !string.IsNullOrEmpty(Params.RunCommandline))
		{
			...
			if (SC.StageTargetPlatform.PlatformType == UnrealTargetPlatform.Mac || SC.StageTargetPlatform.PlatformType == UnrealTargetPlatform.Win64 || SC.StageTargetPlatform.PlatformType == UnrealTargetPlatform.Linux)
			{
				ProjectFile = "";
			}
