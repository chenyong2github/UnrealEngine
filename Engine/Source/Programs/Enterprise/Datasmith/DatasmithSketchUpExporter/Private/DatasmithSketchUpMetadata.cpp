...
)
{
	...
	if (!MetadataDefinitionMap.Contains(MetadataPtr->SSourceID))
	{
		// Add the metadata definition to our dictionary.
		MetadataDefinitionMap.Add(MetadataPtr->SSourceID, MetadataPtr);
	}
}
...
)
{
	...
	SUEntityGetID(InSEntityRef, &SSourceID); // we can ignore the returned SU_RESULT
...
)
{
	...
	SU_GET_STRING(SUAttributeDictionaryGetName, InSAttributeDictionaryRef, SAttributeDictionaryName);
...
)
{
	...
	FString SAttributePath;
	SU_GET_STRING(SUClassificationAttributeGetPath, InSSchemaAttributeRef, SAttributePath);
...
)
{
	...
	switch (SAttributeType)
	{
		...
		case SUTypedValueType::SUTypedValueType_Bool:
		{
			...
			SAttributeValue = FString::Printf(TEXT("%ls"), BoolValue ? TEXT("true") : TEXT("false"));
			break;
		}
		...
		{
			SU_GET_STRING(SUTypedValueGetString, InSTypedValueRef, SAttributeValue);
			break;
		}
