...
public:
	...
	void RebuildLayerTimeInfoCache(const UE::FUsdStage& UsdStage);
	 ...
	 */
	void InitializeActorTimeCodes(const UE::FUsdStage& UsdStage);
...
private:
	...
	FLayerTimeInfo* RecursivelyCreateLayerTimeInfo(const UE::FSdfLayer& LayerHandle, TSet<FString>& ExploredFiles);
...
void FUsdLevelSequenceHelperImpl::RebuildLayerTimeInfoCache(const UE::FUsdStage& UsdStage)
{
	if (!UsdStage)
	{
		return;
	...
	UE::FSdfLayer RootLayerHandle = UsdStage.GetRootLayer();

	// Used to catch N-item infinite loops (e.g. A/B/C/A/B/C/etc)
	TSet<FString> ExploredPaths;
	FLayerTimeInfo* RootLayerInfo = RecursivelyCreateLayerTimeInfo(RootLayerHandle, ExploredPaths);
	RootLayerInfoIdentifier = RootLayerInfo->Identifier;

	RecursivelyPropagateTimeCodes(RootLayerInfo);
}

void FUsdLevelSequenceHelperImpl::InitializeActorTimeCodes(const UE::FUsdStage& UsdStage)
{
	AUsdStageActor* ValidStageActor = StageActor.Get();
...

	{
		...
		UE::FSdfLayer SessionLayer = UsdStage.GetSessionLayer();
...
FUsdLevelSequenceHelperImpl::FLayerTimeInfo* FUsdLevelSequenceHelperImpl::RecursivelyCreateLayerTimeInfo(const UE::FSdfLayer& LayerRefPtr, TSet<FString>& ExploredFiles)
{
	...
	FString FilePath = LayerRefPtr.GetRealPath();
	...
	FString Identifier = LayerRefPtr.GetIdentifier();
	...
	TArray< FString > SubLayerPaths = LayerRefPtr.GetSubLayerPaths();
	TArray< UE::FSdfLayerOffset > SubLayerOffsetScales = LayerRefPtr.GetSubLayerOffsets();

	for (int64 LayerIndex = 0; LayerIndex < LayerRefPtr.GetNumSubLayerPaths(); ++LayerIndex)
	{
		FString RelativePath = UE::FSdfLayerUtils::SdfComputeAssetPathRelativeToLayer(LayerRefPtr, *SubLayerPaths[LayerIndex]);
		UE::FSdfLayer SubLayer = UE::FSdfLayer::FindOrOpen(*RelativePath);
		...
		const UE::FSdfLayerOffset& SubLayerOffsetScale = SubLayerOffsetScales[LayerIndex];
		...
		if (FUsdLevelSequenceHelperImpl::FLayerTimeInfo* SubLayerInfo = RecursivelyCreateLayerTimeInfo(SubLayer, ExploredFiles))
		{
			UE_LOG(LogUsd, Verbose, TEXT("Adding sublayer '%s' to layer '%s'. Offset: '%f', Scale: '%f'"),
				...
				*LayerTimeInfo.Identifier,
				SubLayerOffsetScale.Offset,
				SubLayerOffsetScale.Scale);

			LayerTimeInfo.SubLayerIdentifiers.Add(SubLayerInfo->Identifier);
			LayerTimeInfo.OffsetTimeCodes.Add(SubLayerOffsetScale.Offset);
			LayerTimeInfo.Scales.Add(SubLayerOffsetScale.Scale);
		}
	}
...
void FUsdLevelSequenceHelperImpl::UpdateInfoFromSection(FUsdLevelSequenceHelperImpl::FLayerTimeInfo* Info, const UMovieSceneSubSection* Section, const UMovieSceneSequence* Sequence)
{
	...
	UE::FSdfLayer Layer = UE::FSdfLayer::FindOrOpen(*Info->Identifier);
	if (!Layer)
	{
	...
	UE::FSdfLayerOffset NewLayerOffset{ModifiedOffset, ModifiedScale};
...
void FUsdLevelSequenceHelper::UpdateLevelSequence(const UE::FUsdStage& UsdStage)
{
#if USE_USD_SDK
	if (UsdSequencerImpl.IsValid() && UsdStage)
	{
