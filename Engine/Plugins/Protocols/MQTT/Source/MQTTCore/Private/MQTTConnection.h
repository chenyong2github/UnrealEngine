	...
	-> TFuture<typename OperationType::FResponseType>
{
	...
	{
		UE_LOG(LogMQTTCore, Warning, TEXT("Tried to queue operation while stopping: %s"), ANSI_TO_TCHAR(typeid(OperationType).name()));
	}

	UE_LOG(LogMQTTCore, Verbose, TEXT("Queued Operation: %s"), ANSI_TO_TCHAR(typeid(OperationType).name()));
#endif

	// const auto Operation = MakeShared<TMQTTOperation<RequestType, ResponseType>, ESPMode::ThreadSafe>(MoveTemp(InRequestPacket), MessageIdGenerator->Poke());
