...
FGLTFJsonImageIndex FGLTFImageBuilder::AddImage(const TArray64<uint8>& RawData, int32 Width, int32 Height, ERGBFormat RawFormat, int32 BitDepth, bool bFloatFormat, const FString& Name, EGLTFJsonMimeType MimeType, int32 Quality)
{
	...
	else
	{
		...
		case EGLTFJsonMimeType::PNG:
            if (!FGLTFBuilderUtility::CompressImage(RawData, Width, Height, RawFormat, BitDepth, ImageData, EImageFormat::PNG, Quality))
            {
                return FGLTFJsonImageIndex(INDEX_NONE);
            }
			break;
		case EGLTFJsonMimeType::JPEG:
            if (!FGLTFBuilderUtility::CompressImage(RawData, Width, Height, RawFormat, BitDepth, ImageData, EImageFormat::JPEG, Quality))
            {
                return FGLTFJsonImageIndex(INDEX_NONE);
            }
			break;
...
FGLTFJsonImageIndex FGLTFImageBuilder::AddImage(const FTextureSource& Image, const FString& Name, EGLTFJsonMimeType MimeType, int32 Quality)
{
	...
	return AddImage(RawData, Image.GetSizeX(), Image.GetSizeY(), RawFormat, BitDepth, bFloatFormat, Name, MimeType, Quality);
}
