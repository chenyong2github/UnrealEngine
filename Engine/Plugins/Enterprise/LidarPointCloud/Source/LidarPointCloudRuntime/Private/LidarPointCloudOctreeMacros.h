...
#define FOR(ElemName, Array) for(auto ElemName = Array.GetData(), DataEnd = ElemName + Array.Num(); ElemName != DataEnd; ++ElemName)
...
#define ITERATE_POINTS(Action) \
{\
	FOR(Point, CurrentNode->AllocatedPoints) { Action }\
	FOR(Point, CurrentNode->PaddingPoints) { Action }\
}
...
#define PROCESS_BODY(Action, PointTest) \
{\
	/* If node fully inside the box - do not check individual points */\
	if (bNodeFullyContained)\
	{\
		if (!IS_VIS_CHECK_REQUIRED) { ITERATE_POINTS(Action) }\
		else { ITERATE_POINTS({ if (Point->bVisible) { Action } }) }\
	}\
	else\
	{\
		if (!IS_VIS_CHECK_REQUIRED) { ITERATE_POINTS({ if (PointTest) { Action } }) }\
		else { ITERATE_POINTS({ if (Point->bVisible && PointTest) { Action } }) }\
	}\
}

#define PROCESS_ALL_BODY(Action) \
{\
	if (!bVisibleOnly || CurrentNode->NumVisiblePoints > 0)\
	{\
		const bool bNodeFullyContained = true;\
		PROCESS_BODY(Action, true) \
	}\
}

#define PROCESS_IN_SPHERE_BODY(Action) \
{\
	if (!bVisibleOnly || CurrentNode->NumVisiblePoints > 0)\
	{\
		const bool bNodeFullyContained = CurrentNode->GetSphereBounds(this).IsInside(Sphere);\
		PROCESS_BODY(Action, POINT_IN_SPHERE) \
	}\
}

#define PROCESS_IN_BOX_BODY(Action) \
{\
	if (!bVisibleOnly || CurrentNode->NumVisiblePoints > 0)\
	{\
		...
		PROCESS_BODY(Action, POINT_IN_BOX) \
	}\
}

#define PROCESS_IN_FRUSTUM_BODY(Action) \
{\
	if (!bVisibleOnly || CurrentNode->NumVisiblePoints > 0)\
	{\
		bool bNodeFullyContained;\
		Frustum.IntersectBox(CurrentNode->Center, SharedData[CurrentNode->Depth].Extent, bNodeFullyContained);\
		PROCESS_BODY(Action, POINT_IN_FRUSTUM) \
	}\
}

#define PROCESS_BY_RAY_BODY(Action) \
{\
	if (!bVisibleOnly || CurrentNode->NumVisiblePoints > 0)\
	{\
		if (Ray.Intersects(CurrentNode->GetBounds(this)))\
		{\
			if (!IS_VIS_CHECK_REQUIRED) { ITERATE_POINTS({ if (POINT_BY_RAY) { Action } }) }\
			else { ITERATE_POINTS({ if (Point->bVisible && POINT_BY_RAY) { Action } }) }\
\
...
{\
	...
	const FBox Box(Sphere.Center - FVector(Sphere.W), Sphere.Center + FVector(Sphere.W));\
	const float RadiusSq = Sphere.W * Sphere.W;\
	ITERATE_NODES_CONST(PROCESS_IN_SPHERE_BODY(Action), NODE_IN_BOX) \
}

#define PROCESS_ALL(Action) { ITERATE_NODES(PROCESS_ALL_BODY(Action), true) }
#define PROCESS_ALL_CONST(Action) { ITERATE_NODES_CONST(PROCESS_ALL_BODY(Action), true) }

#define PROCESS_IN_BOX(Action) { ITERATE_NODES(PROCESS_IN_BOX_BODY(Action), NODE_IN_BOX) }
#define PROCESS_IN_BOX_CONST(Action) { ITERATE_NODES_CONST(PROCESS_IN_BOX_BODY(Action), NODE_IN_BOX) }

#define PROCESS_IN_FRUSTUM(Action) { ITERATE_NODES(PROCESS_IN_FRUSTUM_BODY(Action), NODE_IN_FRUSTUM) }
#define PROCESS_IN_FRUSTUM_CONST(Action) { ITERATE_NODES_CONST(PROCESS_IN_FRUSTUM_BODY(Action), NODE_IN_FRUSTUM) }
...
{\
	const float RadiusSq = Radius * Radius; \
	ITERATE_NODES_CONST(PROCESS_BY_RAY_BODY(Action), false)\
}