...
namespace CADLibrary
{
	...
	void CreateCTFace_internal(const ON_BrepFace& Face, CT_LIST_IO& dest, ON_BoundingBox& outerBBox, ON_NurbsSurface& Surface, bool ignoreInner)
	{
		...
		for (int LoopIndex = 0; LoopIndex < LoopCount; ++LoopIndex)
		{
			...
			for (int i = 0; i < TrimCount; ++i)
			{
				...
				CheckedCTError setUvCurveError = CT_COEDGE_IO::SetUVCurve(
			...
			CT_OBJECT_ID Loop;
			CheckedCTError LoopCreationError = CT_LOOP_IO::Create(Loop, Coedges);
		...
		CheckedCTError result = CT_FACE_IO::Create(FaceID, SurfaceID, faceOrient, Loops);
		if (!result)
			return;
	...
	CheckedCTError FRhinoCoretechWrapper::AddBRep(ON_Brep& Brep)
	{
