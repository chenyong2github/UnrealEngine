...
//DEFINE_LOG_CATEGORY_STATIC(LogDatasmithDispatcher, Log, All);
...
namespace DatasmithDispatcher
{
...
#if PLATFORM_MAC
		ProcessorPath = FPaths::Combine(FPaths::EngineDir(), TEXT("Binaries/Mac/NotForLicensees/DatasmithCoretechWorker"));
#elif PLATFORM_LINUX
		ProcessorPath = FPaths::Combine(FPaths::EngineDir(), TEXT("Binaries/Linux/NotForLicensees/DatasmithCoretechWorker"));
#else
		ProcessorPath = FPaths::Combine(FPaths::EngineDir(), TEXT("Binaries/Win64/NotForLicensees/DatasmithCoretechWorker.exe"));
#endif
	...
	bool FDatasmithWorkerHandler::StartWorker()
	{
		...
		//StartTime = Stopwatch.GetDuration();
	...
	bool FDatasmithWorkerHandler::Run()
	{
		...
		while (NeedToRun())
		{
			...
			if (!ClientListener.IsConnected())
			{
				...
				{
					Dispatcher.SetTaskState(CurrentTask->Index, ProcessFailed);
					CurrentTask.Reset();
				}
				RestartProcessor();
				bTaskIsRunning = true;
	...
	{
		//FTimespan PingTime = Stopwatch.GetDuration() - StartTime;
		//UE_LOG(LogDatasmithDispatcher, Display, TEXT("Connected in %d s"), PingTime.GetTotalSeconds());
