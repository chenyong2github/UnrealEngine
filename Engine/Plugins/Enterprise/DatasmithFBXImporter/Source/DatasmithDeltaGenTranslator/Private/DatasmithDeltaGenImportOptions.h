...
public:
	...
	UPROPERTY(config, EditAnywhere, Category = Processing, meta = (ToolTip = "Don't keep nodes that marked invisible in FBX(an din the original scene), except switch variants"))
	bool bRemoveInvisibleNodes;

	UPROPERTY(config, EditAnywhere, Category = Processing, meta = (ToolTip = "Collapse nodes that have identity transform, have no mesh and not used in animation/variants/switches"))
	bool bSimplifyNodeHierarchy;

	UPROPERTY(config, EditAnywhere, Category=AssetImporting, meta=(DisplayName="Import Variants", ToolTip="import VAR files"))
	bool bImportVar;
	...
	UPROPERTY(config, EditAnywhere, Category=AssetImporting, meta=(DisplayName="Import POS States", ToolTip="import POS files"))
	bool bImportPos;

	UPROPERTY(config, EditAnywhere, Category=AssetImporting, meta=(DisplayName="POS file path", EditCondition = "bImportPos", ToolTip="Path to the *.pos file. By default it will search for a *.pos file in the same folder as the FBX file, with the same base filename as it", FilePathFilter="pos"))
	FFilePath PosPath;

	UPROPERTY(config, EditAnywhere, Category=AssetImporting, meta=(DisplayName="Import TML Animations", ToolTip="import TML files"))
	bool bImportTml;

	UPROPERTY(config, EditAnywhere, Category=AssetImporting, meta=(DisplayName="TML file path", EditCondition = "bImportTml", ToolTip="Path to the *.tml file. By default it will search for a *.tml file in the same folder as the FBX file, with the same base filename as it", FilePathFilter="tml"))
	FFilePath TmlPath;

	UPROPERTY(config, EditAnywhere, Category=AssetImporting, meta=(DisplayName="Shadow Textures", ToolTip="How to handle shadow textures"))
