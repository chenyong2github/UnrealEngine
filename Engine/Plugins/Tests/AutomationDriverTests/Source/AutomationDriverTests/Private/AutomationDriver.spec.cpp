...
void FAutomationDriverSpec::Define()
{
	BeforeEach([this]() {
		...
		if (!SuiteWindow.IsValid())
		{
			...
			// Clean up Share pointer if the window is closed. Because if the window is closed, it can not be brought back to front.
			SuiteWindow->SetOnWindowClosed(FOnWindowClosed::CreateLambda([this](const TSharedRef<SWindow>& Window){ SuiteWindow.Reset(); }));
		}
