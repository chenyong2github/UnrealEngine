	...
	namespace Private
	{
		...
		*****************************************************************************/
		...
		std::shared_ptr<DeviceConnection> DeviceCache::GetDevice(const AJADeviceOptions& InDeviceOption)
		{
			...
			{
				LOG_ERROR(TEXT("DeviceCache: The device index '%d' is invalid.\n"), InDeviceOption.DeviceIndex);
		...
		*****************************************************************************/
		...
		bool DeviceConnection::CommandList::RegisterChannel(ETransportType InTransportType, NTV2InputSource InInputSource, NTV2Channel InChannel, bool bInIsInput, bool bConnectChannel, ETimecodeFormat InTimecodeFormat, EPixelFormat InUEPixelFormat, NTV2VideoFormat InDesiredInputFormat, bool bInAsOwner, bool bInIsAutoDetected)
		{
			...
			std::vector<ChannelInfo*>& ChannelInfos = Connection.ChannelInfos;
			...
			bool bReinitialize = FoundIterator != std::end(ChannelInfos) && (*FoundIterator)->bIsAutoDetected;
			...
			else
			{
				...
				if (bReinitialize)
				{
					...
					{
						LOG_WARNING(TEXT("Device: Autodetect reinitialized channel '%d' on device '%S' but the channel was not in autodetect mode.\n")
				...
				{
					LOG_ERROR(TEXT("Device: Trying to enable the channel '%d' on device '%S' as the owner but it was already enabled. Do you already have an '%S' running on the same channel?\n")
						, uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str()
				...
				{
					LOG_ERROR(TEXT("Device: Trying to enable the channel '%d' on device '%S' as a '%S' but it was already enabled as a '%S'.\n")
				...
				{
					LOG_ERROR(TEXT("Device: Trying to enable the channel '%d' on device '%S' as an '%S' but it was already enabled as an '%S'.\n")
						, uint32_t(InChannel) + 1
						, Connection.Card->GetDisplayName().c_str()
				...
				if (bResult)
				{
					...
					{
						LOG_ERROR(TEXT("Device: Initialization of the input failed for channel %d on device %S. %S"), uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str(), FailureReason.c_str());
						bResult = false;
					}
					...
					if (bResult && !bInIsAutoDetected && VideoFormat != FoundChannel->VideoFormat)
					{
						...
						LOG_ERROR(TEXT("Device: Try to enable with the video format '%S' the channel '%d' on device '%S' that was already enabled with '%S'.\n")
							, NTV2FrameRateToString(GetNTV2FrameRateFromVideoFormat(VideoFormat), bForRetailDisplay)
							, uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str()
							, NTV2FrameRateToString(GetNTV2FrameRateFromVideoFormat(FoundChannel->VideoFormat), bForRetailDisplay));
						bResult = false;
					}
				}
				...
				{
					LOG_WARNING(TEXT("Device: Try to enable the channel '%d' on device '%S' with a timecode format that is not the same as the previous one. Timecode may not be decoded properly.\n")
						, uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str());
				}
		...
		bool DeviceConnection::CommandList::UnregisterChannel(NTV2Channel InChannel, bool bInAsOwner)
		{
			...
			if (FoundIterator == std::end(ChannelInfos))
			{
				LOG_ERROR(TEXT("Device: Try to unregister the channel '%d' on device '%S' when it's not registered.\n"), uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str());
				assert(false);
				return false;
			}
			...
			if (FoundChannel->RefCounter == 0)
			{
				...
				{
					AJAAutoLock Lock(&Connection.ChannelInfoLock);
					ChannelInfos.erase(FoundIterator);
				}
				delete FoundChannel;
		...
		void DeviceConnection::CommandList::SetChannelConnected(NTV2Channel InChannel, bool bConnected)
		{
			...
			auto Found = std::find_if(std::begin(ChannelInfos), std::end(ChannelInfos), [=](const ChannelInfo* Info) {return Info->Channel == InChannel; });
			assert(Found != std::end(ChannelInfos));
			if (Found != std::end(ChannelInfos))
			{
		...
		bool DeviceConnection::CommandList::RegisterAnalogLtc(EAnalogLTCSource InSource, NTV2FrameRate InFrameRate, bool bUseReferencePin)
		{
			...
			{
				LOG_ERROR(TEXT("Device: Trying to read LTC from the reference input '%d' on device '%S' but it is not supported by the device.\n"), uint32_t(InSource) + 1, Connection.Card->GetDisplayName().c_str());
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Device: Trying to read LTC from the LTC input '%d' on device '%S' but it is not supported by the device.\n"), uint32_t(InSource) + 1, Connection.Card->GetDisplayName().c_str());
				bResult = false;
			}
			...
			{
				LOG_ERROR(TEXT("Device: The device '%S' is already reading timecode from LTC input with frame rate '%S' and only one is supported."), Connection.Card->GetDisplayName().c_str(), NTV2FrameRateToString(Connection.LTCFrameRate).c_str());
				bResult = false;
			}
			...
			if (bResult && bUseReferencePin)
			{
				...
				{
					LOG_ERROR(TEXT("Device: Trying to read LTC from reference pin on device '%S' but it doesn't have that capability.\n"), Connection.Card->GetDisplayName().c_str());
					bResult = false;
				}
				...
				{
					LOG_ERROR(TEXT("Device: The reference pin is already used to read LTC timecode on device '%S'.\n"), Connection.Card->GetDisplayName().c_str());
					bResult = false;
				}
				...
				{
					LOG_ERROR(TEXT("Device: Trying to read LTC from the reference input on device '%S' but the reference is already used as a genlock source.\n"), Connection.Card->GetDisplayName().c_str());
					bResult = false;
				}
			}
			...
			if (bResult)
			{
				...
				if (Found != std::end(Connection.ChannelInfos))
				{
					...
					if (::NTV2DeviceCanDoMultiFormat(Connection.Card->GetDeviceID()))
					{
						...
						{
							LOG_ERROR(TEXT("Device: Trying to read LTC with FrameRate '%S' but it's not compatible with card's (channel 1) current FrameRate of '%S' on device '%S'.\n"), NTV2FrameRateToString(InFrameRate).c_str(), NTV2FrameRateToString(ChannelFrameRate).c_str(), Connection.Card->GetDisplayName().c_str());
							bResult = false;
						}
					}
					else if (InFrameRate != ChannelFrameRate)
					{
						LOG_ERROR(TEXT("Device: Trying to read LTC with FrameRate '%S' but card's (channel 1) current FrameRate is '%S' and device '%S' can't do multi-format.\n"), NTV2FrameRateToString(InFrameRate).c_str(), NTV2FrameRateToString(ChannelFrameRate).c_str(), Connection.Card->GetDisplayName().c_str());
						bResult = false;
					}
				}
				else
		...
		bool DeviceConnection::CommandList::RegisterReference(EAJAReferenceType InOutputReferenceType, NTV2Channel InOutputReferenceChannel)
		{
			...
			if (Connection.bOutputReferenceSet)
			{
				...
				{
					LOG_ERROR(TEXT("Device: Couldn't set the reference for output on device %S. The type was already setup with %d.\n"), Connection.Card->GetDisplayName().c_str(), Connection.OutputReferenceType);
					return false;
				}
				...
				{
					LOG_ERROR(TEXT("Device: Couldn't set the reference for output channel %d on device %S. The input was already setup with %d.\n"), Connection.Card->GetDisplayName().c_str(), Connection.OutputReferenceChannel);
					return false;
				}
			...
			case EAJAReferenceType::EAJA_REFERENCETYPE_EXTERNAL:
			{
				...
				{
					LOG_ERROR(TEXT("Device: Couldn't set the reference for output channel %d on device %S. The reference is used to read analog LTC.\n"), InOutputReferenceChannel, Connection.Card->GetDisplayName().c_str());
					return false;
				}
				AJA_CHECK(Connection.Card->SetLTCInputEnable(false));
			...
			case EAJAReferenceType::EAJA_REFERENCETYPE_INPUT:
			{
				...
				{
					LOG_ERROR(TEXT("Device: Couldn't set the reference source on device '%S. The Channel index is invalid."), Connection.Card->GetDisplayName().c_str());
					return false;
				}
		...
		*****************************************************************************/
		CNTV2Card* DeviceConnection::NewCNTV2Card(uint32_t InDeviceIndex)
		{
			...
			while (!Result->IsDeviceReady())
			{
				...
				{
					LOG_WARNING(TEXT("Device: Can't get the device initialized.\n"));
					return false;
				}
				++Counter;
				::Sleep(SleepMilli);
		...
		bool DeviceConnection::Lock_EnableChannel_SDILinkHelper(CNTV2Card* InCard, ETransportType InTransportType, NTV2Channel InChannel) const
		{
			...
			for (ChannelInfo* Itt : ChannelInfos)
			{
				if (Itt->Channel == InChannel)
				{
					...
					{
						LOG_ERROR(TEXT("Device: Trying to enable the channel '%d' on device '%S' as a '%S' but channel '%d' was already enabled.\n")
							, int32_t(InChannel) + 1
				...
				else
				{
					...
					{
						LOG_ERROR(TEXT("Device: Trying to enable the channel '%d' on device '%S' as a '%S' but it was already enabled as a '%S' by channel '%d'.\n")
							, uint32_t(InChannel) + 1
							, InCard->GetDisplayName().c_str()
							, Helpers::TransportTypeToString(InTransportType)
		...
		bool DeviceConnection::Lock_EnableChannel_Helper(CNTV2Card* InCard, ETransportType InTransportType, NTV2InputSource InInputSource, NTV2Channel InChannel, bool bIsInput, bool bConnectChannel, bool bIsAutoDetected, ETimecodeFormat InTimecodeFormat, EPixelFormat InUEPixelFormat, NTV2VideoFormat InDesiredInputFormat, NTV2VideoFormat& OutFoundVideoFormat)
		{
			...
			{
				LOG_ERROR(TEXT("Device: The device '%S' doesn't support channel '%d'"), InCard->GetDisplayName().c_str(), uint32_t(InChannel) + 1);
				return false;
			}
			...
			if (NotMultiVideoFormat != NTV2_FORMAT_UNKNOWN && NotMultiVideoFormat != OutFoundVideoFormat)
			{
				...
				if (bIsMultiFormatEnabled)
				{
					...
					{
						LOG_ERROR(TEXT("Device: The device '%S' does support Multi Format but channel %d requested format ('%S') and it's not compatible with the previous format ('%S')."), InCard->GetDisplayName().c_str(), uint32_t(InChannel) + 1, NTV2VideoFormatToString(OutFoundVideoFormat).c_str(), NTV2VideoFormatToString(NotMultiVideoFormat).c_str()); 
						return false;
					}
				}
				else if (!bIsAutoDetected)
				{
					LOG_ERROR(TEXT("Device: The device '%S' doesn't support Multi Format and channel %d requested format ('%S') which is not the same as previous format ('%S')."), InCard->GetDisplayName().c_str(), uint32_t(InChannel) + 1, NTV2VideoFormatToString(OutFoundVideoFormat).c_str(), NTV2VideoFormatToString(NotMultiVideoFormat).c_str());
					return false;
				}
			}
			...
			if (bIsInput && bIsSDI)
			{
				...
				{
					LOG_ERROR(TEXT("Device: Initialization of the input failed for channel %d on device '%S'. %S"), uint32_t(InChannel) + 1, InCard->GetDisplayName().c_str(), FailureReason.c_str());
					return false;
				}
			}
		...
		bool DeviceConnection::Lock_Initialize()
		{
			...
			{
				LOG_ERROR(TEXT("Device: Couldn't get current stream using Aja. Something is wrong when tryin to access registers on device '%S'."), Card->GetDisplayName().c_str());
				delete Card;
				Card = nullptr;
				return false;
			}
			...
			if (!Card->AcquireStreamForApplication(UEAppType, static_cast<uint32_t>(AJAProcess::GetPid())))
			{
				...
				if (CurrentAppPID)
				{
					...
					LOG_ERROR(TEXT("Device: Couldn't acquire stream for Unreal Engine application on device '%S'. Used by application '%S' with PID '%d'."), Card->GetDisplayName().c_str(), FourCCBuffer, CurrentAppPID);
				}
				else
				{
					LOG_ERROR(TEXT("Device: Couldn't acquire stream for Unreal Engine application on device '%S'."), Card->GetDisplayName().c_str());
				}
			...
			{
				LOG_INFO(TEXT("Connected to %S with SDK %d.%d.%d.%d"), Card->GetDeviceVersionString().c_str(), AJA_NTV2_SDK_VERSION_MAJOR, AJA_NTV2_SDK_VERSION_MINOR, AJA_NTV2_SDK_VERSION_POINT, AJA_NTV2_SDK_BUILD_NUMBER);
				if (AJA_NTV2_SDK_VERSION_MAJOR < 14)
				{
					LOG_WARNING(TEXT("Unreal Engine's implementation of AJA support SDK 14 and up."));
				}
			}
			...
			if (DeviceOption.bWantMultiFormatMode)
			{
				...
				{
					LOG_WARNING(TEXT("Device: The device '%S' doesn't support Multi Format."), Card->GetDisplayName().c_str());
				}
			}
		...
		uint32_t DeviceConnection::Lock_AcquireBaseFrameIndex(NTV2Channel InChannel, NTV2VideoFormat InVideoFormat) const
		{
			...
			{
				LOG_ERROR(TEXT("Device: Too many Frames Indexes requested on device '%S'.\n"), Card->GetDisplayName().c_str());
