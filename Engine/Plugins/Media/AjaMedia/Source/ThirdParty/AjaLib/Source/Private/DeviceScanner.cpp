...
#include "Private/DeviceScanner.h"
		...
		*****************************************************************************/
		...
		bool DeviceScanner::GetDeviceInfo(int32_t InDeviceIndex, AJADeviceScanner::DeviceInfo& OutDeviceInfo) const
		{
			...
			if (OutDeviceInfo.bIsSupported)
			{
				...
				OutDeviceInfo.NumSdiInput = min(DeviceInfo.numVidInputs, NumFrameStores);
				OutDeviceInfo.NumSdiOutput = min(DeviceInfo.numVidOutputs, NumFrameStores);
				OutDeviceInfo.NumHdmiInput = min(DeviceInfo.numHDMIVidInputs, NumFrameStores);
				OutDeviceInfo.NumHdmiOutput = min(DeviceInfo.numHDMIVidOutputs, NumFrameStores);
			}
