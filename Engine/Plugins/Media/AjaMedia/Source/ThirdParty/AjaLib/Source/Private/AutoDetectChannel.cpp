		...
		*****************************************************************************/
		...
		bool AutoDetectChannelImpl::Thread_Configure(DeviceConnection::CommandList& InCommandList)
		{
			...
			for (int32_t DeviceIndex = 0; DeviceIndex < NumberOfDevices; ++DeviceIndex)
			{
				...
				if (!bValidDeviceInfo)
				{
					LOG_INFO(TEXT("AutoDetectChannel: Device %d is not valid.\n"), DeviceIndex);
					break;
				}
		...
		bool AutoDetectChannelImpl::Thread_Detect(int32_t InDeviceIndex, const AJADeviceScanner::DeviceInfo& InDeviceInfo)
		{
			...
			while (!NewCard->IsDeviceReady())
			{
				...
				{
					LOG_INFO(TEXT("AutoDetectChannel: Can't get the %d device initialized.\n"), InDeviceIndex);
					return false;
				}
				++Counter;
				::Sleep(SleepMilli);
			...
			{
				LOG_INFO(TEXT("AutoDetectChannel: Couldn't acquire stream for Unreal Engine application on device '%S'."), NewCard->GetDisplayName().c_str());
				return false;
			}
