	...
	namespace Private
	{
		...
		*****************************************************************************/
			...
			, bStopRequested(false)
		{
			...
			{
				LOG_ERROR(TEXT("AJAChannel: The port index '%d' is invalid.\n"), Options.ChannelIndex);
			}
			else
			{
		...
		bool ChannelThreadBase::CanInitialize() const
		{
			...
			if (Helpers::GetNumberOfLinkChannel(GetOptions().TransportType) != 1)
			{
				...
				{
					LOG_ERROR(TEXT("Device: Can't do a %S on channel '%d'. Did you mean channel '%d'?")
		...
		bool ChannelThreadBase::ChannelThread_Initialization(DeviceConnection::CommandList& InCommandList)
		{
			...
			{
				LOG_ERROR(TEXT("ConfigureVideo: The expected video format is invalid for %d.\n"), uint32_t(Channel) + 1);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("ConfigureVideo: 4k Key and Fill output is not supported for this card.\n"));
				return false;
			}
		...
		bool ChannelThreadBase::DeviceThread_ConfigureVideo(DeviceConnection::CommandList& InCommandList)
		{
			...
			if ((bValidInput || bValidOutput) && bValidKey)
			{
				if (IsInput())
				{
					...
					{
						LOG_ERROR(TEXT("Device: Initialization of the input failed for channel %d on device '%S'. '%S'"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str(), FailureReason.c_str());
						return false;
					}
				}
				else
				{
					...
					{
						LOG_ERROR(TEXT("ConfigureVideo: Unknown video format for output channel %d on device '%S'.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						return false;
					}
				}
				...
				{
					LOG_ERROR(TEXT("ConfigureVideo: The device '%S' doesn't support the video format '%S'.\n"), GetDevice().GetDisplayName().c_str(), ::NTV2VideoFormatToString(VideoFormat).c_str());
					return false;
				}
				...
				if (IsOutput())
				{
					...
					if (GetOptions().OutputReferenceType == AJA::EAJAReferenceType::EAJA_REFERENCETYPE_INPUT)
					{
						...
						{
							LOG_ERROR(TEXT("ConfigureVideo: The synchronization port index '%d' is invalid.\n"), GetOptions().SynchronizeChannelIndex);
							return false;
						}
						else
						{
			...
			{
				LOG_ERROR(TEXT("ConfigureVideo: Invalid video options for channel %d on device '%S'."), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
			...
			else
			{
				...
				if (Helpers::IsSdiTransport(GetOptions().TransportType))
				{
					if (!Helpers::SetSDIOutLevelAtoLevelBConversion(Device->GetCard(), GetOptions().TransportType, Channel, VideoFormat, GetOptions().bConvertOutputLevelAToB))
					{
						...
						{
							LOG_WARNING(TEXT("ConfigureVideo: Can't convert level A to B for channel %d on device '%S'."), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						}
					}
					...
					if (UseKey())
					{
						const bool bConnectedChannel = false;
						const bool bAsOwner = true;
						...
						if (!InCommandList.IsChannelConnect(KeyChannel))
						{
							if (!Helpers::SetSDIOutLevelAtoLevelBConversion(Device->GetCard(), GetOptions().TransportType, KeyChannel, VideoFormat, GetOptions().bConvertOutputLevelAToB))
							{
								if (GetOptions().bConvertOutputLevelAToB)
								{
									LOG_WARNING(TEXT("ConfigureVideo: Can't convert level A to B on key for channel %d on device '%S'."), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
								}
							}
		...
		bool ChannelThreadBase::DeviceThread_ConfigureAnc(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (UseAncillary() || UseAncillaryField2())
			{
				...
				if (!bSupportAnc)
				{
					LOG_WARNING(TEXT("ConfigureAncillary: Device doesn't support Anc.\n"));
		...
		bool ChannelThreadBase::DeviceThread_ConfigureAudio(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (UseAudio() && GetOptions().CallbackInterface != nullptr)
			{
				...
				{
					LOG_ERROR(TEXT("ConfigureAudio: Could not get valid AudioSystem from InputSource %d for channel %d on device %S."), uint32_t(InputSource), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					return false;
				}
			}
			else
			{
				if (UseAudio())
				{
					LOG_ERROR(TEXT("ConfigureAudio: Invalid audio options for channel %d on device %S."), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					return false;
				}
			}
			...
			if (NTV2_IS_VALID_AUDIO_SYSTEM(AudioSystem))
			{
				CNTV2Card& Device = GetDevice();

				if (IsInput())
				{
					//	Have the audio system capture audio from the designated device input (i.e., ch1 uses SDIIn1, ch2 uses SDIIn2, etc.)...
					Device.SetAudioSystemInputSource(AudioSystem, NTV2_AUDIO_EMBEDDED, ::NTV2ChannelToEmbeddedAudioInput(Channel));
				}

				ULWord MaxNumOfChannels = ::NTV2DeviceGetMaxAudioChannels(Device.GetDeviceID());
				...
				Device.SetNumberAudioChannels(NumberOfAudioChannel, AudioSystem);

				//	The on-device audio buffer should be 4MB to work best across all devices & platforms...
				Device.SetAudioBufferSize(NTV2_AUDIO_BUFFER_BIG, AudioSystem);

				if (IsOutput())
				{
					//	Set up the output audio embedders...
					if (::NTV2DeviceGetNumAudioSystems(Device.GetDeviceID()) > 1)
					{
						Device.SetSDIOutputAudioSystem(Channel, AudioSystem);
					}
				}

				Device.SetAudioLoopBack(NTV2_AUDIO_LOOPBACK_OFF, AudioSystem);
