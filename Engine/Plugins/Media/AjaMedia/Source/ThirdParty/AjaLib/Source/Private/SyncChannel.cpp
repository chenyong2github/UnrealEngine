	...
	namespace Private
	{
		...
		*****************************************************************************/
			...
			, PreviousVerticalInterruptCount(0)
		{
			...
			{
				LOG_ERROR(TEXT("SyncChannel: The port index '%d' is invalid.\n"), InOptions.ChannelIndex);
			}
			else
			{
		...
		bool SyncChannelImpl::Thread_Configure(DeviceConnection::CommandList& InCommandList)
		{
			...
			{
				LOG_ERROR(TEXT("SyncChannel: The expected video format is invalid for %d.\n"), uint32_t(Channel) + 1);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("SyncChannel: Was not able to lock on device %S for channel %d.\n"), GetDevice().GetDisplayName().c_str(), uint32_t(Channel) + 1);
				return false;
			}
		...
		bool SyncChannelImpl::WaitForSync()
		{
			if (!SyncOption.bOutput)
			{
				...
				const NTV2InputSource InputSource = Helpers::IsHdmiTransport(SyncOption.TransportType) ? GetNTV2HDMIInputSourceForIndex(Channel) : GetNTV2InputSourceForIndex(Channel);
				std::optional<NTV2VideoFormat> FoundFormat = Helpers::GetInputVideoFormat(Device->GetCard(), SyncOption.TransportType, Channel, InputSource, DesiredVideoFormat, false, FailureReason);

				if (!FoundFormat.has_value())
				{
					LOG_ERROR(TEXT("Sync: The VideoFormat was invalid for channel %d on device %S. %S\n")
						, uint32_t(Channel) + 1
				...
				if (!Helpers::CompareFormats(DesiredVideoFormat, FoundFormat.value(), FailureReason))
				{
					...
					{
						LOG_ERROR(TEXT("Sync: The VideoFormat changed for channel %d on device %S. %S\n")
							, uint32_t(Channel) + 1
							, GetDevice().GetDisplayName().c_str()
							, FailureReason.c_str()
						);
		...
		bool SyncChannelImpl::GetVideoFormat(NTV2VideoFormat& OutVideoFormat)
		{
			...
			{
				InputSource = GetNTV2HDMIInputSourceForIndex(Channel);
			}
			else
			{
				InputSource = GetNTV2InputSourceForIndex(Channel);
			}
