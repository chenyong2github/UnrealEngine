...
namespace AJA
{
	namespace Private
	{
		...
		*****************************************************************************/
		...
		bool OutputChannel::SetVideoFrameData(const AJAOutputFrameBufferData& InFrameData, void* RHITexture)
		{
			if (ChannelThread)
			{
		...
		*****************************************************************************/
		...
		bool OutputChannelThread::SetAncillaryFrameData(const AJAOutputFrameBufferData& InFrameData, uint8_t* InAncillaryBuffer, uint32_t InAncillaryBufferSize)
		{
			...
			{
				LOG_ERROR(TEXT("SetFrameData: Can't set the ancillary. The buffer is invalid or the buffer size is not the same as the AJA for output channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		bool OutputChannelThread::SetAudioFrameData(const AJAOutputFrameBufferData& InFrameData, uint8_t* InAudioBuffer, uint32_t InAudioBufferSize)
		{
			...
			{
				LOG_ERROR(TEXT("SetAudioFrameData: Can't set the audio. The buffer is invalid or the buffer size is not the same as the AJA Audio Format for output channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		bool OutputChannelThread::SetVideoFrameData(const AJAOutputFrameBufferData& InFrameData, uint8_t* InVideoBuffer, uint32_t InVideoBufferSize)
		{
			...
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video. The buffer is invalid or the buffer size is not the same as the AJA Video Format for output channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		bool OutputChannelThread::SetVideoFrameData(const AJAOutputFrameBufferData& InFrameData, void* RHITexture)
		{
			const bool bIsProgressive = ::IsProgressivePicture(VideoFormat);

			if (!bIsProgressive)
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video, GPUTextureTransfer is not supported with interlaced video for output channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
			...
			if (!RHITexture)
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video. The RHI texture is invalid for output channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}

			if (!TextureTransfer || !GetOptions().bUseGPUDMA)
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video. GPU DMA was not setup correctly for output channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
			...
			if (!bDMABuffersRegistered)
			{
				for (Frame* IttFrame : AllFrames)
				{
					...
					Args.PixelFormat = GetOptions().PixelFormat == EPixelFormat::PF_8BIT_YCBCR ? GPUTextureTransfer::EPixelFormat::PF_8Bit : GPUTextureTransfer::EPixelFormat::PF_10Bit;
			...
			const GPUTextureTransfer::ETransferDirection Direction = GPUTextureTransfer::ETransferDirection::GPU_TO_CPU;
			TextureTransfer->TransferTexture(AvailableWritingFrame->VideoBuffer, RHITexture, Direction);
		...
		bool OutputChannelThread::DeviceThread_ConfigureAudio(DeviceConnection::CommandList& InCommandList)
		{
			...
			if(bResult && UseAudio())
			{
				...
				if (AudioBufferSize > 0)
				{
					...
					if (bResult == false)
					{
						LOG_ERROR(TEXT("ConfigureAudio: Could not start audio output for audio system %d for channel %d on device %S."), uint32_t(AudioSystem), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					}
				}
			}
		...
		void OutputChannelThread::Thread_AutoCirculateLoop()
		{
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bRunning)
				{
					LOG_ERROR(TEXT("AutoCirculate: Can't wait for the output field for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				}
				...
				if (!bRunning)
				{
					LOG_ERROR(TEXT("AutoCirculate: Can't get the status for output channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					break;
				}
		...
		void OutputChannelThread::Thread_PingPongLoop()
		{
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bRunning)
				{
					LOG_ERROR(TEXT("PingPong: Can't wait for the output field for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					break;
				}
				...
				if (AvailableReadingFrame)
				{
					...
					if (UseAudio())
					{
						...
						if (bAudioSuccess)
						{
							...
							{
								LOG_WARNING(TEXT("PingPong: Audio play head is catching up to write head for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
							}

							bool bAudioRunning = false;
							if (GetDevice().IsAudioOutputRunning(AudioSystem, bAudioRunning) && !bAudioRunning)
							{
								LOG_WARNING(TEXT("PingPong: Audio output option is true but the audio output is not running for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
							}
						}
						else
						{
					...
					if (!bRunning)
					{
						LOG_ERROR(TEXT("PingPong: Can't do the DMA frame transfer for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						Thread_PushAvailableReadingFrame(AvailableReadingFrame);
						break;
					}
				...
				{
					LOG_WARNING(TEXT("PingPong: No frames are available for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				}
			}
		...
		void OutputChannelThread::Thread_TestInterlacedOutput(Frame* InFrame)
		{
			...
			{
				LOG_ERROR(TEXT("INTERLACED TEST - The 2 lines are the same color. %d\n"), InterlacedTest_FrameCounter);
			}
			if (bIsFirstLineWhite != bInterlacedTest_ExpectFirstLineToBeWhite)
			{
				...
				LOG_ERROR(TEXT("INTERLACED TEST - The lines has swap color. %d\n"), InterlacedTest_FrameCounter);
			}
			++InterlacedTest_FrameCounter;
		}
	}
