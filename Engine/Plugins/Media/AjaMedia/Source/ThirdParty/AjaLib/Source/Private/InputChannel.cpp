	...
	namespace Private
	{
		...
		*****************************************************************************/
		...
		bool InputChannelThread::DeviceThread_ConfigureAutoCirculate(DeviceConnection::CommandList& InCommandList)
		{
			...
			{
				LOG_ERROR(TEXT("AutoCirculate: The device '%S' couldn't not capture."), GetDevice().GetDisplayName().c_str());
				return false;
			}
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bRunning)
				{
					LOG_ERROR(TEXT("AutoCirculate: Can't get the status for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					bRunning = false;
				}
				...
				{
					LOG_ERROR(TEXT("AutoCirculate: Can't get the Channel running for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					bRunning = false;
				}
		...
		void InputChannelThread::Thread_AutoCirculateLoop()
		{
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (ChannelStatus.IsRunning() && ChannelStatus.HasAvailableInputFrame())
				{
					...
					if (!bRunning)
					{
						LOG_ERROR(TEXT("AutoCirculate: Can't transfer the buffer for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						break;
					}
					...
					if (!bWaitResult)
					{
						...
						{
							LOG_WARNING(TEXT("AutoCirculate: The device '%S' missed an interrupt signal."), GetDevice().GetDisplayName().c_str());
						}
					}
				}
				else
		...
		bool InputChannelThread::DeviceThread_ConfigurePingPong(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (!::NTV2DeviceCanDoCapture(DeviceId))
			{
				LOG_ERROR(TEXT("PingPong: The device '%S' couldn't not capture."), GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		void InputChannelThread::Thread_PingPongLoop()
		{
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bWaitResult)
				{
					...
					{
						LOG_WARNING(TEXT("PingPong: The device '%S' missed an interrupt signal."), GetDevice().GetDisplayName().c_str());
						::Sleep(0);
						continue;
					}
				}
				...
				if (!bRunning)
				{
					LOG_ERROR(TEXT("PingPong: Can't wait for the input field for channel %d on device %S.\n"), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					break;
				}
		...
		InputChannelThread::EVerifyFormatResult InputChannelThread::VerifyFormat()
		{
			...
			{ 
				...
				while (Counter * SleepMilli < TimeoutMilli)
				{
					...
					if (Counter * SleepMilli > TimeoutMilli)
					{
						LOG_ERROR(TEXT("%S: Can't get the Channel running for channel %d on device %S.\n"), *ModeName, uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						break;
					}
				...
				if (!FoundFormat.has_value())
				{
					...
					if (!bAutoDetectFormat)
					{
						LOG_ERROR(TEXT("%S: Could not detect format for channel %d on device %S. %S\n")
							, *ModeName
							, uint32_t(Channel) + 1
			...
			if (!Helpers::CompareFormats(DesiredVideoFormat, FoundFormat.value(), FailureReason))
			{
				...
				{
					LOG_ERROR(TEXT("%S: The VideoFormat changed for channel %d on device %S. %S\n")
						, *ModeName
						, uint32_t(Channel) + 1
						, GetDevice().GetDisplayName().c_str()
						, FailureReason.c_str()
