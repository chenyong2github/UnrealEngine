		...
		*****************************************************************************/
		...
		bool TimecodeChannelImpl::Thread_Configure(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (TimecodeOption.bUseDedicatedPin)
			{
				...
				default:
					LOG_ERROR(TEXT("TimecodeChannel: The LTC source Index is invalid on device %S.\n"), GetDevice().GetDisplayName().c_str());
					return false;
				}
			...
			else
			{
				...
				{
					LOG_ERROR(TEXT("TimecodeChannel: The expected video format is invalid for %d.\n"), uint32_t(Channel) + 1);
					return false;
				}
				...
				{
					LOG_ERROR(TEXT("TimecodeChannel: Was not able to lock on device %S for channel %d.\n"), GetDevice().GetDisplayName().c_str(), uint32_t(Channel) + 1);
					return false;
				}
			}
