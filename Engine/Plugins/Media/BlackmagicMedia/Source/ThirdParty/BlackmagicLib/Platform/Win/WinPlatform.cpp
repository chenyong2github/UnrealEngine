...
namespace BlackmagicPlatform
{
	...
	bool InitializeAPI()
	{
		...
		bool bResult = !FAILED(Result);
		if (!bResult)
		{
			LOG_ERROR(TEXT("Initialization of Blackmagic COM failed - %08x."), Result);
		}
		return bResult;
	}
	...
	IDeckLinkIterator* CreateDeckLinkIterator()
	{
		...
		if (FAILED(Result))
		{
			LOG_ERROR(TEXT("A DeckLink iterator could not be created - %08x. The DeckLink drivers may not be installed."), Result);
	...
	IDeckLinkVideoConversion* CreateDeckLinkVideoConversion()
	{
		...
		if (FAILED(Result))
		{
			LOG_ERROR(TEXT("A DeckLink video conversion could not be created - %08x. The DeckLink drivers may not be installed."), Result);
