	...
	namespace Private
	{
		...
		HRESULT FInputChannelNotificationCallback::VideoInputFormatChanged(BMDVideoInputFormatChangedEvents InNotificationEvents, IDeckLinkDisplayMode* InNewDisplayMode, BMDDetectedVideoInputFormatFlags InDetectedSignalFlags)
		{
			...
			if ((InNotificationEvents & bmdVideoInputDisplayModeChanged) != 0)
			{
				...
				if (InNewDisplayMode->GetDisplayMode() != InputChannel->DisplayMode)
				{
					...
					LOG_INFO(TEXT("The video input format changed on the device '%d'. The new display mode is '%S'"), InputChannel->ChannelInfo.DeviceIndex, DisplayModeName.c_str());
			...
			if ((InNotificationEvents & bmdVideoInputColorspaceChanged) != 0)
			{
				...
				if ((InDetectedSignalFlags & bmdDetectedVideoInputYCbCr422) == 0)
				{
					...
					{
						LOG_INFO(TEXT("The color space have changed on the device '%d'. Only YUV is supported."), InputChannel->ChannelInfo.DeviceIndex);

						if (InDetectedSignalFlags == bmdDetectedVideoInputYCbCr422)
						{
							LOG_INFO(TEXT("VideoInputYCbCr422"));
						}
						if (InDetectedSignalFlags == bmdDetectedVideoInputRGB444)
						{
							LOG_INFO(TEXT("VideoInputRGB444"));
						}
					}
				}
			}
			...
			if ((InNotificationEvents & bmdVideoInputFieldDominanceChanged) != 0)
			{
				...
				{
					LOG_INFO(TEXT("The FieldDominance have changed on the device '%d'."), InputChannel->ChannelInfo.DeviceIndex);
					bProcessFormatChanged = true;
				}
			}
		...
		HRESULT FInputChannelNotificationCallback::VideoInputFrameArrived(IDeckLinkVideoInputFrame* InVideoFrame, IDeckLinkAudioInputPacket* InAudioPacket)
		{
			...
			if (Helpers::IsInterlacedDisplayMode(InputChannel->DisplayMode) && FrameInfo.bHasInputSource)
			{
				...
				{
					LOG_WARNING(TEXT("Video frame has arrived faster than expected on device '%d'"), InputChannel->ChannelInfo.DeviceIndex);
				}
			}
		...
		bool FInputChannel::InitializeInterfaces()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not obtain the IDeckLinkInput interface for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not obtain the IDeckLinkStatus interface for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FInputChannel::VerifyFormatSupport()
		{
			...
			if (!bSupported)
			{
				LOG_ERROR(TEXT("The desired video mode '%S' is not supported on device '%d'."), BlackmagicPlatform::GetName(DeckLinkDisplayMode.Get()).c_str(), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK || Denominator == 0)
			{
				LOG_ERROR(TEXT("Could not get a valid frame rate for input on device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FInputChannel::VerifyDeviceUsage()
		{
			...
			{
				LOG_ERROR(TEXT("Device %d can't be used because it's already being used as an input."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (DuplexStatus == ENUM(BMDDuplexMode)::bmdDuplexHalf)
			{
				...
				{
					LOG_ERROR(TEXT("Device %d can't be used because it's already being used as an output and it can't do input at the same time."), ChannelInfo.DeviceIndex);
					return false;
				}
			}
		...
		bool FInputChannel::InitializeAudio()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not enable the audio input on device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
...
;
			...
			{
				LOG_ERROR(TEXT("The device '%d' doesn't have a dedicated LTC input - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FInputChannel::InitializeStream()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not enable video input for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not flush the streams video for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not start the streams video for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
			bStreamStarted = true;
		...
		bool FInputChannel::InitializeListener_Helper(const FListener InListener)
		{
			...
			if (bNeedToPauseStream)
			{
				...
				if (Result != S_OK)
				{
					LOG_ERROR(TEXT("Could not pause the the streams video for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
					return false;
				}
			}
			...
			if (bResult && InListener.Option.bReadVideo)
			{
				if (ReadVideoCounter == 0)
				{
					...
					{
						LOG_ERROR(TEXT("Could not find the pixel format '%d'."), (int32_t)InListener.Option.PixelFormat);
						bResult = false;
					}
