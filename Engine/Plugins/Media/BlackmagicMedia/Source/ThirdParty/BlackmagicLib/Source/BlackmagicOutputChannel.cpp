	...
	namespace Private
	{
		...
		*****************************************************************************/
		...
		bool FOutputChannel::Initialize()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Couldn't get the attribute flags for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}

			if ((VideoCapabilities & bmdDeviceSupportsPlayback) == 0)
			{
				LOG_ERROR(TEXT("Device '%d' doesn't support playback mode."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Could not create output buffers for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Could not find the pixel format '%d'."), (int32_t)ChannelOptions.PixelFormat);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Could not create notification callback object for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not set callback for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FOutputChannel::InitializeDeckLink()
		{
			...
			if (DeckLink == nullptr)
			{
				LOG_ERROR(TEXT("Could not find the device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::InitializeInterfaces()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not obtain the IDeckLinkAttributes interface for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not obtain the IDeckLinkOutput interface for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not obtain the IDeckLinkConfiguration interface for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FOutputChannel::InitializeLinkConfiguration()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not change device '%d' link mode."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Configuration == bmdLinkConfigurationQuadLink)
			{
				...
				{
					LOG_ERROR(TEXT("Could not change device '%d' Quad-link output. It is not supported by the device."), ChannelInfo.DeviceIndex);
					return false;
				}
				else if (Result == E_FAIL && Result == E_INVALIDARG)
				{
					LOG_ERROR(TEXT("Could not change device %d Quad-link output to Square Division Quad Split mode."), ChannelInfo.DeviceIndex);
					return false;
				}
			}
		...
		void FOutputChannel::OnScheduledFrameCompleted(IDeckLinkVideoFrame* InCompletedFrame, BMDOutputFrameCompletionResult InResult)
		{
...

			{
				...
				LOG_VERBOSE(TEXT("\tOnScheduledFrame %d, %d:%d"), FoundFrame->FrameIdentifier, FoundFrame->Timecode.Seconds, FoundFrame->Timecode.Frames);

				CHECK(FoundFrame);
				if (FoundFrame->InFlightCount == 0)
				{
					LOG_ERROR(TEXT("No frame is currently in flight, verify that the engine framerate is high enough to support the output framerate."));
				}
				else
				{
				...
				if (InFlightFrames <= 0)
				{
					...
					LOG_VERBOSE(TEXT("\tResending last frame: %d"), LastScheduledFrame.load()->FrameIdentifier);
		...
		{
			{
				...
				LOG_WARNING(TEXT("RenderAudio, %d, NumSamples: %d"), millisec_since_epoch, BufferedSamples);
#endif
		...
		bool FOutputChannel::ScheduleOutputFrame(FOutputFrame* InFrame)
		{
			...
			LOG_ERROR(TEXT("Could not schedule frame for Output device %d"), ChannelInfo.DeviceIndex);
			return false;
		}
...

				{
					...
					if (OutputFrameToSchedule)
					{
						...
						if (ChannelOptions.bUseGPUDMA)
						{
							...
							TextureTransfer->BeginSync(DestinationBuffer, GPUTextureTransfer::ETransferDirection::GPU_TO_CPU);
						}
#endif //_WINDOWS
		...
		void FOutputChannel::AudioThread_ScheduleAudioSamples()
		{
			while (true)
			{
				...
				if (bIsPrerollingAudio && NumBufferedAudioSamples < AudioSamplesWaterLevel)
				{
					...
					{
						LOG_ERROR(TEXT("Could not preroll audio for Output device %d"), ChannelInfo.DeviceIndex);
					}
				}
...
#if AUDIO_DEBUGGING
						auto millisec_since_epoch = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
						LOG_WARNING(TEXT("(%d) Samples Buffered: %d, Expected: %d, Written: %d, PacketsToScheduleSize: %d"), millisec_since_epoch, NumBufferedAudioSamples.load(), PacketToSchedule->NumAudioSamples, SamplesWritten, AudioPacketsToSchedule.Size());
#endif
					}
				}
				...
				{
					LOG_ERROR(TEXT("Could not schedule audio for Output device %d"), ChannelInfo.DeviceIndex);
				}
			}
		}
		...
		bool FOutputChannel::VerifyFormatSupport()
		{
			...
			{
				LOG_ERROR(TEXT("Could not check if the video mode is supported on device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Could not get the display mode on device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Could not get a valid frame rate for output on device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (!bSupported)
			{
				LOG_ERROR(TEXT("The video format '%d' is not support on device '%d'."), DisplayMode, ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::VerifyIfDeviceUsed()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Couldn't get the DeckLinkStatus for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Couldn't get the Busy status for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Device %d can't be used because it's already being used as an output."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Couldn't get the DuplexMode status for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Device %d is inactive."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (DuplexStatus == ENUM(BMDDuplexMode)::bmdDuplexHalf)
			{
				...
				{
					LOG_ERROR(TEXT("Device %d can't be used because it's already being used as an input and it can't do output at the same time."), ChannelInfo.DeviceIndex);
					return false;
				}
			}
		...
		bool FOutputChannel::InitializeKeyer()
		{
			...
			if (ChannelOptions.bOutputKey)
			{
				if (!Helpers::Is8Bits(ChannelOptions.PixelFormat))
				{
					LOG_WARNING(TEXT("Can only key if the pixel format contains alpha."));
				}

				HRESULT Result = DeckLink->QueryInterface(IID_IDeckLinkKeyer, (void**)&Keyer);
				if (Result != S_OK)
				{
					LOG_ERROR(TEXT("Could not obtain the IDeckLinkKeyer interface for device '%d'."), ChannelInfo.DeviceIndex);
					return false;
				}
				...
				if (Result != S_OK)
				{
					LOG_ERROR(TEXT("Could not enable external Keyer for device '%d'. Does the device need special configuration in Blackmagic Desktop Video Setup utility?"), ChannelInfo.DeviceIndex);
					return false;
				}
				...
				if (Result != S_OK)
				{
					LOG_ERROR(TEXT("Could not set Keyer level for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
					return false;
				}
			}
		...
		bool FOutputChannel::InitializeAudio()
		{
			...
			{
				CHECK(false);
				LOG_ERROR(TEXT("Unsupported audio bit depth."));
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Could not enable audio output for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}

			if (FAILED(DeckLinkOutput->BeginAudioPreroll()))
			{
				LOG_ERROR(TEXT("Could not begin audio preroll for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::InitializeVideo()
		{
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not enable video output for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				LOG_ERROR(TEXT("Could not send preroll frames for device '%d'."), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				LOG_ERROR(TEXT("Could not start playback for device '%d' - result = %08x."), ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FOutputChannel::SendPreRollFrames()
		{
			...
			{
				LOG_ERROR(TEXT("Can't initiate PreRoll. Need at least 3 frames and only '%d' are available."), FrameReadyToWrite.size());
				return false;
			}
		...
		void FOutputChannel::EmbedTimecodeInFrame(FOutputFrame* InFrame)
		{
			...
			if (Result != S_OK)
			{
				LOG_WARNING(TEXT("Could not set Timecode on frame for Output device %d"), ChannelInfo.DeviceIndex);
			}
		}
		...
		FOutputChannel::~FOutputChannel()
		{
			...
			if (DeckLink)
			{
				...
				if (Result == S_OK)
				{
					...
					if ((BusyStatus & bmdDevicePlaybackBusy) != 0)
					{
						LOG_WARNING(TEXT("Could not stop capture activity on device '%d' while closing it."), ChannelInfo.DeviceIndex);
					}
				}
			}
		...
		bool FOutputChannel::SetVideoFrameData(const FFrameDescriptor& InFrame)
		{
			...
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video. The buffer is invalid or the buffer size is not the same as the Blackmagic Video Format for output channel %d.\n"), ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::SetVideoFrameData(FFrameDescriptor_GPUDMA& InFrame)
		{
			...
			if (!bIsProgressive)
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video, GPUTextureTransfer is not supported with interlaced video.\n"));
				return false;
			}

			if (InFrame.RHITexture == nullptr)
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video. The texture is invalid.\n"));
				return false;
			}

			if (!ChannelOptions.bUseGPUDMA || !TextureTransfer)
			{
				LOG_ERROR(TEXT("SetVideoFrameData: Can't set the video. GPU DMA was not setup correctly.\n"));
				return false;
			}
			...
			if (!bRegisteredDMABuffers)
			{
				for (FOutputFrame* Frame : AllFrames)
				{
					...
					{
						GPUTextureTransfer::FRegisterDMABufferArgs Args;
						Args.Height = VideoHeight;
						Args.Width = VideoWidth;
						Args.Stride = Stride;
						Args.Buffer = Buffer;
						Args.PixelFormat = ChannelOptions.PixelFormat == EPixelFormat::pf_8Bits ? GPUTextureTransfer::EPixelFormat::PF_8Bit : GPUTextureTransfer::EPixelFormat::PF_10Bit;
			...
			GPUTextureTransfer::ETransferDirection Direction = GPUTextureTransfer::ETransferDirection::GPU_TO_CPU;
		...
		bool FOutputChannel::SetAudioFrameData(const FAudioSamplesDescriptor& InSamples)
		{
			...
			{
				LOG_ERROR(TEXT("SetAudioFrameData: Can't set the audio. The buffer is invalid or the buffer size is not the same as the Blackmagic Audio Format for output channel %d.\n"), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (FAudioPacket* Packet = AudioPacketPool.Acquire())
			{
				...
				LOG_ERROR(TEXT("SetAudio (%d), (%d:%d) %d samples"), millisec_since_epoch, InSamples.Timecode.Seconds, InSamples.Timecode.Frames, Packet->NumAudioSamples);;
#endif
		...
		BlackmagicDesign::Private::FOutputChannel::FOutputFrame* FOutputChannel::FetchAvailableWritingFrameInterlaced(const FBaseFrameData& InFrame)
		{
			...
			if (!ChannelOptions.bOutputInterlacedFieldsTimecodeNeedToMatch)
			{
				if (!InFrame.bEvenFrame)
				{
					...
					{
						LOG_WARNING(TEXT("Could not find frame. id %d, total flight %d, available %d"), InFrame.FrameIdentifier, InFlightFrames.load(), FrameReadyToWrite.size());
					}
		...
		void FOutputChannel::PushWhenFrameReady(FOutputFrame* InFrame)
		{
			...
			{
				LOG_VERBOSE(TEXT("Pushing frame %d, FrameReadyToWriteSize = %d"), InFrame->FrameIdentifier, FrameReadyToWrite.size());
