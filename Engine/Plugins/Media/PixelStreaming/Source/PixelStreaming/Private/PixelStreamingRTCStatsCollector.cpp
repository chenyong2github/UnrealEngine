...
void FPixelStreamingRTCStatsCollector::AddRef() const
{
	FPlatformAtomics::InterlockedIncrement(&RefCount);
}

rtc::RefCountReleaseStatus FPixelStreamingRTCStatsCollector::Release() const
{
...
void FPixelStreamingRTCStatsCollector::OnStatsDelivered(const rtc::scoped_refptr<const webrtc::RTCStatsReport>& Report)
{
	FPixelStreamingStats* PSStats = FPixelStreamingStats::Get();
	...
	for (webrtc::RTCStatsReport::ConstIterator Iter = Report->begin(); Iter != Report->end(); ++Iter)
	{
		...
		// For debugging
		// UE_LOG(PixelStreamer, Log, TEXT("------------%s---%s------------"), *FString(Stats.id().c_str()), *FString(Stats.type()));
		// for (const webrtc::RTCStatsMemberInterface* StatMember : StatMembers)
		// {
		// 	UE_LOG(PixelStreamer, Log, TEXT("%s"), *FString(StatMember->name()));
		// }
