...
namespace UE::PixelStreaming
{
	...
	FPixelStreamingVideoInputBackBufferComposited::FPixelStreamingVideoInputBackBufferComposited()
	{
		...
		CompositedFrame = CreateRHITexture(DefaultSize.X, DefaultSize.Y);
	...
	void FPixelStreamingVideoInputBackBufferComposited::OnBackBufferReady(SWindow& SlateWindow, const FTexture2DRHIRef& FrameBuffer)
	{
		...
		FTextureRHIRef StagingTexture = CreateRHITexture(FrameBuffer->GetSizeXY().X, FrameBuffer->GetSizeXY().Y);
		// Re-render FrameBuffer to StagingTexture (ensure format match)
		CopyTexture(RHICmdList, FrameBuffer, StagingTexture, nullptr);
		...
		if (WindowsRendered == TopLevelWindows.Num())
		{
			...
			for (TSharedRef<SWindow> Window : TopLevelWindows)
			{
				...
				if (CompositedX < TotalX || CompositedY < TotalY)
				{
					...
					StagingTexture = CreateRHITexture(SizeX, SizeY);
	...
	TSharedPtr<FPixelCaptureCapturer> FPixelStreamingVideoInputBackBufferComposited::CreateCapturer(int32 FinalFormat, float FinalScale)
	{
		switch (FinalFormat)
		{
