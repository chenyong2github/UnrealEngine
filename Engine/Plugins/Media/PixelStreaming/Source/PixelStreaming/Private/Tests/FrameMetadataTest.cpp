...
namespace UE::PixelStreaming
{
	...
	bool FFrameMetadataTest::RunTest(const FString& Parameters)
	{
		...
		TestTrue("Layer initialized", Metadata.Layer == 0);
		TestTrue("SourceTime initialized", Metadata.SourceTime == 0);
		TestTrue("AdaptCallTime initialized", Metadata.AdaptCallTime == 0);
		TestTrue("AdaptProcessStartTime initialized", Metadata.AdaptProcessStartTime == 0);
		TestTrue("AdaptProcessFinalizeTime initialized", Metadata.AdaptProcessFinalizeTime == 0);
		TestTrue("AdaptProcessEndTime initialized", Metadata.AdaptProcessEndTime == 0);
		TestTrue("UseCount initialized", Metadata.UseCount == 0);
		TestTrue("FirstEncodeStartTime initialized", Metadata.FirstEncodeStartTime == 0);
		TestTrue("LastEncodeStartTime initialized", Metadata.LastEncodeStartTime == 0);
		TestTrue("LastEncodedEndTime initialized", Metadata.LastEncodeEndTime == 0);
