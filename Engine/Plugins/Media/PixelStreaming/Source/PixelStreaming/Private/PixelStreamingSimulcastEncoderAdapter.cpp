...
PixelStreamingSimulcastEncoderAdapter::PixelStreamingSimulcastEncoderAdapter(webrtc::VideoEncoderFactory* factory,
                                                 const webrtc::SdpVideoFormat& format)
    : PixelStreamingSimulcastEncoderAdapter(factory, nullptr, format) {}

PixelStreamingSimulcastEncoderAdapter::PixelStreamingSimulcastEncoderAdapter(
    webrtc::VideoEncoderFactory* primary_factory,
    ...
    : inited_(0),
      ...
      boost_base_layer_quality_(webrtc::RateControlSettings::ParseFromFieldTrials()
                                    .Vp8BoostBaseLayerQuality()),
      prefer_temporal_support_on_base_layer_(webrtc::field_trial::IsEnabled(
...
void PixelStreamingSimulcastEncoderAdapter::SetFecControllerOverride(
...
int PixelStreamingSimulcastEncoderAdapter::InitEncode(
  ...
  RTC_DCHECK_LE(number_of_streams, webrtc::kMaxSimulcastStreams);
  bool doing_simulcast_using_adapter = (number_of_streams > 1);
  int num_active_streams = NumActiveStreams(*inst);

  codec_ = *inst;
  webrtc::SimulcastRateAllocator rate_allocator(codec_);
...
int PixelStreamingSimulcastEncoderAdapter::Encode(
  ...
  rtc::scoped_refptr<webrtc::I420BufferInterface> src_buffer;
  ...
  for (size_t stream_idx = 0; stream_idx < streaminfos_.size(); ++stream_idx) {
    ...
    FPixelStreamingSimulcastFrameBuffer* SimulcastFrameBuffer = static_cast<FPixelStreamingSimulcastFrameBuffer*>(input_image.video_frame_buffer().get());
    FPixelStreamingLayerFrameSource* LayerFrameSource = SimulcastFrameBuffer->GetLayerFrameSource(stream_idx);
    rtc::scoped_refptr<FPixelStreamingLayerFrameBuffer> LayerFrameBuffer = new rtc::RefCountedObject<FPixelStreamingLayerFrameBuffer>(LayerFrameSource);
    webrtc::VideoFrame new_frame(input_image);
    new_frame.set_video_frame_buffer(LayerFrameBuffer);

    const uint32_t frame_timestamp_ms =
        1000 * new_frame.timestamp() / 90000;  // kVideoPayloadTypeFrequency;
    ...
    std::vector<webrtc::VideoFrameType> stream_frame_types(
    ...
    if ((dst_width == src_width && dst_height == src_height) ||
        (new_frame.video_frame_buffer()->type() ==
             webrtc::VideoFrameBuffer::Type::kNative &&
         streaminfos_[stream_idx]
             .encoder->GetEncoderInfo()
             .supports_native_handle)) {

      int ret = streaminfos_[stream_idx].encoder->Encode(new_frame,
                                                         &stream_frame_types);
      ...
      rtc::scoped_refptr<webrtc::I420Buffer> dst_buffer =
          webrtc::I420Buffer::Create(dst_width, dst_height);
      ...
      webrtc::VideoFrame frame(input_image);
...
int PixelStreamingSimulcastEncoderAdapter::RegisterEncodeCompleteCallback(
...
void PixelStreamingSimulcastEncoderAdapter::SetRates(
...
void PixelStreamingSimulcastEncoderAdapter::OnLossNotification(
...
void PixelStreamingSimulcastEncoderAdapter::PopulateStreamCodec(
    const webrtc::VideoCodec& inst,
    int stream_index,
  ...
  *stream_codec = inst;
  ...
  if (stream_resolution == StreamResolution::LOWEST) {
    ...
    if (inst.mode == webrtc::VideoCodecMode::kScreensharing) {
...
bool PixelStreamingSimulcastEncoderAdapter::Initialized() const {
  return rtc::AtomicOps::AcquireLoad(&inited_) == 1;
}

void PixelStreamingSimulcastEncoderAdapter::DestroyStoredEncoders() {
