	...
	SampleDuration(0)
{
	...
	FrameRate.Numerator = FrameRate.Denominator = 0;
}
...
ULONG WmfMediaHAPDecoder::AddRef()
{
	return FPlatformAtomics::InterlockedIncrement(&RefCount);
}


ULONG WmfMediaHAPDecoder::Release()
{
...
HRESULT WmfMediaHAPDecoder::GetStreamLimits(DWORD* pdwInputMinimum, DWORD* pdwInputMaximum, DWORD* pdwOutputMinimum, DWORD* pdwOutputMaximum)
{
...
HRESULT WmfMediaHAPDecoder::GetStreamCount(DWORD* pcInputStreams, DWORD* pcOutputStreams)
{
...
HRESULT WmfMediaHAPDecoder::GetStreamIDs(DWORD /*dwInputIDArraySize*/, DWORD* /*pdwInputIDs*/, DWORD /*dwOutputIDArraySize*/, DWORD* /*pdwOutputIDs*/)
{
	return E_NOTIMPL;   
}


HRESULT WmfMediaHAPDecoder::GetInputStreamInfo(DWORD dwInputStreamID, MFT_INPUT_STREAM_INFO* pStreamInfo)
{
	if (pStreamInfo == NULL)
	{
		return E_POINTER;
...
HRESULT WmfMediaHAPDecoder::GetOutputStreamInfo(DWORD dwOutputStreamID, MFT_OUTPUT_STREAM_INFO* pStreamInfo)
{
	if (pStreamInfo == NULL)
	{
		return E_POINTER;
...
HRESULT WmfMediaHAPDecoder::GetInputStreamAttributes(DWORD /*dwInputStreamID*/, IMFAttributes** /*ppAttributes*/)
{
	return E_NOTIMPL;
}


HRESULT WmfMediaHAPDecoder::GetOutputStreamAttributes(DWORD /*dwOutputStreamID*/, IMFAttributes** /*ppAttributes*/)
{
	return E_NOTIMPL;
}


HRESULT WmfMediaHAPDecoder::DeleteInputStream(DWORD /*dwStreamID*/)
{
	return E_NOTIMPL; 
}


HRESULT WmfMediaHAPDecoder::AddInputStreams(DWORD /*cStreams*/, DWORD* /*adwStreamIDs*/)
{
	return E_NOTIMPL; 
}


HRESULT WmfMediaHAPDecoder::GetInputAvailableType(DWORD /*dwInputStreamID*/, DWORD /*dwTypeIndex*/, IMFMediaType** /*ppType*/)
{
...
HRESULT WmfMediaHAPDecoder::SetInputType(DWORD dwInputStreamID, IMFMediaType* pType, DWORD dwFlags)
{
	if (dwInputStreamID != 0)
	{
		return MF_E_INVALIDSTREAMNUMBER;
...
HRESULT WmfMediaHAPDecoder::SetOutputType(DWORD dwOutputStreamID, IMFMediaType* pType, DWORD dwFlags)
{
	if (dwOutputStreamID != 0)
	{
		return MF_E_INVALIDSTREAMNUMBER;
...
HRESULT WmfMediaHAPDecoder::GetInputCurrentType(DWORD dwInputStreamID, IMFMediaType** ppType)
{
	if (ppType == NULL)
	{
		return E_POINTER;
...
HRESULT WmfMediaHAPDecoder::GetOutputCurrentType(DWORD dwOutputStreamID, IMFMediaType** ppType)
{
	if (ppType == NULL)
	{
		return E_POINTER;
...
HRESULT WmfMediaHAPDecoder::GetInputStatus(DWORD dwInputStreamID, DWORD* pdwFlags)
{
	if (pdwFlags == NULL)
	{
		return E_POINTER;
...
HRESULT WmfMediaHAPDecoder::GetOutputStatus(DWORD* pdwFlags)
{
	if (pdwFlags == NULL)
	{
		return E_POINTER;
...
HRESULT WmfMediaHAPDecoder::SetOutputBounds(LONGLONG /*hnsLowerBound*/, LONGLONG /*hnsUpperBound*/)
{
	return E_NOTIMPL;
}


HRESULT WmfMediaHAPDecoder::ProcessEvent(DWORD /*dwInputStreamID*/, IMFMediaEvent* pEvent)
{
			...
			if (DXGIManager.IsValid())
			{
				...
				if (DXGIManager->OpenDeviceHandle(&DeviceHandle) == S_OK)
				{
					...
					if (D3D11Device.IsValid())
					{
						...
						{
							UE_LOG(LogWmfMedia, Verbose, TEXT("D3D11Device from Device manager: %p"), D3D11Device.Get());
							if (InitPipeline())
							{
								UE_LOG(LogWmfMedia, Error, TEXT("Unable to initialize 3D pipeline for Hap Decoding"));
								hr = E_NOTIMPL;
							}
						}
					}
...
HRESULT WmfMediaHAPDecoder::ProcessInput(DWORD dwInputStreamID, IMFSample* pSample, DWORD dwFlags)
{
	if (pSample == NULL)
	{
		return E_POINTER;
...
HRESULT WmfMediaHAPDecoder::OnCheckOutputType(IMFMediaType* InMediaType)
{
	if (OutputType)
	{
		DWORD dwFlags = 0;
...
HRESULT WmfMediaHAPDecoder::OnSetOutputType(IMFMediaType* InMediaType)
{
...
HRESULT WmfMediaHAPDecoder::OnDiscontinuity()
{
	InternalTimeStamp = 0;
	EmplyQueues();
	return S_OK;
...
HRESULT WmfMediaHAPDecoder::OnFlush()
{
	OnDiscontinuity();
	return S_OK;
}
...
void WmfMediaHAPDecoder::EmplyQueues()
{
	InputQueue.Empty();
	OutputQueue.Empty();
}
...
bool WmfMediaHAPDecoder::InitPipeline()
{
	...
	if (ErrorMsgs)
	{
		...
		UE_LOG(LogWmfMedia, Error, TEXT("D3DCompile Error/warning: %s"), *FString(pMessage));
		if (!PSCode.IsValid())
		{
			return true;
		}
