...
void FWebAPICodeGenSettings::GetModuleDependencies(TSet<FString>& OutModules) const
{
	...
	if(ParentType.IsValid() && UE::WebAPI::FWebAPIEditorUtilities::Get()->GetModulesForClass(ParentType.Get(), BaseClassModuleDependencies))
	{
...
void FWebAPICodeGenSettings::GetIncludePaths(TArray<FString>& OutIncludePaths) const
{
	...
	if(ParentType.IsValid() && UE::WebAPI::FWebAPIEditorUtilities::Get()->GetHeadersForClass(ParentType.Get(), BaseClassHeaders))
	{
