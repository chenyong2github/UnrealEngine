...
const TObjectPtr<UWebAPITypeInfo>& UWebAPITypeInfo::Duplicate(const TObjectPtr<UWebAPITypeRegistry>& InTypeRegistry) const
{
	check(InTypeRegistry);
	UWebAPITypeInfo* DuplicatedObject = DuplicateObject(this, InTypeRegistry.Get());
	const TObjectPtr<UWebAPITypeInfo>& DuplicatedObjectRef = InTypeRegistry->GetOrMakeGeneratedType(DuplicatedObject->SchemaType, DuplicatedObject->Name, DuplicatedObject->JsonName, DuplicatedObject);
	if(DuplicatedObjectRef->DebugString.IsEmpty())
	{
		DuplicatedObjectRef->DebugString +=	TEXT("Duplicated from another type.");
	}
	return DuplicatedObjectRef;
}
