...
const TObjectPtr<UWebAPITypeInfo>& UWebAPITypeRegistry::GetOrMakeGeneratedType(
...
const TObjectPtr<UWebAPITypeInfo>& UWebAPITypeRegistry::GetOrMakeGeneratedType(
	...
	const FString& InName,
	const FString& InJsonName,
	const TObjectPtr<UWebAPITypeInfo>& InTemplateTypeInfo)
{
	ensureAlways(!InName.IsEmpty());
	...
	const FName TypeName = UE::WebAPI::MakeTypeInfoName(this, InSchemaType, InName);
	UWebAPITypeInfo* TypeInfo = DuplicateObject(InTemplateTypeInfo, this, TypeName);
...
FName UE::WebAPI::MakeTypeInfoName(UObject* InOuter, const EWebAPISchemaType& InSchemaType, const FString& InName)
{
	...
	{
		return MakeUniqueObjectName(InOuter, UWebAPITypeInfo::StaticClass(), FName(FString::Format(TEXT("{ClassName}_{TypeName}_"), NameArgs)));
	}

	return MakeUniqueObjectName(InOuter, UWebAPITypeInfo::StaticClass(), FName(FString::Format(TEXT("{ClassName}_{SchemaType}_{TypeName}_"), NameArgs)));
}
