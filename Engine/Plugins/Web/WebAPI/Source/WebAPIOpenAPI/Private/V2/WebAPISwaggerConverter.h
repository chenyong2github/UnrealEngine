	...
	namespace WebAPI
	{
		namespace Swagger
		{
			...
			public:
				...
				template <>
				TSharedPtr<OpenAPI::V2::FSchema> ResolveReference(const FString& InDefinitionName);

				template <>
				TSharedPtr<OpenAPI::V2::FParameter> ResolveReference(const FString& InDefinitionName);
...
*/
				...
				/** Modifies an existing Model using the supplied source object. */
				template <>
				bool PatchModel(const TSharedPtr<OpenAPI::V2::FParameter>& InSrcSchema, const FWebAPITypeNameVariant& InModelTypeName, const TObjectPtr<UWebAPIModel>& OutModel);
