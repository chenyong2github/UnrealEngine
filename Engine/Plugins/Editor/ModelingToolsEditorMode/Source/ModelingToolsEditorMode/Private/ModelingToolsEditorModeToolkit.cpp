...
TSharedPtr<SWidget> FModelingToolsEditorModeToolkit::MakeAssetConfigPanel()
{
	...
	{
		AutogenSubfolder = SNew(SEditableTextBox)
		.Text(this, &FModelingToolsEditorModeToolkit::GetRestrictiveModeAutogenSubfolderText)
		.OnTextCommitted(this, &FModelingToolsEditorModeToolkit::OnRestrictiveModeAutogenSubfolderTextCommitted);
		...
		Content->AddSlot()[
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("AutogenSubfolder", "Autogen Subfolder"))
				.ToolTipText(LOCTEXT("AutogenSubfolderToolTip", "Override for the name of the subfolder in which new assets get stored in. "
					"If nothing is set, the username is used as the subfolder name."))
			]
			+ SHorizontalBox::Slot().HAlign(HAlign_Fill).Padding(0).FillWidth(1.0f)
			[
				AutogenSubfolder->AsShared()
			]
		];
	}
	else
...
FText FModelingToolsEditorModeToolkit::GetRestrictiveModeAutogenSubfolderText() const
{
	const UModelingToolsEditorModeSettings* Settings = GetDefault<UModelingToolsEditorModeSettings>();
	return FText::FromString(Settings->GetRestrictiveModeAutogenSubfolder());
}

void FModelingToolsEditorModeToolkit::OnRestrictiveModeAutogenSubfolderTextCommitted(const FText& InNewText, ETextCommit::Type InTextCommit) const
{
	UModelingToolsEditorModeSettings* Settings = GetMutableDefault<UModelingToolsEditorModeSettings>();
	Settings->SetRestrictiveModeAutogenSubfolder(InNewText.ToString());
}
