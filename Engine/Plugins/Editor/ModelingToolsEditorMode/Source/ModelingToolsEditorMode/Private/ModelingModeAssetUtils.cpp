...
FString UE::Modeling::GetNewAssetPathName(const FString& BaseNameIn, const UWorld* TargetWorld, FString SuggestedFolder)
{
	...
	else
	{
		...
		{
			const FString AutogenSubfolder = Settings->GetRestrictiveModeAutogenSubfolder().TrimStartAndEnd();
			if (!AutogenSubfolder.IsEmpty())
			{
				PackageFolderPath = FPaths::Combine(PackageFolderPath, AutogenSubfolder);
			}
			else
			{
