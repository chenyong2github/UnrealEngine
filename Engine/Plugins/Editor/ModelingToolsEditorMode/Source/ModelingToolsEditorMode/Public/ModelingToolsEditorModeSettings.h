...
public:
	...
	bool GetEnableDynamicMeshActors() const
	{
	...
	virtual FString GetRestrictiveModeAutogenSubfolder() const
	{
		return bRestrictiveMode ? RestrictiveModeAutogenSubfolder : FString();
	}

	virtual bool SetRestrictiveModeAutogenSubfolder(const FString& AutogenSubfolder)
	{
		if (bRestrictiveMode)
		{
			RestrictiveModeAutogenSubfolder = AutogenSubfolder;
			return true;
		}
		return false;
	}
...
protected:
	...
	/** Override for the name of the subfolder in which new assets get stored in when in restrictive mode. */
	UPROPERTY(Config)
	FString RestrictiveModeAutogenSubfolder;
