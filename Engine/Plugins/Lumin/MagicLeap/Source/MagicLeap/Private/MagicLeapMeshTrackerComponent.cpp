...
#if WITH_MLSDK
	MLMeshingSettings CreateSettings(const UMagicLeapMeshTrackerComponent& MeshTrackerComponent)
	{
...
public:
	...
	bool Create(const UMagicLeapMeshTrackerComponent& MeshTrackerComponent)
	{
#if WITH_MLSDK
	...
	bool Update(const UMagicLeapMeshTrackerComponent& MeshTrackerComponent)
	{
#if WITH_MLSDK
	...
	, PreviousWorldScale(100.0f)
{
	...
	if (GIsEditor)
	{
		FEditorDelegates::PrePIEEnded.AddUObject(this, &UMagicLeapMeshTrackerComponent::PrePIEEnded);
	}
#endif
}
...
void UMagicLeapMeshTrackerComponent::ConnectMRMesh(UMRMeshComponent* InMRMeshPtr)
{
	...
	{
		UE_LOG(LogMagicLeap, Warning, 
			TEXT("MRMesh is already connected to a UMagicLeapMeshTrackerComponent. Ignoring this connect."));
		return;
	}
	else
	{
...
#if WITH_EDITOR
void UMagicLeapMeshTrackerComponent::PostEditChangeProperty(FPropertyChangedEvent& e)
{
#if WITH_MLSDK
...
void UMagicLeapMeshTrackerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
		...
		if (BricksPerFrame > 0)
		{
			...
			for (numBricksProcessed = 0; numBricksProcessed < BricksPerFrame && PendingIter; ++numBricksProcessed)
			{
				...
				if (CachedMeshData->WorldVertices.Num() > 0)
				{
					UE_LOG(LogMagicLeap,
						Log,
						TEXT("UMagicLeapMeshTrackerComponent: ADDING/UPDATING brick %s"),
				...
				else
				{
					UE_LOG(LogMagicLeap,
						Log,
						TEXT("UMagicLeapMeshTrackerComponent: REMOVING brick %s"),
						*(CachedMeshData->BlockID.ToString()));
				}
...
void UMagicLeapMeshTrackerComponent::ConnectBlockSelector(TScriptInterface<IMagicLeapMeshBlockSelectorInterface> Selector)
{
	if (Impl != nullptr)
	{
		...
		{
			UE_LOG(LogMagicLeap, Warning, TEXT("Invalid selector passed to UMagicLeapMeshTrackerComponent::ConnectBlockSelector(). Using default block selector from MeshTrackerComponent."));
			Impl->BlockSelector = this;	
		}
	}
}
...
void UMagicLeapMeshTrackerComponent::BeginDestroy()
{
	if (MRMesh != nullptr)
	{
...
void UMagicLeapMeshTrackerComponent::FinishDestroy()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
...
void UMagicLeapMeshTrackerComponent::RequestMeshInfo()
{
#if WITH_MLSDK
...
bool UMagicLeapMeshTrackerComponent::GetMeshInfoResult()
{
#if WITH_MLSDK
	const FAppFramework& AppFramework = static_cast<FMagicLeapHMD *>(GEngine->XRSystem->GetHMDDevice())->GetAppFrameworkConst();
	const float WorldToMetersScale = AppFramework.GetWorldToMetersScale();
				...
				switch (MeshInfoData.state)
				{
					...
					case MLMeshingMeshState_Updated:
					{
						...
						if (MeshInfoData.state == MLMeshingMeshState_Unchanged)
						{
							...
							UE_LOG(LogMagicLeap,
								Log,
								TEXT("UMagicLeapMeshTrackerComponent: Received 'unchanged' event for unseen block %s"),
								*(BlockID.ToString()));
#endif
						}
					...
					case MLMeshingMeshState_Deleted:
					{
						...
						else
						{
							...
							UE_LOG(LogMagicLeap,
								Log,
								TEXT("UMagicLeapMeshTrackerComponent: Received 'deleted' event for unseen block %s"),
								*(BlockID.ToString()));
#endif
						}
						break;
...
void UMagicLeapMeshTrackerComponent::RequestMesh()
{
#if WITH_MLSDK
...
bool UMagicLeapMeshTrackerComponent::GetMeshResult()
{
#if WITH_MLSDK
	const FAppFramework& AppFramework = static_cast<FMagicLeapHMD *>(GEngine->XRSystem->GetHMDDevice())->GetAppFrameworkConst();
	const float WorldToMetersScale = AppFramework.GetWorldToMetersScale();
		...
		else
		{
			...
			for (uint32_t MeshIndex = 0; MeshIndex < Mesh.data_count; ++ MeshIndex)
			{
				...
				if (pendingBlockInfo == nullptr)
				{
					...
					UE_LOG(LogMagicLeap,
						Log,
						TEXT("UMagicLeapMeshTrackerComponent: Received unrequested mesh with ID %s"),
						*(BlockID.ToString()));
#endif
					continue;
				}
			...
			if (Impl->LatestMeshInfo.BlockInfoByGuid.Num() != 0)
			{
				UE_LOG(LogMagicLeap,
					Log,
					TEXT("UMagicLeapMeshTrackerComponent: Failed to receive meshes for %d requests:"),
					Impl->LatestMeshInfo.BlockInfoByGuid.Num());
				for (const auto& BlockInfo : Impl->LatestMeshInfo.BlockInfoByGuid)
				{
					UE_LOG(LogMagicLeap,
						Log,
						TEXT("UMagicLeapMeshTrackerComponent: No data received for %s"),
...
#if WITH_EDITOR
void UMagicLeapMeshTrackerComponent::PrePIEEnded(bool bWasSimulatingInEditor)
{
	Impl->Destroy();
}
#endif
