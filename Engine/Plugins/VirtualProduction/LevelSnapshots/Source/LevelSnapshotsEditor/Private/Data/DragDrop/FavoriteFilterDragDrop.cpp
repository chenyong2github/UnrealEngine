...
FFavoriteFilterDragDrop::FFavoriteFilterDragDrop(const TSubclassOf<ULevelSnapshotFilter>& InClassToInstantiate, const TSharedRef<FLevelSnapshotsEditorFilters> InSelectedFilterSetter)
	:
...
bool FFavoriteFilterDragDrop::OnDropOnRow(const TSharedRef<SLevelSnapshotsEditorFilterRow>& DroppedOnRow)
{
	...
	SelectedFilterSetter->SetActiveFilter(CreatedFilter->GetChildFilter());
	return true;
}
...
void FFavoriteFilterDragDrop::ShowCannotDropFilter()
{
	...
	ShowFeedbackMessage(StatusSymbol, LOCTEXT("CannotDropFilter", "Drop onto a condition to create filter"));
}
