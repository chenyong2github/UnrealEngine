...
UWorld* SLevelSnapshotsEditorContextPicker::GetEditorWorld()
{
...
{
	if (const TSharedPtr<FLevelSnapshotsEditorViewBuilder> Builder = BuilderPtr.Pin())
	{
...
{
	const bool bDoesAssetPointToEditorWorld = AssetPath == FSoftObjectPath(GetEditorWorld());
...
void SLevelSnapshotsEditorContextPicker::SetSelectedWorld(const FSoftObjectPath& SelectedWorld)
{
	...
	if (SelectedWorld == GetEditorWorld())
	{
		if (const TSharedPtr<FLevelSnapshotsEditorViewBuilder> Builder = BuilderPtr.Pin())
		{
			const TWeakObjectPtr<ULevelSnapshotsEditorData> EditorData = Builder->EditorDataPtr;
