...
public:
 ...
 // --------------------------------------------------------------------------------------------------------------------
IMPLEMENT_GLOBAL_SHADER(FCalibratedMapDerivedDataCS, "/Plugin/CameraCalibration/Private/DistortionSTMapProcessor.usf", "DistortionSTMapProcessorMainCS", SF_Compute);
...
void FCalibratedMapProcessor::Update_RenderThread()
{
	...
	for(TSharedPtr<FDerivedDistortionDataJob>& CompletedJob : ToComplete)
	{
		...
		{
			UE_LOG(LogCameraCalibration, Error, TEXT("Failed to retrieve edge points distorted UVs"));
