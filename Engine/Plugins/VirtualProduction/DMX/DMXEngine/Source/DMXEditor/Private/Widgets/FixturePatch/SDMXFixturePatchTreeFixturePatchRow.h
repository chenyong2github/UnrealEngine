...
/** A fixture patch row in a fixture patch tree */
...
public:
	SLATE_BEGIN_ARGS(SDMXFixturePatchTreeFixturePatchRow)
	{}
	...
	void Construct(const FArguments& InArgs, TSharedPtr<FDMXEntityTreeEntityNode> InEntityNode, TSharedPtr<STableViewBase> InOwnerTableView, TWeakPtr<SDMXFixturePatchTree> InFixturePatchTree);
...
private:
	...
	FText GetFixturePatchDisplayText() const;

	/** Returns the text of the starting channel */
	FText GetStartingChannelText() const;

	/** Returns the text of the ending channel */
	FText GetEndingChannelText() const;
	...
	/** The fixture patch tree widget that owns this row */
	TWeakPtr<SDMXFixturePatchTree> WeakFixturePatchTree;

	/** The tooltip of the status icon */
	...
	/** The editable text block to display and change the name of the patch */
	TSharedPtr<SInlineEditableTextBlock> InlineEditableFixturePatchNameWidget;
};
