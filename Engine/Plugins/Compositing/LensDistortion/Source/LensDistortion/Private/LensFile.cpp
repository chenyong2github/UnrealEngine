...
{	
	...
	{
		UE_LOG(LogLensDistortion, Warning, TEXT("Can't evaluate LensFile '%s' - Invalid Lens Handler"), *GetName());
		return false;
	}
	
	if (LensHandler->GetUVDisplacementMap() == nullptr)
	{
		UE_LOG(LogLensDistortion, Warning, TEXT("Can't evaluate LensFile '%s' - Invalid displacement map in LensHandler '%s'"), *GetName(), *LensHandler->GetName());
		return false;
	}

	if (LensInfo.LensModel == nullptr)
	{
		UE_LOG(LogLensDistortion, Warning, TEXT("Can't evaluate LensFile '%s' - Invalid Lens Model"), *GetName());
	...
	{
		UE_LOG(LogLensDistortion, Warning, TEXT("Can't evaluate LensFile '%s' - LensHandler '%s' doesn't support lens model '%s'"), *GetName(), *LensHandler->GetName(), *LensInfo.LensModel.GetDefaultObject()->GetModelName().ToString());
		SetupNoDistortionOutput(LensHandler, OutDistortionData);
		return false;
	}
...
bool ULensFile::EvaluateDistortionForParameters(float InFocus, float InZoom, ULensDistortionModelHandlerBase* LensHandler, FDistortionData& OutDistortionData)
{
	...
	else //DisplacementMap blending
	{
		...
		{
			UE_LOG(LogLensDistortion, Verbose, TEXT("Could not find distortion data for Focus = '%0.2f' and Zoom = '%0.2f' on LensFile '%s'"), InFocus, InZoom, *GetName());
			SetupNoDistortionOutput(LensHandler, OutDistortionData);
			return false;
		}
	}
...
bool ULensFile::EvaluteDistortionForSTMaps(float InFocus, float InZoom, ULensDistortionModelHandlerBase* LensHandler, FDistortionData& OutDistortionData)
{
	...
	{
		UE_LOG(LogLensDistortion, Verbose, TEXT("Can't evaluate LensFile '%s' - %d data points still being computed. Clearing render target for no distortion"), *GetName(), DerivedDataInFlightCount);
		SetupNoDistortionOutput(LensHandler, OutDistortionData);
		return true;
	}
	...
	{
		UE_LOG(LogLensDistortion, Verbose, TEXT("Can't evaluate LensFile '%s' - No calibrated maps"), *GetName());
		SetupNoDistortionOutput(LensHandler, OutDistortionData);
		return true;
	}
