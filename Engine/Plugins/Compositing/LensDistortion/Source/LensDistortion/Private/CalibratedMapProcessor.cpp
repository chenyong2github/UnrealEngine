...
public:
 ...
 // --------------------------------------------------------------------------------------------------------------------
IMPLEMENT_GLOBAL_SHADER(FCalibratedMapDerivedDataCS, "/Plugin/LensDistortion/Private/DistortionSTMapProcessor.usf", "DistortionSTMapProcessorMainCS", SF_Compute);
...
void FCalibratedMapProcessor::Update_RenderThread()
{
	...
	for(TSharedPtr<FDerivedDistortionDataJob>& CompletedJob : ToComplete)
	{
		...
		{
			UE_LOG(LogLensDistortion, Error, TEXT("Failed to retrieve edge points distorted UVs"));
