...
struct IMoviePipelineQueueTreeItem : TSharedFromThis<IMoviePipelineQueueTreeItem>
{
	...
	virtual TSharedRef<SWidget> ConstructWidget(TWeakPtr<SMoviePipelineQueueEditor> InQueueWidget) = 0;
};
...
struct FMoviePipelineQueueJobTreeItem : IMoviePipelineQueueTreeItem
{
	...
	explicit FMoviePipelineQueueJobTreeItem(UMoviePipelineExecutorJob* InJob)
		: WeakJob(InJob)
	{}
	...
	virtual TSharedRef<SWidget> ConstructWidget(TWeakPtr<SMoviePipelineQueueEditor> InQueueWidget) override
	{
		...
		[
			...
			[
				...
				.ObjectPath(this, &FMoviePipelineQueueJobTreeItem::GetSequencePath)
				.AllowedClass(ULevelSequence::StaticClass())
				.OnObjectChanged(this, &FMoviePipelineQueueJobTreeItem::SetSequencePath)
				.AllowClear(false)
			...
			[
				SNew(SObjectPropertyEntryBox)
				.ObjectPath(this, &FMoviePipelineQueueJobTreeItem::GetMasterConfigPath)
				.AllowedClass(UMoviePipelineMasterConfig::StaticClass())
				.OnObjectChanged(this, &FMoviePipelineQueueJobTreeItem::SetMasterConfigPath)
				.AllowClear(false)
				.DisplayUseSelected(false)
				.DisplayBrowse(true)
				.DisplayThumbnail(true)
			...
			[
				SNew(SButton)
				.OnClicked(InQueueWidget.Pin().Get(), &SMoviePipelineQueueEditor::OnEditMasterConfigForJob, WeakJob)
				.Content()
				[
					SNew(STextBlock)
			...
			[
				SNew(STextBlock)
				.Text(this, &FMoviePipelineQueueJobTreeItem::GetOutputLabel)
				// .TextStyle(FTakeRecorderStyle::Get(), "TakeRecorder.Source.Label")
				.ColorAndOpacity(this, &FMoviePipelineQueueJobTreeItem::GetOutputColorAndOpacity)
			]
			...
			[
				...
				.WidgetIndex(this, &FMoviePipelineQueueJobTreeItem::GetStatusIndex)
				...
				[
					SNew(SProgressBar)
					// .Style(FTakeRecorderStyle::Get(), "TakeRecorder.Source.Switch")
					.Percent(this, &FMoviePipelineQueueJobTreeItem::GetProgressPercent)
				]
...
private:
	...
	FString GetMasterConfigPath() const
	{
		...
		if (Job)
		{
			return Job->Configuration->GetPathName();
		}
	...
	void SetMasterConfigPath(const FAssetData& AssetData)
	{
		...
		if (Job)
		{
			Job->Configuration = CastChecked<UMoviePipelineMasterConfig>(AssetData.GetAsset());
		}
	}
	...
	FText GetOutputLabel() const
	{
		...
		{
			UMoviePipelineOutputSetting* OutputSetting = Job->Configuration->FindSetting<UMoviePipelineOutputSetting>();
			check(OutputSetting);
	...
	FSlateColor GetOutputColorAndOpacity() const
	{
		UMoviePipelineExecutorJob* Job = WeakJob.Get();
		return (Job && Job->HasFinished()) ? FSlateColor::UseSubduedForeground() : FSlateColor::UseForeground();
	}
...
struct FMoviePipelineMapTreeItem : IMoviePipelineQueueTreeItem
{
	...
	virtual TSharedRef<SWidget> ConstructWidget(TWeakPtr<SMoviePipelineQueueEditor> InQueueWidget) override
	{
		return SNew(SHorizontalBox)
			...
			[
				SNew(STextBlock)
				.Text(this, &FMoviePipelineMapTreeItem::GetMapLabel)
			];
	}
...
private:
	FText GetMapLabel() const
	{
		...
		if (Job)
		{
			...
			if (World)
			{
				return FText::FromString(World->GetName());
			}
		}

		return LOCTEXT("MissingMap_Label", "[Missing Map]");
	}
};
...
void SMoviePipelineQueueEditor::Construct(const FArguments& InArgs)
{
	...
	for(int32 i = 0; i < 5; i++)
	{
		UMoviePipelineExecutorJob* NewJob = NewObject<UMoviePipelineExecutorJob>();
		NewJob->AddToRoot();
		NewJob->Sequence = FSoftObjectPath(TEXT("/Game/Sequencer/MovieRenderPipeline/PipelineTestSeq"));
		NewJob->Map = FSoftObjectPath(TEXT("/Game/Sequencer/MovieRenderPipeline/MovieRenderPipelineTest"));

		TSharedPtr< FMoviePipelineQueueJobTreeItem> ExampleJobTreeItem = MakeShared<FMoviePipelineQueueJobTreeItem>(NewJob);
		RootNodes.Add(ExampleJobTreeItem);

		TSharedPtr<FMoviePipelineMapTreeItem> MapLine = MakeShared<FMoviePipelineMapTreeItem>(NewJob);
		ExampleJobTreeItem->Children.Add(MapLine);

		for (int32 j = 0; j < 2; j++)
		{
			TSharedPtr<FMoviePipelineShotItem> Shot = MakeShared<FMoviePipelineShotItem>(NewJob);
			ExampleJobTreeItem->Children.Add(Shot);
		}
	}
}
	...
	return SNew(SComboButton)
		...
		[
			...
			[
				SNew(STextBlock)
				.TextStyle(FEditorStyle::Get(), "NormalText.Important")
				.Text(LOCTEXT("AddNewSetting_Text", "Job"))
			]
...
void SMoviePipelineQueueEditor::ReconstructTree()
{
	...
	for (UMoviePipelineExecutorJob* Job : ActiveQueue->GetJobs())
	{
		...
		TSharedPtr< FMoviePipelineQueueJobTreeItem> JobTreeItem = MakeShared< FMoviePipelineQueueJobTreeItem>(Job);
...
{
	return
		SNew(STableRow<TSharedPtr<IMoviePipelineQueueTreeItem>>, Tree)
		[
...
FReply SMoviePipelineQueueEditor::OnEditMasterConfigForJob(TWeakObjectPtr<UMoviePipelineExecutorJob> InJob)
{
	...
	TSharedRef<SMoviePipelineConfigPanel> ConfigEditorPanel = 
		...
		.BasePreset(InJob->Configuration);
