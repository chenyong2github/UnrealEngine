...
class FNiagaraAttributeTrimmerHelper : public GraphBridge
{
	...
	using FDependencySet = TSet<FScopedPin>;
	...
	using FDependencyMap = TMap<FScopedPin, FDependencyChain>;
	...
	static FScopedPin FindWriteForRead(const FParamMapHistory& ParamMap, const FScopedPin& ReadPin, int32* OutVariableIndex);
	static int32 FindDefaultBinding(const FParamMapHistory& ParamMap, int32 VariableIndex, const FScopedPin& ReadPin);

	// for a specific read of a variable finds the actual name of the attribute being read
	static FName FindAttributeForRead(const FParamMapHistory& ParamMap, const FScopedPin& ReadPin);
