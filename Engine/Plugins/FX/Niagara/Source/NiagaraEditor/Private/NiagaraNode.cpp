...
bool UNiagaraNode::ReallocatePins(bool bMarkNeedsResynchronizeOnChange)
{
	...
	for (int32 OldPinIndex = 0; OldPinIndex < OldPins.Num(); ++OldPinIndex)
	{
		...
		auto PinMatchPredicate = [&](UEdGraphPin* Pin) 
		{
			...
			return (OldPin->bOrphanedPin == false || OldPin->PinType == Pin->PinType) && (bPinsHaveIds && Pin->PersistentGuid == OldPin->PersistentGuid || bPinsHaveIds == false && Pin->PinName == OldPin->PinName);
		};
