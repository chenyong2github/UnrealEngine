...
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTextureEnabledName(TEXT("HeightVirtualTextureEnabled_"));
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTextureName(TEXT("HeightVirtualTexture_"));
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTexturePageTableName(TEXT("HeightVirtualTexturePageTable_"));
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTexturePageTableUniform0Name(TEXT("HeightVirtualTexturePackedUniform0_"));
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTexturePageTableUniform1Name(TEXT("HeightVirtualTexturePackedUniform1_"));
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTextureSamplerName(TEXT("HeightVirtualTextureSampler_"));
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTextureUniformsName(TEXT("HeightVirtualTextureUniforms_"));
const FString UNiagaraDataInterfaceLandscape::HeightVirtualTextureWorldToUvTransformName(TEXT("HeightVirtualTextureWorldToUvTransform_"));

// cached texture - height
const FString UNiagaraDataInterfaceLandscape::CachedHeightTextureEnabledName(TEXT("CachedHeightTextureEnabled_"));
const FString UNiagaraDataInterfaceLandscape::CachedHeightTextureName(TEXT("CachedHeightTexture_"));
const FString UNiagaraDataInterfaceLandscape::CachedHeightTextureSamplerName(TEXT("CachedHeightTextureSampler_"));
const FString UNiagaraDataInterfaceLandscape::CachedHeightTextureUvScaleBiasName(TEXT("CachedHeightTextureUvScaleBias_"));
const FString UNiagaraDataInterfaceLandscape::CachedHeightTextureWorldToUvTransformName(TEXT("CachedHeightTextureWorldToUvTransform_"));
...
public:
	...
	FShaderResourceViewRHIRef GetHeightTexture() const { return HeightTexture.SRV; }
...
public:
	...
	TConstArrayView<float> GetHeightValues() { return HeightValues; }
...
public:
	...
	FNDI_Landscape_SharedResourceHandle GetLandscapeData(const FNiagaraSystemInstance& SystemInstance, const FNDILandscapeData_GameThread& InstanceData, FNDI_SharedResourceUsage Usage, bool bNeedsDataImmediately);
...
{
	{
		TConstArrayView<float> SourceHeightValues = Owner.GetHeightValues();
		uint32 TexelStride = sizeof(float);

		HeightTexture.Initialize(TEXT("FLandscapeTextureResource"), sizeof(float), Owner.CellCount.X, Owner.CellCount.Y, EPixelFormat::PF_R32_FLOAT);
		{
			void* TexelData = RHILockTexture2D(HeightTexture.Buffer, 0, RLM_WriteOnly, TexelStride, true);
			FMemory::Memcpy(TexelData, SourceHeightValues.GetData(), SourceHeightValues.Num() * SourceHeightValues.GetTypeSize());
			RHIUnlockTexture2D(HeightTexture.Buffer, 0, true);
		}
	}

#if STATS
	check(GpuMemoryUsage == 0);
	GpuMemoryUsage = HeightTexture.NumBytes;
...
FNDI_Landscape_SharedResourceHandle FNDI_Landscape_GeneratedData::GetLandscapeData(const FNiagaraSystemInstance& SystemInstance, const FNDILandscapeData_GameThread& InstanceData, FNDI_SharedResourceUsage Usage, bool bNeedsDataImmediately)
{
...
bool UNiagaraDataInterfaceLandscape::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (FNDILandscapeData_GameThread* InstanceData = reinterpret_cast<FNDILandscapeData_GameThread*>(PerInstanceData))
	{
		...
		if (InstanceData->RequiresCollisionCacheGpu)
		{
			FNDI_Landscape_GeneratedData& GeneratedData = SystemInstance->GetWorldManager()->EditGeneratedData<FNDI_Landscape_GeneratedData>();
			InstanceData->SharedResourceHandle = GeneratedData.GetLandscapeData(*SystemInstance, *InstanceData, FNDI_SharedResourceUsage(false, true), true);
		}
	}
...
void UNiagaraDataInterfaceLandscape::ApplyLandscape(const FNiagaraSystemInstance& SystemInstance, FNDILandscapeData_GameThread& InstanceData) const
{
	...
	if (InstanceData.Landscape != Landscape)
	{
		...
		// see if we need to support creating our own cached version of the collision geometry in a generated data
...
public:
	...
	{
		HeightVirtualTextureParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTextureName + ParameterInfo.DataInterfaceHLSLSymbol));
		HeightVirtualTexturePageTableParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTexturePageTableName + ParameterInfo.DataInterfaceHLSLSymbol));
		HeightVirtualTextureSamplerParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTextureSamplerName + ParameterInfo.DataInterfaceHLSLSymbol));
		HeightVirtualTextureWorldToUvTransformParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTextureWorldToUvTransformName + ParameterInfo.DataInterfaceHLSLSymbol));
		HeightVirtualTextureEnabledParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTextureEnabledName + ParameterInfo.DataInterfaceHLSLSymbol));
		HeightVirtualTexturePageTableUniform0Param.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTexturePageTableUniform0Name + ParameterInfo.DataInterfaceHLSLSymbol));
		HeightVirtualTexturePageTableUniform1Param.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTexturePageTableUniform1Name + ParameterInfo.DataInterfaceHLSLSymbol));
		HeightVirtualTextureUniformsParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::HeightVirtualTextureUniformsName + ParameterInfo.DataInterfaceHLSLSymbol));

		CachedHeightTextureParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::CachedHeightTextureName + ParameterInfo.DataInterfaceHLSLSymbol));
		CachedHeightTextureSamplerParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::CachedHeightTextureSamplerName + ParameterInfo.DataInterfaceHLSLSymbol));
		CachedHeightTextureWorldToUvTransformParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::CachedHeightTextureWorldToUvTransformName + ParameterInfo.DataInterfaceHLSLSymbol));
		CachedHeightTextureUvScaleBiasParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::CachedHeightTextureUvScaleBiasName + ParameterInfo.DataInterfaceHLSLSymbol));
		CachedHeightTexturEnabledParam.Bind(ParameterMap, *(UNiagaraDataInterfaceLandscape::CachedHeightTextureEnabledName + ParameterInfo.DataInterfaceHLSLSymbol));
	}
	...
	bool SetHeightVirtualTextureParameters(FRHICommandList& RHICmdList, FRHIComputeShader* ComputeShaderRHI, const FNDILandscapeData_RenderThread& ProxyData) const
	{
		...
		constexpr uint32 HeightVirthaTextureLayerIndex = 0;
		constexpr bool HeightVirtualTextureSrgb = false;

		FRHIShaderResourceView* PhysicalTextureSrv = HeightAllocatedTexture->GetPhysicalTextureSRV(HeightVirthaTextureLayerIndex, HeightVirtualTextureSrgb);
		...
		SetTextureParameter(RHICmdList, ComputeShaderRHI, HeightVirtualTexturePageTableParam, HeightAllocatedTexture->GetPageTableTexture(HeightVirthaTextureLayerIndex));
		...
		HeightAllocatedTexture->GetPackedUniform(&HeightVirtualTextureUniforms, HeightVirthaTextureLayerIndex);
	...
	bool SetCachedHeightTextureParameters(FRHICommandList& RHICmdList, FRHIComputeShader* ComputeShaderRHI, const FNDILandscapeData_RenderThread& ProxyData) const
	{
		...
		{
			FShaderResourceViewRHIRef HeightTextureSrv = ProxyData.TextureResources->GetHeightTexture();
	...
	void Set(FRHICommandList& RHICmdList, const FNiagaraDataInterfaceSetArgs& Context) const
	{
		...
		if (ProxyData)
		{
			...
			else if (SetCachedHeightTextureParameters(RHICmdList, ComputeShaderRHI, *ProxyData))
			{
