...
bool FNiagaraParameterStore::AddParameter(const FNiagaraVariable& Param, bool bInitInterfaces, bool bTriggerRebind, int32* OutOffset)
{
	...
	if (Param.GetType().IsDataInterface())
	{
		...
		EObjectFlags DataInterfaceObjectFlags = RF_Transactional;
		if(Owner != nullptr && Owner->IsTemplate())
		{
			DataInterfaceObjectFlags |= RF_Public;
		}
		DataInterfaces[Offset] = bInitInterfaces ? NewObject<UNiagaraDataInterface>(Owner, const_cast<UClass*>(Param.GetType().GetClass()), NAME_None, DataInterfaceObjectFlags) : nullptr;
		bInterfacesDirty = true;
	}
