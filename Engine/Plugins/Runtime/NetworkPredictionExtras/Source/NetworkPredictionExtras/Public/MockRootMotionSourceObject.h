...
class UMockTargetTask : public UObject
{
	GENERATED_BODY()

public:
	
	virtual void DoTask(FMockTargetHandle Handle) const { }
};

UCLASS(Blueprintable)
class UMockTargetTask_Blueprintable : public UMockTargetTask
{
	GENERATED_BODY()

public:
	
	virtual void DoTask(FMockTargetHandle Handle) const override { BP_DoTask(Handle); }

	UFUNCTION(BlueprintImplementableEvent)
	void BP_DoTask(FMockTargetHandle Handle) const;
};

UCLASS()
class UMockTargetTask_SphereOverlap: public UMockTargetTask
{
	GENERATED_BODY()

public:
	
	virtual void DoTask(FMockTargetHandle Handle) const override { }

	UPROPERTY(EditAnywhere, Category="Targeting")
	float DistanceFromTargetOrigin;
...
// Bad name
USTRUCT(BlueprintType)
struct FMockTargetTaskSequence
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, Instanced, Category="Targeting")
	TArray<UMockTargetTask*> Tasks;
};


UCLASS()
class UMockTargetDataAsset : public UDataAsset
{
	GENERATED_BODY()
...
public:
	...
	FMockTargetTaskSequence Tasks;
};


// Bad name
USTRUCT(BlueprintType)
struct FSparseTargetTaskContainer
{
	GENERATED_BODY()
	...
	UPROPERTY(EditDefaultsOnly, Category="Targeting", meta=(ShowOnlyInnerProperties))
	FMockTargetTaskSequence MySparseTasks;
};


UCLASS(Blueprintable, SparseClassDataTypes = SparseTargetTaskContainer)
class AMyTargetTaskSequenceHavingActor : public AActor
{
	GENERATED_BODY()

public:

	// Not sparse - every instance of the actor has their own instanced copy of the objects containers here!
	UPROPERTY(EditAnywhere, Category="Targeting", meta=(ShowOnlyInnerProperties))
	FMockTargetTaskSequence NonSparseTasks;

	void Foo()
	{
		FSparseTargetTaskContainer* SparseData = GetSparseTargetTaskContainer();
	}
...
UCLASS(Blueprintable, SparseClassDataTypes = TestClassData)
...
public:
	...
	void Foo()
	{
		FTestClassData* SparseData = GetTestClassData();
		SparseData->MyIntProperty;
	}
};

UCLASS(Blueprintable)
class ATestActorChild_NoNewSparse : public ATestActor
{
	GENERATED_BODY()
};
...
// Bad name
USTRUCT(BlueprintType)
struct FTestClassDataChild : public FTestClassData
{
	GENERATED_BODY()
...
UCLASS(Blueprintable, SparseClassDataTypes = TestClassDataChild )
class ATestActorChild : public ATestActor
{
	GENERATED_BODY()
...
public:
	...
	void Bar()
	{
		...
		FTestClassDataChild* SparseDataChild = GetTestClassDataChild();
		SparseDataChild->MyIntProperty;
	}
};
