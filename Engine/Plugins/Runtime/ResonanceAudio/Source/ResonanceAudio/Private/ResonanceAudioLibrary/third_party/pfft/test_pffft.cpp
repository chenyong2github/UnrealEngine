...
#ifndef M_PI
#define DEFINED_M_PI 1
#define M_PI 	3.14159265358979323846
#endif

#ifndef M_LN2
#define DEFINED_M_LN2 1
#define M_LN2 0.693147180559945309417
#endif

#ifndef M_SQRT2
#define DEFINED_M_SQRT2 1
...
void pffft_validate_N(int N, int cplx) {
  ...
  ref = (float*) pffft_aligned_malloc(Nbytes);
  in = (float*) pffft_aligned_malloc(Nbytes);
  out = (float*) pffft_aligned_malloc(Nbytes);
  tmp = (float*) pffft_aligned_malloc(Nbytes);
  tmp2 = (float*) pffft_aligned_malloc(Nbytes);
  ...
  for (pass=0; pass < 2; ++pass) {
    ...
    if (pass == 0) {
      float *wrk = (float*) malloc(2*Nbytes+15*sizeof(float));
      for (k=0; k < Nfloat; ++k) {
...
void benchmark_ffts(int N, int cplx) {
  ...
  int Nbytes = Nfloat * sizeof(float);
  float* X = (float*)pffft_aligned_malloc(Nbytes);
  float* Y = (float*) pffft_aligned_malloc(Nbytes);
  float* Z = (float*) pffft_aligned_malloc(Nbytes);
...
int TestPFFFT(int argc, char **argv) {
