...
UMetasoundEditorGraphNode_Base::UMetasoundEditorGraphNode_Base(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UEdGraphPin* UMetasoundEditorGraphNode_Base::GetOutputPin()
{
...
void UMetasoundEditorGraphNode_Base::GetInputPins(TArray<class UEdGraphPin*>& OutInputPins)
{
	OutInputPins.Empty();
...
UEdGraphPin* UMetasoundEditorGraphNode_Base::GetInputPin(int32 InputIndex)
{
...
int32 UMetasoundEditorGraphNode_Base::GetInputCount() const
{
	int32 InputCount = 0;
...
void UMetasoundEditorGraphNode_Base::InsertNewNode(UEdGraphPin* FromPin, UEdGraphPin* NewLinkPin, TSet<UEdGraphNode*>& OutNodeList)
{
...
void UMetasoundEditorGraphNode_Base::AllocateDefaultPins()
{
	check(Pins.Num() == 0);
	...
	if (!IsRootNode())
	{
		// Todo --> conform to UMetasound outputs
		CreatePin(EGPD_Output, TEXT("Metasound"), TEXT("Output"));
	}
}

void UMetasoundEditorGraphNode_Base::ReconstructNode()
{
...
void UMetasoundEditorGraphNode_Base::AutowireNewNode(UEdGraphPin* FromPin)
{
	if (FromPin)
	{
		const UMetasoundEditorGraphSchema* Schema = CastChecked<UMetasoundEditorGraphSchema>(GetSchema());
...
bool UMetasoundEditorGraphNode_Base::CanCreateUnderSpecifiedSchema(const UEdGraphSchema* Schema) const
{
	return Schema->IsA(UMetasoundEditorGraphSchema::StaticClass());
}

FString UMetasoundEditorGraphNode_Base::GetDocumentationLink() const
{
