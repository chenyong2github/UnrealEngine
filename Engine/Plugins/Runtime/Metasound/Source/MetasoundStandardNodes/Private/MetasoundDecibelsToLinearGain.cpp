...
namespace Metasound
{
	...
	const FVertexInterface& FDecibelsToLinearGainOperator::GetVertexInterface()
	{
		...
		static const FVertexInterface Interface(
			FInputVertexInterface(
				TInputDataVertexModel<float>(METASOUND_GET_PARAM_NAME_AND_TT(InputDecibelGain), 0.0f)
			),
			FOutputVertexInterface(
				TOutputDataVertexModel<float>(METASOUND_GET_PARAM_NAME_AND_TT(OutputLinearGain))
			)
		);
	...
	const FNodeClassMetadata& FDecibelsToLinearGainOperator::GetNodeInfo()
	{
		auto InitNodeInfo = []() -> FNodeClassMetadata
		{
			...
			const FText NodeDisplayName = LOCTEXT("Metasound_DecibelsToLinearGainName", "Decibels to Linear Gain");
