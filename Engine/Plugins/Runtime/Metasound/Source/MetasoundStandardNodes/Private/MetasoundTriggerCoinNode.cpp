	...
	const FVertexInterface& FTriggerCoinOperator::GetVertexInterface()
	{
		...
		static const FVertexInterface Interface(
			FInputVertexInterface(
				...
				TInputDataVertexModel<int32>(METASOUND_GET_PARAM_NAME_AND_TT(InputSeed), -1),
				TInputDataVertexModel<float>(METASOUND_GET_PARAM_NAME_AND_TT(InputProbability), 1.0f)
			),
			FOutputVertexInterface(
				TOutputDataVertexModel<FTrigger>(METASOUND_GET_PARAM_NAME_AND_TT(OutputTrueTrigger)),
				TOutputDataVertexModel<FTrigger>(METASOUND_GET_PARAM_NAME_AND_TT(OutputFalseTrigger))
			)
		);
