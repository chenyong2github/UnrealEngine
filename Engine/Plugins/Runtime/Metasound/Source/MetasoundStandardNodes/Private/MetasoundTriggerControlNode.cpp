...
namespace Metasound
{
	...
	const FVertexInterface& FTriggerControlOperator::GetVertexInterface()
	{
		...
		static const FVertexInterface Interface(
			FInputVertexInterface(
				...
				TInputDataVertexModel<FTrigger>(METASOUND_GET_PARAM_NAME_AND_TT(InputOpen)),
				TInputDataVertexModel<FTrigger>(METASOUND_GET_PARAM_NAME_AND_TT(InputClose)),
				TInputDataVertexModel<FTrigger>(METASOUND_GET_PARAM_NAME_AND_TT(InputToggle)),
				TInputDataVertexModel<bool>(METASOUND_GET_PARAM_NAME_AND_TT(InputStartClosed), true)
			),
	...
	const FNodeClassMetadata& FTriggerControlOperator::GetNodeInfo()
	{
		auto InitNodeInfo = []() -> FNodeClassMetadata
		{
			...
			Info.ClassName = { Metasound::StandardNodes::Namespace, TEXT("Trigger Control"), TEXT("") };
			Info.MajorVersion = 1;
			Info.MinorVersion = 0;
			Info.DisplayName = LOCTEXT("Metasound_TriggerControlNodeDisplayName", "Trigger Control");
			Info.Description = LOCTEXT("Metasound_TriggerControlNodeDescription", "Control whether input triggers are passed through.");
