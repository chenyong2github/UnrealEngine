...
namespace Metasound
{
	...
	const FVertexInterface& FFrequencyMultiplierToSemitoneOperator::GetVertexInterface()
	{
		...
		static const FVertexInterface Interface(
			FInputVertexInterface(
				TInputDataVertexModel<float>(METASOUND_GET_PARAM_NAME_AND_TT(InputFrequencyMultiplier), 1.0f)
			),
			FOutputVertexInterface(
				TOutputDataVertexModel<float>(METASOUND_GET_PARAM_NAME_AND_TT(OutputSemitone))
			)
		);
	...
	const FNodeClassMetadata& FFrequencyMultiplierToSemitoneOperator::GetNodeInfo()
	{
		auto InitNodeInfo = []() -> FNodeClassMetadata
		{
			...
			const FText NodeDisplayName = LOCTEXT("Metasound_FrequencyMultiplierToSemitoneName", "Frequency Multiplier to Semitone");
			const FText NodeDescription = LOCTEXT("Metasound_FrequencyMultiplierToSemitoneDescription", "Converts a frequency multiplier to the corresponding difference in semitones.");
