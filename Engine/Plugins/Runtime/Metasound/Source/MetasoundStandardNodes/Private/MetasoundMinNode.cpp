...
namespace Metasound
{
	...
	namespace MetasoundMinNodePrivate
	{
		FNodeClassMetadata CreateNodeClassMetadata(const FName& InDataTypeName, const FName& InOperatorName, const FText& InDisplayName, const FText& InDescription, const FVertexInterface& InDefaultInterface)
		{
			...
			{
				FNodeClassName{FName("Min"), InOperatorName, InDataTypeName},
				1, // Major Version
				0, // Minor Version
				InDisplayName,
				InDescription,
	...
	public:
		static const FVertexInterface& GetDefaultInterface()
		{
			...
			static const FVertexInterface DefaultInterface(
				FInputVertexInterface(
					TInputDataVertexModel<ValueType>(METASOUND_GET_PARAM_NAME_AND_TT(InputAValue)),
					TInputDataVertexModel<ValueType>(METASOUND_GET_PARAM_NAME_AND_TT(InputBValue))
				),
		...
		static const FNodeClassMetadata& GetNodeInfo()
		{
			auto CreateNodeClassMetadata = []() -> FNodeClassMetadata
			{
				...
				FName OperatorName = TEXT("Min");
				FText NodeDisplayName = FText::Format(LOCTEXT("MinDisplayNamePattern", "Min ({0})"), FText::FromString(GetMetasoundDataTypeString<ValueType>()));
				FText NodeDescription = LOCTEXT("MinDesc", "Returns the min of A and B.");
