...
-Related, we are also not all in on a "predict everything: seemlessly and automtically" solution. We still feel player prediction is best kept to a minimum (meaning: predict the minimum amount of stuff you can get away with).
-But, we are tring to make it way easier to write predictive gameplay systems. Easier to write, to debug and reason about.
