...
-The simulation has tightly defined input and output.
...
-Forward Predict: Must be explicitly enabled by outside call, ties simulated proxy sim to an autonomous proxy sim. Sims will be reconciled together in step. Basic implementation is there but not complete. Still work to do here.
...
User Structures
-User defines these strutures along with NetSerialize/Log function.
	-InputCmd: client generated input state. (E.g, FMockInputCmd)
	-Sync: frame-to-frame evolving state. (E.g, FMockSyncState)
	-Aux: state that does not always change frame to frame (E.g, FMockAuxState)

User Simulation Class (E.g, FMockNetworkSimulation)
-Networking agnostic, user defined simulation class. Has ::SimulationTick function. Can hold non networked state.
-Takes Inputcmd/Sync/Aux state as input and produces an output Sync and (optional) Aux state.

TNetworkedSimulationModel
-Core class that ties things together. Mostly empty itself.

	Model Definition (FNetSimModelDefBase. E.g, FMockNetworkModelDef)
		-Sets user defined structures and simulation class.
		-Implements networking, user defined functions: ShouldReconcile, Interpolate.

	TNetworkedSimulationState
		-Holds all internal networked state: user frame buffers and ticking/frame info.

	RepControllers (Server, Autonomous Proxy, Simulated Proxy)
		-Network role based classes that define NetSeriaize, Reconcile, and Tick behavior.
		-These hold the logic to actually advance and network the simulation machinery. 

 
Driver (TNetworkedSimulationModelDriver. E.g, UMockNetworkSimulationComponent)
-UE4 side that binds with the TNetworkedSimulationModel
-Resonsible for ::FinalizeFrame, ::ProduceInput functions. Also debugging/logging helpers.
...
Synthesize: In general means "making stuff up". For example, "synthesizing user commands" would mean "creating user commands that were not generated by a player, but instead guessing what it might be".
...
Update (4-1-20)
-Spring cleaning refactor. User code should be unaffected but this has hopefully simplified a lot of things.
-Moving away from individual Input/Sync/Time buffers into a single internal "FrameState" structure.
	-This is more cache effecient and less error prone. Previously was difficult to reason about which buffer's "head" was correct and how to keep them in sync.
	-"Gaps" in frames are now allowed, rather than wiping previous contents. Gap frames are skipped.
	-This was needed as we experimented with input buffering and packet loss.

Update (3-16-20)
-Network Prediction Insights is the new Unreal Insights based tool debugging tool for Network rediction.
-Enable the NetworkPredictionInsights plugin in your project. 
-Run with -trace=NP or -trace=NetworkPrediction. You can also enable the NetworkPrediction channel via the TraceFilterChannels window.
-Launch NPI through editor under Tools -> Network Prediction Insights
-Alternatively, you can connect standalone UnrealInsights.exe directly to your game or open a saved .utrace file.
-Existing canvas based debugging will be removed soon from Network Prediction.
-Added TSimulationDoTickFunc<Model> to replace TScopedSimulationTick. This better encapsulates 'running a tick'. 

Videos of Network Prediction Insights:
https://youtu.be/_rdt-v1nFlY
https://youtu.be/18BNvoGP-E0
