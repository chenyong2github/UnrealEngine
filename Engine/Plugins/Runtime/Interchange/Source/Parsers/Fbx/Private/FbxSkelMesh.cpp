...
namespace UE
{
	namespace Interchange
	{
		namespace Private
		{
			void FFbxSkelMesh::FindSkeletalMeshes(FbxScene* SDKScene, TArray< TArray<FbxNode*> >& outSkelMeshArray, bool bCombineSkeletalMesh, bool bForceFindRigid)
			{
					...
					if (bForceFindRigid)
					{
						...
						for (int32 i = 0; i < outSkelMeshArray.Num(); i++)
						{
							...
							for (FbxNode* InspectedNode : NodeArray)
							{
								...
								if (LodGroup != nullptr)
								{
									FbxNode* SkelMeshNode = FindLODGroupNode(InspectedNode, 0, nullptr);
			...
			void FFbxSkelMesh::RecursiveGetAllMeshNode(TArray<FbxNode*> & OutAllNode, FbxNode * Node)
			{
				if (Node == nullptr)
				{
					return;
			...
			void FFbxSkelMesh::FindAllLODGroupNode(TArray<FbxNode*> & OutNodeInLod, FbxNode * NodeLodGroup, int32 LodIndex)
			{
			...
			bool FFbxSkelMesh::IsUnrealBone(FbxNode * Link)
			{
			...
			void FFbxSkelMesh::RecursiveBuildSkeleton(FbxNode * Link, TArray<FbxNode*> & OutSortedLinks)
			{
			...
			bool FFbxSkelMesh::RetrievePoseFromBindPose(FbxScene * SDKScene, const TArray<FbxNode*> & NodeArray, FbxArray<FbxPose*> & PoseArray)
			{
			...
			FbxNode* FFbxSkelMesh::GetRootSkeleton(FbxScene * SDKScene, FbxNode * Link)
			{
			...
			void FFbxSkelMesh::BuildSkeletonSystem(FbxScene * SDKScene, TArray<FbxCluster*> & ClusterArray, TArray<FbxNode*> & OutSortedLinks)
			{
				FbxNode* Link;
			...
			bool FFbxSkelMesh::FindSkeletonJoints(FbxScene * SDKScene, TArray<FbxNode*> & NodeArray, TArray<FbxNode*> & SortedLinks, FbxArray<FbxAMatrix> & LocalsPerLink)
			{
			...
			FbxNode* FFbxSkelMesh::RecursiveGetFirstMeshNode(FbxNode* Node, FbxNode* NodeToFind)
			{
				if (Node == nullptr)
				{
					return nullptr;
			...
			FbxNode* FFbxSkelMesh::FindLODGroupNode(FbxNode* NodeLodGroup, int32 LodIndex, FbxNode* NodeToFind)
			{
				check(NodeLodGroup->GetChildCount() >= LodIndex);
				FbxNode* ChildNode = NodeLodGroup->GetChild(LodIndex);
				if (ChildNode == nullptr)
			...
			void FFbxSkelMesh::RecursiveFindFbxSkelMesh(FbxScene * SDKScene, FbxNode * Node, TArray< TArray<FbxNode*> > & outSkelMeshArray, TArray<FbxNode*> & SkeletonArray)
			{
				...
				else if (Node->GetNodeAttribute() && Node->GetNodeAttribute()->GetAttributeType() == FbxNodeAttribute::eLODGroup)
				{
					...
					SkelMeshNode = FindLODGroupNode(Node, 0, nullptr);
			...
			void FFbxSkelMesh::RecursiveFindRigidMesh(FbxScene * SDKScene, FbxNode * Node, TArray< TArray<FbxNode*> > & outSkelMeshArray, TArray<FbxNode*> & SkeletonArray)
			{
			...
			void FFbxSkelMesh::RecursiveFixSkeleton(FbxScene * SDKScene, FbxNode * Node, TArray<FbxNode*> & SkelMeshes, bool bImportNestedMeshes)
			{
