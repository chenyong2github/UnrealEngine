...
TOptional<Interchange::FImportImage> UInterchangePSDTranslator::GetTexturePayloadData(const UInterchangeSourceData* SourceData, const FString& PayLoadKey) const
{
	...
	if (!Interchange::FImportImageHelper::IsImportResolutionValid(PSDHeader.Width, PSDHeader.Height, bAllowNonPowerOfTwo))
	{
		return TOptional<Interchange::FImportImage>();
	}
	...
	if (!PSDParserHelper::ReadData(PayloadData.RawData.GetData(), Buffer, PSDHeader))
	{
