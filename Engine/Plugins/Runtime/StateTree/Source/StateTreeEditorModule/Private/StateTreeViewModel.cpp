...
void FStateTreeViewModel::PasteStates(UStateTreeState* AfterState)
{
	UStateTreeEditorData* TreeData = TreeDataWeak.Get();
	if (TreeData == nullptr)
	{
	...
	if (AfterState)
	{
		...
		if (Index != INDEX_NONE)
		{
			PasteStatesAsChildren(AfterState->Parent, Index + 1);
		}
	}
}

void FStateTreeViewModel::PasteStatesAsChildren(UStateTreeState* ParentState, const int32 Index)
{
	UStateTreeEditorData* TreeData = TreeDataWeak.Get();
	if (TreeData == nullptr)
	{
