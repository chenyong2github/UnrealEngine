...
bool FTextureShareD3D12SharedResourceSecurityAttributes::Initialize()
{
	...
	if (!AllocateAndInitializeSid(&SIDAuthWorld, 1,
		...
		&pEveryoneSID))
	{
		UE_LOG(LogTextureShareD3D12, Error, TEXT("AllocateAndInitializeSid Error %u"), GetLastError());
		Release();
		return false;
	}
	...
	{
		UE_LOG(LogTextureShareD3D12, Error, TEXT("SetEntriesInAcl Error %u"), GetLastError());
		Release();
		return false;
	...
	if (NULL == pSD)
	{
		UE_LOG(LogTextureShareD3D12, Error, TEXT("LocalAlloc Error %u"), GetLastError());
		Release();
		return false;
	...
	{
		UE_LOG(LogTextureShareD3D12, Error, TEXT("InitializeSecurityDescriptor Error %u"), GetLastError());
		Release();
		return false;
	}
	...
	{
		UE_LOG(LogTextureShareD3D12, Error, TEXT("SetSecurityDescriptorDacl Error %u"), GetLastError());
		Release();
		return false;
	}
