...
FMPCDIRegion::FMPCDIRegion(const wchar_t* Name, int InW, int InH)
	: ID(Name)
	, X(0), Y(0)
	, W(1), H(1)
	, ResX(InW), ResY(InH)
...
bool FMPCDIRegion::Load(mpcdi::Region* mpcdiRegion, const IMPCDI::EMPCDIProfileType ProfileType)
{
	...
	if (ProfileType != IMPCDI::EMPCDIProfileType::mpcdi_SL)
	{
		...
		if (geometryFile)
		{
			//Load texture warp map
			if (!WarpData)
			{
				WarpData = new FMPCDIWarpTexture();
			}

			FMPCDIWarpTexture* WarpMap = static_cast<FMPCDIWarpTexture*>(WarpData);
			if (WarpMap) 
			{
...
bool FMPCDIRegion::SetStaticMeshWarp(UStaticMeshComponent* MeshComponent, USceneComponent* OriginComponent)
{
	if (!WarpData)
	{
		WarpData = new FMPCDIWarpMesh();
	}

	FMPCDIWarpMesh* WarpMesh = static_cast<FMPCDIWarpMesh*>(WarpData);
	if (WarpMesh)
	{
		return WarpMesh->SetStaticMeshWarp(MeshComponent, OriginComponent);
	}
