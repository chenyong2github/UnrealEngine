...
public:
	...
	{
		const FName ClassName = T::StaticClass()->GetFName();
		RegisteredDataModelGenerators.Add(ClassName, CreateGeneratorDelegate);
	}

private:
	/** A list of data model generators, indexed by class name, that have been registered */
	static TMap<FName, FGetColorGradingDataModelGenerator> RegisteredDataModelGenerators;
...
public:
	...
	const TSharedRef<IPropertyRowGenerator> GetPropertyRowGenerator() const { return PropertyRowGenerator.ToSharedRef(); }
...
private:
	...
	TMap<FName, TSharedPtr<IDisplayClusterColorGradingDataModelGenerator>> DataModelGeneratorInstances;
