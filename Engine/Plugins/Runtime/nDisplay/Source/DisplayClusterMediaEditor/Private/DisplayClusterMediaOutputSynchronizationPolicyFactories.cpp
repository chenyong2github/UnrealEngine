...
/**
 * UDisplayClusterMediaOutputSynchronizationPolicyRivermax
 */
UDisplayClusterMediaOutputSynchronizationPolicyRivermaxFactory::UDisplayClusterMediaOutputSynchronizationPolicyRivermaxFactory(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SupportedClass = UDisplayClusterMediaOutputSynchronizationPolicyRivermax::StaticClass();
	bCreateNew = true;
	bEditAfterNew = true;
}

UObject* UDisplayClusterMediaOutputSynchronizationPolicyRivermaxFactory::FactoryCreateNew(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	return NewObject<UDisplayClusterMediaOutputSynchronizationPolicyRivermax>(InParent, InClass, InName, Flags);
}

uint32 UDisplayClusterMediaOutputSynchronizationPolicyRivermaxFactory::GetMenuCategories() const
{
	return EAssetTypeCategories::None;
}

bool UDisplayClusterMediaOutputSynchronizationPolicyRivermaxFactory::ShouldShowInNewMenu() const
{
	return true;
}
