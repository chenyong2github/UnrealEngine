...
void SDisplayClusterConfiguratorConstraintCanvas::Construct(const FArguments& InArgs)
{
	SConstraintCanvas::Construct(SConstraintCanvas::FArguments());
}

void SDisplayClusterConfiguratorConstraintCanvas::ArrangeLayeredChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, FArrangedChildLayers& ArrangedChildLayers) const
{
	if (Children.Num() > 0)
	{
...
int32 SDisplayClusterConfiguratorConstraintCanvas::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
