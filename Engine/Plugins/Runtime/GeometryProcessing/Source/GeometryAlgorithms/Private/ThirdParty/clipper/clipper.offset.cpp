...
void ClipperOffset::DoSquare(PathGroup& group, const Path64& path, size_t j, size_t k)
{
	...
	if (delta_ > 0)
	{
		group.path_.push_back(Point64(
			 static_cast<int64>(static_cast<double>(path[j].x) + delta_ * (norms[k].x - norms[k].y)),
			 static_cast<int64>(static_cast<double>(path[j].y) + delta_ * (norms[k].y + norms[k].x))));
		group.path_.push_back(Point64(
			static_cast<int64>(static_cast<double>(path[j].x) + delta_ * (norms[j].x + norms[j].y)),
			static_cast<int64>(static_cast<double>(path[j].y) + delta_ * (norms[j].y - norms[j].x))));		
	}
	else
	{
		group.path_.push_back(Point64(
...
void ClipperOffset::DoMiter(PathGroup& group, const Path64& path, size_t j, size_t k, double cos_a)
{
	...
	group.path_.push_back(Point64(
		static_cast<int64>(static_cast<double>(path[j].x) + (norms[k].x + norms[j].x) * q),
		static_cast<int64>(static_cast<double>(path[j].y) + (norms[k].y + norms[j].y) * q)));
	// @UE END
}
...
void ClipperOffset::OffsetPoint(PathGroup& group, Path64& path, size_t j, size_t& k)
{
	...
	if (sin_a * delta_ < 0) // a concave offset
	{
		...
		// explicit casts
		Point64 p1 = Point64(
			static_cast<int64>(static_cast<double>(path[j].x) + norms[k].x * delta_),
			static_cast<int64>(static_cast<double>(path[j].y) + norms[k].y * delta_));
		Point64 p2 = Point64(
			static_cast<int64>(static_cast<double>(path[j].x) + norms[j].x * delta_),
			static_cast<int64>(static_cast<double>(path[j].y) + norms[j].y * delta_));
		// @UE END
...
void ClipperOffset::OffsetOpenPath(PathGroup& group, Path64& path, EndType end_type)
{
	...
	case EndType::Butt:
		...
		// explicit casts
		group.path_.push_back(Point64(
			static_cast<int64>(static_cast<double>(path[0].x) + norms[1].x * delta_),
			static_cast<int64>(static_cast<double>(path[0].y) + norms[1].y * delta_)));
		group.path_.push_back(Point64(
			static_cast<int64>(static_cast<double>(path[0].x) - norms[1].x * delta_),
			static_cast<int64>(static_cast<double>(path[0].y) - norms[1].y * delta_)));
		// @UE END
	...
	for(path_iter = group.paths_in_.cbegin(); path_iter != group.paths_in_.cend(); ++path_iter)
	{
		...
		if (cnt == 1) // single point - only valid with open paths
		{
			...
			else
			{
				...
				// @UE BEGIN
				// explicit casts
				group.path_.push_back(Point64(static_cast<int64>(static_cast<double>(path[0].x) - delta_), static_cast<int64>(static_cast<double>(path[0].y) - delta_)));
				group.path_.push_back(Point64(static_cast<int64>(static_cast<double>(path[0].x) + delta_), static_cast<int64>(static_cast<double>(path[0].y) - delta_)));
				group.path_.push_back(Point64(static_cast<int64>(static_cast<double>(path[0].x) + delta_), static_cast<int64>(static_cast<double>(path[0].y) + delta_)));
				group.path_.push_back(Point64(static_cast<int64>(static_cast<double>(path[0].x) - delta_), static_cast<int64>(static_cast<double>(path[0].y) + delta_)));
				// @UE END
