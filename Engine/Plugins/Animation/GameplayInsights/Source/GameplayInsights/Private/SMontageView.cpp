...
FName FMontageViewCreator::GetTargetTypeName() const
{
	static FName TargetTypeName = "AnimInstance";
	return TargetTypeName;
}

FName FMontageViewCreator::GetName() const
{
	return MontageName;
}

FText FMontageViewCreator::GetTitle() const
{
	return LOCTEXT("Montages", "Montages");
}

FSlateIcon FMontageViewCreator::GetIcon() const
{
#if WITH_EDITOR
...
TSharedPtr<IRewindDebuggerView> FMontageViewCreator::CreateDebugView(uint64 ObjectId, double CurrentTime, const TraceServices::IAnalysisSession& AnalysisSession) const
{
	return SNew(SMontageView, ObjectId, CurrentTime, AnalysisSession);
}
