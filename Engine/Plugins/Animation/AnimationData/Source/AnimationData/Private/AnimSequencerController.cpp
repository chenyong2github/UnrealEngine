		...
		if (NewLength != Model->GetNumberOfFrames())
		{
			...
			if (PlayRange.Contains(T0))
			{
				...
				if (T0 < T1)
				{
					...
					ConditionalAction<UE::Anim::FResizePlayLengthInFramesAction>(bShouldTransact, Model, T0, T1);
...
void UAnimSequencerController::ResetModel(bool bShouldTransact /*= true*/)
{
	...
	if (Model->bPopulated)
	{
		...
		SetFrameRate(FCommonFrameRates::FPS_30());
		SetNumberOfFrames(1);
	...
	if (TrackIndex == INDEX_NONE)
	{
		...
		else
		{
			...
			if (BoneIndex != INDEX_NONE || IgnoreSkeletonValidation())
			{
				...
				{
					ReportError(LOCTEXT("FailedToAddBoneControl", "Failed to add bone control"));
				}
		...
		if (bValidPosKeys && bValidRotKeys && bValidScaleKeys)
		{
			if (FBoneAnimationTrack* TrackPtr = Model->FindMutableBoneTrackByName(BoneName))
			{
...

				{
					...
					for (int32 KeyIndex = 0; KeyIndex < PositionalKeys.Num(); ++KeyIndex)
					{
						TransformKeys.Add(FTransform(FQuat(RotationalKeys[KeyIndex]), FVector(PositionalKeys[KeyIndex]), FVector(ScalingKeys[KeyIndex])));
						TimeValues.Add(FrameRate.AsSeconds(KeyIndex));
					}

					if(!SetBoneCurveKeys(BoneName, TransformKeys, TimeValues))
					{
...
void UAnimSequencerController::PopulateWithExistingModel(TScriptInterface<IAnimationDataModel> InModel)
{
	...
	if (InModel.GetObject() && InModel.GetObject()->IsA<UAnimDataModel>())
	{
		...
		{
			for (const FBoneAnimationTrack& BoneTrack : InModel->GetBoneAnimationTracks())
			{
...
void UAnimSequencerController::InitializeModel()
{
	if (const UAnimSequence* AnimationSequence = Model->GetAnimationSequence())
	{
		if (UMovieScene* MovieScene = NewObject<UMovieScene>(Model, FName("MovieScene"), RF_Transactional))
		{
			...
			{
				ReportError(LOCTEXT("FailedToCreateTrack", "Failed to create add (master) UMovieSceneControlRigParameterTrack"));
			}
...
bool UAnimSequencerController::SetBoneCurveKeys(const FName& BoneName, const TArray<FTransform>& Keys, const TArray<float>& TimeValues) const
{
	if (UMovieSceneControlRigParameterSection* Section = Model->GetFKControlRigSection())
	{		
		...
		{
			URigHierarchy* Hierarchy = FKRig->GetHierarchy();
			if(Hierarchy || IgnoreSkeletonValidation())
			{
				...
				if (bContainsBone && bContainsBoneControl)
				{
					Section->RemoveTransformParameter(BoneNameControlKey.Name);
					Section->AddTransformParameter(BoneNameControlKey.Name, TOptional<FEulerTransform>(), Model->bPopulated);
					
					FTransformParameterNameAndCurves* ParameterCurvePair = Section->GetTransformParameterNamesAndCurves().FindByPredicate([BoneNameControlKey]( FTransformParameterNameAndCurves Parameter)
					{
						return Parameter.ParameterName == BoneNameControlKey.Name;
					});
					check(ParameterCurvePair);
					
					const FTransform Offset = Hierarchy ? Hierarchy->GetControlOffsetTransform(Hierarchy->FindChecked<FRigControlElement>(BoneNameControlKey), ERigTransformType::InitialLocal) : FTransform::Identity;

					TArray<FFrameNumber> FrameNumbers;
					TArray<FMovieSceneFloatValue> Values[9];

					const int32 NumKeys = Keys.Num();
					FrameNumbers.Reserve(NumKeys);
					for (int32 Index = 0; Index < 9; ++Index)
					{
						Values[Index].Reserve(NumKeys);
					}
					...
					for (int32 KeyIndex = 0; KeyIndex < NumKeys; ++KeyIndex)
					{
						...
						OffsetTransform.NormalizeRotation();
						FrameNumbers.Add(Model->MovieScene->GetTickResolution().AsFrameTime(TimeValues[KeyIndex]).RoundToFrame());
						...
						for (int32 ChannelIndex = 0; ChannelIndex < 3; ++ChannelIndex)
						{
							{
								// Pos
								FMovieSceneFloatValue& Value = Values[ChannelIndex].AddDefaulted_GetRef();
							...
							{
								// Rot
								FMovieSceneFloatValue& Value = Values[3 + ChannelIndex].AddDefaulted_GetRef();
								Value.Value = EulerAngles[ChannelIndex];
								Value.InterpMode = ERichCurveInterpMode::RCIM_Linear;
							}

							{
								// Scale
								FMovieSceneFloatValue& Value = Values[6 + ChannelIndex].AddDefaulted_GetRef();
					...
					for (int32 ChannelIndex = 0; ChannelIndex < 3; ++ChannelIndex)
					{
						ParameterCurvePair->Translation[ChannelIndex].AddKeys(FrameNumbers, Values[ChannelIndex]);
						ParameterCurvePair->Rotation[ChannelIndex].AddKeys(FrameNumbers, Values[3 + ChannelIndex]);
						ParameterCurvePair->Scale[ChannelIndex].AddKeys(FrameNumbers,  Values[6 + ChannelIndex]);
					}
				...
				if (URigHierarchyController* Controller = RigHierarchy->GetController())
				{
					...
					{											
						...
						if (NewCurveKey.IsValid())
						{
							...
							{											
								const FRigElementKey NewCurveControlKey = Controller->RenameElement(CurveControlKey, UFKControlRig::GetControlName(NewCurveName, ERigElementType::Curve), false);
								if (NewCurveControlKey.IsValid())
								{
									...
									{
										ParameterCurvePair->ParameterName = UFKControlRig::GetControlName(NewCurveName, ERigElementType::Curve);
									}
	...
	if (UMovieSceneControlRigParameterSection* Section = Model->GetFKControlRigSection())
	{
		if (UFKControlRig* FKRig = Cast<UFKControlRig>(Section->GetControlRig()))
		{
			if (URigHierarchy* Hierarchy = FKRig->GetHierarchy())
			{
				if (URigHierarchyController* HierarchyController = Hierarchy->GetController())
				{
					const FRigElementKey NewCurveKey(DuplicateCurveName, ERigElementType::Curve);
					const FRigElementKey NewCurveControlKey(UFKControlRig::GetControlName(DuplicateCurveName, ERigElementType::Curve), ERigElementType::Control);
