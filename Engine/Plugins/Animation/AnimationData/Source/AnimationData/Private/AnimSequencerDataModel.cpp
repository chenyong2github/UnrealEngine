...
{
	UAnimationAsset* AnimationAsset = CastChecked<UAnimationAsset>(GetOuter());	
...
{
	Algo::Transform(LegacyBoneAnimationTracks, OutNames, [](FBoneAnimationTrack Track)
	{
		return Track.Name; 
	});
}
...
{	
	...
	else
	{
		...
		auto UpdateWithChannel = [UpdateSHAWithArray](const auto& Channel)
		{
...
{
	return NewObject<UAnimSequencerController>();
}
		...
		if(const UMovieSceneControlRigParameterSection* Section = GetFKControlRigSection())
		{
			if (const UControlRig* ControlRig = Section->GetControlRig())
			{
				if (const URigHierarchy* Hierarchy = ControlRig->GetHierarchy())
				{
					...
					// Loop over all control elements in hierarchy - and for any found bone controllers sample their curve data to a new track entry
					Hierarchy->ForEach<FRigControlElement>([this, Section, ReferenceSkeleton, TargetSkeleton, Hierarchy, &Transforms, &FrameNumbers](const FRigControlElement* ControlElement) -> bool
					{
						const FName TargetBoneName = UFKControlRig::GetControlTargetName(ControlElement->GetName(), ERigElementType::Bone);
						const FRigElementKey BoneElementKey(TargetBoneName, ERigElementType::Bone);

						if (Hierarchy->Contains(BoneElementKey))
						{
							const FTransformParameterNameAndCurves* ParameterCurvePair = Section->GetTransformParameterNamesAndCurves().FindByPredicate([ControlName = ControlElement->GetName()](const FTransformParameterNameAndCurves Parameter)
							{
								return Parameter.ParameterName == ControlName;
							});

							// Only populate the track if any curve keys were set
							const bool bContainsAnyKeys = ParameterCurvePair && [ParameterCurvePair]()
							{
								for (int32 ChannelIndex = 0; ChannelIndex < 3; ++ChannelIndex)
								{
									if (ParameterCurvePair->Translation[ChannelIndex].GetNumKeys())
										return true;
									if (ParameterCurvePair->Rotation[ChannelIndex].GetNumKeys())
										return true;
									if (ParameterCurvePair->Scale[ChannelIndex].GetNumKeys())
										return true;				
								}
							...
							FBoneAnimationTrack& BoneTrack = LegacyBoneAnimationTracks.AddDefaulted_GetRef();
							BoneTrack.Name = TargetBoneName;
							BoneTrack.BoneTreeIndex = ReferenceSkeleton.FindBoneIndex(BoneTrack.Name);
							ensureMsgf(BoneTrack.BoneTreeIndex != INDEX_NONE, TEXT("Generating track data for a bone %s that does not exist on the skeleton %"), *TargetBoneName.ToString(), *TargetSkeleton->GetPathName());
							...
							if(bContainsAnyKeys)
							{
								...
								GenerateTransformKeysForControl(TargetBoneName, Transforms, FrameNumbers);
								for (const FTransform& Transform : Transforms)
								{
									BoneTrack.InternalTrackData.PosKeys.Add(FVector3f(Transform.GetLocation()));
									BoneTrack.InternalTrackData.RotKeys.Add(FQuat4f(Transform.GetRotation()));
									BoneTrack.InternalTrackData.ScaleKeys.Add(FVector3f(Transform.GetScale3D()));					
								}
							}
						}
						return true;
...
void UAnimationSequencerDataModel::ValidateSequencerData() const
{
	...
	checkf(NumberOfMasterTracks == 1, TEXT("Invalid number of Master Tracks in Movie Scene expected 1 but found %i"), NumberOfMasterTracks);
		
	const UMovieSceneControlRigParameterTrack* Track = MovieScene->FindMasterTrack<UMovieSceneControlRigParameterTrack>();
	checkf(Track, TEXT("Unable to find Control Rig Master Track"));

	const int32 NumberOfSections = Track->GetAllSections().Num();
	checkf(NumberOfSections == 1, TEXT("Invalid number of Sections found for Control Rig Master Track expected 1 but found %i"), NumberOfSections);

	UMovieSceneControlRigParameterSection* Section = GetFKControlRigSection();
	checkf(Section, TEXT("Unable to find Control Rig Master Section"));
}
...
void UAnimationSequencerDataModel::ValidateLegacyAgainstControlRigData() const
{
	...
	if (const USkeleton* Skeleton = OuterSequence->GetSkeleton())
	{
		...
		for (const FBoneAnimationTrack& Track : LegacyBoneAnimationTracks)
		{
			...
			FTransformParameterNameAndCurves* BoneCurveParameter = Section->GetTransformParameterNamesAndCurves().FindByPredicate([BoneControlKey](const FTransformParameterNameAndCurves& ParameterPair)
			{
		...
		for (const FFloatCurve& FloatCurve : LegacyCurveData.FloatCurves)
		{
			...
			FScalarParameterNameAndCurve* CurveControlParameter = Section->GetScalarParameterNamesAndCurves().FindByPredicate([CurveControlKey](const FScalarParameterNameAndCurve& ParameterPair)
			{
...
void UAnimationSequencerDataModel::IterateTransformControlCurve(const FName& BoneName, TFunction<void(const FTransform&, const FFrameNumber&)> IterationFunction) const
{
	...
	if (Hierarchy->Contains(BoneControlKey))
	{
		...
		}))
		{
			...
			AnimSequencerHelpers::GenerateUniqueFramesForTransformCurve(ControlCurve, Frames);
	...
	if (ControlRig)
	{
		if (const URigHierarchy* RigHierarchy = ControlRig->GetHierarchy())
		{
			...
			// Populate bone/curve elements to Pose/Curve indices
			{
				...
				RigHierarchy->ForEach<FRigControlElement>([this, &RefSkeleton, &Curve, &RequiredBones, &RigHierarchy, &RetargetingScope, &MeshPoses, bValidCurve = Curve.IsValid(), SmartNameContainer=RequiredBones.GetSkeletonAsset()->GetSmartNameContainer(USkeleton::AnimCurveMappingName)](const FRigControlElement* ControlElement) -> bool
				{
					if (ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
					{
						...
						if (BoneIndex != INDEX_NONE)
						{
							...
							if (CompactPoseBoneIndex != INDEX_NONE)
							{
								...
								const FTransform ComponentTransform = RigHierarchy->GetGlobalTransform(Key);
								MeshPoses.SetComponentSpaceTransform(CompactPoseBoneIndex, ComponentTransform);
							}
						}
			...
			{
				QUICK_SCOPE_CYCLE_COUNTER(STAT_ConvertToLocalTransforms);
				// Convert back to local-space bone transforms
				FCSPose<FCompactPose>::ConvertComponentPosesToLocalPosesSafe(MeshPoses, RigPose);
...
void UAnimationSequencerDataModel::EvaluateTrack(UMovieSceneControlRigParameterTrack* CR_Track, const UE::Anim::DataModel::FEvaluationContext& EvaluationContext) const
{
	...
	if (SectionsInRange.Num())
	{
		...
		if (TransformParameters.Num())
		{
			...
			FEvaluationInfo ZeroAlphaInfo;
			UE::MovieScene::EvaluateTime(TransformParameters[0].Translation[0].GetTimes(), BoneSampleTime.FrameNumber, ZeroAlphaInfo.Index1, ZeroAlphaInfo.Index2, ZeroAlphaInfo.Interp);
			
			FEvaluationInfo BlendFromInfo;
			UE::MovieScene::EvaluateTime(TransformParameters[0].Translation[0].GetTimes(), BoneSampleTime.FloorToFrame(), BlendFromInfo.Index1, BlendFromInfo.Index2, BlendFromInfo.Interp);

			FEvaluationInfo BlendToInfo;
			UE::MovieScene::EvaluateTime(TransformParameters[0].Translation[0].GetTimes(), BoneSampleTime.CeilToFrame(), BlendToInfo.Index1, BlendToInfo.Index2, BlendToInfo.Interp);
			...
			for (const FTransformParameterNameAndCurves& TypedParameter : TransformParameters)
			{
				...
				if (ControlElement && ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
				{
					...
					auto EvaluateToTransform = [&TypedParameter](const FFrameNumber Frame) -> FTransform
					{
						FVector3f Translation(ForceInitToZero), Scale(FVector3f::OneVector);
						FRotator3f Rotator(0.0f, 0.0f, 0.0f);

						TypedParameter.Translation[0].Evaluate(Frame,Translation[0]);
						TypedParameter.Translation[1].Evaluate(Frame, Translation[1]);
						TypedParameter.Translation[2].Evaluate(Frame, Translation[2]);

						TypedParameter.Scale[0].Evaluate(Frame, Scale[0]);
						TypedParameter.Scale[1].Evaluate(Frame, Scale[1]);
						TypedParameter.Scale[2].Evaluate(Frame, Scale[2]);
						
						TypedParameter.Rotation[0].Evaluate(Frame, Rotator.Roll);
						TypedParameter.Rotation[1].Evaluate(Frame, Rotator.Pitch);
						TypedParameter.Rotation[2].Evaluate(Frame, Rotator.Yaw);

						return FTransform(FRotator(Rotator), static_cast<FVector>(Translation), static_cast<FVector>(Scale));
					};

					auto ExtractTransform = [&TypedParameter](const FFrameNumber Frame) -> FTransform
					{
						FVector3f Translation(ForceInitToZero), Scale(FVector3f::OneVector);	

						Translation[0] = TypedParameter.Translation[0].GetValues()[Frame.Value].Value;
						Translation[1] = TypedParameter.Translation[1].GetValues()[Frame.Value].Value;
						Translation[2] = TypedParameter.Translation[2].GetValues()[Frame.Value].Value;

						Scale[0] = TypedParameter.Scale[0].GetValues()[Frame.Value].Value;
						Scale[1] = TypedParameter.Scale[1].GetValues()[Frame.Value].Value;
						Scale[2] = TypedParameter.Scale[2].GetValues()[Frame.Value].Value;

						FRotator3f Rotator;
						Rotator.Roll = TypedParameter.Rotation[0].GetValues()[Frame.Value].Value;
						Rotator.Pitch = TypedParameter.Rotation[1].GetValues()[Frame.Value].Value;
						Rotator.Yaw = TypedParameter.Rotation[2].GetValues()[Frame.Value].Value;
					...
					if (FMath::IsNearlyZero(Alpha))
					{
						...
						{
							EulerTransform = FEulerTransform(EvaluateToTransform(BoneSampleTime.FrameNumber));
						}
						else if (EvaluationContext.InterpolationType == EAnimInterpolationType::Step)
						{
							EulerTransform = FEulerTransform(ExtractTransform(BoneSampleTime.FrameNumber));
						}
					}
					...
					else
					{
						...
						const FTransform FromBoneTransform = EvaluateToTransform(FromFrame);
						const FTransform ToBoneTransform = EvaluateToTransform(ToFrame);
