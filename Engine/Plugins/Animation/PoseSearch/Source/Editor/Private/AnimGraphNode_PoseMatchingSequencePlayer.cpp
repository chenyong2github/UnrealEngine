...
#include "AnimGraphNode_PoseMatchingSequencePlayer.h"
...
public:
		...
		: FEdGraphSchemaAction_K2NewNode(LOCTEXT("Animation", "Animations"), Title, LOCTEXT("EvalAnimSequenceToMakePose", "Evaluates an animation sequence to produce a pose"), 0, FText::FromName(InAssetInfo.ObjectPath))
	{
		...
		UAnimGraphNode_PoseMatchingSequencePlayer* Template = NewObject<UAnimGraphNode_PoseMatchingSequencePlayer>();
		NodeTemplate = Template;
	}

	virtual UEdGraphNode* PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2D Location, bool bSelectNewNode = true) override
	{
		UAnimGraphNode_PoseMatchingSequencePlayer* SpawnedNode = CastChecked<UAnimGraphNode_PoseMatchingSequencePlayer>(FEdGraphSchemaAction_K2NewNode::PerformAction(ParentGraph, FromPin, Location, bSelectNewNode));
...
/////////////////////////////////////////////////////
// UAnimGraphNode_PoseMatchingSequencePlayer

UAnimGraphNode_PoseMatchingSequencePlayer::UAnimGraphNode_PoseMatchingSequencePlayer(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UAnimGraphNode_PoseMatchingSequencePlayer::PreloadRequiredAssets()
{
...
FLinearColor UAnimGraphNode_PoseMatchingSequencePlayer::GetNodeTitleColor() const
{
...
FText UAnimGraphNode_PoseMatchingSequencePlayer::GetTooltipText() const
{
	if (!Node.Sequence)
	{
		return FText();
...
FText UAnimGraphNode_PoseMatchingSequencePlayer::GetNodeTitleForSequence(ENodeTitleType::Type TitleType, UAnimSequenceBase* InSequence) const
{
...
FText UAnimGraphNode_PoseMatchingSequencePlayer::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	if (Node.Sequence == nullptr)
	{
		// we may have a valid variable connected or default pin value
		UEdGraphPin* SequencePin = FindPin(GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_PoseMatchingSequencePlayer, Sequence));
		if (SequencePin && SequencePin->LinkedTo.Num() > 0)
		{
			return LOCTEXT("SequenceNodeTitleVariable", "Play Animation Sequence (pose matching)");
		}
		...
		{
			return LOCTEXT("SequenceNullTitle", "Play (None) (pose matching)");
		}
	}
	else
	{
...
FText UAnimGraphNode_PoseMatchingSequencePlayer::GetTitleGivenAssetInfo(const FText& AssetName, bool bKnownToBeAdditive)
{
	FFormatNamedArguments Args;
	...
	{
		return FText::Format(LOCTEXT("SequenceNodeTitleAdditive", "Play {AssetName} (additive, pose matching)"), Args);
	}
	else
	{
		return FText::Format(LOCTEXT("SequenceNodeTitle", "Play {AssetName} (pose matching)"), Args);
	}
}

FText UAnimGraphNode_PoseMatchingSequencePlayer::GetMenuCategory() const
{
	return FEditorCategoryUtils::GetCommonCategory(FCommonEditorCategory::Animation);
}

void UAnimGraphNode_PoseMatchingSequencePlayer::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
	auto LoadedAssetSetup = [](UEdGraphNode* NewNode, bool /*bIsTemplateNode*/, TWeakObjectPtr<UAnimSequence> SequencePtr)
	{
		UAnimGraphNode_PoseMatchingSequencePlayer* SequencePlayerNode = CastChecked<UAnimGraphNode_PoseMatchingSequencePlayer>(NewNode);
...
bool UAnimGraphNode_PoseMatchingSequencePlayer::IsActionFilteredOut(class FBlueprintActionFilter const& Filter)
{
...
EAnimAssetHandlerType UAnimGraphNode_PoseMatchingSequencePlayer::SupportsAssetClass(const UClass* AssetClass) const
{
...
void UAnimGraphNode_PoseMatchingSequencePlayer::ValidateAnimNodeDuringCompilation(class USkeleton* ForSkeleton, class FCompilerResultsLog& MessageLog)
{
...
void UAnimGraphNode_PoseMatchingSequencePlayer::GetNodeContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
...
void UAnimGraphNode_PoseMatchingSequencePlayer::SetAnimationAsset(UAnimationAsset* Asset)
{
...
void UAnimGraphNode_PoseMatchingSequencePlayer::ReplaceReferredAnimations(const TMap<UAnimationAsset*, UAnimationAsset*>& AnimAssetReplacementMap)
{
	HandleAnimReferenceReplacement(Node.Sequence, AnimAssetReplacementMap);
}


bool UAnimGraphNode_PoseMatchingSequencePlayer::DoesSupportTimeForTransitionGetter() const
{
	return true;
}

UAnimationAsset* UAnimGraphNode_PoseMatchingSequencePlayer::GetAnimationAsset() const 
{
...
const TCHAR* UAnimGraphNode_PoseMatchingSequencePlayer::GetTimePropertyName() const 
{
	return TEXT("InternalTimeAccumulator");
}

UScriptStruct* UAnimGraphNode_PoseMatchingSequencePlayer::GetTimePropertyStruct() const 
{
	return FAnimNode_PoseMatchingSequencePlayer::StaticStruct();
}

void UAnimGraphNode_PoseMatchingSequencePlayer::CustomizePinData(UEdGraphPin* Pin, FName SourcePropertyName, int32 ArrayIndex) const
{
	Super::CustomizePinData(Pin, SourcePropertyName, ArrayIndex);

	if (Pin->PinName == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_PoseMatchingSequencePlayer, PlayRate))
	{
		if (!Pin->bHidden)
		{
			// Draw value for PlayRateBasis if the pin is not exposed
			UEdGraphPin* PlayRateBasisPin = FindPin(GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_PoseMatchingSequencePlayer, PlayRateBasis));
			if (!PlayRateBasisPin || PlayRateBasisPin->bHidden)
			{
				if (Node.PlayRateBasis != 1.f)
				{
					...
					Pin->PinFriendlyName = FText::Format(LOCTEXT("FAnimNode_PoseMatchingSequencePlayer_PlayRateBasis_Value", "({PinFriendlyName} / {PlayRateBasis})"), Args);
				}
			}
			else // PlayRateBasisPin is visible
			{
				FFormatNamedArguments Args;
				Args.Add(TEXT("PinFriendlyName"), Pin->PinFriendlyName);
				Pin->PinFriendlyName = FText::Format(LOCTEXT("FAnimNode_PoseMatchingSequencePlayer_PlayRateBasis_Name", "({PinFriendlyName} / PlayRateBasis)"), Args);
			}
...
void UAnimGraphNode_PoseMatchingSequencePlayer::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	const FName PropertyName = (PropertyChangedEvent.Property ? PropertyChangedEvent.Property->GetFName() : NAME_None);

	// Reconstruct node to show updates to PinFriendlyNames.
	if ((PropertyName == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_PoseMatchingSequencePlayer, PlayRateBasis))
