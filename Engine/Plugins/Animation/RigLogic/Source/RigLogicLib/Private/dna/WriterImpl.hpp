...
#endif
template<class TWriterBase>
void WriterImpl<TWriterBase>::setName(const char* name) {
    dna.descriptor.name = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setArchetype(Archetype archetype) {
    dna.descriptor.archetype = static_cast<std::uint16_t>(archetype);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGender(Gender gender) {
    dna.descriptor.gender = static_cast<std::uint16_t>(gender);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAge(std::uint16_t age) {
    dna.descriptor.age = age;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setMetaData(const char* key, const char* value) {
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setTranslationUnit(TranslationUnit unit) {
    dna.descriptor.translationUnit = static_cast<std::uint16_t>(unit);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setRotationUnit(RotationUnit unit) {
    dna.descriptor.rotationUnit = static_cast<std::uint16_t>(unit);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setCoordinateSystem(CoordinateSystem system) {
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setLODCount(std::uint16_t lodCount) {
    dna.descriptor.lodCount = lodCount;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setDBMaxLOD(std::uint16_t lod) {
    dna.descriptor.maxLOD = lod;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setDBComplexity(const char* name) {
    dna.descriptor.complexity = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setDBName(const char* name) {
    dna.descriptor.dbName = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGUIControlName(std::uint16_t index, const char* name) {
    ensureHasSize(dna.definition.guiControlNames, index + 1ul, memRes);
    dna.definition.guiControlNames[index] = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setRawControlName(std::uint16_t index, const char* name) {
    ensureHasSize(dna.definition.rawControlNames, index + 1ul, memRes);
    dna.definition.rawControlNames[index] = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointName(std::uint16_t index, const char* name) {
    ensureHasSize(dna.definition.jointNames, index + 1ul, memRes);
    dna.definition.jointNames[index] = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointIndices(std::uint16_t index, const std::uint16_t* jointIndices, std::uint16_t count) {
    dna.definition.lodJointMapping.clearIndices(index);
    dna.definition.lodJointMapping.addIndices(index, jointIndices, count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setLODJointMapping(std::uint16_t lod, std::uint16_t index) {
    dna.definition.lodJointMapping.associateLODWithIndices(lod, index);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointHierarchy(const std::uint16_t* jointIndices, std::uint16_t count) {
    dna.definition.jointHierarchy.assign(jointIndices, jointIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeChannelName(std::uint16_t index, const char* name) {
    ensureHasSize(dna.definition.blendShapeChannelNames, index + 1ul, memRes);
    dna.definition.blendShapeChannelNames[index] = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeChannelIndices(std::uint16_t index,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setLODBlendShapeChannelMapping(std::uint16_t lod, std::uint16_t index) {
    dna.definition.lodBlendShapeMapping.associateLODWithIndices(lod, index);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapName(std::uint16_t index, const char* name) {
    ensureHasSize(dna.definition.animatedMapNames, index + 1ul, memRes);
    dna.definition.animatedMapNames[index] = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapIndices(std::uint16_t index,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setLODAnimatedMapMapping(std::uint16_t lod, std::uint16_t index) {
    dna.definition.lodAnimatedMapMapping.associateLODWithIndices(lod, index);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setMeshName(std::uint16_t index, const char* name) {
    ensureHasSize(dna.definition.meshNames, index + 1ul, memRes);
    dna.definition.meshNames[index] = name;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setMeshIndices(std::uint16_t index, const std::uint16_t* meshIndices, std::uint16_t count) {
    dna.definition.lodMeshMapping.clearIndices(index);
    dna.definition.lodMeshMapping.addIndices(index, meshIndices, count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setLODMeshMapping(std::uint16_t lod, std::uint16_t index) {
    dna.definition.lodMeshMapping.associateLODWithIndices(lod, index);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::addMeshBlendShapeChannelMapping(std::uint16_t meshIndex, std::uint16_t blendShapeChannelIndex) {
    dna.definition.meshBlendShapeChannelMapping.add(meshIndex, blendShapeChannelIndex);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setNeutralJointTranslations(const Vector3* translations, std::uint16_t count) {
    dna.definition.neutralJointTranslations.assign(translations, translations + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setNeutralJointRotations(const Vector3* rotations, std::uint16_t count) {
    dna.definition.neutralJointRotations.assign(rotations, rotations + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGUIToRawInputIndices(const std::uint16_t* inputIndices, std::uint16_t count) {
    dna.behavior.controls.conditionals.inputIndices.assign(inputIndices, inputIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGUIToRawOutputIndices(const std::uint16_t* outputIndices, std::uint16_t count) {
    dna.behavior.controls.conditionals.outputIndices.assign(outputIndices, outputIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGUIToRawFromValues(const float* fromValues, std::uint16_t count) {
    dna.behavior.controls.conditionals.fromValues.assign(fromValues, fromValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGUIToRawToValues(const float* toValues, std::uint16_t count) {
    dna.behavior.controls.conditionals.toValues.assign(toValues, toValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGUIToRawSlopeValues(const float* slopeValues, std::uint16_t count) {
    dna.behavior.controls.conditionals.slopeValues.assign(slopeValues, slopeValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setGUIToRawCutValues(const float* cutValues, std::uint16_t count) {
    dna.behavior.controls.conditionals.cutValues.assign(cutValues, cutValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setPSDCount(std::uint16_t count) {
    dna.behavior.controls.psdCount = count;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setPSDRowIndices(const std::uint16_t* rowIndices, std::uint16_t count) {
    dna.behavior.controls.psds.rows.assign(rowIndices, rowIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setPSDColumnIndices(const std::uint16_t* columnIndices, std::uint16_t count) {
    dna.behavior.controls.psds.columns.assign(columnIndices, columnIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setPSDValues(const float* weights, std::uint16_t count) {
    dna.behavior.controls.psds.values.assign(weights, weights + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointRowCount(std::uint16_t rowCount) {
    dna.behavior.joints.rowCount = rowCount;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointColumnCount(std::uint16_t columnCount) {
    dna.behavior.joints.colCount = columnCount;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointGroupLODs(std::uint16_t jointGroupIndex, const std::uint16_t* lods, std::uint16_t count) {
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointGroupInputIndices(std::uint16_t jointGroupIndex,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointGroupOutputIndices(std::uint16_t jointGroupIndex,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointGroupValues(std::uint16_t jointGroupIndex, const float* values, std::uint32_t count) {
    auto& jointGroups = dna.behavior.joints.jointGroups;
    ensureHasSize(jointGroups, jointGroupIndex + 1ul, memRes);
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setJointGroupJointIndices(std::uint16_t jointGroupIndex,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeChannelLODs(const std::uint16_t* lods, std::uint16_t count) {
    dna.behavior.blendShapeChannels.lods.assign(lods, lods + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeChannelInputIndices(const std::uint16_t* inputIndices, std::uint16_t count) {
    dna.behavior.blendShapeChannels.inputIndices.assign(inputIndices, inputIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeChannelOutputIndices(const std::uint16_t* outputIndices, std::uint16_t count) {
    dna.behavior.blendShapeChannels.outputIndices.assign(outputIndices, outputIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapLODs(const std::uint16_t* lods, std::uint16_t count) {
    dna.behavior.animatedMaps.lods.assign(lods, lods + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapInputIndices(const std::uint16_t* inputIndices, std::uint16_t count) {
    dna.behavior.animatedMaps.conditionals.inputIndices.assign(inputIndices, inputIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapOutputIndices(const std::uint16_t* outputIndices, std::uint16_t count) {
    dna.behavior.animatedMaps.conditionals.outputIndices.assign(outputIndices, outputIndices + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapFromValues(const float* fromValues, std::uint16_t count) {
    dna.behavior.animatedMaps.conditionals.fromValues.assign(fromValues, fromValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapToValues(const float* toValues, std::uint16_t count) {
    dna.behavior.animatedMaps.conditionals.toValues.assign(toValues, toValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapSlopeValues(const float* slopeValues, std::uint16_t count) {
    dna.behavior.animatedMaps.conditionals.slopeValues.assign(slopeValues, slopeValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setAnimatedMapCutValues(const float* cutValues, std::uint16_t count) {
    dna.behavior.animatedMaps.conditionals.cutValues.assign(cutValues, cutValues + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setVertexPositions(std::uint16_t meshIndex, const Position* positions, std::uint32_t count) {
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setVertexTextureCoordinates(std::uint16_t meshIndex,
                                                          const TextureCoordinate* textureCoordinates,
                                                          std::uint32_t count) {
    ensureHasSize(dna.geometry.meshes, meshIndex + 1ul, memRes);
    auto& destination = dna.geometry.meshes[meshIndex].textureCoordinates;
    destination.clear();
    destination.us.reserve(count);
    destination.vs.reserve(count);
    for (std::size_t i = 0ul; i < count; ++i) {
        destination.us.push_back(textureCoordinates[i].u);
        destination.vs.push_back(textureCoordinates[i].v);
    }
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setVertexNormals(std::uint16_t meshIndex, const Normal* normals, std::uint32_t count) {
    ensureHasSize(dna.geometry.meshes, meshIndex + 1ul, memRes);
    dna.geometry.meshes[meshIndex].normals.assign(normals, normals + count);
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setVertexLayouts(std::uint16_t meshIndex, const VertexLayout* layouts, std::uint32_t count) {
    ensureHasSize(dna.geometry.meshes, meshIndex + 1ul, memRes);
    auto& destination = dna.geometry.meshes[meshIndex].layouts;
    destination.clear();
    destination.positions.reserve(count);
    destination.textureCoordinates.reserve(count);
    destination.normals.reserve(count);
    for (std::size_t i = 0ul; i < count; ++i) {
        destination.positions.push_back(layouts[i].position);
        destination.textureCoordinates.push_back(layouts[i].textureCoordinate);
        destination.normals.push_back(layouts[i].normal);
    }
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setFaceVertexLayoutIndices(std::uint16_t meshIndex,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setMaximumInfluencePerVertex(std::uint16_t meshIndex, std::uint16_t maxInfluenceCount) {
    ensureHasSize(dna.geometry.meshes, meshIndex + 1ul, memRes);
    dna.geometry.meshes[meshIndex].maximumInfluencePerVertex = maxInfluenceCount;
}

template<class TWriterBase>
void WriterImpl<TWriterBase>::setSkinWeightsValues(std::uint16_t meshIndex,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setSkinWeightsJointIndices(std::uint16_t meshIndex,
                                                         std::uint32_t vertexIndex,
                                                         const std::uint16_t* jointIndices,
                                                         std::uint16_t count) {
    ensureHasSize(dna.geometry.meshes, meshIndex + 1ul, memRes);
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeChannelIndex(std::uint16_t meshIndex,
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeTargetDeltas(std::uint16_t meshIndex,
                                                        std::uint16_t blendShapeTargetIndex,
                                                        const Delta* deltas,
                                                        std::uint32_t count) {
    ensureHasSize(dna.geometry.meshes, meshIndex + 1ul, memRes);
...
template<class TWriterBase>
void WriterImpl<TWriterBase>::setBlendShapeTargetVertexIndices(std::uint16_t meshIndex,
                                                               std::uint16_t blendShapeTargetIndex,
