...
class FilteredDNAInputArchiveTest : public ::testing::TestWithParam<LODConstraint> {
    protected:
        void SetUp() override {
            ...
            dna::FilteredInputArchive archive{&stream, dna::DataLayer::All, lodConstraint.maxLOD, lodConstraint.minLOD, &amr};
            archive >> *dnaInstance;
        }
...
TEST_P(FilteredDNAInputArchiveTest, FilterJoints) {
    ...
    for (std::size_t jointGroupIdx = 0ul; jointGroupIdx < expected.jointGroups.size(); ++jointGroupIdx) {
        ...
        ASSERT_EQ(jntGrp.lods, expectedJntGrp.lods);
        ASSERT_EQ(jntGrp.inputIndices, expectedJntGrp.inputIndices);
        ASSERT_EQ(jntGrp.outputIndices, expectedJntGrp.outputIndices);
        ASSERT_ELEMENTS_NEAR(jntGrp.values, expectedJntGrp.values, 0.005f);
        ASSERT_EQ(jntGrp.jointIndices, expectedJntGrp.jointIndices);
    }
}
...
TEST_P(FilteredDNAInputArchiveTest, FilterBlendShapes) {
    ...
    ASSERT_EQ(result.inputIndices, expected.inputIndices);
    ASSERT_EQ(result.outputIndices, expected.outputIndices);
    ASSERT_EQ(result.lods, expected.lods);
}
...
TEST_P(FilteredDNAInputArchiveTest, FilterAnimatedMaps) {
    ...
    ASSERT_EQ(result.lods, expected.lods);
    ASSERT_EQ(result.conditionals.inputIndices, expected.conditionals.inputIndices);
    ASSERT_EQ(result.conditionals.outputIndices, expected.conditionals.outputIndices);
    ASSERT_ELEMENTS_NEAR(result.conditionals.fromValues, expected.conditionals.fromValues, 0.005f);
    ASSERT_ELEMENTS_NEAR(result.conditionals.toValues, expected.conditionals.toValues, 0.005f);
    ASSERT_ELEMENTS_NEAR(result.conditionals.slopeValues, expected.conditionals.slopeValues, 0.005f);
    ASSERT_ELEMENTS_NEAR(result.conditionals.cutValues, expected.conditionals.cutValues, 0.005f);
}
...
TEST_P(FilteredDNAInputArchiveTest, FilterDefinition) {
    ...
    ASSERT_EQ(result.jointHierarchy, dna::decoded::jointHierarchy[index]);
...
TEST_F(GeometryFilteringTest, IgnoreBlendShapeTargets) {
    dna::FilteredInputArchive archive{&stream, dna::DataLayer::GeometryWithoutBlendShapes, 0u,
