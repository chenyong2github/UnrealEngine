...
#include "MassMovementTestingActor.h"
...
//////////////////////////////////////////////////////////////////////////
// FMassMovementTestingSceneProxy
FMassMovementTestingSceneProxy::FMassMovementTestingSceneProxy(const UPrimitiveComponent& InComponent)
	: FDebugRenderSceneProxy(&InComponent)
{
	DrawType = WireMesh;
}

SIZE_T FMassMovementTestingSceneProxy::GetTypeHash() const
{
	static size_t UniquePointer;
	return reinterpret_cast<size_t>(&UniquePointer);
}

FPrimitiveViewRelevance FMassMovementTestingSceneProxy::GetViewRelevance(const FSceneView* View) const
{
...
uint32 FMassMovementTestingSceneProxy::GetMemoryFootprint(void) const
{
...
UMassMovementTestingComponent::UMassMovementTestingComponent(const FObjectInitializer& ObjectInitialize)
...
#if WITH_EDITOR
void UMassMovementTestingComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UMassMovementTestingComponent::OnRegister()
{
	Super::OnRegister();
#if WITH_EDITOR
	// Force to update tests when ever the data changes.
	OnDataChangedHandle = UE::ZoneGraphDelegates::OnZoneGraphDataBuildDone.AddUObject(this, &UMassMovementTestingComponent::OnZoneGraphDataBuildDone);
#endif
	OnDataAddedHandle = UE::ZoneGraphDelegates::OnPostZoneGraphDataAdded.AddUObject(this, &UMassMovementTestingComponent::OnZoneGraphDataChanged);
	OnDataRemovedHandle = UE::ZoneGraphDelegates::OnPreZoneGraphDataRemoved.AddUObject(this, &UMassMovementTestingComponent::OnZoneGraphDataChanged);
...
void UMassMovementTestingComponent::OnUnregister()
{
	Super::OnUnregister();
#if WITH_EDITOR
...
void UMassMovementTestingComponent::OnZoneGraphDataChanged(const AZoneGraphData* ZoneGraphData)
{
...
#if WITH_EDITOR
void UMassMovementTestingComponent::OnZoneGraphDataBuildDone(const struct FZoneGraphBuildData& BuildData)
{
	UpdateTests();
}
#endif

FBoxSphereBounds UMassMovementTestingComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	const FVector ActorPosition = LocalToWorld.GetTranslation();
	return FBox(ActorPosition - SearchExtent, ActorPosition + SearchExtent);
}

void UMassMovementTestingComponent::UpdateTests()
{
	if (!ZoneGraph)
	{
		return;
...
void UMassMovementTestingComponent::PinLane()
{
	if (LaneLocation.IsValid())
	{
...
void UMassMovementTestingComponent::ClearPinnedLane()
{
	PinnedLane.Reset();
	UpdateTests();
}
...
AMassMovementTestingActor::AMassMovementTestingActor(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	DebugComp = CreateDefaultSubobject<UMassMovementTestingComponent>(TEXT("DebugComp"));
...
#if WITH_EDITOR
void AMassMovementTestingActor::PostEditMove(bool bFinished)
{
	if (DebugComp)
	{
...
void AMassMovementTestingActor::PinLane()
{
	if (DebugComp)
	{
		DebugComp->PinLane();
...
void AMassMovementTestingActor::ClearPinnedLane()
{
	if (DebugComp)
	{
