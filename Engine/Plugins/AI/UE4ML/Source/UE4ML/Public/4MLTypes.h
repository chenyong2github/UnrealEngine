...
DECLARE_LOG_CATEGORY_EXTERN(LogUnrealEditor4ML, Log, All);
...
namespace F4ML
{
	...
	/** UE4ML-flavored new object creation. We're using this with all the objects 
...
struct F4MLMemoryWriter : FMemoryWriter
{
	F4MLMemoryWriter(TArray<uint8>& InBytes, bool bIsPersistent = false, bool bSetOffset = false, const FName InArchiveName = NAME_None)
...
struct F4MLMemoryReader : FMemoryReader
{
	F4MLMemoryReader(TArray<uint8>& InBytes, bool bIsPersistent = false)
