...
U4MLSensor_Camera::U4MLSensor_Camera(const FObjectInitializer& ObjectInitializer)
...
bool U4MLSensor_Camera::ConfigureForAgent(U4MLAgent& Agent)
{
	if (Agent.GetAvatar())
	{
...
void U4MLSensor_Camera::Configure(const TMap<FName, FString>& Params)
{
...
TSharedPtr<F4ML::FSpace> U4MLSensor_Camera::ConstructSpaceDef() const
{
	return MakeShareable(new F4ML::FSpace_Box({ Width, Height, 4 }, 0, 255));
}

void U4MLSensor_Camera::HandleScreenshotData(int32 InSizeX, int32 InSizeY, const TArray<FColor>& InImageData)
{
	FScopeLock Lock(&ObservationCS);
	LastTickData.Reset(Width * Height);
}

void U4MLSensor_Camera::SenseImpl(const float DeltaTime)
{
...
void U4MLSensor_Camera::OnAvatarSet(AActor* Avatar)
{
	if (Avatar == nullptr)
	{
		if (CaptureComp)
	...
	if (AsPC)
	{
		...
		{
			ScreenshotDataCallbackHandle = LocalPlayer->ViewportClient->OnScreenshotCaptured().AddUObject(this, &U4MLSensor_Camera::HandleScreenshotData);
...
void U4MLSensor_Camera::ClearPawn(APawn& InPawn)
{
	CaptureComp = nullptr;
}

void U4MLSensor_Camera::GetObservations(F4MLMemoryWriter& Ar)
{
