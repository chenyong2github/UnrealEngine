...
def set_debug(debug):
    ...
    __DEBUG = debug
    if __DEBUG:
        EXEC_NAME = 'UE4Editor-Win64-Debug.exe'
    else:
        EXEC_NAME = 'UE4Editor.exe'
...
class UE4Runner:
    @staticmethod
    def run(project_name, ue4params, server_port):
        ue4params.add_param('4mlport', '{}'.format(server_port))
        if is_debug():
            ue4params.add_option('debug')
        commandline = '{} {} {}'.format(_get_exec_path(), project_name, ue4params.as_command_line())
