...
class SpaceBuildingTest(unittest.TestCase):
    def test_invalid_json(self): 
        self.assertIsNone(spaces.gym_space_from_ue4ml(''))
        self.assertIsNone(spaces.gym_space_from_ue4ml(None))
        with self.assertRaises(json.JSONDecodeError):
            spaces.gym_space_from_ue4ml('notjson')
    ...
    def test_space_creation(self):
        ...
        for sample in json_samples:    
            space = spaces.gym_space_from_ue4ml(sample[0])
