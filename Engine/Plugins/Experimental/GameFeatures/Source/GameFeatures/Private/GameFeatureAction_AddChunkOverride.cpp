...
int32 UGameFeatureAction_AddChunkOverride::GenerateUniqueChunkId() const
{
	...
	if (NewChunkId < LowestAllowedChunkIndexForAutoGeneration)
	{
		...
		FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("GameFeatureAction_AddChunkOverride", "Autogenerated ChunkID is lower than config specified LowestAllowedChunkIndexForAutoGeneration. Please manually assign a valid Chunk Id"));
		NewChunkId = -1;
	}
	else if (GameFeatureAction_AddChunkOverride::ChunkIdToPluginMap.Contains(NewChunkId))
	{
		...
		FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("GameFeatureAction_AddChunkOverride", "Unable to auto generate unique valid Chunk Id. Please manually assign a valid Chunk Id"));
		NewChunkId = -1;
	}
