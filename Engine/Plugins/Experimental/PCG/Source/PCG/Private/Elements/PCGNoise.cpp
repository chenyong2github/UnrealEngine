...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGNoise)

#define LOCTEXT_NAMESPACE "PCGNoise"

TArray<FPCGPinProperties> UPCGNoiseSettings::InputPinProperties() const
{
	return Super::DefaultPointInputPinProperties();
}

TArray<FPCGPinProperties> UPCGNoiseSettings::OutputPinProperties() const
{
	return Super::DefaultPointOutputPinProperties();
}

FPCGElementPtr UPCGNoiseSettings::CreateElement() const
{
	return MakeShared<FPCGNoise>();
}
...
namespace PCGNoise
{
	...
	void DoFractal2D(const FSharedParams& SharedParams, const FBufferParams& BufferParams, const UPCGNoiseSettings& Settings, FractalNoiseFunc&& FractalNoise)
	{
	...
	template<typename FractalNoiseFunc>
	void DoEdgeMask2D(const FSharedParams& SharedParams, const FBufferParams& BufferParams, const UPCGNoiseSettings& Settings, FractalNoiseFunc&& FractalNoise)
	{
		const TArray<FPCGPoint>& SrcPoints = BufferParams.InputPointData->GetPoints();
	...
	void DoVoronoi2DInternal(const FSharedParams& SharedParams, const FBufferParams& BufferParams, const UPCGNoiseSettings& Settings, VoronoiHashFunc&& HashFunc)
	{
		const TArray<FPCGPoint>& SrcPoints = BufferParams.InputPointData->GetPoints();
	...
	void DoVoronoi2D(const FSharedParams& SharedParams, const FBufferParams& BufferParams, const UPCGNoiseSettings& Settings)
	{
		if (SharedParams.bTiling)
		{
			/**
...
bool FPCGNoise::ExecuteInternal(FPCGContext* Context) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGNoise::Execute);

	const UPCGNoiseSettings* Settings = Context->GetInputSettings<UPCGNoiseSettings>();
	check(Settings);
	...
	PCGNoise::FSharedParams SharedParams;
	...
	SharedParams.Transform = Settings->Transform * FTransform(FRotator::ZeroRotator, RandomOffset, FVector(PCGNoise::MAGIC_SCALE_FACTOR));
	...
	{
		PCGNoise::FBufferParams BufferParams;
		...
		{
		case PCGNoiseMode::Caustic2D:
			{
				DoFractal2D(SharedParams, BufferParams, *Settings, &PCGNoise::CalcCaustic2D);
			}
			break;

		case PCGNoiseMode::Perlin2D:
			{
				DoFractal2D(SharedParams, BufferParams, *Settings, &PCGNoise::CalcPerlin2D);
			}
			break;

		case PCGNoiseMode::FractionalBrownian2D:
			{
				DoFractal2D(SharedParams, BufferParams, *Settings, &PCGNoise::CalcFractionalBrownian2D);
			}
			break;

		case PCGNoiseMode::Voronoi2D:
			{
				DoVoronoi2D(SharedParams, BufferParams, *Settings);
			}
			break;

		case PCGNoiseMode::EdgeMask2D:
			{
				switch (Settings->EdgeMask2DMode)
				{
				case PCGNoiseMask2DMode::Perlin:
					DoEdgeMask2D(SharedParams, BufferParams, *Settings, &PCGNoise::CalcPerlin2D);
					break;

				case PCGNoiseMask2DMode::Caustic:
					DoEdgeMask2D(SharedParams, BufferParams, *Settings, &PCGNoise::CalcCaustic2D);
					break;

				case PCGNoiseMask2DMode::FractionalBrownian:
					DoEdgeMask2D(SharedParams, BufferParams, *Settings, &PCGNoise::CalcFractionalBrownian2D);
					break;
