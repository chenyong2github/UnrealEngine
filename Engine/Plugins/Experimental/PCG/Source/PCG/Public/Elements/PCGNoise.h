...
#include "PCGNoise.generated.h"

UENUM()
enum class PCGNoiseMode
{
...
class UPCGNoiseSettings : public UPCGSettings
{
	GENERATED_BODY()
...
#if WITH_EDITOR
	virtual FName GetDefaultNodeName() const override { return FName(TEXT("Noise")); }
	virtual FText GetDefaultNodeTitle() const override { return NSLOCTEXT("PCGNoise", "NodeTitle", "Noise"); }
...
public:
	...
	PCGNoiseMode Mode = PCGNoiseMode::Perlin2D;

	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Mode == PCGNoiseMode::EdgeMask2D", EditConditionHides, PCG_Overridable))
	PCGNoiseMask2DMode EdgeMask2DMode = PCGNoiseMask2DMode::Perlin;

	// this is how many times the fractal method recurses. A higher number will mean more detail
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (ClampMin = "1", ClampMax = "100", EditCondition = "Mode != PCGNoiseMode::Voronoi2D", EditConditionHides, PCG_Overridable))
	int32 Iterations = 4;
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Mode != PCGNoiseMode::Voronoi2D || !bTiling", EditConditionHides))
	FTransform Transform = FTransform::Identity;

	// the less random this is, the more it returns to being a grid
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (ClampMin = "0.0", ClampMax = "1.0", EditCondition = "Mode == PCGNoiseMode::Voronoi2D", EditConditionHides, PCG_Overridable))
	double VoronoiCellRandomness = 1.0;

	// The output attribute name to write the voronoi cell id, if not 'None'
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Mode == PCGNoiseMode::Voronoi2D", EditConditionHides, PCG_Overridable))
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Mode == PCGNoiseMode::Voronoi2D && bTiling", EditConditionHides, PCG_Overridable, ClampMin = "1"))
	int32 TiledVoronoiResolution = 8;

	// how many cells around the edge will tile
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Mode == PCGNoiseMode::Voronoi2D && bTiling", EditConditionHides, PCG_Overridable))
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Mode == PCGNoiseMode::EdgeMask2D", EditConditionHides, ClampMin = "0", PCG_Overridable))
...
class FPCGNoise : public FPCGPointProcessingElementBase
{
protected:
