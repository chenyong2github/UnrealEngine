...
)
{
	...
	if (ensure(FullMesh.HasAttributes() && FullMesh.Attributes()->NumUVLayers() >= 1 && FullMesh.Attributes()->NumNormalLayers() == 3))
	{
		...
		FDynamicMeshNormalOverlay* N = Attribs.PrimaryNormals();
		FDynamicMeshUVOverlay* UV = Attribs.PrimaryUV();
		TMap<FIndex2i, int> ElIDsToVID;
		...
		for (int VID = 0; VID < OrigMaxVID; VID++)
		{
			...
			ElIDsToVID.Reset();
			FullMesh.EnumerateVertexTriangles(VID, [&FullMesh, &Triangles, &OutMesh, &N, &UV, &ElIDsToVID, Pos, VID](int32 TID)
			{
				FIndex3i InTri = FullMesh.GetTriangle(TID);
				int VOnT = IndexUtil::FindTriIndex(VID, InTri);
				FIndex2i ElIDs(
					N->GetTriangle(TID)[VOnT],
				...
				{
					FVector3f Normal = N->GetElement(ElIDs.A);
					FVertexInfo Info(Pos, Normal, FVector3f(1, 1, 1), UV->GetElement(ElIDs.B));

					int OutVID = OutMesh.AppendVertex(Info);
					OutTri[VOnT] = OutVID;
					FVector3f T, B;
					VectorUtil::MakePerpVectors(Normal, T, B); // just put arbitrary tangents, because we haven't loaded the tangents from the mesh
					UE::PlanarCutInternals::AugmentDynamicMesh::SetTangent(OutMesh, OutVID, Normal, T, B);
