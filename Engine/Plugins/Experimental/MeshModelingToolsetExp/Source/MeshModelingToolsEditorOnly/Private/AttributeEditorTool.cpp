...
void UAttributeEditorTool::InitializeAttributeLists()
{
	...
	TArray<TArray<FAttributeEditorAttribInfo>*> AttribInfos = {
		&this->VertexAttributes, &this->PolygonAttributes, &this->TriangleAttributes };
...
void UAttributeEditorTool::ClearNormals()
{
	...
	{
		Cast<IMeshDescriptionCommitter>(Targets[ComponentIdx])->CommitMeshDescription([&](const IMeshDescriptionCommitter::FCommitterParams& CommitParams)
		{
...
void UAttributeEditorTool::ResetLightmapUVsChannels()
{
	...
	{
		TVertexInstanceAttributesConstRef<FVector2f> InstanceUVs = 
			Cast<IMeshDescriptionProvider>(Targets[ComponentIdx])->GetMeshDescription()->VertexInstanceAttributes().GetAttributesRef<FVector2f>(MeshAttribute::VertexInstance::TextureCoordinate);
