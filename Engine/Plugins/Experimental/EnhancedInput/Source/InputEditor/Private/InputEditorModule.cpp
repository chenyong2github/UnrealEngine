...
private:
	...
	void OnAssetDefaultsRequireUpdate(const FAssetData& AssetData);
	void OnAssetRenamed(const FAssetData& AssetData, const FString&);

	template<typename T>
	TSharedPtr<IDetailsView> AddClassDetailsView();
	template<typename T>
	void  UpdateClassDetailsView(TSharedPtr<IDetailsView>& DetailsView);

	static EAssetTypeCategories::Type InputAssetsCategory;

	TArray< TSharedPtr<IAssetTypeActions> > CreatedAssetTypeActions;

	TSharedPtr<IDetailsView> TriggerDetailsView;
	TSharedPtr<IDetailsView> ModifierDetailsView;
...
TSharedPtr<IDetailsView> FInputEditorModule::AddClassDetailsView()
{
	...
	UpdateClassDetailsView<T>(DetailsView);

	return DetailsView;
}

template<typename T>
void  FInputEditorModule::UpdateClassDetailsView(TSharedPtr<IDetailsView>& DetailsView)
{
	TArray<UObject*> CDOs;
	...
	{
		if (!ClassIt->IsNative() || !ClassIt->IsChildOf(T::StaticClass()))
		{
			continue;
		}
	...
	for (FAssetData& Asset : BlueprintAssetData)
	{
		...
		if (Result.IsSet())
		{
			...
			{
				if (ParentClass->IsChildOf(T::StaticClass()))
				{
	...
	CDOs.RemoveAll([](UObject* Object) {
		UClass* BaseClass = Object->GetClass();
		if (BaseClass->GetMetaData(TEXT("NotInputConfigurable")).ToBool())
		{
			return true;
		}
		while (BaseClass)
	...
	DetailsView->SetObjects(CDOs);
}

TSharedRef<SWidget> FInputEditorModule::CreateSettingsPanel()
{
	TriggerDetailsView = AddClassDetailsView<UInputTrigger>();
	ModifierDetailsView = AddClassDetailsView<UInputModifier>();
...
void FInputEditorModule::StartupModule()
{
	...
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(FName("AssetRegistry"));
	IAssetRegistry& AssetRegistry = AssetRegistryModule.Get();
	AssetRegistry.OnAssetAdded().AddRaw(this, &FInputEditorModule::OnAssetDefaultsRequireUpdate);
	AssetRegistry.OnAssetRemoved().AddRaw(this, &FInputEditorModule::OnAssetDefaultsRequireUpdate);
...
void FInputEditorModule::OnAssetDefaultsRequireUpdate(const FAssetData& AssetData)
{
	...
	if (Result.IsSet())
	{
		...
		if (UClass* ParentClass = FindObjectSafe<UClass>(ANY_PACKAGE, *ClassName, true))
		{
			...
			{
				UpdateClassDetailsView<UInputTrigger>(TriggerDetailsView);
			}
			else if (ParentClass->IsChildOf(UInputModifier::StaticClass()))
			{
				UpdateClassDetailsView<UInputModifier>(ModifierDetailsView);
			}
		}
	}
}

void FInputEditorModule::OnAssetRenamed(const FAssetData& AssetData, const FString& OldObjectPath)
{
	OnAssetDefaultsRequireUpdate(AssetData);
}
