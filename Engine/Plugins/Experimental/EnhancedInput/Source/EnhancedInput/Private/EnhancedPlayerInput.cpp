...
FInputActionInstance& UEnhancedPlayerInput::ProcessActionValue(const UInputAction* Action, float DeltaTime, bool bGamePaused, FInputActionValue RawKeyValue, EKeyEvent KeyEvent, ETriggerState& LastTriggerState, const TArray<UInputModifier*>& Modifiers, const TArray<UInputTrigger*>& Triggers)
{
	...
	//UE_CLOG(TriggerState != ETriggerState::None || LastTriggerState != TriggerState, LogTemp, Warning, TEXT("%s %s - Got trigger state %s - trigger event %s : %.2f"), *Mapping.Key.ToString(), *Mapping.Action.ToString(), *UEnum::GetValueAsString(TEXT("EnhancedInput.ETriggerState"), TriggerState), *UEnum::GetValueAsString(TEXT("EnhancedInput.ETriggerEvent"), TriggerEvent), ModifiedValue.Get<float>());
...
void UEnhancedPlayerInput::InitializeMappingActionModifiers(const FEnhancedActionKeyMapping& Mapping)
{
	...
	for (UInputModifier* Modifier : Mapping.Modifiers)
	{
		...
		checkf(Modifier->GetExecutionPhase() != EModifierExecutionPhase::FinalValue, TEXT("Mapping '%s:%s' is applying a final value modifier '%s', which is only supported on actions."), *Mapping.Action->GetName(), *Mapping.Key.GetFName().ToString(), *Modifier->GetName());
	}
#endif
