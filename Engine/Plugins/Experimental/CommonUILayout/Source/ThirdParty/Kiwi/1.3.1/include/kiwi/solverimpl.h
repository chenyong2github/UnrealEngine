...
public:
	...
	*/
	bool addConstraint( const Constraint& constraint )
	{
		if( m_cns.find( constraint ) != m_cns.end() )
			return false; //throw DuplicateConstraint( constraint );
		...
		if( subject.type() == Symbol::Invalid && allDummies( *rowptr ) )
		{
			if( !nearZero( rowptr->constant() ) )
				return false; //throw UnsatisfiableConstraint( constraint );
			else
				subject = tag.marker;
		}
		...
		return optimize( *m_objective ); //optimize( *m_objective );
	}
	...
	*/
	bool removeConstraint( const Constraint& constraint )
	{
		...
		if( cn_it == m_cns.end() )
			return false; //throw UnknownConstraint( constraint );
		...
		else
		{
			...
			if( row_it == m_rows.end() )
				return false; //throw InternalSolverError( "failed to find leaving row" );
	...
	*/
	bool addEditVariable( const Variable& variable, double strength )
	{
		if( m_edits.find( variable ) != m_edits.end() )
			return false; //throw DuplicateEditVariable( variable );
		strength = strength::clip( strength );
		if( strength == strength::required )
			return false; //throw BadRequiredStrength();
	...
	*/
	bool removeEditVariable( const Variable& variable )
	{
		auto it = m_edits.find( variable );
		if( it == m_edits.end() )
			return false; //throw UnknownEditVariable( variable );
	...
	*/
	bool suggestValue( const Variable& variable, double value )
	{
		auto it = m_edits.find( variable );
		if( it == m_edits.end() )
			return false; //throw UnknownEditVariable( variable );
	...
	*/
	void updateVariables()
	{
		...
		{
			Variable& var = const_cast<Variable&>(varPair.first);
...
private:
	...
	*/
	bool optimize( const Row& objective )
	{
		while( true )
		{
			...
			if( it == m_rows.end() )
				return false; //throw InternalSolverError( "The objective is unbounded." );
	...
	bool dualOptimize()
	{
		while( !m_infeasible_rows.empty() )
		{
				...
				it->second->constant() < 0.0 )
			{
				...
				if( entering.type() == Symbol::Invalid )
					return false; //throw InternalSolverError( "Dual optimize failed." );
				// pivot the entering symbol into the basis
				Row* row = it->second;
				m_rows.erase( it );
				row->solveFor( leaving, entering );
