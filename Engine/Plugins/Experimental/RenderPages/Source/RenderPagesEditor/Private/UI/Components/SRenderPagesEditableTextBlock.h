...
{
	DECLARE_DELEGATE_RetVal_TwoParams(FText, FOnRenderPagesEditableTextBlockTextCommitted, const FText&, ETextCommit::Type);

	/**
	 * A reusable text widget that can be modified by pressing a pencil button that's shown next to it.
	 */
	class SRenderPagesEditableTextBlock : public SCompoundWidget
	{
	public:
		SLATE_BEGIN_ARGS(SRenderPagesEditableTextBlock) {}
			SLATE_ATTRIBUTE(FText, Text)// The text displayed in this text block
			SLATE_EVENT(FOnRenderPagesEditableTextBlockTextCommitted, OnTextCommitted)// Called when the text is committed
		SLATE_END_ARGS()
	...
	protected:
		...
		FOnRenderPagesEditableTextBlockTextCommitted OnTextCommittedDelegate;
	};
}
