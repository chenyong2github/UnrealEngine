...
{
	DECLARE_DELEGATE_RetVal_TwoParams(FText, FOnRenderPagesFileSelectorTextBlockTextCommitted, const FText&, ETextCommit::Type);
	 ...
	 */
	class SRenderPagesFileSelectorTextBlock : public SCompoundWidget
	{
	public:
		SLATE_BEGIN_ARGS(SRenderPagesFileSelectorTextBlock) {}
			...
			SLATE_EVENT(FOnRenderPagesFileSelectorTextBlockTextCommitted, OnTextCommitted)// Callback when the text is committed
		SLATE_END_ARGS()
	...
	protected:
		...
		FOnRenderPagesFileSelectorTextBlockTextCommitted OnTextCommittedDelegate;
	};
}
