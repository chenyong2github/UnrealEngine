...
UEnum* UNiagaraDataInterfacePhysicsField::FieldVectorTargetEnum = StaticEnum<EFieldVectorType>();
UEnum* UNiagaraDataInterfacePhysicsField::FieldScalarTargetEnum = StaticEnum<EFieldScalarType>();
UEnum* UNiagaraDataInterfacePhysicsField::FieldIntegerTargetEnum = StaticEnum<EFieldIntegerType>();
...
void FNDIPhysicsFieldData::Update(FNiagaraSystemInstance* SystemInstance)
{
	if (SystemInstance != nullptr && SystemInstance->GetWorld())
	{
		...
		if (FieldComponent)
		{
			FieldCommands = FieldComponent->FieldCommands;
		}
	}
}
...
void UNiagaraDataInterfacePhysicsField::PostInitProperties()
{
	...
	if (HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		FNiagaraTypeRegistry::Register(FNiagaraTypeDefinition(FieldVectorTargetEnum), true, true, false);
		FNiagaraTypeRegistry::Register(FNiagaraTypeDefinition(FieldScalarTargetEnum), true, true, false);
		FNiagaraTypeRegistry::Register(FNiagaraTypeDefinition(FieldIntegerTargetEnum), true, true, false);
	}
}
...
{
	{
		...
		Sig.Inputs.Add(FNiagaraVariable(FieldVectorTargetEnum, TEXT("Target Type")));
	...
	}
	{
		...
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Physics Field")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetVec3Def(), TEXT("World Position")));
		Sig.Inputs.Add(FNiagaraVariable(FieldScalarTargetEnum, TEXT("Target Type")));
	...
	}
	{
		...
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Physics Field")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetVec3Def(), TEXT("World Position")));
		Sig.Inputs.Add(FNiagaraVariable(FieldIntegerTargetEnum, TEXT("Target Type")));
...
void UNiagaraDataInterfacePhysicsField::SamplePhysicsVectorField(FVectorVMContext& Context)
{
	...
	if (InstData)
	{
		...
		SampleMax.Init(FVector(-FLT_MAX, -FLT_MAX, -FLT_MAX), Context.NumInstances);
