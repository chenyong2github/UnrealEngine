...
Status LoopImpl::Initialize() {
  ...
  auto cpu_allocator = session_state_.GetExecutionProviders()
                           .Get(onnxruntime::kCpuExecutionProvider)
                           ->GetAllocator(0, OrtMemTypeDefault);
  iter_num_mlvalue_ = MakeScalarMLValue<int64_t>(cpu_allocator, 0, iter_num_rank != 0); // WITH_UE: C4800: Implicit conversion from 'int32_t' to bool. Possible information loss
  condition_mlvalue_ = MakeScalarMLValue<bool>(cpu_allocator, condition_, condition_rank != 0); // WITH_UE: C4800: Implicit conversion from 'int32_t' to bool. Possible information loss
