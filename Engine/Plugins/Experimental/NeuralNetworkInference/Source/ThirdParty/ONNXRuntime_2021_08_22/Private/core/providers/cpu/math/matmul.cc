...
Status MatMul<float>::PrePack(const Tensor& tensor, int input_idx, /*out*/ AllocatorPtr alloc,
  ...
  if (input_idx == 1) {
    ...
    is_packed = GemmPackBFp32(alloc, tensor, trans_b_attr_ != 0, packed_b_, packed_b_size, b_shape_); // WITH_UE: C4800: Implicit conversion from 'int64_t' to bool. Possible information loss
