...
bool FNeuralNetworkInferenceQAUtils::EstimateTensorL1DiffError(const FNeuralTensor& InTensorA, const FNeuralTensor& InTensorB, const float InZeroThreshold, const FString& InDebugName)
{
	if (InTensorA.GetDataType() != InTensorB.GetDataType())
	{
		UE_LOG(LogNeuralNetworkInferenceQA, Warning, TEXT("InTensorA.GetDataType() != InTensorB.GetDataType() failed, %d != %d."), InTensorA.GetDataType(), InTensorB.GetDataType());
		return false;
	}
	if (InTensorA.Num() != InTensorB.Num())
	{
		UE_LOG(LogNeuralNetworkInferenceQA, Warning, TEXT("InTensorA.Num() == InTensorB.Num() failed, %d != %d."), InTensorA.Num(), InTensorB.Num());
		return false;
	}
	...
	else
	{
		for (int32 Index = 0; Index < TensorSize; ++Index)
		{
			...
			else
			{
				Value1 = 0.f;
				Value2 = 0.f;
				ensureMsgf(false, TEXT("DataType = %d not implemented yet for FNeuralNetworkInferenceQAUtils::EstimateTensorL1DiffError()!"), InTensorA.GetDataType());
			}
			// Update L1
			AverageL1Value += FMath::Abs(Value1);
			AverageL1Diff += FMath::Abs(Value1 - Value2);
	...
	{
		UE_LOG(LogNeuralNetworkInferenceQA, Display, TEXT("IsFinite(Value1) != IsFinite(Value2) for %d values (skipped %d valid ones): %s"), TensorSize - SizeMinusInvalid, SizeMinusInvalid, *FiniteVsNon);
	}
	// Average L1
	...
	{
		UE_LOG(LogNeuralNetworkInferenceQA, Warning, TEXT("FNeuralNetworkInferenceQAUtils::EstimateTensorL1DiffError(): NaN|Inf found: AverageL1Diff %f for an L1 average value of %f."), AverageL1Diff, AverageL1Value);
		return false;
	}
	// Did it pass?
	if (AverageL1Diff > InZeroThreshold)
	{
		UE_LOG(LogNeuralNetworkInferenceQA, Warning, TEXT("%s: AverageL1Diff <= ZeroThreshold failed (%fe-3 > %fe-3), AverageL1Value = %f, AverageL1Diff/ZeroThreshold = %f."),
