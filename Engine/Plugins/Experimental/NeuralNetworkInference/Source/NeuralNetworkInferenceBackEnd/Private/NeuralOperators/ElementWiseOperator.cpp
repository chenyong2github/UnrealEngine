...
bool IElementWiseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks()
{
	...
	if (Attributes.Num() > 1)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IElementWiseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): So far only IElementWiseOperators with 0 or 1"
			" attributes, not implemented for %d."), Attributes.Num());
		return bIsLoaded;
	}
	// Input sanity checks
	const TArray<FNeuralTensor*>& InputTensors = GetInputTensorsConst();
	if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(InputTensors, 1))
	{
		return bIsLoaded;
	}
...
void IElementWiseOperator::ForwardGPU_RenderThread(FRDGBuilder* InOutGraphBuilder)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IElementWiseOperator::ForwardGPU_RenderThread(): Sanity checks failed."));
		return;
	}
	// Set parameters
...
void IElementWiseOperator::ForwardCPUWithFunction(float InOperatorFunction(const float))
{
	...
	if (!bIsLoaded)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IElementWiseOperator::ForwardCPUWithFunction(): bIsLoaded was false."));
		return;
	}
	if (Attributes.Num() != 0)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IElementWiseOperator::ForwardCPUWithFunction(): Attributes.Num() = %d != 0."), Attributes.Num());
		return;
	}
...
void IElementWiseOperator::ForwardCPUWithFunction(float InOperatorFunction(const float, const float))
{
	...
	if (Attributes.Num() != 1)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IElementWiseOperator::ForwardCPUWithFunction(): Attributes.Num() = %d != 1."), Attributes.Num());
		return;
	}
	// Not inlined layer (output is different than input)
	if (InlinedTensor < 0)
