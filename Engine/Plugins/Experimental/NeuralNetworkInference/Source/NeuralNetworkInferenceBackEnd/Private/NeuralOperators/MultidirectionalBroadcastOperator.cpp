	...
	const TArray<uint32>& InShapeOutput, const TArray<uint32>& InShapeA, const TArray<uint32>& InShapeB, float OperationFunction(const float, const float))
{
	...
	else
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::RunMultidirectionalBroadcastElementWiseOperation():"
	...
	const uint32 InOutputNumberDimensions)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::FillShapeMultidirectionalBroadcast():"
...
bool IMultidirectionalBroadcastOperator::ConfigureOutputAndInternalVariablesAndSanityChecks()
{
	...
	if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(InputTensors, 2))
	{
		return bIsLoaded;
	}
	if (ShapesOutput.Num() != ShapesX.Num() || ShapesOutput.Num() != ShapesY.Num())
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ConfigureOutputAndInternalVariablesAndSanityChecks():"
			" ShapesOutput.Num() == ShapesX.Num() == ShapesY.Num() failed: %d vs. %d vs. %d."), ShapesOutput.Num(), ShapesX.Num(), ShapesY.Num());
		return bIsLoaded;
	}
	if (InlinedTensor < -1)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): Invalid InlinedTensor = %d."), InlinedTensor);
		return bIsLoaded;
	}
	...
	if (InlinedTensor < 0)
	{
		...
		// Sanity checks
		if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(OutputTensors, 1))
		{
			return bIsLoaded;
		}
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): Only implemented for"
...
void IMultidirectionalBroadcastOperator::ToGPU_RenderThread()
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ToGPU_RenderThread():"
...
void IMultidirectionalBroadcastOperator::ForwardGPU_RenderThread(FRDGBuilder* InOutGraphBuilder)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ForwardGPU_RenderThread():"
	...
	else
	{
		...
		else
		{
			...
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ForwardGPU_RenderThread(): Unknown InlinedTensor = %d."), InlinedTensor);
			return;
		}
	}
	// Set shader
...
void IMultidirectionalBroadcastOperator::ForwardCPUWithFunction(float InOperatorFunction(const float, const float))
{
	...
	if (!bIsLoaded)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ForwardCPUWithFunction(): bIsLoaded was false."));
		return;
	}
	if (InlinedTensor < -1)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ForwardCPUWithFunction(): Invalid InlinedTensor = %d."), InlinedTensor);
		return;
	}
...
bool IMultidirectionalBroadcastOperator::EstimateInlinedTensorFromPotentialOnes()
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::EstimateInlinedTensorFromPotentialOnes(): InputTensors.Num() = %d != 2."), InputTensors.Num());
		return false;
	}
...
void IMultidirectionalBroadcastOperator::ShapesToGPU()
{
	...
	if (ShapesX.IsReadBufferValid() || ShapesY.IsReadBufferValid() || ShapesOutput.IsReadBufferValid())
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("IMultidirectionalBroadcastOperator::ShapesToGPU():"
			" ShapesX, ShapesY and/or ShapesOutput FReadBuffers were not nullptr."));
	}
