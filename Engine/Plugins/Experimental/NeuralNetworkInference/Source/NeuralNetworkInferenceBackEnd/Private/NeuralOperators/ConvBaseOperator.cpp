...
FNeuralInt64ArrayUInt32Buffer FPrivateConvBaseOperator::TransposePadsFromConvPads(const FNeuralInt64ArrayUInt32Buffer& InPads, const FConvBaseOperator::EAutoPad InAutoPad, const TArray<int64>& InEffectiveKernelSizes)
{
	...
	for (int32 Index = 0; Index < PadsArrayTemporary.Num(); ++Index)
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator: PadsArrayTemporary[%d] = %d, ConvTranspose only implemented for PadsArrayTemporary[%d] >= 0 for now."), Index, PadsArrayTemporary[Index], Index);
			return Pads;
		}
	}
...
bool FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks()
{
	...
	if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(InputTensors, 2, 3, 3, -1, 0, 1) || !FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(InputTensors, 2, 3, 1, 1, 2, 2))
	{
		return false;
	}
	...
	{
		if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(GetOutputTensorsConst(), 1, 1, -1, -1))
		{
			return bIsLoaded;
		}
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): OutputPadding.Num() should be 0 or %d, but not %d."), NumberConvolutionalDimensions, OutputPadding.Num());
		return bIsLoaded;
	}
	// OutputShape sanity check
	if (OutputShape.Num() != 0 && OutputShape.Num() != NumberConvolutionalDimensions)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): OutputShape.Num() should be 0 or %d, but not %d."), NumberConvolutionalDimensions, OutputShape.Num());
		return bIsLoaded;
	}
	...
	if (OutputShape.Num() > 0 && Pads.Num() > 0)
	{
		for (int64 DimensionIndex = 0; DimensionIndex < FMath::Min(NumberConvolutionalDimensions, Pads.Num()); ++DimensionIndex) // Min to avoid crash if wrong Pad (wrong Pad checked later)
		{
			...
			{
				UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): You can only set OutputShape or Pads, but not both. Pads will be overridden."));
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): FillOutputShape() failed."));
		return bIsLoaded;
	}
	// Set Dilations
	if (!FillDilationsIfEmpty(NumberConvolutionalDimensions))
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): FillDilationsIfEmpty() failed."));
		return bIsLoaded;
	}
	// Set Strides
	if (!SetAndConfigureStrides(NumberConvolutionalDimensions))
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): SetAndConfigureStrides() failed."));
		return bIsLoaded;
	}
	// Strides sanity checks
	if (Strides.Num() > 0 && Strides.Num() != NumberConvolutionalDimensions) // Right dimensions
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): Strides.Num() == 0 || Strides.Num() == NumberConvolutionalDimensions failed (%d != 0 || %d)."),
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): SetAndConfigureAuxiliaryTensor() failed."));
		return bIsLoaded;
	}
	for (const int64 Stride : Strides) // Strides >= 1 on each dimension
	{
		if (Stride < 1)
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): Stride (%d) < 1 found! Stride must be >= 1."), Stride);
			return bIsLoaded;
		}
	}
	...
	if (AutoPad != EAutoPad::NotSet && Pads.Num() != 0)
	{
		...
		for (const int64 Pad : Pads)
		{
			if (Pad != 0)
			{
				UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): If AutoPad (%d) is different than NotSet (%d), explicit padding is not allowed."),
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureAuxiliaryTensor(): AuxiliaryTensors.Num() should be 0 or 1, not %d."), AuxiliaryTensors.Num());
		return false;
	}
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): EstimateAndFillPads() failed."));
		return bIsLoaded;
	}
	...
	for (const int64 Pad : Pads) // Pads >= 0 on each dimension
	{
		if (Pad < 0)
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): Pads >= 0 failed (found %d)."), Pad);
			return bIsLoaded;
		}
	}
	// Final sanity checks
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): FillOutputSizesAndPaddedMarginsAndSanityChecks() failed."));
		return bIsLoaded;
	}
	// Return true
	bIsLoaded = true;
...
void FConvBaseOperator::ForwardCPU()
{
	...
	if (!bIsLoaded)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ForwardCPU(): bIsLoaded was false."));
		return;
	}
...
void FConvBaseOperator::ForwardGPU_RenderThread(FRDGBuilder* InOutGraphBuilder)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ForwardGPU_RenderThread(): NumberConvolutionalDimensions <= FConvBaseCS::MAX_NUMBER_DIMENSIONS failed (%d vs. %d). I.e.,"
...
bool FConvBaseOperator::SanityChecksForTensors(const TArray<int64>& InPartiallySetOutputSizes) const
{
	...
	if (InlinedTensor > -1)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::SanityChecksForTensors(): This operator cannot be inlined."));
		return false;
	}
	// Input sanity checks
	const TArray<FNeuralTensor*>& InputTensors = GetInputTensorsConst();
...
bool FConvBaseOperator::FillOutputShape(const TArray<int64>& InXSizes)
{
	...
	if (bIsTransposed)
	{
		...
		if (AutoPad == FConvBaseOperator::EAutoPad::SameLower || AutoPad == FConvBaseOperator::EAutoPad::SameUpper)
		{
			...
			{
				UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::FillOutputShape(): OutputShape == OutputShapeTemporary failed, %s != %s."),
...
bool FConvBaseOperator::FillDilationsIfEmpty(const int32 InNumberConvolutionalDimensions)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::FillDilationsIfEmpty(): Dilations.Num() == 0 || Dilations.Num() == InNumberConvolutionalDimensions failed (%d != 0 || %d)."),
	...
	for (const int64 Dilation : Dilations) // Dilation >= 1 on each dimension
	{
		if (Dilation < 1)
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::FillDilationsIfEmpty(): Dilation (%d) < 1 found! Dilation must be >= 1."), Dilation);
			return false;
		}
	}
	return true;
...
bool FConvBaseOperator::EstimateAndFillPads(const TArray<int64>& InEffectiveKernelSizes, const int32 InNumberDimensions, const TArray<int64>& InXSizes)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::EstimateAndFillPads(): Unknown AutoPad = %d."), (int32)AutoPad);
		return false;
	}
	return true;
}
...
bool FConvBaseOperator::FillOutputSizesAndPaddedMarginsAndSanityChecks(TArray<int64>& InOutPartiallySetOutputSizes, const TArray<int64>& InEffectiveKernelSizes)
{
	...
	// Sanity check
	if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(GetOutputTensorsConst(), 1, 1, 3, -1))
	{
		return false;
	}
	return true;
