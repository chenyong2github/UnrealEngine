...
bool FBatchNormalizationOperator::ConfigureOutputAndInternalVariablesAndSanityChecks()
{
	...
	if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(InputTensors, 5))
	{
		return bIsLoaded;
	}
	...
	if (InlinedTensor < 0)
	{
		if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(GetOutputTensorsConst(), 1))
		{
			return bIsLoaded;
		}
...
void FBatchNormalizationOperator::ForwardCPU()
{
	...
	if (!bIsLoaded)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FBatchNormalizationOperator::ForwardCPU(): bIsLoaded was false."));
		return;
	}
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FBatchNormalizationOperator::ForwardCPU(): OutputTensor.GetNumberDimensions() = %d but should be >= 1."), OutputTensor.GetNumberDimensions());
	}
}
...
void FBatchNormalizationOperator::ForwardGPU_RenderThread(FRDGBuilder* InOutGraphBuilder)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FBatchNormalizationOperator::ForwardGPU_RenderThread(): Sanity checks failed."));
		return;
	}
	// Set parameters
	FNeuralTensor& OutputTensor = (InlinedTensor < 0 ? GetOutputTensorNoConst() : *GetInputTensorsNoConst()[0]);
