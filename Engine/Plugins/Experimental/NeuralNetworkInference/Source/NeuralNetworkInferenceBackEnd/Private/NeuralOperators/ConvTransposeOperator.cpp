...
FConvTransposeOperator FPrivateConvTransposeOperator::NodeProtoToConvTransposeOperator(const FNodeProto& InNodeProto)
{
	...
	if (const FAttributeProto* EpsilonAttribute = FModelProto::FindElementInArray(TEXT("AutoPad"), InNodeProto.Attribute, /*bMustValueBeFound*/false))
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator(): Unknown EpsilonAttribute->S = %s."), *EpsilonAttribute->S);
		}
	}
...
bool FConvTransposeOperator::ConfigureOutputAndInternalVariablesAndSanityChecks()
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): Zeros.Num() == NumberConvolutionalDimensions failed, %d != %d."), Zeros.Num(), NumberConvolutionalDimensions);
		return bIsLoaded;
	}
	bIsLoaded = true;
	return bIsLoaded;
...
void FConvTransposeOperator::ToGPU_RenderThread()
{
	...
	if (bIsXWithZerosNeeded)
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ToGPU_RenderThread():"
...
void FConvTransposeOperator::ForwardCPU()
{
	...
	if (bIsXWithZerosNeeded)
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("%dD transposed convolution not implemented yet."), NumberDimensions);
			return;
		}
	}
...
void FConvTransposeOperator::ForwardGPU_RenderThread(FRDGBuilder* InOutGraphBuilder)
{
	...
	if (bIsXWithZerosNeeded)
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ForwardGPU_RenderThread(): Sanity checks failed."));
			return;
		}
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvBaseOperator::ForwardGPU_RenderThread(): Only 1D, 2D, 3D, or 5D convolutions implemented, but not %dD."), NumberConvolutionalDimensions);
			return;
		}
		// Set parameters
...
bool FConvTransposeOperator::SetAndConfigureStrides(const int32 InNumberConvolutionalDimensions)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureStrides(): ZerosFromConvStrides() failed."));
		return false;
	}
	// Zeros sanity checks
	if (Zeros.Num() > 0 && Zeros.Num() != InNumberConvolutionalDimensions) // Right dimensions
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureStrides(): Zeros.Num() == 0 || Zeros.Num() == InNumberConvolutionalDimensions failed (%d != 0 || %d)."),
	...
	for (const int64 Zero : Zeros) // Zeros >= 0 on each dimension
	{
		if (Zero < 0)
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureStrides(): Zeros (%d) < 1 found! Zeros must be >= 1."), Zero);
			return false;
		}
		else if (!bIsXWithZerosNeeded && Zero > 0)
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureStrides(): bIsXWithZerosNeeded was false but Zeros (%d) > 0 found! Only one of the 2 can be true."), Zero);
			return false;
		}
	}
...
bool FConvTransposeOperator::SetAndConfigureAuxiliaryTensor()
{
	...
	if (Zeros.Num() < 1)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureAuxiliaryTensor(): Zeros.Num() should not be 0."));
		return false;
	}
	if (bIsXWithZerosNeeded)
	{
		...
		if (XSizes.Num() < 2)
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureAuxiliaryTensor(): XSizes.Num() should be at least 2, not %d."), XSizes.Num());
			return false;
		}
		else if (AuxiliaryTensors.Num() != 1)
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FConvTransposeOperator::SetAndConfigureAuxiliaryTensor(): AuxiliaryTensors.Num() should be 1, not %d."), AuxiliaryTensors.Num());
			return false;
		}
		...
		XWithZeros.SetTensorTypeGPU(ENeuralTensorTypeGPU::IntermediateInitialized);
		// Set XSizesForGPU
