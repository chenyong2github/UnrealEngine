...
void FSqueezeOperator::ForwardCPU()
{
	...
	if (!bIsLoaded)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FSqueezeOperator::ForwardCPU(): bIsLoaded was false."));
		return;
	}
...
{
	// Sanity checks
	if (!FNeuralNetworkInferenceBackEndUtilsGPU::GPUSanityChecks(InOutGraphBuilder, bIsLoaded))
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FSqueezeOperator::ForwardGPU_RenderThread(): Sanity checks failed."));
		return;
	}
	// Squeeze is 0-cost if inlined, otherwise, just copy memory from input to output
	const TArray<FNeuralTensor*> InputTensors = GetInputTensorsConst();
...
TArray<int64> FSqueezeOperator::Remove1sFromShape(const FNeuralTensor& InTensor, const FNeuralTensor& InAxesTensor) const
{
	...
	if (isDuplicateRemoved)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FSqueezeOperator::Remove1sFromShape(): Axes input had duplicate(s) and was removed."));
	}
	...
	for (int64 Axes : AxesList)
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FSqueezeOperator::Remove1sFromShape(): Axes has an invalid index to remove (Shape value is not equal to one)."));
