...
bool FGemmOperator::ConfigureOutputAndInternalVariablesAndSanityChecks()
{
	...
	if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(InputTensors, 2, 3, 1, 2))
	{
		return bIsLoaded;
	}
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): CommonK1 == CommonK2 failed: %d != %d"), CommonK1, CommonK2);
		return bIsLoaded;
	}
	if (InlinedTensor >= 0)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): FGemmOperator cannot be inlined (InlinedTensor = %d)."), InlinedTensor);
		return bIsLoaded;
	}
	...
	if (!FNeuralNetworkInferenceBackEndUtils::SizeSanityChecks(GetOutputTensorsNoConst(), 1, 1, 2, 3))
	{
		return bIsLoaded;
	}
	// Check bias tensor (C)
	if (InputTensors.Num() == 3)
	{
		...
		{
			UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): Dimension mismatch between the output tensor and"
		...
		for (int32 CDimension = 0; CDimension < C.GetNumberDimensions(); ++CDimension) // CSizes.Num() might be smaller than OutputSizes.Num()
		{
			...
			{
				UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): CPadded.GetSize(%d) == 1 |"
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ConfigureOutputAndInternalVariablesAndSanityChecks(): AColsOrBRows == B.GetSize(1): %d vs. %d"),
...
void FGemmOperator::ForwardCPU()
{
	...
	if (!bIsLoaded)
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ForwardCPU(): bIsLoaded was false."));
		return;
	}
...
void FGemmOperator::ForwardGPU_RenderThread(FRDGBuilder* InOutGraphBuilder)
{
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ForwardGPU_RenderThread(): Sanity checks failed."));
		return;
	}
	// Set parameters
	...
	{
		UE_LOG(LogNeuralNetworkInferenceBackEnd, Warning, TEXT("FGemmOperator::ForwardGPU_RenderThread(): Unknown GemmMode = %d."), (int32)GemmMode);
		return;
	}
	// Set shader
