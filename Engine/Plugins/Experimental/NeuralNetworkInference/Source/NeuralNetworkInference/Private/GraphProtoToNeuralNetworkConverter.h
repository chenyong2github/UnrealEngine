...
private:
	 ...
	 *
	 * 2 alternatives:
	 * - Game and/or Editor (efficient): If InModelPath is empty, InOutTensorManager is assumed to be a const input variable (assuming the tensors are all already loaded, e.g., on de-serialization).
	 * - Editor-only: If InModelPath is not empty, InOutTensorManager is assumed to be an output variable that will be filled based on InModelPath.
	 *
	...
	static bool CreateOperatorsAndEditTensorArray(TArray<TSharedPtr<FNeuralOperator>>& OutOperators, TMap<FString, int32>& OutputNameIndexMap, TArray<FNeuralTensor>& InOutTensors,
		...
		const FGraphProto& InGraphProto, const FString& InModelPath = TEXT(""));
