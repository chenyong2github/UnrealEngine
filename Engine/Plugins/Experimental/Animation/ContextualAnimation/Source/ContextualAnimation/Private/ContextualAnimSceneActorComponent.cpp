	...
	if (SceneInstance)
	{
		...
		for (const FContextualAnimIKTargetDefinition& IKTargetDef : SceneActorDataPtr->GetSettings()->IKTargetDefinitions)
		{
			if (UAnimInstance* AnimInstance = SceneActorDataPtr->GetAnimInstance())
			{
				...
				float Alpha = 0.f;
				bool bCurveFound = AnimInstance->GetCurveValue(IKTargetDef.GoalName, Alpha);
				...
				{
					if (const FContextualAnimSceneActorData* TargetSceneActorData = SceneInstance->FindSceneActorDataForRole(IKTargetDef.TargetRoleName))
					{
						if (USkeletalMeshComponent* TargetSkelMeshComp = TargetSceneActorData->GetSkeletalMeshComponent())
						{
							...
							IKTargetTransform = SceneActorDataPtr->GetAnimData()->IKTargetData.ExtractTransformAtTime(IKTargetDef.GoalName, Time) * IKTargetParentTransform;
