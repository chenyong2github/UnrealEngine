...
void UContextualAnimSceneActorComponent::OnJoinedScene(const FContextualAnimSceneActorData* SceneActorData)
{
	...
	if (SceneActorData->GetIKTargetDefs().IKTargetDefs.Num() > 0)
	{
...
void UContextualAnimSceneActorComponent::UpdateIKTargets()
{
	...
	check(SceneActorDataPtr);

	const FAnimMontageInstance* MontageInstance = SceneActorDataPtr->GetAnimMontageInstance();
	if(MontageInstance == nullptr)
	{
		return;
	}

	for (const FContextualAnimIKTargetDefinition& IKTargetDef : SceneActorDataPtr->GetIKTargetDefs().IKTargetDefs)
	{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		...
		if (const FContextualAnimSceneActorData* TargetSceneActorData = SceneActorDataPtr->GetSceneInstance().FindSceneActorDataByRole(IKTargetDef.TargetRoleName))
		{
			if (const USkeletalMeshComponent* TargetSkelMeshComp = TargetSceneActorData->GetSkeletalMeshComponent())
			{
				if (IKTargetDef.Provider == EContextualAnimIKTargetProvider::Autogenerated)
				{
					...
					IKTargetTransform = SceneActorDataPtr->GetAnimTrack().IKTargetData.ExtractTransformAtTime(IKTargetDef.GoalName, Time) * IKTargetParentTransform;
