	...
	, FTrajectorySample& InOutSample)
{
	...
	{
		const FVector AccelDir = InOutSample.LinearAcceleration.GetSafeNormal();
		const float VelSize = InOutSample.LinearVelocity.Size();
		InOutSample.LinearVelocity = InOutSample.LinearVelocity - (InOutSample.LinearVelocity - AccelDir * VelSize) * FMath::Min(IntegrationDelta * MovementComponent->GroundFriction, 1.f);
		...
		InOutSample.LinearVelocity += InOutSample.LinearAcceleration * IntegrationDelta;
...

	{
		...
		InOutSample.LinearAcceleration = ControlRotationDelta.RotateVector(InOutSample.LinearAcceleration);
