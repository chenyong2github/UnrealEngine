	...
	FString& ObjectBaseNameOut )
{
	...
	{
		PackageFolderPath = AssetAPI->GetWorldRelativeAssetRootPath(TargetWorld);
	}
	...
	{
		FString CurrentAssetPath = AssetAPI->GetActiveAssetFolderPath();
	...
	{
		FString SelectedPath = AssetAPI->InteractiveSelectAssetPath(ObjectBaseName, LOCTEXT("GenerateStaticMeshActorPathDialogWarning", "Choose Folder Path and Name for New Asset. Cancel to Discard New Asset."));
	...
	FGeneratedStaticMeshAssetConfig&& AssetConfig)
{
	...
	UPackage* AssetPackage = AssetAPI->MakeNewAssetPackage(UsePackageFolderPath, UseBaseName, UniqueAssetName);
	...
	{
		AssetAPI->NotifyGeneratedAssetModified(Builder.NewStaticMesh, AssetPackage);
	}
	...
	const UObject* RelativeToAsset)
{
	...
	if (AutoGenMode == EModelingModeAssetGenerationBehavior::InteractivePromptToSave)
	{
		FString SelectedPath = AssetAPI->InteractiveSelectAssetPath(ObjectBaseName, LOCTEXT("GenerateTexture2DAssetPathDialogWarning", "Choose Folder Path and Name for New Asset. Cancel to Discard New Asset."));
		if (SelectedPath.IsEmpty() == false)
		{
			PackageFolderPath = FPaths::GetPath(SelectedPath);
			ObjectBaseName = FPaths::GetBaseFilename(SelectedPath, true);
	...
	// create new package
	FString UniqueAssetName;
	UPackage* AssetPackage = AssetAPI->MakeNewAssetPackage(PackageFolderPath, UseBaseName, UniqueAssetName);
	...
	if (AutoGenMode == EModelingModeAssetGenerationBehavior::AutoGenerateAndAutosave)
	{
		AssetAPI->AutoSaveGeneratedAsset(GeneratedTexture, AssetPackage);
	}
	else if (AutoGenMode == EModelingModeAssetGenerationBehavior::InteractivePromptToSave)
	{
		AssetAPI->AutoSaveGeneratedAsset(GeneratedTexture, AssetPackage);
	...
	{
		AssetAPI->NotifyGeneratedAssetModified(GeneratedTexture, AssetPackage);
	}
