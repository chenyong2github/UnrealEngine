...
class ANIMNEXTINTERFACE_API FAnimationDataRegistry
{
public:

	// Access the global registry
	static FAnimationDataRegistry* Get();
	...
	FAnimationDataHandle RegisterReferencePose(USkeletalMeshComponent* SkeletalMeshComponent);

	// Returns a ref counted handle to the refence pose of the given skeletal mesh component
	FAnimationDataHandle GetOrGenerateReferencePose(USkeletalMeshComponent* SkeletalMeshComponent);
	...
	void RegisterData(const FName& Id, const FAnimationDataHandle& AnimationDataHandle);
	...
	FAnimationDataHandle GetRegisteredData(const FName& Id) const;
	...
	FAnimationDataHandle AllocateData(const int32 NumElements, ArgTypes&&... Args)
	{
		FAnimationDataHandle Handle = PreAllocateMemory<DataType>(NumElements);
...
private:
	...
	struct FReferencePoseData
	{
		...
		FReferencePoseData(const FAnimationDataHandle& InAnimationDataHandle, const FDelegateHandle& InDelegateHandle)
			: AnimationDataHandle(InAnimationDataHandle)
			, DelegateHandle(InDelegateHandle)
		{
		}

		FReferencePoseData(FAnimationDataHandle&& InAnimationDataHandle, FDelegateHandle&& InDelegateHandle)
			: AnimationDataHandle(MoveTemp(InAnimationDataHandle))
			, DelegateHandle(MoveTemp(InDelegateHandle))
		{
		}

		FAnimationDataHandle AnimationDataHandle;
	...
	TMap <FName, FAnimationDataHandle> StoredData;
...
private:
	...
	friend struct FAnimationDataHandle;
