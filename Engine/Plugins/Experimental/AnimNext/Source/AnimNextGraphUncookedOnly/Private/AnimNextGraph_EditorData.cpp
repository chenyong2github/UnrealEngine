...
void UAnimNextGraph_EditorData::Initialize(bool bRecompileVM)
{
	...
	if (RigVMClient.GetController(0) == nullptr)
	{
		...
		if(bRecompileVM)
		{
			UE::AnimNext::GraphUncookedOnly::FUtils::Compile(AnimNextGraph);
		}
	}
...
{
	UE::AnimNext::GraphUncookedOnly::FUtils::Compile(GetTypedOuter<UAnimNextGraph>());
}
...
void UAnimNextGraph_EditorData::UpdateGraphReturnType()
{
	if (UAnimNextGraph* Graph = GetTypedOuter<UAnimNextGraph>())
	{
		...
		UE::AnimNext::FParamTypeHandle Handle = UE::AnimNext::GraphUncookedOnly::FUtils::GetParameterHandleFromPin(GraphReturnPinType);
