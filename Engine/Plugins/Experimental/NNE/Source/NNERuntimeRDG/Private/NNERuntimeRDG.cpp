...
FGuid FRuntimeRDG::GUID = FGuid((int32)'R', (int32)'R', (int32)'D', (int32)'G');
int32 FRuntimeRDG::Version = 0x00000001;

struct FInferenceModelRDG::FReadbackEntry
{
...
bool FRuntimeRDG::CanCreateModelData(FString FileType, TConstArrayView<uint8> FileData) const
{
	return FileType.Compare("onnx", ESearchCase::IgnoreCase) == 0;
}

bool FRuntimeRDG::CanCreateModel(TConstArrayView<uint8> ModelData) const
{
	int32 GuidSize = sizeof(FRuntimeRDG::GUID);
	int32 VersionSize = sizeof(FRuntimeRDG::Version);
	if (ModelData.Num() <= GuidSize + VersionSize)
	{
		return false;
	}
	bool bResult = FGenericPlatformMemory::Memcmp(&(ModelData[0]), &(FRuntimeRDG::GUID), GuidSize) == 0;
	bResult &= FGenericPlatformMemory::Memcmp(&(ModelData[GuidSize]), &(FRuntimeRDG::Version), VersionSize) == 0;
	return bResult;
};
...
//
//
FInferenceModelRDG::FInferenceModelRDG()
...
//
//
bool FInferenceModelRDG::LoadModel(TConstArrayView<uint8> ModelData, FMLRuntimeFormat& Format)
{
	int32 GuidSize = sizeof(FRuntimeRDG::GUID);
	int32 VersionSize = sizeof(FRuntimeRDG::Version);
 ...
 */
int FInferenceModelRDG::RunSync(TConstArrayView<FMLTensorBinding> InInputBindings, TConstArrayView<FMLTensorBinding> InOutputBindings)
{
...
int FInferenceModelRDG::SetInputTensorShapes(TConstArrayView<FTensorShape> InInputShapes)
{
 ...
 */
int FInferenceModelRDG::EnqueueRDG(FRDGBuilder& RDGBuilder, TConstArrayView<FMLTensorBinding> InInputBindings, TConstArrayView<FMLTensorBinding> InOutputBindings)
{
 ...
 */
int FInferenceModelRDG::SetTensors(FRDGBuilder& GraphBuilder, FTensorRDGArray& InTensorRDGs, FIntArray& OutIndices, 
...
//
//
void FInferenceModelRDG::AddTensorUploads_RenderThread(FRDGBuilder& GraphBuilder, TConstArrayView<int32> InUploadIndices, 
...
//
//
void FInferenceModelRDG::AddTensorReadbacks_RenderThread(FRDGBuilder& GraphBuilder, TConstArrayView<int32> InReadbackIndices,
	TConstArrayView<FTensorRDG> InTensorRDGs, TConstArrayView<FMLTensorBinding> InBindings)
{
	check(InTensorRDGs.Num() == InBindings.Num());
...
TArray<uint8> ConvertToModelData(TArrayView<uint8> ModelBuffer)
{
	...
	Writer << FRuntimeRDG::GUID;
	Writer << FRuntimeRDG::Version;
