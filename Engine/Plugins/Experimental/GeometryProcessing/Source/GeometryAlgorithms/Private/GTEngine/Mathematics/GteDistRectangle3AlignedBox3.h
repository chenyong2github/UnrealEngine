...
template <typename Real>
void DCPQuery<Real, Rectangle3<Real>, AlignedBox3<Real>>::SetMaxLCPIterations(int maxLCPIterations)
{
    mLCP.SetMaxIterations(maxLCPIterations);
}

template <typename Real>
typename DCPQuery<Real, Rectangle3<Real>, AlignedBox3<Real>>::Result
DCPQuery<Real, Rectangle3<Real>, AlignedBox3<Real>>::operator()(
    Rectangle3<Real> const& rectangle, AlignedBox3<Real> const& box)
{
    Result result;

    // Translate the rectangle and aligned box so that the aligned box becomes
    // a canonical box.
