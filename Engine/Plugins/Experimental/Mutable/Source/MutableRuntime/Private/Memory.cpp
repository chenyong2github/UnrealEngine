...
//! allowed.
//-------------------------------------------------------------------------------------------------
void* mu::mutable_malloc( size_t size )
{
	if (!size)
	{
		return nullptr;
...
void* mu::mutable_malloc_aligned( size_t size, uint32_t alignment )
{
	if (!size)
	{
		return nullptr;
...
void mu::mutable_free( void* ptr, size_t size )
{
	if (!ptr || !size)
	{
		return;
...
void mu::mutable_free_aligned( void* ptr, size_t size )
{
	if (!size || !ptr)
	{
		return;
