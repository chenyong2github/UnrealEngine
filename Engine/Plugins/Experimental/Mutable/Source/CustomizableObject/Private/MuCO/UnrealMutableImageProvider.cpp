...
//-------------------------------------------------------------------------------------------------
	...
	TTuple<UE::Tasks::FTask, TFunction<void()>> FUnrealMutableImageProvider::GetImageAsync(const FExternalImageID& id, uint8 MipmapsToSkip, TFunction<void(mu::Ptr<mu::Image>)>& ResultCallback)
#else
	TTuple<FGraphEventRef, TFunction<void()>> FUnrealMutableImageProvider::GetImageAsync(const FExternalImageID& id, uint8 MipmapsToSkip, TFunction<void(mu::Ptr<mu::Image>)>& ResultCallback)
#endif
{
	// Thread: worker
...
mu::FImageDesc FUnrealMutableImageProvider::GetImageDesc(const FExternalImageID& id, uint8 MipmapsToSkip) 
{
...
void FUnrealMutableImageProvider::CacheImage(const FExternalImageID& Id, bool bUser)
{
		...
		else
		{
			...
			for (int32 p = 0; !pResult && p < ImageProviders.Num(); ++p)
			{
				...
				if (Provider.IsValid())
				{
					...
					switch (Provider->HasTextureParameterValue(Id.GetName()))
					{

					case UCustomizableSystemImageProvider::ValueType::Raw:
					{
						FIntVector desc = Provider->GetTextureParameterValueSize(Id.GetName());
						pResult = new mu::Image(desc[0], desc[1], 1, mu::EImageFormat::IF_RGBA_UBYTE, mu::EInitializationType::Black);
						Provider->GetTextureParameterValueData(Id.GetName(), pResult->GetData());
						break;
					}

					case UCustomizableSystemImageProvider::ValueType::Unreal:
					{
						UTexture2D* UnrealTexture = Provider->GetTextureParameterValue(Id.GetName());
					...
					{
						UnrealDeferredTexture = Provider->GetTextureParameterValue(Id.GetName());
						break;
					}
			...
			{
				UE_LOG(LogMutable, Warning, TEXT("Failed to cache external image %s."), *Id.GetName());
				pResult = CreateDummy();
			}
...
void FUnrealMutableImageProvider::UnCacheImage(const FExternalImageID& Id, bool bUser)
{
	check(IsInGameThread());
...
void FUnrealMutableImageProvider::ClearCache(bool bUser)
{
	...
	FScopeLock Lock(&ExternalImagesLock);
	for (TTuple<FExternalImageID, FUnrealMutableImageInfo> Tuple : GlobalExternalImages)
	{
...
void FUnrealMutableImageProvider::CacheImages(const mu::Parameters& Parameters)
{
	...
	for (int32 ParamIndex = 0; ParamIndex < NumParams; ++ParamIndex)
	{
		...
		{
			const FExternalImageID TextureId = Parameters.GetImageValue(ParamIndex);
		...
		for (int32 ValueIndex = 0; ValueIndex < NumValues; ++ValueIndex)
		{
			...
			const FExternalImageID TextureId = Parameters.GetImageValue(ParamIndex, Range);
...
{
	for (TPair<FExternalImageID, FUnrealMutableImageInfo>& Image : GlobalExternalImages)
	{
