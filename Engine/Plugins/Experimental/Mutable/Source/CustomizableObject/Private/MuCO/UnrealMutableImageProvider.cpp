...
//-------------------------------------------------------------------------------------------------
	...
	TTuple<UE::Tasks::FTask, TFunction<void()>> FUnrealMutableImageProvider::GetImageAsync(mu::FExternalImageID id, uint8 MipmapsToSkip, TFunction<void(mu::Ptr<mu::Image>)>& ResultCallback)
#else
	TTuple<FGraphEventRef, TFunction<void()>> FUnrealMutableImageProvider::GetImageAsync(mu::FExternalImageID id, uint8 MipmapsToSkip, TFunction<void(mu::Ptr<mu::Image>)>& ResultCallback)
#endif
{
	// Thread: worker
...
mu::FImageDesc FUnrealMutableImageProvider::GetImageDesc(mu::FExternalImageID id, uint8 MipmapsToSkip) 
{
...
void FUnrealMutableImageProvider::CacheImage(const mu::FExternalImageID& Id, bool bUser)
{
...
void FUnrealMutableImageProvider::UnCacheImage(const mu::FExternalImageID& Id, bool bUser)
{
	check(IsInGameThread());
...
{
	for (TPair<mu::FExternalImageID, FUnrealMutableImageInfo>& Image : GlobalExternalImages)
	{
