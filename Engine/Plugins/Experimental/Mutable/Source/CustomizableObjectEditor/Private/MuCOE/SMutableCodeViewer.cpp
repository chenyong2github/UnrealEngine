...
namespace
{
	...
	public:
		...
		TTuple<UE::Tasks::FTask, TFunction<void()>> GetImageAsync(const FExternalImageID& id, uint8 MipmapsToSkip, TFunction<void(mu::Ptr<mu::Image>)>& ResultCallback) override
#else
		TTuple<FGraphEventRef, TFunction<void()>> GetImageAsync(const FExternalImageID& id, uint8 MipmapsToSkip, TFunction<void(mu::Ptr<mu::Image>)>& ResultCallback) override
#endif
		{
		...
		mu::FImageDesc GetImageDesc(const FExternalImageID& Id, uint8 MipmapsToSkip) override
		{
			return IMAGE_DESC;
		}
	};
