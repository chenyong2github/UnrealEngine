...
namespace DatasmithRuntime
{
	...
	TSharedPtr<IDatasmithUEPbrMaterialElement> ValidatePbrMaterial( TSharedPtr<IDatasmithUEPbrMaterialElement> PbrMaterialElement, FSceneImporter& SceneImporter )
	{
		...
		if (PbrMaterialElement->GetUseMaterialAttributes())
		{
			for (int32 Index = 0; Index < PbrMaterialElement->GetExpressionsCount(); ++Index)
			{
				...
				if (Expression && Expression->IsSubType((uint64)EDatasmithMaterialExpressionType::FunctionCall))
				{
	...
	void FDatasmithExpressionEvaluator::EvaluateExpression(const IDatasmithMaterialExpression& InExpression, FDatasmithInputValue& OutValue)
	{
		...
		if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::ConstantBool))
		{
		...
		else if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::ConstantColor))
		{
		...
		else if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::ConstantScalar))
		{
		...
		else if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::FlattenNormal))
		{
		...
		else if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::FunctionCall))
		{
		...
		else if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::Generic))
		{
		...
		else if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::Texture))
		{
		...
		else if (InExpression.IsSubType((uint64)EDatasmithMaterialExpressionType::TextureCoordinate))
		{
