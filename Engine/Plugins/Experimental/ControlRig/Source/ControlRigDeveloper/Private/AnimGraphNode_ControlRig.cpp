...
void UAnimGraphNode_ControlRig::ReallocatePinsDuringReconstruction(TArray<UEdGraphPin*>& OldPins)
{
	...
	for (const TPair<FName, FRigVMParameter>& ParameterPair : InputParameters)
	{
		FName PropertyName = ParameterPair.Key;
		...
		{
			const FRigVMParameter& Parameter = ParameterPair.Value;

			FEdGraphPinType PinType;

			if (Parameter.GetScriptStruct())
			{
				PinType.PinCategory = UEdGraphSchema_K2::PC_Struct;
				PinType.PinSubCategoryObject = Parameter.GetScriptStruct();
			}
			else if (Parameter.GetCPPType() == TEXT("bool"))
			{
				PinType.PinCategory = UEdGraphSchema_K2::PC_Boolean;
			}
			else if (Parameter.GetCPPType() == TEXT("float"))
			{
				PinType.PinCategory = UEdGraphSchema_K2::PC_Float;
			}
			else if (Parameter.GetCPPType() == TEXT("int32"))
			{
				PinType.PinCategory = UEdGraphSchema_K2::PC_Int;
			}
			else if (Parameter.GetCPPType() == TEXT("FName"))
			{
				PinType.PinCategory = UEdGraphSchema_K2::PC_Name;
			}
			else if (Parameter.GetCPPType() == TEXT("FString"))
			{
...
void UAnimGraphNode_ControlRig::RebuildExposedProperties()
{
	...
	GetIOParameters(true, InputParameters);
	// we still update OUtputvariables
	// we don't want output to be exposed
	GetIOParameters(false, OutputParameters);

	// clear IO variables that don't exist anymore
	auto ClearInvalidMapping = [](const TMap<FName, FRigVMParameter>& InParameters, TMap<FName, FName>& InOutMapping)
	{
		...
		for (int32 Index=0; Index<KeyArray.Num(); ++Index)
		{
			...
			if (!InParameters.Contains(KeyArray[Index]))
			{
	...
	ClearInvalidMapping(InputParameters, Node.InputMapping);
	ClearInvalidMapping(OutputParameters, Node.OutputMapping);

	for (auto Iter = InputParameters.CreateConstIterator(); Iter; ++Iter)
	{
...
bool UAnimGraphNode_ControlRig::IsInputProperty(const FName& PropertyName) const
{
	...
	return InputParameters.Contains(PropertyName);
}
...
bool UAnimGraphNode_ControlRig::IsAvailableToMapToCurve(const FName& PropertyName, bool bInput) const
{
	...
	const FRigVMParameter* Parameter = (bInput) ? InputParameters.Find(PropertyName) : OutputParameters.Find(PropertyName);
	if (ensure(Parameter))
	{
		return Parameter->GetCPPType().Equals(TEXT("float"));
	}
...
void UAnimGraphNode_ControlRig::GetIOParameters(bool bInput, TMap<FName, FRigVMParameter>& OutParameters) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
void UAnimGraphNode_ControlRig::CreateVariableMapping(const FString& FilteredText, TArray< TSharedPtr<FVariableMappingInfo> >& OutArray, bool bInput)
{
	...
	const TMap<FName, FRigVMParameter>& Parameters = (bInput)? InputParameters : OutputParameters;
