...
#include "ChaosTireConfig.generated.h"

class UPhysicalMaterial;

/** Allows overriding of friction of this tire config on a specific material */
USTRUCT()
struct FTireFrictionPerMaterial
{
	GENERATED_USTRUCT_BODY()

	/** Physical material for friction scale */
	UPROPERTY(EditAnywhere, Category = FTireFrictionPerMaterial)
...
UCLASS()
class CHAOSVEHICLES_API UChaosTireConfig : public UDataAsset
{
	GENERATED_BODY()
...
private:
	...
	UPROPERTY(EditAnywhere, Category = TireFrictionConfig)
	float FrictionScale;

	/** Tire friction scales for specific physical materials. */
	UPROPERTY(EditAnywhere, Category = TireFrictionConfig)
	TArray<FTireFrictionPerMaterial> TireFrictionScales;
...
public:
	...
	static TArray<TWeakObjectPtr<UChaosTireConfig>> AllTireConfigs;
