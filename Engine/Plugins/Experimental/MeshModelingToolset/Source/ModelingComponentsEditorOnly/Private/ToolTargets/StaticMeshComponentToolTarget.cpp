...
void UStaticMeshComponentToolTarget::CalculateAutoGeneratedAttributes(FMeshDescription& MeshDescription)
{
	if (ensure(IsValid()))
	{
		UStaticMesh* StaticMesh = Cast<UStaticMeshComponent>(Component)->GetStaticMesh();
		if (EditingLOD == EStaticMeshEditingLOD::HiResSource)
		{
			...
			UE::MeshDescription::InitializeAutoGeneratedAttributes(MeshDescription, &SourceModel.BuildSettings);
		}
		else
		{
			UE::MeshDescription::InitializeAutoGeneratedAttributes(MeshDescription, StaticMesh, (int32)EditingLOD);
		}
	}
}
