...
void UE::MeshDescription::InitializeAutoGeneratedAttributes(FMeshDescription& Mesh, const FMeshBuildSettings* BuildSettings)
{
	...
	if (BuildSettings->bRecomputeNormals || BuildSettings->bRecomputeTangents)
	{
		...
		{
			// get the normal/tangent buffers
			...
			TVertexInstanceAttributesRef<FVector> VertexInstanceTangents = Mesh.VertexInstanceAttributes().GetAttributesRef<FVector>(MeshAttribute::VertexInstance::Tangent);

			// If normals or tangents are missing on polygons, create them because that's how FStaticMeshOperations::ComputeTangentsAndNormals works
			// (is this true for MikkT tangents?)
			if (Mesh.PolygonAttributes().GetAttributesRef<FVector>(MeshAttribute::Polygon::Normal).IsValid() == false
				|| Mesh.PolygonAttributes().GetAttributesRef<FVector>(MeshAttribute::Polygon::Tangent).IsValid() == false)
			{
				FStaticMeshOperations::ComputePolygonTangentsAndNormals(Mesh);
			}
...
{
#if WITH_EDITOR
	if (ensure(Cast<UStaticMeshComponent>(StaticMeshComponent) != nullptr))
	{
