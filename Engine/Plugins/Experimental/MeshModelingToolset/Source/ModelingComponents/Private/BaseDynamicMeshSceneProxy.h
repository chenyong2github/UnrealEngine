...
public:
	...
	void UploadUpdate(bool bPositions, bool bNormals, bool bColors)
	{
		check(IsInRenderingThread());
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			if (VisibilityMap & (1 << ViewIndex))
			{
				...
				for (FMeshRenderBufferSet* BufferSet : Buffers)
				{
					...
					DrawBatch(Collector, *BufferSet, MaterialProxy, false, DepthPriority, ViewIndex, DynamicPrimitiveUniformBuffer);
					if (bWireframe)
					{
						DrawBatch(Collector, *BufferSet, WireframeMaterialProxy, true, DepthPriority, ViewIndex, DynamicPrimitiveUniformBuffer);
					}
				}
			}
		}
