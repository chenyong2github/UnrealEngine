	...
	void RegisterPlacementModeItems()
	{
			...
			; Info && GEditor)
		{
			FAssetData VirtualCamera2ActorAssetData(
				TEXT("/VirtualCameraMacPreset/VCamActor"),
				TEXT("/VirtualCameraMacPreset"),
				TEXT("VCamActor"),
			...
			const UClass* VCamActorBlueprintClass = LoadClass<UObject>(nullptr, TEXT("/VirtualCameraMacPreset/VCamActor.VCamActor_C"));
			UE_CLOG(VCamActorBlueprintClass == nullptr, LogVirtualCameraPixelStreamingPresetEditor, Warning, TEXT("Failed to load '/VirtualCameraMacPreset/VCamActor.VCamActor_C'. Has the Blueprint been moved?"));
		}
	}
...
IMPLEMENT_MODULE(FVirtualCameraMacPresetEditorModule, VirtualCameraMacPresetEditor)
