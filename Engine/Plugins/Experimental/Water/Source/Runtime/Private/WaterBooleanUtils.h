...
namespace UE {
	...
	namespace WaterUtils {
		 ...
		 */
		void ExtractMesh(AVolume* Volume, FDynamicMesh3& Mesh)
		{
		 ...
		 */
		void ExtractMesh(FBoxSphereBounds Bounds, FDynamicMesh3& Mesh)
		{
		 ...
		 */
		void ExtractMesh(FAxisAlignedBox3d Bounds, FDynamicMesh3& Mesh)
		{
			FMinimalBoxMeshGenerator MeshGen;
			FOrientedBox3d Box;
		 ...
		 */
		void ApplyBooleanRepairs(FDynamicMesh3& Mesh, double MergeTolerance = FMathf::ZeroTolerance * 10.0)
		{
			if (Mesh.IsClosed() == false)
			{
		 ...
		 */
		void MakeNormalizationTransform(
		 ...
		 */
		void SetToFaceNormals(FDynamicMesh3& Mesh)
		{
		 ...
		 */
		FDynamicMesh3 AccumulateExtrusionVolumes(const TArray<AWaterBodyExclusionVolume*>& ExclusionVolumes, const FTransform3d& Transform)
		{
		 ...
		 */
		FDynamicMesh3 MakeBoxCollisionMesh(FAxisAlignedBox3d WorldBoxBounds, const TArray<AWaterBodyExclusionVolume*>& ExclusionVolumes)
		{
		...
		FAxisAlignedBox3d GetBounds(const AWaterBodyExclusionVolume* Volume, double ExpansionSize = 0)
		{
		...
		FAxisAlignedBox3d GetBounds(const TArray<AWaterBodyExclusionVolume*>& Volumes)
		{
		 ...
		 */
		void FindRemainingBoxSpaceXY(const FAxisAlignedBox3d& OuterBox, const FAxisAlignedBox3d& RemoveBox,
		 ...
		 */
		void FillEmptySpaceAroundBoxesXY(
		 ...
		 */
		bool IsConvex(const FPolygon2d& Polygon)
		{
		 ...
		 */
		void MakeSweepConvex(const TArray<FVector3d>& VertexLoop, const FVector3d& SweepVector, FKConvexElem& ConvexOut)
		{
		 ...
		 */
		void MakeSweepConvex(const TArray<FVector3d>& VertexLoop, const FFrame3d& BasePlane, FKConvexElem& ConvexOut)
		{
			int32 N = VertexLoop.Num();
			ConvexOut.VertexData.SetNum(2 * N);
			for (int32 k = 0; k < N; ++k)
		 ...
		 */
		void MakeSweepConvex(const FPolygon2d& Polygon, const FFrame3d& Plane, const FVector3d& SweepVector, FKConvexElem& ConvexOut)
		{
			const TArray<FVector2d>& Vertices = Polygon.GetVertices();
			int32 N = Vertices.Num();
			ConvexOut.VertexData.SetNum(2 * N);
		 ...
		 */
		void MakePerTriangleSweepConvexDecomposition(const FDynamicMesh3& Mesh, TArray<FKConvexElem>& Convexes, double DotThreshold = 0.95)
		{
		 ...
		 */
		void FindConvexPairedTrisFromPlanarMesh(FDynamicMesh3& Mesh, const FFrame3d& Plane, TArray<FPolygon2d>& PlanePolygons)
		{
		 ...
		 */
		bool GetTriangleSetBoundaryLoop(const FDynamicMesh3& Mesh, const TArray<int32>& Tris, FEdgeLoop& Loop)
		{
			Loop.Mesh = &Mesh;
		 ...
		 */
		bool TriSetToBoundaryPolygon(FDynamicMesh3& MeshXY, const TArray<int32>& Tris, FPolygon2d& PolygonOut)
		{
		 ...
		 */
		bool CanAppendTriConvex(FDynamicMesh3& MeshXY, const TArray<int32>& Tris, int32 TestTri)
		{
		 ...
		 */
		void FindConvexPolygonsFromPlanarMesh(FDynamicMesh3& Mesh, const FFrame3d& Plane, TArray<FPolygon2d>& PlanePolygons)
		{
			auto GetXY = [](const FVector3d& P) { return FVector2d(P.X, P.Y); };		// useful for this code
		  ...
		  * @param DotThreshold only faces where Dot(Normal,+Z) > DotThreshold are swept. Default 0.95 (~18 deg)
		  */
		void MakeClusteredTrianglesSweepConvexDecomposition(const FDynamicMesh3& Mesh, TArray<FKConvexElem>& Convexes, double DotThreshold = 0.95)
		{
			FAxisAlignedBox3d Bounds = Mesh.GetBounds();
			FVector3d VerticalDir = FVector3d::UnitZ();
			FVector3d OffsetVec = (-VerticalDir) * (Bounds.Max.Z - Bounds.Min.Z);
		 ...
		 * 
		 */
		void GenerateSubtractSweepConvexDecomposition(const FDynamicMesh3& MeshIn, 
		 ...
		 *
		 */
		void BuildOceanCollisionComponents(
