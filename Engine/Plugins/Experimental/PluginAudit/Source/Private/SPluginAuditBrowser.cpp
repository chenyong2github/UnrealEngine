			...
			if (AssetRegistry->GetDependencies(FAssetIdentifier(AssetInPlugin.PackageName), FoundDependencies, UE::AssetRegistry::EDependencyCategory::All, DependencyRequirements))
			{
				for (const FAssetIdentifier& DependencyId : FoundDependencies)
				{
					...
					if (DependencyId.ObjectName == NAME_GameplayTag && DependencyId.PackageName == GameplayTagStructPackage)
					{
						...
						const EDoesPluginDependGameplayTagSource DependencyResult = DoesPluginDependOnGameplayTagSource(Manager, Plugin.Plugin, DependencyId.ValueName, PossibleSources);
						if (DependencyResult != EDoesPluginDependGameplayTagSource::Yes)
						{
							...
							if (DependencyResult == EDoesPluginDependGameplayTagSource::UnknownTag)
							{
...
/*static*/ SPluginAuditBrowser::EDoesPluginDependGameplayTagSource SPluginAuditBrowser::DoesPluginDependOnGameplayTagSource(const UGameplayTagsManager& Manager, const TSharedPtr<IPlugin>& DependentPlugin, FName TagName, TArray<TSharedPtr<IPlugin>>& OutPossibleSources)
{
	FString Comment;
	...
	if (Manager.GetTagEditorData(TagName, Comment, TagSources, bIsTagExplicit, bIsRestrictedTag, bAllowNonRestrictedChildren))
	{
		...
		if (TagSourcePlugins.Num() == 0)
		{
			...
			return EDoesPluginDependGameplayTagSource::Yes;
		}
		...
		return EDoesPluginDependGameplayTagSource::No;
	}

	// If there's no source data for the tag then the tag is unknown and hasn't been registered yet.
	return EDoesPluginDependGameplayTagSource::UnknownTag;
}
