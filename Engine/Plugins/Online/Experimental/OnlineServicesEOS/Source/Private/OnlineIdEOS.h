...
private:
	...
	TMap<EOS_EpicAccountId, uint32> EpicAccountIdToHandle; // Map of EOS_EpicAccountId to the associated handle.
	TMap<EOS_ProductUserId, uint32> ProductUserIdToHandle; // Map of EOS_ProductUserId to the associated handle.
};
...
{
	return Handle.Type == EOnlineServices::Epic && Handle.IsValid();
}
